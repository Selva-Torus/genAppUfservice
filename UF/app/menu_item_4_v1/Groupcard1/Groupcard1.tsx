...
const Groupcard1 = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:Dashboard3:AFVK:v1",componentId:"700eecb58bc94182aa8d396f4d7dced1",from:"GroupCard1",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("")){
      setdd147({...dd147,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const card1dced1Ref = useRef<any>(null);
  const handleClearSearch = () => {
    card1dced1Ref.current?.setSearchParams();
    card1dced1Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(card1dced1) && Object.keys(card1dced1)?.length>0)
      {
        setcard1dced1({})
      }
    }else 
      prevRefreshRef.current= true
  }, [card1dced1Props?.refresh])
