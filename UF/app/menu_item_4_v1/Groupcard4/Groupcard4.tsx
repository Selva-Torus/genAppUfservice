...
const Groupcard4 = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:Dashboard3:AFVK:v1",componentId:"862965596d384079aee41f013332e38a",from:"GroupCard4",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("")){
      set6b783({...6b783,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const card42e38aRef = useRef<any>(null);
  const handleClearSearch = () => {
    card42e38aRef.current?.setSearchParams();
    card42e38aRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(card42e38a) && Object.keys(card42e38a)?.length>0)
      {
        setcard42e38a({})
      }
    }else 
      prevRefreshRef.current= true
  }, [card42e38aProps?.refresh])
