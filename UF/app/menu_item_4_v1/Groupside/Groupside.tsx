...
const Groupside = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:Dashboard3:AFVK:v1",componentId:"f5b36dda2fd54aba9ca30b404275fa55",from:"GroupSide",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("")){
      set4441d({...4441d,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("dashboard")){
      setdashboard0074d({...dashboard0074d,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("account")){
      setaccount895d0({...account895d0,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("help")){
      sethelp1a4ca({...help1a4ca,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("settings")){
      setsettings67c80({...settings67c80,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("notification")){
      setnotification7a80c({...notification7a80c,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const side5fa55Ref = useRef<any>(null);
  const handleClearSearch = () => {
    side5fa55Ref.current?.setSearchParams();
    side5fa55Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(side5fa55) && Object.keys(side5fa55)?.length>0)
      {
        setside5fa55({})
      }
    }else 
      prevRefreshRef.current= true
  }, [side5fa55Props?.refresh])
  ...
  return (
    ...
    >
          {allowedControls.includes("") ?<Text   /* 4441d */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
        {allowedControls.includes("dashboard")?<Icondashboard /* 0074d */ encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
        {allowedControls.includes("account")?<Iconaccount /* 895d0 */ encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
        {allowedControls.includes("help")?<Iconhelp /* 1a4ca */ encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
        {allowedControls.includes("settings")?<Iconsettings /* 67c80 */ encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
        {allowedControls.includes("notification")?<Iconnotification /* 7a80c */ encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
    </div>
 )
}
