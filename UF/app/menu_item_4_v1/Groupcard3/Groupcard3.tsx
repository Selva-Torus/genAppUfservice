...
const Groupcard3 = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:Dashboard3:AFVK:v1",componentId:"3dca09515ce7418e92db9b717ec5fd72",from:"GroupCard3",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("")){
      set0cce7({...0cce7,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const card35fd72Ref = useRef<any>(null);
  const handleClearSearch = () => {
    card35fd72Ref.current?.setSearchParams();
    card35fd72Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(card35fd72) && Object.keys(card35fd72)?.length>0)
      {
        setcard35fd72({})
      }
    }else 
      prevRefreshRef.current= true
  }, [card35fd72Props?.refresh])
