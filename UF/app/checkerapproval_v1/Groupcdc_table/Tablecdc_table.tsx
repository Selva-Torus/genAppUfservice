...
let defaultColumns = [
  {
    "id": "setup_cd",
    "nodeid": "cbc12e00740b4e2da1dcc1651db7cc97",
    "name": "Setup Code",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "api_nm",
    "nodeid": "3547de9808a144d699d80613965498ae",
    "name": "API Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "api_endpt",
    "nodeid": "bee6b7c2ef094d6a8e65ecb279339a78",
    "name": "API Endpoint",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "pdt_cd",
    "nodeid": "05301927bc084f94a03955f561c70f76",
    "name": "Product Code",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "http_mthd",
    "nodeid": "c001ef341fae442ca95896fca3a3f6f2",
    "name": "HTTP Method",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "aprl_id",
    "nodeid": "f11f4ca2197e4c8ea336f80d78447e91",
    "name": "Approval Id",
    "meta": {
      "sort": true
    },
...
const Tablecdc_table = ({ lockedData,setLockedData,primaryTableData, setPrimaryTableData,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData }: any)=>{
  ...
  const GetTableDetails = async () => {
    const orchestrationData = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        componentId: '5def3a2d3cd44fd49c3d321c5de8e54d',
        isTable: true,
        from :"Table",
  ...
  const [SearchParams,setSearchParams] = useState<any>({})
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      if (ids.length == 0) {
        ...
        keys={}       
        Object.keys(details_group46bbeProps?.presetValues).map((item: any) => {
          keys[item] = null
        }) 
        setdetails_group46bbeProps((pre:any)=>({...pre.presetValues,...keys}))
        setcdc_table8e54dProps((pre:any)=>({...pre, selectedIds:[]}))
        setdetails_group46bbe({})
        setdetails_group46bbeProps({...details_group46bbeProps,presetValues:{}})
        setLockedData((pre:any)=>({...pre,data:[]}))
        return
      }

      cdc_table8e54d.filter((item:any,id:number)=>{
        if (ids[ids.length - 1] == id.toString()){
          selectedData?.push(allData[id])
          postIds.push(item.approval_id)
          processIds.push(item?.trs_process_id)
        }
      })

      //////////
        setdetails_group46bbe(allData[ids])
        setdetails_group46bbeProps({...details_group46bbeProps,presetValues:{}})
      //////////
      setcdc_table8e54dProps((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))      
    }
      ...
      setcdc_table8e54dProps((pre:any)=>({...pre, selectedIds:ids}))
    }
      setdetails_group46bbe(selectedData[0]||{})
      setdetails_group46bbeProps({...details_group46bbeProps,presetValues:selectedData[0]||{}})
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setcdc_table8e54dProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
    ...
    try {
      ...
      if (isRulePresent==false) {
        ...
        const api_paginationBody: api_paginationDto = {
          ...
          count: parseInt(pageSize),
          filterDetails: {
            ufKey:'CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:VGPH001:AFK:CDC_Checker_Action_Screen:AFVK:v1:UO', 
            nodeId: '5def3a2d3cd44fd49c3d321c5de8e54d',
            elementId: '5def3a2d3cd44fd49c3d321c5de8e54d'
          },
          searchFilter: searchParams
        }
        if(encryptionFlagCont) {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( cdc_table8e54dProps?.presetValues&&Object.keys(cdc_table8e54dProps?.presetValues).length > 0) {
          filtertedData = [cdc_table8e54dProps?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setcdc_table8e54d(uf_paginationDataFilter.data||[])
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(cdc_table8e54dProps.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [cdc_table8e54dProps.filterProps])
  ...
  const handlePrimaryTable = () => {
    let findData = cdc_table8e54dProps?.selectedIds[cdc_table8e54dProps?.selectedIds?.length-1]
    if(Array.isArray(cdc_table8e54d) && cdc_table8e54d.length>0)
    {
      let data = cdc_table8e54d[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "approval_id",
  ...
  useEffect(() => {
    if (cdc_table8e54dProps?.selectedIds?.length != 0) handlePrimaryTable()
  }, [cdc_table8e54dProps?.selectedIds])
    ...
    <div className='w-full h-full'>
          <div className=' w-full flex flex-row h-[80%]'>
            <Table
              ...
              edgePadding={true}
              tableSelection={true}
              selectedIds={cdc_table8e54dProps?.selectedIds}  
