...
const TextInputapprove_id = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:V001:AFGK:VGPH001:AFK:CDC_Checker_Action_DFD:AFVK:v1|0ffd283841c3428d98777604be56573c|properties.approval_id"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:VGPH001:AFK:CDC_Checker_Action_Screen:AFVK:v1|2eff021532cd4b80834bce1263346bbe|8c6867aa141c4359bd90470cd7882664"
    }
  ],
  "schemaData": {
    "type": "number"
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'approval_id',type:"text"})
  ...
  const handleChange = async(e: any) => {
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['cdc_group']  = {...cdc_group2e1e4,approval_id:newInputValue},
      codeStates['setcdc_group'] = setcdc_group2e1e4,
      codeStates['details_group']  = {...details_group46bbe,approval_id:newInputValue},
      codeStates['setdetails_group'] = setdetails_group46bbe,
      codeStates['table_group']  = {...table_group05951,approval_id:newInputValue},
      codeStates['settable_group'] = settable_group05951,
      codeStates['cdc_table']  = {...cdc_table8e54d,approval_id:newInputValue},
      codeStates['setcdc_table'] = setcdc_table8e54d,
    codeExecution(code,codeStates);
    }  
    setError('');
    setValidate((pre:any)=>({...pre,approval_id:undefined}));
    if(dynamicStateandType.type=="number"){
    setdetails_group46bbe((prev: any) => ({ ...prev, approval_id: +e.target.value }));
    }
    else{
    setdetails_group46bbe((prev: any) => ({ ...prev, approval_id: e.target.value }));
    }
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "8c6867aa141c4359bd90470cd7882664",
          isTable: false,
          from:"TextInputapprove_id",
      ...
      if(orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties){
        let type:any={name:'approval_id',type:'text'};
        type={
          name:'approval_id',
          type: orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.approval_id.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.approval_id.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.approval_id.type
        }
        ...
        type={
          name:'approval_id',
          type: orchestrationData?.data?.schemaData[0].schema.properties.approval_id.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.properties.approval_id.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.properties.approval_id.type
        }
        setDynamicStateandType(type);
      }
      }
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setdetails_group46bbe((pre:any)=>({...pre,approval_id:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
  ...
  if(dfd_cdc_checker_action_dfd_v1Props?.setSearchFilters && dfd_cdc_checker_action_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_cdc_checker_action_dfd_v1Props.data) && dfd_cdc_checker_action_dfd_v1Props.data.length > 0){
      setdetails_group46bbe((pre:any)=>({...pre,approval_id:dfd_cdc_checker_action_dfd_v1Props.data[0]?.approval_id}));
    }
  }
  },[dfd_cdc_checker_action_dfd_v1Props?.setSearchFilters])
  if (approve_id82664?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `18 / 25`,gridRow: `44 / 67`, gap:``, height: `100%`, overflow: 'auto'}} >
      <TextInput
        require={isRequredData}
        className=""
        label={keyset("Approve Id")}
        onChange= {handleChange}
        onBlur={handleBlur}
        type={dynamicStateandType.type}
        value={details_group46bbe?.approval_id||""}
         disabled= {approve_id82664?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Approve Id"
        validationState={validate?.approval_id ? "invalid" : undefined}
        errorMessage={error}
      />
    </div> 
  )
...
export default TextInputapprove_id
