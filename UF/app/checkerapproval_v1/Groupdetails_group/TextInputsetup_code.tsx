...
const TextInputsetup_code = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:V001:AFGK:VGPH001:AFK:CDC_Checker_Action_DFD:AFVK:v1|0ffd283841c3428d98777604be56573c|properties.setup_code"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:VGPH001:AFK:CDC_Checker_Action_Screen:AFVK:v1|2eff021532cd4b80834bce1263346bbe|d4f4298cde0641ae91d933fcb684eedf"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'setup_code',type:"text"})
  ...
  const handleChange = async(e: any) => {
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['cdc_group']  = {...cdc_group2e1e4,setup_code:newInputValue},
      codeStates['setcdc_group'] = setcdc_group2e1e4,
      codeStates['details_group']  = {...details_group46bbe,setup_code:newInputValue},
      codeStates['setdetails_group'] = setdetails_group46bbe,
      codeStates['table_group']  = {...table_group05951,setup_code:newInputValue},
      codeStates['settable_group'] = settable_group05951,
      codeStates['cdc_table']  = {...cdc_table8e54d,setup_code:newInputValue},
    ...
    setdetails_group46bbe((prev: any) => ({ ...prev, setup_code: +e.target.value }));
    }
    else{
    setdetails_group46bbe((prev: any) => ({ ...prev, setup_code: e.target.value }));
    }
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "d4f4298cde0641ae91d933fcb684eedf",
          isTable: false,
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setdetails_group46bbe((pre:any)=>({...pre,setup_code:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
  ...
  if(dfd_cdc_checker_action_dfd_v1Props?.setSearchFilters && dfd_cdc_checker_action_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_cdc_checker_action_dfd_v1Props.data) && dfd_cdc_checker_action_dfd_v1Props.data.length > 0){
      setdetails_group46bbe((pre:any)=>({...pre,setup_code:dfd_cdc_checker_action_dfd_v1Props.data[0]?.setup_code}));
    }
  }
  },[dfd_cdc_checker_action_dfd_v1Props?.setSearchFilters])
  if (setup_code4eedf?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 8`,gridRow: `20 / 42`, gap:``, height: `100%`, overflow: 'auto'}} >
      <TextInput
        ...
        value={details_group46bbe?.setup_code||""}
         disabled= {setup_code4eedf?.isDisabled ? true : false}
        pin='brick-brick'     
