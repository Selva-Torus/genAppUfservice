...
const Buttoncontinue = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "3773ca523a1849fa92542912ce1599e4"
      );
  ...
  useEffect(()=>{
    if (!continue599e4?.trigger) return;
      if(continue599e4?.trigger){
      (async()=>{
        await handleClick();
      })();
      setcontinue599e4((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[continue599e4?.trigger])

  useEffect(()=>{
    if(continue599e4?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[continue599e4?.refresh])
  ...
  const handleClick=async()=>{
    try{  
      ...
      setoverallgroup05ff6((prev: any) => ({ ...prev, continue: true }));
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
      setoverallgroup05ff6((prev: any) => ({ ...prev, continue: false }));
 ...
 if (continue599e4?.isHidden) {
    return <></>
  }

  return (
    <div
      style={{gridColumn: `19 / 23`,gridRow: `57 / 68`, gap:``, height: `100%`, overflow: 'auto'}} 
      >
        {showFlag && <Button 
          ref={buttonRef}
          ...
          onClick={handleClick}
          view='action'
          disabled= {continue599e4?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
        >
          {keyset("Continue")}
