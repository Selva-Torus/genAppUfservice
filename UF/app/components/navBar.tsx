...
const Navbar = ({ data }: any) => {
  ...
  const {accessProfile, setAccessProfile, encAppFalg} = useContext(TotalContext) as TotalContextProps
  ...
  const getScreenAccess = async () => {
    ...
    data.map((group:any)=>{
      ...
      group?.screenDetails?.map((screens:any)=>{
        screenDetails.push({ screenName: screens?.name, ufKey: screens?.key,version:screens?.key?.split(":").at(-1) })
      })
    ...
    data[0].screenDetails.map((value: any, index: any) => {
      screenData.push({ screenName: value.name, ufKey: value.key,version:value.key.split(":").at(-1) })
    })
    let allNames:any=[]
    ...
    for (let i = 0; i < allScreenDetails.length; i++) {
      ...
      for(let j=0;j<allScreenDetails[i]?.screenDetails.length;j++)
      {
        ...
        try {
          ...
          const orchestrationData = await AxiosService.post("/UF/Orchestration",orchestrationDataBody,{
            headers: {
            ...
            if (security == 'AA') {
              screenDetails.push({
                action: () => routes.push('/' + specificGroup[j].screenName+"_"+specificGroup[j].version),
