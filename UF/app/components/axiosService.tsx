...
AxiosService.interceptors.request.use(
  async (config) => {
     if (config.data && ['post', 'put', 'patch'].includes(config.method || '') && config.data.dpdKey && config.data.method ) {
      ...
      let ciphertext : any;
      if(method == "vault"){
        const encrypt = { Credentials: encryptionData.credentials, value: config.data, context: "tt407_cgfa_tg4cgfa_v1" }
        const vaultEncrypt = await localEncrypt(encrypt)
        ciphertext = await encryptData(vaultEncrypt)
      }else{
        ciphertext = await clientEncrypt(encryptionData.credentials,config.data,"tt407_cgfa_tg4cgfa_v1")
      }
      if(method == "AESGCM"){
  ...
  async(response:any) => {
    if ( response.data.dpdKey && response.data.method ) {
      ...
      if(method == "vault"){
        ...
        response.data = await clientDecrypt(encryptionData.credentials,response.data,"tt407_cgfa_tg4cgfa_v1")
      }
    }
    return response
  },
