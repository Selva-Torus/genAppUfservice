...
const CardActive_APIs = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  const handleMapperDetails=async()=>{
    ...
    let orchestrationDataBody:any = {
      ...
      controlId:"a998bc9b77c945e4895126025c219760",
      isTable:false,
      accessProfile:accessProfile,
      from:"cardActive APIs",
    }
    ...
    code = orchestrationData?.data?.code
      ...
      if(Array.isArray(orchestrationData?.data?.dstData)&&orchestrationData?.data?.dstData?.length>0){
        setAPI_Report360bc((pre:any)=>({...pre,active_apis:orchestrationData?.data?.dstData[0]?.active_apis}))
      } 
    }catch(err){
      console.log(err)
    }
  ...
  useEffect(() => {
    handleMapperDetails()
        setDisable((prev: any) => ({ ...prev, cardActive_APIs19760:false }));
  }, [refresh.cardActive_APIs19760])
  ...
  if (hide.cardActive_APIs19760) {
    return <></>
  }  
  return (
    <div className="col-start-4 col-end-7 row-start-2 row-end-2 gap-10px" >
      <TorusCard 
      style={style}
      className="rounded-lg shadow-lg p-6 bg-white"
      disabled={disable.cardActive_APIs19760 || false}
      >
      ...
      {API_Report360bc?.active_apis?API_Report360bc?.active_apis:"0"}
      </Text>
    </div>
      </TorusCard>
    </div>
