...
const Carderror = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  const handleMapperDetails=async()=>{
    ...
    let orchestrationDataBody:any = {
      ...
      controlId:"530bec9565af48ca9617cb55ea1bc458",
      isTable:false,
    ...
    code = orchestrationData?.data?.code
      ...
      if(Array.isArray(orchestrationData?.data?.dstData)&&orchestrationData?.data?.dstData?.length>0){
        setAPI_Report360bc((pre:any)=>({...pre,error:orchestrationData?.data?.dstData[0]?.error}))
      } 
    }catch(err){
      console.log(err)
    }
  ...
  useEffect(() => {
    handleMapperDetails()
        setDisable((prev: any) => ({ ...prev, carderrorbc458:false }));
  }, [refresh.carderrorbc458])
  ...
  if (hide.carderrorbc458) {
    return <></>
  }  
  return (
    <div className="col-start-10 col-end-13 row-start-2 row-end-2 gap-10px" >
      <TorusCard 
      style={style}
      className="rounded-lg shadow-lg p-6 bg-white"
      disabled={disable.carderrorbc458 || false}
      >
      ...
      {API_Report360bc?.error?API_Report360bc?.error:"0"}
      </Text>
    </div>
      </TorusCard>
    </div>
