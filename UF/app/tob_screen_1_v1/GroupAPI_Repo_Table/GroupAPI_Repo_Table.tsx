...
import TableAPI_Repo_Table  from './TableAPI_Repo_Table';  
import TextAPI_Repository  from "./TextAPI_Repository";
import { useInfoMsg } from "@/app/components/infoMsgHandler";
import { getCookie } from "@/app/components/cookieMgment"
import { TotalContext, TotalContextProps } from '@/app/globalContext';


const GroupApiRepoTable = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData}:any) => {
  ...
  async function securityCheck() {
    let orchestrationBody:any ={
      key:"CK:CT242:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:TOB_Dashboard_Screen:AFVK:v1",componentId:"3dde18f1597841a992060519b608836e",
      from:"GroupApiRepoTable",
isTable : true,
accessProfile:accessProfile
  ...
  useEffect(() => {    
    securityCheck()   
    handleOnload()
  }, [refresh?.groupAPI_Repo_Table8836e])
  return (
    <div className="col-start-1 col-end-7 row-start-6 row-end-9 gap-10px border border-slate-300 p-2 rounded-md groupStyle">
      <Grid containerClass='grid grid-cols-12 gap-2 '>
        {allowedComponent.includes("api_repo_table")|| allowedComponent.includes("ALL") ? <TableAPI_Repo_Table lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
          {allowedComponent.includes("api_repository")|| allowedComponent.includes("ALL")  ? <TextAPI_Repository   /* 09e8a */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} /> : <div></div>}
      </Grid>
    </div>             
  )
}

export default GroupApiRepoTable