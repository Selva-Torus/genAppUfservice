...
const TableAPI_Repo_Table=({ lockedData,setLockedData,primaryTableData, setPrimaryTableData,refetch, setRefetch,setData,encryptionFlagCompData }: any)=>{
...
//another screen
  ...
  const handleUpdate: PaginationProps['onUpdate'] = (page, pageSize) =>{
    setAPI_Repo_Table8836eContainValidataion((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setAPI_Repo_Table8836eContainValidataion((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page = 1, pageSize = 10, searchParams = {},dfKey:any,isRulePresent:any=false) {
    ...
    try {
      ...
      if (isRulePresent==false) {
        ...
        const api_paginationBody: api_paginationDto = {
          ...
          filterDetails: {
            ...
            nodeId: '3dde18f1597841a992060519b608836e',
            elementId: '3dde18f1597841a992060519b608836e'
          },
          searchFilter: searchParams
        }
        if(encryptionFlagCont) {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        ...
        setAPI_Repo_Table8836e(uf_paginationDataFilter.data||[])
  ...
  useEffect(() => {
    GetTableDetails()
  }, [refresh?.tableAPI_Repo_Table8836e])
  ...
  const handlePrimaryTable = () => {
    let findData = isAPI_Repo_Table8836eContainValidataion.selectedIds[isAPI_Repo_Table8836eContainValidataion.selectedIds.length-1]
    if(Array.isArray(API_Repo_Table8836e) && API_Repo_Table8836e.length>0)
    {
      let data = API_Repo_Table8836e[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "id",
        value: data["id"],
  ...
  useEffect(() => {
    if (isAPI_Repo_Table8836eContainValidataion.selectedIds.length != 0) handlePrimaryTable()
  }, [isAPI_Repo_Table8836eContainValidataion.selectedIds])
  ...
  if (hide.tableAPI_Repo_Table8836e) {
    return <></>
  }
  return(
        ...
        <Col>
          <Flex direction='column' >
            <MyTable
              ...
              columns={columns}
              edgePadding={true}
              selectedIds={isAPI_Repo_Table8836eContainValidataion.selectedIds}  
...
export default TableAPI_Repo_Table