...
const TableConnected_App_Table=({ lockedData,setLockedData,primaryTableData, setPrimaryTableData,refetch, setRefetch,setData,encryptionFlagCompData }: any)=>{
...
//another screen
  ...
  // for all data without pagaination
    ...
    try {
      ...
      if (isRulePresent==false) {
        ...
        const api_paginationBody: api_paginationDto = {
          ...
          count: pageSize,
          filterDetails: {
            ufKey:'CK:CT242:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:TOB_Dashboard_Screen:AFVK:v1:UO', 
            nodeId: '4bf572e7f63547d59e62cdd358ccff73',
            elementId: '4bf572e7f63547d59e62cdd358ccff73'
          },
          searchFilter: searchParams
        }
        if(encryptionFlagCont) {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        ...
        setConnected_App_Tablecff73(uf_paginationDataFilter.data||[])
  ...
  useEffect(() => {
    GetTableDetails()
  }, [refresh?.tableConnected_App_Tablecff73])

  useEffect(() => {
      if(DFkeyAndRule?.dfKey!=""){
        fetchData(0, 0,{},DFkeyAndRule,DFkeyAndRule?.isRulePresent)
      }
    setConnected_App_Tablecff73ContainValidataion((pre:any)=>({...pre, selectedIds:[]}))
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [refetch])

  const handlePrimaryTable = () => {
    let findData = isConnected_App_Tablecff73ContainValidataion.selectedIds[isConnected_App_Tablecff73ContainValidataion.selectedIds.length-1]
    if(Array.isArray(Connected_App_Tablecff73) && Connected_App_Tablecff73.length>0)
    {
      let data = Connected_App_Tablecff73[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "id",
        value: data["id"],
  ...
  useEffect(() => {
    if (isConnected_App_Tablecff73ContainValidataion.selectedIds.length != 0) handlePrimaryTable()
  }, [isConnected_App_Tablecff73ContainValidataion.selectedIds])
  ...
  if (hide.tableConnected_App_Tablecff73) {
    return <></>
  }
  return(
        ...
        <Col>
          <Flex direction='column' >
            <MyTable
              ...
              columns={columns}
              edgePadding={true}
              selectedIds={isConnected_App_Tablecff73ContainValidataion.selectedIds}  
...
export default TableConnected_App_Table