...
const TextInputtax_payers_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_View_Details:AFVK:v1|cf8c6e9383ea49e7be4639ad0e373f21|2787045a518f4bba90245bd199138781"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,ITAX_View_Details_v1:{...pre?.ITAX_View_Details_v1,tax_payer_full_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setview_detail_group73f21((prev: any) => ({ ...prev, tax_payer_full_name: +e.target.value }));
    }
    else{
    setview_detail_group73f21((prev: any) => ({ ...prev, tax_payer_full_name: e.target.value }));
    }
    ...
    let code:string=allCode;
     if (code != '') {
      ...
      codeStates['view_detail_group']  = {...view_detail_group73f21,tax_payer_full_name:newInputValue},
      codeStates['setview_detail_group'] = setview_detail_group73f21,
      codeStates['prn_no_datails_table']  = {...prn_no_datails_tablefc106,tax_payer_full_name:newInputValue},
  ...
  if(dfd_itax_source_tran_dfd_v1Props?.setSearchFilters && dfd_itax_source_tran_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_itax_source_tran_dfd_v1Props.data) && dfd_itax_source_tran_dfd_v1Props.data.length > 0){
      setview_detail_group73f21((pre:any)=>({...pre,tax_payer_full_name:dfd_itax_source_tran_dfd_v1Props.data[0]?.tax_payer_full_name}));
    }
  }
  ...
  return (   
    <div  
      ...
      <TextInput
        ...
        value={view_detail_group73f21?.tax_payer_full_name||""}
        ...
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.ITAX_View_Details_v1?.tax_payer_full_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
