...
const Texteslip_no = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      if ("hasLogicCenter" in dfd_itax_source_tran_dfd_v1Props && !dfd_itax_source_tran_dfd_v1Props.hasLogicCenter) {
        ...
        setview_detail_group73f21((pre: any) => {
          return { ...pre, eslip_no: api_paginationData.data.records[0]?.eslip_no }
        })
        }
      }
      else{
      if(Array.isArray(dfd_itax_source_tran_dfd_v1Props) && dfd_itax_source_tran_dfd_v1Props && !view_detail_group73f21.eslip_no){
        setview_detail_group73f21((pre:any)=>({...pre,eslip_no:dfd_itax_source_tran_dfd_v1Props[0]?.eslip_no}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[eslip_no1e386?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_itax_source_tran_dfd_v1Props) && !view_detail_group73f21.eslip_no){
    setview_detail_group73f21((pre:any)=>({...pre,eslip_no:dfd_itax_source_tran_dfd_v1Props[0]?.eslip_no}));
  }
  },[dfd_itax_source_tran_dfd_v1Props])

  if (eslip_no1e386?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `2 / 10`,gridRow: `1 / 11`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className=""
  variant="subheader-3"
  color="primary"
>
      {keyset(isDynamic ? item?.eslip_no : (view_detail_group73f21?.eslip_no || ""))}
</Text>
  </div>
  )
}

export default Texteslip_no
