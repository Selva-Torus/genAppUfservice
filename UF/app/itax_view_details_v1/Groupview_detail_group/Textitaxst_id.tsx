...
const Textitaxst_id = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_itax_source_tran_dfd_v1Props) && dfd_itax_source_tran_dfd_v1Props && !view_detail_group73f21.itaxst_id){
        setview_detail_group73f21((pre:any)=>({...pre,itaxst_id:dfd_itax_source_tran_dfd_v1Props[0]?.itaxst_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(() => {
  if(Array.isArray(dfd_itax_source_tran_dfd_v1Props) && !view_detail_group73f21.itaxst_id){
    setview_detail_group73f21((pre:any)=>({...pre,itaxst_id:dfd_itax_source_tran_dfd_v1Props[0]?.itaxst_id}));
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.itaxst_id : (view_detail_group73f21?.itaxst_id || ""))}
</Text>
  </div>
  )
}
