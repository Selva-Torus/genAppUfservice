...
import Groupview_detail_group  from "./Groupview_detail_group/Groupview_detail_group";


export default function PageItaxViewDetailsV1() {
  ...
  const [checkview_detail_group,setCheckview_detail_group,]=useState<boolean>(false);
  const [checkprn_no_datails_table,setCheckprn_no_datails_table,]=useState<boolean>(false);
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function itax_source_tran_dfd_v1(pagination:any): Promise<void>{
        ...
        if(itax_view_details_v1Props.length > 0){
          let filterData :any[] =[];
          for(let i=0;i< itax_view_details_v1Props.length;i++){
            if(itax_view_details_v1Props[i].DFDkey == "CK:CT010:FNGK:AF:FNK:DF-DFD:CATK:I001:AFGK:ITAX:AFK:ITAX_Source_Tran_DFD:AFVK:v1"){
              delete itax_view_details_v1Props[i].DFDkey;
              filterData.push(itax_view_details_v1Props[i])
            }           
          }
  ...
  useEffect(()=>{
    ...
    async function itax_source_tran_dtl_dfd_v1(pagination:any): Promise<void>{
        ...
        if(itax_view_details_v1Props.length > 0){
          let filterData :any[] =[];
          for(let i=0;i< itax_view_details_v1Props.length;i++){
            if(itax_view_details_v1Props[i].DFDkey == "CK:CT010:FNGK:AF:FNK:DF-DFD:CATK:I001:AFGK:ITAX:AFK:ITAX_Source_Tran_Dtl_DFD:AFVK:v1"){
              delete itax_view_details_v1Props[i].DFDkey;
              filterData.push(itax_view_details_v1Props[i])
            }           
          }
  ...
  async function securityCheck(): Promise<void> {
    const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_View_Details:AFVK:v1",accessProfile:[user],from:"pageItaxViewDetailsV1"},{
      headers: {
        Authorization: `Bearer ${token}`
      }});
  ...
  "events": {}
    ...
    await itax_source_tran_dtl_dfd_v1(pagination)
          ...
          allowedGroup.map((nodes:AllowedGroupNode)=>{
            ...
            if(nodes?.groupName == 'view_detail_group' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setCheckview_detail_group(true)
            }
            if(nodes?.groupName == 'prn_no_datails_table' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setCheckprn_no_datails_table(true)
            }
          })
          }
           }catch(err:any)
    ...
    <>
     ...
     style={{
      ...
      {isProcessing && (
        ...
        {checkview_detail_group && initialLoad &&<Groupview_detail_group
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          primaryTableData={primaryTableData}
          setPrimaryTableData={setPrimaryTableData}
