...
const Tableprn_no_datails_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing}: any)=>{
 ...
 /////////////
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setprn_no_datails_tablefc106Props((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setprn_no_datails_tablefc106Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["itaxst_id"]:prn_no_datails_tablefc106Props?.presetValues?.itaxst_id}
 
    let dstKey=dfKey?.dfKey
    dstKey=dstKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    try {
    if(!prn_no_datails_tablefc106Props?.presetValues || Object.keys(prn_no_datails_tablefc106Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_View_Details:AFVK:v1:UO', 
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setprn_no_datails_tablefc106(uf_paginationDataFilter.data||[])
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(prn_no_datails_tablefc106))
    {
      GetTableDetails()
    }
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(prn_no_datails_tablefc106Props.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [prn_no_datails_tablefc106Props.filterProps])
  ...
  useEffect(() => {
    if (prn_no_datails_tablefc106Props?.clearData === true) {
      setprn_no_datails_tablefc106([]);
      setAllDataObject([]);
      setAllData([]);
      setSelectedPaginationData([]);
      setLockedData((pre:any) => ({...pre, data:[]}));
      setprn_no_datails_tablefc106Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [prn_no_datails_tablefc106Props?.clearData])
  useEffect(() => {
    setprn_no_datails_tablefc106([])
    setprn_no_datails_tablefc106Props((pre:any)=>({...pre, selectedIds:[]}))
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [prn_no_datails_tablefc106Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = prn_no_datails_tablefc106Props?.selectedIds[prn_no_datails_tablefc106Props?.selectedIds?.length-1]
    if(Array.isArray(prn_no_datails_tablefc106) && prn_no_datails_tablefc106.length>0)
    {
      let data = prn_no_datails_tablefc106[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "itaxstd_id",
  ...
  useEffect(() => {
    if (prn_no_datails_tablefc106Props?.selectedIds?.length != 0) handlePrimaryTable()
  }, [prn_no_datails_tablefc106Props?.selectedIds])
  ...
  if (prn_no_datails_tablefc106?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div className=' w-full h-full flex flex-row'>
            <Table
              ...
              edgePadding={true}
              selectedIds={prn_no_datails_tablefc106Props?.selectedIds}  
...
export default Tableprn_no_datails_table
