...
const Cardactive_msg = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  const handleMapperDetails=async()=>{
    ...
    try{
      ...
      setmaindashboard_cards0d32d((pre:any)=>({...pre,active_msg:dfd_mongo_maindashboard_v1Props[0]?.active_msg}));
  }
    }catch(err){
      console.log(err)
    }
...
useEffect(() => {
  ...
  if (active_msg28606?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `4 / 7`,gridRow: `1 / 42`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Card 
      style={style}
      className=""      
      size="m"
      view="filled"
      disabled= {active_msg28606?.isDisabled ? true : false}
      onClick={handleClick}  
      >
...
{maindashboard_cards0d32d?.active_msg?maindashboard_cards0d32d?.active_msg:"0"}
      </Text>
    </div>
      </Card>
    </div>
