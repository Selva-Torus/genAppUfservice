...
const Cardmost_used_msg = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  const handleMapperDetails=async()=>{
    ...
    try{
      ...
      setmaindashboard_cards0d32d((pre:any)=>({...pre,most_used_msg:dfd_mongo_maindashboard_v1Props[0]?.most_used_msg}));
  }
    }catch(err){
      console.log(err)
    }
...
useEffect(() => {
  ...
  if (most_used_msgfc11a?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `1 / 4`,gridRow: `1 / 42`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Card 
      style={style}
      className=""      
      size="m"
      view="filled"
      disabled= {most_used_msgfc11a?.isDisabled ? true : false}
      onClick={handleClick}  
      >
...
{maindashboard_cards0d32d?.most_used_msg?maindashboard_cards0d32d?.most_used_msg:"0"}
      </Text>
    </div>
      </Card>
    </div>
...
export default Cardmost_used_msg
