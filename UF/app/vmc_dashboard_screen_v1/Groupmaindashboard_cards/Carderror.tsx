...
const Carderror = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  const handleMapperDetails=async()=>{
    try{
    let code:any;
    const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT261:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:VMC_Dashboard_Screen:AFVK:v1",  componentId:"21672813eca346948142a314edb0d32d",controlId:"09205bbb770c4b1f8ce71506b23403d0",isTable:false,accessProfile:accessProfile,from:"cardError"},{
      headers: {
    ...
    try{
      ...
      setmaindashboard_cards0d32d((pre:any)=>({...pre,error:dfd_mongo_maindashboard_v1Props[0]?.error}));
  }
    }catch(err){
      console.log(err)
    }
...
useEffect(() => {
  ...
  if (error403d0?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `10 / 13`,gridRow: `1 / 42`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Card 
      style={style}
      className=""      
      size="m"
      view="filled"
      disabled= {error403d0?.isDisabled ? true : false}
      onClick={handleClick}  
      >
...
{maindashboard_cards0d32d?.error?maindashboard_cards0d32d?.error:"0"}
      </Text>
    </div>
      </Card>
    </div>
