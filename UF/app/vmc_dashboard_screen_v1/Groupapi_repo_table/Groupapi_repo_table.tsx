...
import Groupapi_repositorys  from "../Groupapi_repositorys/Groupapi_repositorys";
...
const Groupapi_repo_table = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT261:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:VMC_Dashboard_Screen:AFVK:v1",componentId:"e6cf2b3cbe00478c8c495a8ad2883529",from:"GroupApiRepoTable",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("api_repositorys")){
      setapi_repositorysb8178({...api_repositorysb8178,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const api_repo_table83529Ref = useRef<any>(null);
  const handleClearSearch = () => {
    api_repo_table83529Ref.current?.setSearchParams();
    api_repo_table83529Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(api_repo_table83529) && Object.keys(api_repo_table83529)?.length>0)
      {
        setapi_repo_table83529({})
      }
    }else 
      prevRefreshRef.current= true
  }, [api_repo_table83529Props?.refresh])
  ...
  return (
    ...
    >
        {allowedComponent.includes("api_repositorys")  &&<Groupapi_repositorys  
