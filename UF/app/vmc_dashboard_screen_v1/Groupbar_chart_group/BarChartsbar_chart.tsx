...
export default function BarChartsbar_chart({ encryptionFlagCompData }: any) {
  ...
  const handleMapperDetails=async()=>{
    ...
    '/UF/Orchestration',
      {
        ...
        componentId: "461923a0e17f4aa0a4a218b629893773",
        controlId: "5bd2043d79ef448d84f1d0093666e196",
        isTable: false,
        accessProfile:accessProfile,
        from:"checkboxBar Chart"
      },
      {
        headers: {
    ...
    let code:any= orchestrationData?.data?.code;
      ...
      if(Array.isArray(dfd_mongo_bar_chart_v1Props) && dfd_mongo_bar_chart_v1Props?.length > 0){
        setData(dfd_mongo_bar_chart_v1Props)
        setbar_chart_group93773((pre:any)=>({...pre,name:dfd_mongo_bar_chart_v1Props[0]?.name}))
      }
      if(Array.isArray(dfd_mongo_bar_chart_v1Props)){
        return
      }
    }catch(err){
      console.log(err)
  ...
  const colors = [
    ...
    '#00BCD4'
      ...
      title  = "Message Calls Over Hours"
  if (bar_chart6e196?.isHidden) {
    return <></>
  }
  return (
    <div 
      style={{gridColumn: `1 / 13`,gridRow: `1 / 33`, gap:``, height: `100%`, overflow: 'auto'}} >
