...
import Tableprn_no_details_table  from './Tableprn_no_details_table';  
...
const Groupprn_no_details_table = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_PRN_Approval_Details:AFVK:v1",componentId:"1cae4dcf33404c0ca0a1036d82dfc106",from:"GroupPrnNoDetailsTable",isTable : true,accessProfile:accessProfile},{
    headers: {
  ...
  const prn_no_details_tablefc106Ref = useRef<any>(null);
  const handleClearSearch = () => {
    prn_no_details_tablefc106Ref.current?.setSearchParams();
    prn_no_details_tablefc106Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(prn_no_details_tablefc106) && Object.keys(prn_no_details_tablefc106)?.length>0)
      {
        setprn_no_details_tablefc106({})
      }
    }else 
      prevRefreshRef.current= true
  }, [prn_no_details_tablefc106Props?.refresh,token])
  ...
  return (
    ...
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<Tableprn_no_details_table headerButtonsRenders={renderBUttons}
        lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={prn_no_details_tablefc106Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing}/>}
      </div>
      </div>
    </div>
 )
...
export default Groupprn_no_details_table
