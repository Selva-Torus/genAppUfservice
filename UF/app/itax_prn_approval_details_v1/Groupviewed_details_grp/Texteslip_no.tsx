...
const Texteslip_no = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_itax_source_tran_dfd_v1Props) && dfd_itax_source_tran_dfd_v1Props && !viewed_details_grp73f21.eslip_no){
        setviewed_details_grp73f21((pre:any)=>({...pre,eslip_no:dfd_itax_source_tran_dfd_v1Props[0]?.eslip_no}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(() => {
  if(Array.isArray(dfd_itax_source_tran_dfd_v1Props) && !viewed_details_grp73f21.eslip_no){
    setviewed_details_grp73f21((pre:any)=>({...pre,eslip_no:dfd_itax_source_tran_dfd_v1Props[0]?.eslip_no}));
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.eslip_no : (viewed_details_grp73f21?.eslip_no || ""))}
</Text>
  </div>
  )
}
