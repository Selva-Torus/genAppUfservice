...
const TextInputprn_registration_date = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT010:FNGK:AF:FNK:DF-DFD:CATK:I001:AFGK:ITAX:AFK:ITAX_Source_Tran_DFD:AFVK:v1|e94f0b0bfd204105bf68851e77eecc7b|properties.payment_advice_date"
      ],
      "targetKey": "CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_PRN_Approval_Details:AFVK:v1|cf8c6e9383ea49e7be4639ad0e373f21|fdaf4582fcb44ca0b672f50679567d15"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'payment_advice_date',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,ITAX_PRN_Approval_Details_v1:{...pre?.ITAX_PRN_Approval_Details_v1,payment_advice_date:undefined}}));
    if(dynamicStateandType.type=="number"){
    setviewed_details_grp73f21((prev: any) => ({ ...prev, payment_advice_date: +e.target.value }));
    }
    else{
    setviewed_details_grp73f21((prev: any) => ({ ...prev, payment_advice_date: e.target.value }));
    }
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['view_detail_back_group']  = {...view_detail_back_group50bce,payment_advice_date:newInputValue},
      codeStates['setview_detail_back_group'] = setview_detail_back_group50bce,
      codeStates['viewed_details_grp']  = {...viewed_details_grp73f21,payment_advice_date:newInputValue},
      codeStates['setviewed_details_grp'] = setviewed_details_grp73f21,
      codeStates['prn_no_details_table']  = {...prn_no_details_tablefc106,payment_advice_date:newInputValue},
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "fdaf4582fcb44ca0b672f50679567d15",
          isTable: false,
          from:"TextInputprn_registration_date",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'payment_advice_date', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'payment_advice_date',type:'text'};
      //   type={
      //     name:'payment_advice_date',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.payment_advice_date.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.payment_advice_date.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.payment_advice_date.type
      //   }
      ...
      //   type={
      //     name:'payment_advice_date',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.payment_advice_date.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.payment_advice_date.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.payment_advice_date.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  if(dfd_itax_source_tran_dfd_v1Props?.setSearchFilters && dfd_itax_source_tran_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_itax_source_tran_dfd_v1Props.data) && dfd_itax_source_tran_dfd_v1Props.data.length > 0){
      setviewed_details_grp73f21((pre:any)=>({...pre,payment_advice_date:dfd_itax_source_tran_dfd_v1Props.data[0]?.payment_advice_date}));
    }
  }
  },[dfd_itax_source_tran_dfd_v1Props?.setSearchFilters])
  if (prn_registration_date67d15?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `17 / 25`,gridRow: `57 / 67`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={viewed_details_grp73f21?.payment_advice_date||""}
        numberFormat={dynamicStateandType.type === "number" ? "" : "none"}
         disabled= {prn_registration_date67d15?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.ITAX_PRN_Approval_Details_v1?.payment_advice_date ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputprn_registration_date
