...
import Groupprn_no_details_table  from "../Groupprn_no_details_table/Groupprn_no_details_table";
...
const Groupviewed_details_grp = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_PRN_Approval_Details:AFVK:v1",componentId:"cf8c6e9383ea49e7be4639ad0e373f21",from:"GroupViewedDetailsGrp",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    ...
    if(orchestrationData?.data?.readableControls.includes("prn_no_details_table")){
      setprn_no_details_tablefc106({...prn_no_details_tablefc106,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const handleOnChange=()=>{
      // copyFormData for group
          setview_detail_back_group50bce((prev:any) => ({ ...prev, ...viewed_details_grp73f21 }));

  }
  const viewed_details_grp73f21Ref = useRef<any>(null);
  const handleClearSearch = () => {
    viewed_details_grp73f21Ref.current?.setSearchParams();
    viewed_details_grp73f21Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(viewed_details_grp73f21) && Object.keys(viewed_details_grp73f21)?.length>0)
      {
        setviewed_details_grp73f21({})
      }
    }else 
      prevRefreshRef.current= true
  }, [viewed_details_grp73f21Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("prn_no_details_table")  &&<Groupprn_no_details_table  
...
export default Groupviewed_details_grp
