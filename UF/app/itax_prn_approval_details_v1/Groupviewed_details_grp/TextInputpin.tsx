...
const TextInputpin = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT010:FNGK:AF:FNK:DF-DFD:CATK:I001:AFGK:ITAX:AFK:ITAX_Source_Tran_DFD:AFVK:v1|e94f0b0bfd204105bf68851e77eecc7b|properties.tax_payer_pin"
      ],
      "targetKey": "CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_PRN_Approval_Details:AFVK:v1|cf8c6e9383ea49e7be4639ad0e373f21|24415d0a28dd4a419b58a6a2e407c9eb"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'tax_payer_pin',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,ITAX_PRN_Approval_Details_v1:{...pre?.ITAX_PRN_Approval_Details_v1,tax_payer_pin:undefined}}));
    if(dynamicStateandType.type=="number"){
    setviewed_details_grp73f21((prev: any) => ({ ...prev, tax_payer_pin: +e.target.value }));
    }
    else{
    setviewed_details_grp73f21((prev: any) => ({ ...prev, tax_payer_pin: e.target.value }));
    }
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['view_detail_back_group']  = {...view_detail_back_group50bce,tax_payer_pin:newInputValue},
      codeStates['setview_detail_back_group'] = setview_detail_back_group50bce,
      codeStates['viewed_details_grp']  = {...viewed_details_grp73f21,tax_payer_pin:newInputValue},
      codeStates['setviewed_details_grp'] = setviewed_details_grp73f21,
      codeStates['prn_no_details_table']  = {...prn_no_details_tablefc106,tax_payer_pin:newInputValue},
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "24415d0a28dd4a419b58a6a2e407c9eb",
          isTable: false,
          from:"TextInputpin",
      ...
      //   type={
      //     name:'tax_payer_pin',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.tax_payer_pin.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.tax_payer_pin.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.tax_payer_pin.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  if(dfd_itax_source_tran_dfd_v1Props?.setSearchFilters && dfd_itax_source_tran_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_itax_source_tran_dfd_v1Props.data) && dfd_itax_source_tran_dfd_v1Props.data.length > 0){
      setviewed_details_grp73f21((pre:any)=>({...pre,tax_payer_pin:dfd_itax_source_tran_dfd_v1Props.data[0]?.tax_payer_pin}));
    }
  }
  },[dfd_itax_source_tran_dfd_v1Props?.setSearchFilters])
  if (pin7c9eb?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `9 / 17`,gridRow: `34 / 44`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={viewed_details_grp73f21?.tax_payer_pin||""}
        numberFormat={dynamicStateandType.type === "number" ? "" : "none"}
         disabled= {pin7c9eb?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.ITAX_PRN_Approval_Details_v1?.tax_payer_pin ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
