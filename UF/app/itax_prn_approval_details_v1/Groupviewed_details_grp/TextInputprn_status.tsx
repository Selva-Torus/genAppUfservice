...
const TextInputprn_status = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT010:FNGK:AF:FNK:DF-DFD:CATK:I001:AFGK:ITAX:AFK:ITAX_Source_Tran_DFD:AFVK:v1|e94f0b0bfd204105bf68851e77eecc7b|properties.trs_status"
      ],
      "targetKey": "CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_PRN_Approval_Details:AFVK:v1|cf8c6e9383ea49e7be4639ad0e373f21|4f9a4686d2f643deb4c4005410c83532"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'trs_status',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,ITAX_PRN_Approval_Details_v1:{...pre?.ITAX_PRN_Approval_Details_v1,trs_status:undefined}}));
    if(dynamicStateandType.type=="number"){
    setviewed_details_grp73f21((prev: any) => ({ ...prev, trs_status: +e.target.value }));
    }
    else{
    setviewed_details_grp73f21((prev: any) => ({ ...prev, trs_status: e.target.value }));
    }
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['view_detail_back_group']  = {...view_detail_back_group50bce,trs_status:newInputValue},
      codeStates['setview_detail_back_group'] = setview_detail_back_group50bce,
      codeStates['viewed_details_grp']  = {...viewed_details_grp73f21,trs_status:newInputValue},
      codeStates['setviewed_details_grp'] = setviewed_details_grp73f21,
      codeStates['prn_no_details_table']  = {...prn_no_details_tablefc106,trs_status:newInputValue},
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "4f9a4686d2f643deb4c4005410c83532",
          isTable: false,
          from:"TextInputprn_status",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'trs_status', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'trs_status',type:'text'};
      //   type={
      //     name:'trs_status',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.trs_status.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.trs_status.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.trs_status.type
      //   }
      ...
      //   type={
      //     name:'trs_status',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.trs_status.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.trs_status.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.trs_status.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  if(dfd_itax_source_tran_dfd_v1Props?.setSearchFilters && dfd_itax_source_tran_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_itax_source_tran_dfd_v1Props.data) && dfd_itax_source_tran_dfd_v1Props.data.length > 0){
      setviewed_details_grp73f21((pre:any)=>({...pre,trs_status:dfd_itax_source_tran_dfd_v1Props.data[0]?.trs_status}));
    }
  }
  },[dfd_itax_source_tran_dfd_v1Props?.setSearchFilters])
  if (prn_status83532?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 9`,gridRow: `34 / 44`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={viewed_details_grp73f21?.trs_status||""}
        numberFormat={dynamicStateandType.type === "number" ? "" : "none"}
         disabled= {prn_status83532?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.ITAX_PRN_Approval_Details_v1?.trs_status ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputprn_status
