...
const TextInputprn_amount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_PRN_Approval_Details:AFVK:v1|cf8c6e9383ea49e7be4639ad0e373f21|31dbaf7533d84cfdb7ededdfe8cd22c3"
    }
  ],
  "schemaData": {
    "type": "number"
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,ITAX_PRN_Approval_Details_v1:{...pre?.ITAX_PRN_Approval_Details_v1,total_amount:undefined}}));
    if(dynamicStateandType.type=="number"){
    setviewed_details_grp73f21((prev: any) => ({ ...prev, total_amount: +e.target.value }));
    }
    else{
    setviewed_details_grp73f21((prev: any) => ({ ...prev, total_amount: e.target.value }));
    }
    ...
    let code:string=allCode;
     if (code != '') {
      ...
      codeStates['viewed_details_grp']  = {...viewed_details_grp73f21,total_amount:newInputValue},
      codeStates['setviewed_details_grp'] = setviewed_details_grp73f21,
      codeStates['prn_no_details_table']  = {...prn_no_details_tablefc106,total_amount:newInputValue},
  ...
  if(dfd_itax_source_tran_dfd_v1Props?.setSearchFilters && dfd_itax_source_tran_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_itax_source_tran_dfd_v1Props.data) && dfd_itax_source_tran_dfd_v1Props.data.length > 0){
      setviewed_details_grp73f21((pre:any)=>({...pre,total_amount:dfd_itax_source_tran_dfd_v1Props.data[0]?.total_amount}));
    }
  }
  ...
  return (   
    <div  
      ...
      <TextInput
        ...
        value={viewed_details_grp73f21?.total_amount||""}
        ...
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.ITAX_PRN_Approval_Details_v1?.total_amount ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
