...
import Groupview_detail_back_group  from "./Groupview_detail_back_group/Groupview_detail_back_group";
import Groupviewed_details_grp  from "./Groupviewed_details_grp/Groupviewed_details_grp";


export default function PageItaxPrnApprovalDetailsV1() {
  ...
  const [checkview_detail_back_group,setCheckview_detail_back_group,]=useState<boolean>(false);
  const [checkviewed_details_grp,setCheckviewed_details_grp,]=useState<boolean>(false);
  const [checkprn_no_details_table,setCheckprn_no_details_table,]=useState<boolean>(false);
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function itax_source_tran_dfd_v1(pagination:any): Promise<void>{
        ...
        if(itax_prn_approval_details_v1Props.length > 0){
          let filterData :any[] =[];
          for(let i=0;i< itax_prn_approval_details_v1Props.length;i++){
            if(itax_prn_approval_details_v1Props[i].DFDkey == "CK:CT010:FNGK:AF:FNK:DF-DFD:CATK:I001:AFGK:ITAX:AFK:ITAX_Source_Tran_DFD:AFVK:v1"){
              delete itax_prn_approval_details_v1Props[i].DFDkey;
              filterData.push(itax_prn_approval_details_v1Props[i])
            }           
          }
  ...
  useEffect(()=>{
    ...
    async function itax_source_tran_dtl_dfd_v1(pagination:any): Promise<void>{
        ...
        if(itax_prn_approval_details_v1Props.length > 0){
          let filterData :any[] =[];
          for(let i=0;i< itax_prn_approval_details_v1Props.length;i++){
            if(itax_prn_approval_details_v1Props[i].DFDkey == "CK:CT010:FNGK:AF:FNK:DF-DFD:CATK:I001:AFGK:ITAX:AFK:ITAX_Source_Tran_Dtl_DFD:AFVK:v1"){
              delete itax_prn_approval_details_v1Props[i].DFDkey;
              filterData.push(itax_prn_approval_details_v1Props[i])
            }           
          }
  ...
  async function securityCheck(): Promise<void> {
    const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_PRN_Approval_Details:AFVK:v1",accessProfile:[user],from:"pageItaxPrnApprovalDetailsV1"},{
      headers: {
        Authorization: `Bearer ${token}`
      }});
  ...
  "events": {}
    ...
    await itax_source_tran_dtl_dfd_v1(pagination)
          ...
          allowedGroup.map((nodes:AllowedGroupNode)=>{
            if(nodes?.groupName == 'view_detail_back_group' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setCheckview_detail_back_group(true)
            }
            if(nodes?.groupName == 'viewed_details_grp' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setCheckviewed_details_grp(true)
            }
            if(nodes?.groupName == 'prn_no_details_table' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setCheckprn_no_details_table(true)
            }
          })
          }
           }catch(err:any)
    ...
    <>
     ...
     style={{
      ...
      {isProcessing && (
        ...
        {checkview_detail_back_group && initialLoad &&<Groupview_detail_back_group
        ...
        {checkviewed_details_grp && initialLoad &&<Groupviewed_details_grp
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          primaryTableData={primaryTableData}
          setPrimaryTableData={setPrimaryTableData}
