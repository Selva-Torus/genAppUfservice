...
const TextAreamessage = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'message',type:"string"})
 ...
 /////////////
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "a20fbd3ca61b46ff99b9ba8a5e447a32",
          isTable: false,
          accessProfile:accessProfile,
          from:"textareaTarget Msg"
        },
        {
          headers: {
      ...
      if(orchestrationData?.data?.schemaData){
        ...
        let type:any={name:'message',type:'text'}
      ...
      if(orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties){
        ...
        type={
          name:'message',
          type: orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.message.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.message.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.message.type
        }
        setDynamicStateandType(type)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[message47a32?.refresh])
  
  useEffect(()=>{
    if (prevRefreshRef.current) {
      setviewmsg_groupf2810((pre:any)=>({...pre,message:""}))
    }else 
  ...
  const handleChange = async(e: any) => {
    setviewmsg_groupf2810((prev: any) => ({ ...prev, message: e?.target?.value }))
  }
  const handleFocus=async(e:any)=>{
  }
  if (message47a32?.isHidden) {
    return <></>
  }
return (
  <div 
  style={{gridColumn: `7 / 13`,gridRow: `1 / 120`, gap:``, height: `100%`, overflow: 'auto'}} >
    <TextArea
      ...
      onBlur={handleBlur}
      disabled= {message47a32?.isDisabled ? true : false}
      minRows = {20}
      maxRows = {30}
      placeholder = {'type here...'}
      size = {'m'}
      pin = {'brick-brick'}
      value = { viewmsg_groupf2810?.message != null && typeof viewmsg_groupf2810?.message =='object' ? Object.keys(viewmsg_groupf2810?.message)?.length ?  JSON.stringify(viewmsg_groupf2810?.message,null ,2):"" : viewmsg_groupf2810?.message||""}
    />
  </div>
  )
}
