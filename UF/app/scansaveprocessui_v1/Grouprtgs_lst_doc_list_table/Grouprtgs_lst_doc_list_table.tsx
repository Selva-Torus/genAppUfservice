...
import Tablertgs_lst_doc_list_table  from './Tablertgs_lst_doc_list_table';  
...
const Grouprtgs_lst_doc_list_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "c7a357981ebb493eba732ca11d1e57bb");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrtgs_lst_doc_list_tablee57bbProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("file_name_rtgs_list")){
      setfile_name_rtgs_list61a4b({...file_name_rtgs_list61a4b,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("action_rtgs_list")){
      setaction_rtgs_list543ba({...action_rtgs_list543ba,isDisabled:true});
    }
  //////////////
  }
  ...
  const rtgs_lst_doc_list_tablee57bbRef = useRef<any>(null);
  const handleClearSearch = () => {
    rtgs_lst_doc_list_tablee57bbRef.current?.setSearchParams();
    rtgs_lst_doc_list_tablee57bbRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rtgs_lst_doc_list_tablee57bb) && Object.keys(rtgs_lst_doc_list_tablee57bb)?.length>0)
      {
        setrtgs_lst_doc_list_tablee57bb({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rtgs_lst_doc_list_tablee57bbProps?.refresh,token])
  ...
  return (
    ...
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<Tablertgs_lst_doc_list_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={rtgs_lst_doc_list_tablee57bbRef} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
...
export default Grouprtgs_lst_doc_list_table
