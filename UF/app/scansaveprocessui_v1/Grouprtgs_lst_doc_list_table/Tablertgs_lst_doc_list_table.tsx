 ...
 /////////////
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["vgphsts_id"]:rtgs_lst_doc_list_tablee57bbProps?.presetValues?.vgphsts_id}
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:scanSaveProcessUi:AFVK:v1:UO', 
...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    async function handleSecurity(controller: any = '') {
      ...
      }else if (controller in scansaveprocessui_v1?.rtgs_lst_doc_list_table && scansaveprocessui_v1?.rtgs_lst_doc_list_table[controller]?.itsHaveArtifact== true)
      {
        if(scansaveprocessui_v1?.rtgs_lst_doc_list_table[controller])
        {
          let result :any = await getAftfactLevelRule(scansaveprocessui_v1._artfactPFRule_,{...decodedTokenObj,session:decodedTokenObj,rtgs_lst_doc_list_table:filteredData},{rtgs_lst_doc_list_table:scansaveprocessui_v1?.rtgs_lst_doc_list_table})
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(rtgs_lst_doc_list_tablee57bb))
    {
      GetTableDetails()
    }
