...
import Tablertgs_list_table  from './Tablertgs_list_table';  
...
const Grouprtgs_list_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "38ff05d8ffd64d0d8767f1834437b8d6");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrtgs_list_table7b8d6Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("tran_id")){
      settran_idb50e3({...tran_idb50e3,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("dr_acnt_no")){
      setdr_acnt_no4ba0e({...dr_acnt_no4ba0e,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("cr_acnt_no")){
      setcr_acnt_nobfce7({...cr_acnt_nobfce7,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("amnt")){
      setamnt3f6e2({...amnt3f6e2,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("cr_bank_code")){
      setcr_bank_codee3623({...cr_bank_codee3623,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("created_by")){
      setcreated_byd32da({...created_byd32da,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("created_date")){
      setcreated_datee821e({...created_datee821e,isDisabled:true});
    }
  //////////////
  }
  ...
  const rtgs_list_table7b8d6Ref = useRef<any>(null);
  const handleClearSearch = () => {
    rtgs_list_table7b8d6Ref.current?.setSearchParams();
    rtgs_list_table7b8d6Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rtgs_list_table7b8d6) && Object.keys(rtgs_list_table7b8d6)?.length>0)
      {
        setrtgs_list_table7b8d6({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rtgs_list_table7b8d6Props?.refresh,token])
  ...
  return (
    ...
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<Tablertgs_list_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={rtgs_list_table7b8d6Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
...
export default Grouprtgs_list_table
