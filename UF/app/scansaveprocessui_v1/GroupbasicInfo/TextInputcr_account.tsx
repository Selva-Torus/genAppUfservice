...
const TextInputcr_account = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,cr_account:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasicinfo3d198((prev: any) => ({ ...prev, cr_account: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(basicinfo3d198?.cr_account == "" || basicinfo3d198?.cr_account == undefined){
      basicinfo3d198.cr_account = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, basicinfo3d198?.cr_account);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,cr_account:"invalid"}}));
        }
    }else if(basicinfo3d198?.cr_account !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +basicinfo3d198?.cr_account);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "578d127f6fe44253be91fd1230aa818b"
      );
      ...
      //     controlId: "578d127f6fe44253be91fd1230aa818b",
      //     isTable: false,
  ...
  return (   
    <div  
      ...
      <TextInput
        ...
        value={basicinfo3d198?.cr_account||""}
         disabled= {cr_accounta818b?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
      ...
      errorMessage={error}
        validationState={validate?.scanSaveProcessUi_v1?.cr_account ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
