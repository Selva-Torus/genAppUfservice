...
const TextInputexchange_rate = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:scanSaveProcessUi:AFVK:v1|409b134cde0449b5a031a7686df3d198|54d2007379c34ae88098ab09cbb88caf"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,exchange_rate:undefined}}));
  ...
  const handleValidate=async (e?:any) => {
     ...
     if(dynamicStateandType.type=="number"){
        ...
        if(!validate.success){
          ...
          setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,exchange_rate:"invalid"}}));
        }
  ...
  return (   
    <div  
      style={{gridColumn: `13 / 19`,gridRow: `26 / 41`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={basicinfo3d198?.exchange_rate||""}
         disabled= {exchange_rate88caf?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
      ...
      errorMessage={error}
        validationState={validate?.scanSaveProcessUi_v1?.exchange_rate ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
