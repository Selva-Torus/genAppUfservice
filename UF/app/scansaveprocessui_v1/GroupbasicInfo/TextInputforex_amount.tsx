...
const TextInputforex_amount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'forex_amount',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,forex_amount:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasicinfo3d198((prev: any) => ({ ...prev, forex_amount: +e.target.value }));
    }
    else{
    setbasicinfo3d198((prev: any) => ({ ...prev, forex_amount: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "033d10c820374a4fa40de84cfeca58a5"
      );
      ...
      //     controlId: "033d10c820374a4fa40de84cfeca58a5",
      //     isTable: false,
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'forex_amount', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'forex_amount',type:'text'};
      //   type={
      //     name:'forex_amount',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.forex_amount.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.forex_amount.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.forex_amount.type
      //   }
      ...
      //   type={
      //     name:'forex_amount',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.forex_amount.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.forex_amount.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.forex_amount.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  if (forex_amounta58a5?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 7`,gridRow: `42 / 57`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={basicinfo3d198?.forex_amount||""}
         disabled= {forex_amounta58a5?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
      ...
      errorMessage={error}
        validationState={validate?.scanSaveProcessUi_v1?.forex_amount ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
