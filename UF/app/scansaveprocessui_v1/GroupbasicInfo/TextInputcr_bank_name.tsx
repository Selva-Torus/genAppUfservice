...
const TextInputcr_bank_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,cr_bank_name:undefined}}));
  ...
  const handleValidate=async (e?:any) => {
     ...
     if(dynamicStateandType.type=="number"){
        ...
        if(!validate.success){
          ...
          setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,cr_bank_name:"invalid"}}));
        }
  ...
  return (   
    <div  
      style={{gridColumn: `19 / 25`,gridRow: `10 / 25`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      errorMessage={error}
        validationState={validate?.scanSaveProcessUi_v1?.cr_bank_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
