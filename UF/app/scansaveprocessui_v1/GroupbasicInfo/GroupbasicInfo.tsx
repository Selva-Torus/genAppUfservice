...
import Textbasic_info  from "./Textbasic_info";
import Checkboxwaive_charges  from "./Checkboxwaive_charges";
...
import TextInputcr_bank_name  from "./TextInputcr_bank_name";
import TextInputcr_bank_bic  from "./TextInputcr_bank_bic";
import Dropdownforex_currency  from "./Dropdownforex_currency";
import TextInputexchange_rate  from "./TextInputexchange_rate";
import TextInputrate_code  from "./TextInputrate_code";
import TextInputforex_amount  from "./TextInputforex_amount";
import TextInputbase_amount  from "./TextInputbase_amount";
import TextInputrate_ref_no  from "./TextInputrate_ref_no";
import TextInputrate_cust_id  from "./TextInputrate_cust_id";
...
const GroupbasicInfo = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "409b134cde0449b5a031a7686df3d198");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setbasicinfo3d198Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("basic_info")){
      setbasic_info216f3({...basic_info216f3,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("waive_charges")){
      setwaive_charges929e5({...waive_charges929e5,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("cr_account")){
      setcr_accounta818b({...cr_accounta818b,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("cr_name")){
      setcr_namea4b34({...cr_namea4b34,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("cr_bank_code")){
      setcr_bank_code8a2bc({...cr_bank_code8a2bc,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("cr_bank_name")){
      setcr_bank_name434eb({...cr_bank_name434eb,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("cr_bank_bic")){
      setcr_bank_bic3d26f({...cr_bank_bic3d26f,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("forex_currency")){
      setforex_currency65e0b({...forex_currency65e0b,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("exchange_rate")){
      setexchange_rate88caf({...exchange_rate88caf,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("rate_code")){
      setrate_codee56ad({...rate_codee56ad,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("forex_amount")){
      setforex_amounta58a5({...forex_amounta58a5,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("base_amount")){
      setbase_amount3b226({...base_amount3b226,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("rate_ref_no")){
      setrate_ref_no82399({...rate_ref_no82399,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("rate_cust_id")){
      setrate_cust_idad42a({...rate_cust_idad42a,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  function handleConfirmOnLoad(){
    ...
    const handleOnload=()=>{
      ...
      setbutton_group74f3e((prev:any) => ({ ...prev, ...basicinfo3d198 }));
      // copyFormData for group
      setcommoninfof4607((prev:any) => ({ ...prev, ...basicinfo3d198 }));
  }
  ...
  const basicinfo3d198Ref = useRef<any>(null);
  const handleClearSearch = () => {
    basicinfo3d198Ref.current?.setSearchParams();
    basicinfo3d198Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(basicinfo3d198) && Object.keys(basicinfo3d198)?.length>0)
      {
        setbasicinfo3d198({})
      }
    }else 
      prevRefreshRef.current= true
  }, [basicinfo3d198Props?.refresh,token])
  ...
  return (
    ...
    >
          {allowedControls.includes("basic_info") ?<Textbasic_info   /* 216f3 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("waive_charges") ?<Checkboxwaive_charges   /* 929e5 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("cr_account") ?<TextInputcr_account   /* a818b */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("cr_name") ?<TextInputcr_name   /* a4b34 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        ...
        {allowedControls.includes("cr_bank_name") ?<TextInputcr_bank_name   /* 434eb */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("cr_bank_bic") ?<TextInputcr_bank_bic   /* 3d26f */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("forex_currency") ?<Dropdownforex_currency   /* 65e0b */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData}/>: <div></div>}
        {allowedControls.includes("exchange_rate") ?<TextInputexchange_rate   /* 88caf */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("rate_code") ?<TextInputrate_code   /* e56ad */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("forex_amount") ?<TextInputforex_amount   /* a58a5 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("base_amount") ?<TextInputbase_amount   /* 3b226 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("rate_ref_no") ?<TextInputrate_ref_no   /* 82399 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("rate_cust_id") ?<TextInputrate_cust_id   /* ad42a */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
