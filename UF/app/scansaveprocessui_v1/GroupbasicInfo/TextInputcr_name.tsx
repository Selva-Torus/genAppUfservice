...
const TextInputcr_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,cr_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasicinfo3d198((prev: any) => ({ ...prev, cr_name: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(basicinfo3d198?.cr_name == "" || basicinfo3d198?.cr_name == undefined){
      basicinfo3d198.cr_name = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, basicinfo3d198?.cr_name);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,cr_name:"invalid"}}));
        }
    }else if(basicinfo3d198?.cr_name !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +basicinfo3d198?.cr_name);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "ed5ab433883844f9b9437c7fa4ba4b34"
      );
      ...
      //     controlId: "ed5ab433883844f9b9437c7fa4ba4b34",
      //     isTable: false,
  ...
  return (   
    <div  
      ...
      <TextInput
        ...
        value={basicinfo3d198?.cr_name||""}
         disabled= {cr_namea4b34?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Cr Name"
      errorMessage={error}
        validationState={validate?.scanSaveProcessUi_v1?.cr_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
