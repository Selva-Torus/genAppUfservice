...
const TextInputbase_amount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'base_amount',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,base_amount:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasicinfo3d198((prev: any) => ({ ...prev, base_amount: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(basicinfo3d198?.base_amount == "" || basicinfo3d198?.base_amount == undefined){
      basicinfo3d198.base_amount = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, basicinfo3d198?.base_amount);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,base_amount:"invalid"}}));
        }
    }else if(basicinfo3d198?.base_amount !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +basicinfo3d198?.base_amount);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "43669d503a94467982239c6659b3b226"
      );
      ...
      //     controlId: "43669d503a94467982239c6659b3b226",
      //     isTable: false,
      //     from:"TextInputbase_amount",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'base_amount', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'base_amount',type:'text'};
      //   type={
      //     name:'base_amount',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.base_amount.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.base_amount.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.base_amount.type
      //   }
      ...
      //   type={
      //     name:'base_amount',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.base_amount.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.base_amount.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.base_amount.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  if (base_amount3b226?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `7 / 13`,gridRow: `42 / 57`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      <div style={{ flex: 1, minHeight: 0 }}>
      <TextInput
        require={isRequredData}
        className="!rounded-xl"
        label={keyset("Base Amount")}
        onChange= {handleChange}
        onBlur={handleBlur}
        itsHaveCurrency={false}
        type={dynamicStateandType.type}
        value={basicinfo3d198?.base_amount||""}
         disabled= {base_amount3b226?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
      ...
      errorMessage={error}
        validationState={validate?.scanSaveProcessUi_v1?.base_amount ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputbase_amount
