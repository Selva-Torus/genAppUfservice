...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    async function handleSecurity(controller: any = '') {
      ...
      }else if (controller in scansaveprocessui_v1?.doclisttable && scansaveprocessui_v1?.doclisttable[controller]?.itsHaveArtifact== true)
      {
        if(scansaveprocessui_v1?.doclisttable[controller])
        {
          let result :any = await getAftfactLevelRule(scansaveprocessui_v1._artfactPFRule_,{...decodedTokenObj,session:decodedTokenObj,doclisttable:filteredData},{doclisttable:scansaveprocessui_v1?.doclisttable})
