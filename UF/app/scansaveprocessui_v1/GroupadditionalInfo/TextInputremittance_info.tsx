...
const TextInputremittance_info = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,remittance_info:undefined}}));
    if(dynamicStateandType.type=="number"){
    setadditionalinfod2894((prev: any) => ({ ...prev, remittance_info: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(additionalinfod2894?.remittance_info == "" || additionalinfod2894?.remittance_info == undefined){
      additionalinfod2894.remittance_info = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, additionalinfod2894?.remittance_info);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,remittance_info:"invalid"}}));
        }
    }else if(additionalinfod2894?.remittance_info !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +additionalinfod2894?.remittance_info);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "5d80022ad84947259d615c470dfba5e0"
      );
      ...
      //     controlId: "5d80022ad84947259d615c470dfba5e0",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!additionalinfod2894?.remittance_info)
      { 
        setadditionalinfod2894Props((pre:any)=>({...pre,required:true}));
        setIsRequredData(true);
      }
      if(validateRefetch.init!=0)
        handleValidate();
  },[validateRefetch.value])
  if (remittance_infoba5e0?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 7`,gridRow: `10 / 25`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={additionalinfod2894?.remittance_info||""}
         disabled= {remittance_infoba5e0?.isDisabled ? true : false}
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
      ...
      errorMessage={error}
        validationState={validate?.scanSaveProcessUi_v1?.remittance_info ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
