...
const GroupadditionalInfo = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "18c17fb7694d4c3a944cddb2c4ed2894");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setadditionalinfod2894Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    ...
    if(orchestrationData?.data?.readableControls.includes("additional_ref")){
      setadditional_reff63a3({...additional_reff63a3,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("customwidget")){
      setcustomwidgetd7e47({...customwidgetd7e47,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  function handleConfirmOnLoad(){
    ...
    const handleOnload=()=>{
      ...
      setbutton_group74f3e((prev:any) => ({ ...prev, ...additionalinfod2894 }));
  }
  ...
  const additionalinfod2894Ref = useRef<any>(null);
  const handleClearSearch = () => {
    additionalinfod2894Ref.current?.setSearchParams();
    additionalinfod2894Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(additionalinfod2894) && Object.keys(additionalinfod2894)?.length>0)
      {
        setadditionalinfod2894({})
      }
    }else 
      prevRefreshRef.current= true
  }, [additionalinfod2894Props?.refresh,token])
