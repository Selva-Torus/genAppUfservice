...
import Tablertgs_list_validtn_table  from './Tablertgs_list_validtn_table';  
...
const Grouprtgs_list_validtn_table = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "6092fd816dd9472689ae9c2c5cf39a42");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrtgs_list_validtn_table39a42Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("vld_code_rtgs_lst")){
      setvld_code_rtgs_lstfc45a({...vld_code_rtgs_lstfc45a,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("vld_reason_rtgs_list")){
      setvld_reason_rtgs_listd8f6e({...vld_reason_rtgs_listd8f6e,isDisabled:true});
    }
  //////////////
  }
  ...
  const rtgs_list_validtn_table39a42Ref = useRef<any>(null);
  const handleClearSearch = () => {
    rtgs_list_validtn_table39a42Ref.current?.setSearchParams();
    rtgs_list_validtn_table39a42Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rtgs_list_validtn_table39a42) && Object.keys(rtgs_list_validtn_table39a42)?.length>0)
      {
        setrtgs_list_validtn_table39a42({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rtgs_list_validtn_table39a42Props?.refresh,token])
  ...
  return (
    ...
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<Tablertgs_list_validtn_table headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={rtgs_list_validtn_table39a42Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
...
export default Grouprtgs_list_validtn_table
