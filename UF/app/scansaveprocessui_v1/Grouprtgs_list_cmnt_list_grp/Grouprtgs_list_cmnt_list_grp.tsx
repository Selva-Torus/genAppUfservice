...
import Grouprtgs_list_cmnts_list  from "../Grouprtgs_list_cmnts_list/Grouprtgs_list_cmnts_list";
...
const Grouprtgs_list_cmnt_list_grp = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "248d655f0cb649ba908cef67baeb5728");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrtgs_list_cmnt_list_grpb5728Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("rtgs_list_cmnts_list")){
      setrtgs_list_cmnts_list15716({...rtgs_list_cmnts_list15716,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const rtgs_list_cmnt_list_grpb5728Ref = useRef<any>(null);
  const handleClearSearch = () => {
    rtgs_list_cmnt_list_grpb5728Ref.current?.setSearchParams();
    rtgs_list_cmnt_list_grpb5728Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rtgs_list_cmnt_list_grpb5728) && Object.keys(rtgs_list_cmnt_list_grpb5728)?.length>0)
      {
        setrtgs_list_cmnt_list_grpb5728({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rtgs_list_cmnt_list_grpb5728Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("rtgs_list_cmnts_list")  &&<Grouprtgs_list_cmnts_list  
...
export default Grouprtgs_list_cmnt_list_grp
