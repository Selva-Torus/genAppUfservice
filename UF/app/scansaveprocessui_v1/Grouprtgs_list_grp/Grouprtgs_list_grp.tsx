...
import Grouprtgs_list_table  from "../Grouprtgs_list_table/Grouprtgs_list_table";
...
import Grouprtgs_list_tab_grp  from "../Grouprtgs_list_tab_grp/Grouprtgs_list_tab_grp";
...
const Grouprtgs_list_grp = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "621ededa6d2e455dbc48b297a0ecf7d8");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrtgs_list_grpcf7d8Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("rtgs_list_table")){
      setrtgs_list_table7b8d6({...rtgs_list_table7b8d6,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("rtgs_list_tab_grp")){
      setrtgs_list_tab_grp024e1({...rtgs_list_tab_grp024e1,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const rtgs_list_grpcf7d8Ref = useRef<any>(null);
  const handleClearSearch = () => {
    rtgs_list_grpcf7d8Ref.current?.setSearchParams();
    rtgs_list_grpcf7d8Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rtgs_list_grpcf7d8) && Object.keys(rtgs_list_grpcf7d8)?.length>0)
      {
        setrtgs_list_grpcf7d8({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rtgs_list_grpcf7d8Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("rtgs_list_table")  &&<Grouprtgs_list_table  
        ...
        {allowedComponent.includes("rtgs_list_tab_grp")  &&<Grouprtgs_list_tab_grp  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
...
export default Grouprtgs_list_grp
