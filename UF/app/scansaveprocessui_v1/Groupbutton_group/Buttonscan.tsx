...
const Buttonscan = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "81fd4695b5644dd8b8863c6eb6a31ce1"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("scan" in button_group74f3eProps?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(button_group74f3eProps?.dynamicActionRule?.scan)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("scan" in scansaveprocessui_v1?.button_group && scansaveprocessui_v1?.button_group["scan"]?.itsHaveArtifact== true)
      {
        setShowFlag(scansaveprocessui_v1?.button_group["scan"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(scansaveprocessui_v1?.button_group?.scan?.order)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    if (!scan31ce1?.trigger) return;
      if(scan31ce1?.trigger){
      (async()=>{
        await handleClick();
      })();
      setscan31ce1((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
  ...
  const handleClick=async()=>{
    try{  
      ...
      setbutton_group74f3e((prev: any) => ({ ...prev, scan: true }));
        //onClick
    ...
    button_group74f3ePropsRef.current = mergedGetFormDataProps2;
      ...
      setbutton_group74f3e((prev: any) => ({ ...prev, scan: false }));
  ...
  }, [scansaveprocessui_v1?.button_group?.scan,button_group74f3e?.dynamicActionRule?.scan,control_tab_groupbc3e2])

 if (scan31ce1?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {scan31ce1?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
          icon="MdFlip"
