 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("cancel" in scansaveprocessui_v1?.button_group && scansaveprocessui_v1?.button_group["cancel"]?.itsHaveArtifact== true)
      {
        setShowFlag(scansaveprocessui_v1?.button_group["cancel"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(scansaveprocessui_v1?.button_group?.cancel?.order)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    if (!cancel2bf72?.trigger) return;
      if(cancel2bf72?.trigger){
      (async()=>{
        await handleClick();
      })();
      setcancel2bf72((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
  ...
  const handleClick=async()=>{
    ...
    let filterData12 = await getFilterProps(filterProps12,button_group74f3e);
    settransactionproduct_v1Props([...filterData12 ]);
  ...
  }, [scansaveprocessui_v1?.button_group?.cancel,button_group74f3e?.dynamicActionRule?.cancel,control_tab_groupbc3e2])

 if (cancel2bf72?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {cancel2bf72?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
