...
const Buttondelete = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "13296ded9e0e4d45b1456729e6d3ad2e"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("delete" in button_group74f3eProps?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(button_group74f3eProps?.dynamicActionRule?.delete)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("delete" in scansaveprocessui_v1?.button_group && scansaveprocessui_v1?.button_group["delete"]?.itsHaveArtifact== true)
      {
        setShowFlag(scansaveprocessui_v1?.button_group["delete"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(scansaveprocessui_v1?.button_group?.delete?.order)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    if (!delete3ad2e?.trigger) return;
      if(delete3ad2e?.trigger){
      (async()=>{
        await handleClick();
      })();
      setdelete3ad2e((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
  ...
  const handleClick=async()=>{
    try{  
      ...
      setbutton_group74f3e((prev: any) => ({ ...prev, delete: true }));
        //onClick
    ...
    button_group74f3ePropsRef.current = mergedGetFormDataProps2;
      ...
      setbutton_group74f3e((prev: any) => ({ ...prev, delete: false }));
  ...
  }, [scansaveprocessui_v1?.button_group?.delete,button_group74f3e?.dynamicActionRule?.delete,control_tab_groupbc3e2])

 if (delete3ad2e?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {delete3ad2e?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
