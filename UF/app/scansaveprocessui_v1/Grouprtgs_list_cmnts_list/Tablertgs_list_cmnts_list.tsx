...
let defaultColumns:any = [
  {
    "id": "cmnts_rtgs_list",
    "nodeid": "cacb0fa58c12442eb2d6c4cac3d1952c",
    "name": "Comments",
    "meta": {
      "sort": true
    },
...
const Tablertgs_list_cmnts_list = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const {cmnts_rtgs_list1952c, setcmnts_rtgs_list1952c}= useContext(TotalContext) as TotalContextProps  
  //////////////
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "53c3156dbeb24f39bc1fc71d25615716",
      );
  ...
  const [SearchParams,setSearchParams] = useState<any>({})
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['cmnts_rtgs_list'] = cmnts_rtgs_list1952c,
        codeStates['setcmnts_rtgs_list'] = setcmnts_rtgs_list1952c,
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setrtgs_list_cmnts_list15716Props((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setrtgs_list_cmnts_list15716Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(scansaveprocessui_v1Props.length > 0){
      for(let i=0;i< scansaveprocessui_v1Props.length;i++){
        if(scansaveprocessui_v1Props[i].DFDkey == ""){
    ...
    try {
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: '53c3156dbeb24f39bc1fc71d25615716',
                elementId: '53c3156dbeb24f39bc1fc71d25615716'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( rtgs_list_cmnts_list15716Props?.presetValues&&Object.keys(rtgs_list_cmnts_list15716Props?.presetValues).length > 0) {
          filtertedData = [rtgs_list_cmnts_list15716Props?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setrtgs_list_cmnts_list15716(api_pagination?.data?.records||[])
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(rtgs_list_cmnts_list15716Props.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [rtgs_list_cmnts_list15716Props.filterProps])
  ...
  useEffect(() => {
    if (rtgs_list_cmnts_list15716Props?.clearData === true) {
      setrtgs_list_cmnts_list15716([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setrtgs_list_cmnts_list15716Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [rtgs_list_cmnts_list15716Props?.clearData])
  useEffect(() => {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [rtgs_list_cmnts_list15716Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = rtgs_list_cmnts_list15716Props?.selectedIds[rtgs_list_cmnts_list15716Props?.selectedIds?.length-1]
    if(Array.isArray(rtgs_list_cmnts_list15716) && rtgs_list_cmnts_list15716.length>0)
    {
      let data = rtgs_list_cmnts_list15716[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "",
        value: data[""],
  ...
  useEffect(() => {
    if (rtgs_list_cmnts_list15716Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (rtgs_list_cmnts_list15716Props?.selectedIds?.length == 0){
      handleOnRowClick({},rtgs_list_cmnts_list15716Props?.selectedIds)
    }
  }, [rtgs_list_cmnts_list15716Props?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(rtgs_list_cmnts_list15716?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&rtgs_list_cmnts_list15716?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](rtgs_list_cmnts_list15716?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: rtgs_list_cmnts_list15716?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&rtgs_list_cmnts_list15716?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](rtgs_list_cmnts_list15716?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: rtgs_list_cmnts_list15716?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (rtgs_list_cmnts_list15716?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"53c3156dbeb24f39bc1fc71d25615716"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              selectedIds={rtgs_list_cmnts_list15716Props?.selectedIds}  
...
export default Tablertgs_list_cmnts_list
