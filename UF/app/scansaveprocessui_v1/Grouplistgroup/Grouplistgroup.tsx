...
import Grouplist_tab_group  from "../Grouplist_tab_group/Grouplist_tab_group";
...
const Grouplistgroup = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "542b83f307a44b7cae800db53d4dcdbd");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setlistgroupdcdbdProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("list_tab_group")){
      setlist_tab_groupd6905({...list_tab_groupd6905,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const listgroupdcdbdRef = useRef<any>(null);
  const handleClearSearch = () => {
    listgroupdcdbdRef.current?.setSearchParams();
    listgroupdcdbdRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(listgroupdcdbd) && Object.keys(listgroupdcdbd)?.length>0)
      {
        setlistgroupdcdbd({})
      }
    }else 
      prevRefreshRef.current= true
  }, [listgroupdcdbdProps?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("list_tab_group")  &&<Grouplist_tab_group  
