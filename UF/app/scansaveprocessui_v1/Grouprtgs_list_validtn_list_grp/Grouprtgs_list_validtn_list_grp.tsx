...
import Grouprtgs_list_validtn_table  from "../Grouprtgs_list_validtn_table/Grouprtgs_list_validtn_table";
...
const Grouprtgs_list_validtn_list_grp = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "cbd0e09ff00546f59415cb7efe1c5569");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrtgs_list_validtn_list_grpc5569Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("rtgs_list_validtn_table")){
      setrtgs_list_validtn_table39a42({...rtgs_list_validtn_table39a42,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const rtgs_list_validtn_list_grpc5569Ref = useRef<any>(null);
  const handleClearSearch = () => {
    rtgs_list_validtn_list_grpc5569Ref.current?.setSearchParams();
    rtgs_list_validtn_list_grpc5569Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rtgs_list_validtn_list_grpc5569) && Object.keys(rtgs_list_validtn_list_grpc5569)?.length>0)
      {
        setrtgs_list_validtn_list_grpc5569({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rtgs_list_validtn_list_grpc5569Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("rtgs_list_validtn_table")  &&<Grouprtgs_list_validtn_table  
...
export default Grouprtgs_list_validtn_list_grp
