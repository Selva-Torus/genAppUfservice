...
import Grouprtgs_list_cmnt_list_grp  from "../Grouprtgs_list_cmnt_list_grp/Grouprtgs_list_cmnt_list_grp";
...
const Groupcmnt_list = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "20eabbe7dcfa4f0c8a2eaed92e9ebbbc");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcmnt_listebbbcProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("rtgs_list_cmnt_list_grp")){
      setrtgs_list_cmnt_list_grpb5728({...rtgs_list_cmnt_list_grpb5728,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const cmnt_listebbbcRef = useRef<any>(null);
  const handleClearSearch = () => {
    cmnt_listebbbcRef.current?.setSearchParams();
    cmnt_listebbbcRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(cmnt_listebbbc) && Object.keys(cmnt_listebbbc)?.length>0)
      {
        setcmnt_listebbbc({})
      }
    }else 
      prevRefreshRef.current= true
  }, [cmnt_listebbbcProps?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("rtgs_list_cmnt_list_grp")  &&<Grouprtgs_list_cmnt_list_grp  
