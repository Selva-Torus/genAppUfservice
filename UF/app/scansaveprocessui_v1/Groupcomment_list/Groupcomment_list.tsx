...
import GroupcmntListTable  from "../GroupcmntListTable/GroupcmntListTable";
...
const Groupcomment_list = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "323166f265bc43ecbdcffe2355572944");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcomment_list72944Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cmntlisttable")){
      setcmntlisttable02d0e({...cmntlisttable02d0e,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const comment_list72944Ref = useRef<any>(null);
  const handleClearSearch = () => {
    comment_list72944Ref.current?.setSearchParams();
    comment_list72944Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(comment_list72944) && Object.keys(comment_list72944)?.length>0)
      {
        setcomment_list72944({})
      }
    }else 
      prevRefreshRef.current= true
  }, [comment_list72944Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("cmntlisttable")  &&<GroupcmntListTable  
...
export default Groupcomment_list
