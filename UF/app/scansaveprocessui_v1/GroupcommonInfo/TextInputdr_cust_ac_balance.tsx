...
const TextInputdr_cust_ac_balance = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,dr_cust_ac_balance:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcommoninfof4607((prev: any) => ({ ...prev, dr_cust_ac_balance: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(commoninfof4607?.dr_cust_ac_balance == "" || commoninfof4607?.dr_cust_ac_balance == undefined){
      commoninfof4607.dr_cust_ac_balance = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, commoninfof4607?.dr_cust_ac_balance);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,dr_cust_ac_balance:"invalid"}}));
        }
    }else if(commoninfof4607?.dr_cust_ac_balance !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +commoninfof4607?.dr_cust_ac_balance);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "ff81fb16b4444da4b89336717f6753dd"
      );
      ...
      //     controlId: "ff81fb16b4444da4b89336717f6753dd",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!commoninfof4607?.dr_cust_ac_balance)
      { 
  ...
  if (dr_cust_ac_balance753dd?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `9 / 17`,gridRow: `26 / 41`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={commoninfof4607?.dr_cust_ac_balance||""}
        disabled={ true }
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.scanSaveProcessUi_v1?.dr_cust_ac_balance ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
