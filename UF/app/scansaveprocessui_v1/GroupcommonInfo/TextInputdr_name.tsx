...
const TextInputdr_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:scanSaveProcessUi:AFVK:v1|f2dd7aceaf454c72bdb1327c439f4607|5ade2207705445b0a1cfc28f43184266"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,dr_name:undefined}}));
  ...
  const handleValidate=async (e?:any) => {
     ...
     if(dynamicStateandType.type=="number"){
        ...
        if(!validate.success){
          ...
          setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,dr_name:"invalid"}}));
        }
  ...
  return (   
    <div  
      style={{gridColumn: `9 / 17`,gridRow: `10 / 25`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        headerText="Dr Name"
      errorMessage={error}
        validationState={validate?.scanSaveProcessUi_v1?.dr_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
