...
const TextInputbase_currency = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:scanSaveProcessUi:AFVK:v1|f2dd7aceaf454c72bdb1327c439f4607|9549a13b6e484a73ae4865515e4b386d"
    }
  ],
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,dr_currency:undefined}}));
  ...
  const handleValidate=async (e?:any) => {
     ...
     if(dynamicStateandType.type=="number"){
        ...
        if(!validate.success){
          ...
          setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,dr_currency:"invalid"}}));
        }
  ...
  return (   
    <div  
      style={{gridColumn: `17 / 25`,gridRow: `10 / 25`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      errorMessage={error}
        validationState={validate?.scanSaveProcessUi_v1?.dr_currency ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
