...
const TextInputdr_cust_ac_sanc_lmt = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,dr_cust_ac_sanc_lmt:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcommoninfof4607((prev: any) => ({ ...prev, dr_cust_ac_sanc_lmt: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(commoninfof4607?.dr_cust_ac_sanc_lmt == "" || commoninfof4607?.dr_cust_ac_sanc_lmt == undefined){
      commoninfof4607.dr_cust_ac_sanc_lmt = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, commoninfof4607?.dr_cust_ac_sanc_lmt);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,scanSaveProcessUi_v1:{...pre?.scanSaveProcessUi_v1,dr_cust_ac_sanc_lmt:"invalid"}}));
        }
    }else if(commoninfof4607?.dr_cust_ac_sanc_lmt !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +commoninfof4607?.dr_cust_ac_sanc_lmt);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "ef43257873b2402b995bbd79124b74f7"
      );
      ...
      //     controlId: "ef43257873b2402b995bbd79124b74f7",
      //     isTable: false,
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!commoninfof4607?.dr_cust_ac_sanc_lmt)
      { 
  ...
  if (dr_cust_ac_sanc_lmtb74f7?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 9`,gridRow: `26 / 41`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={commoninfof4607?.dr_cust_ac_sanc_lmt||""}
        disabled={ true }
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.scanSaveProcessUi_v1?.dr_cust_ac_sanc_lmt ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
