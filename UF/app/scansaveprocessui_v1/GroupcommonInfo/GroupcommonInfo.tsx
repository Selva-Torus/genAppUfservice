...
import TextInputdr_cust_ac_sanc_lmt  from "./TextInputdr_cust_ac_sanc_lmt";
import TextInputdr_cust_ac_balance  from "./TextInputdr_cust_ac_balance";
...
const GroupcommonInfo = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  /////////////
    ...
    if(orchestrationData?.data?.readableControls.includes("dr_cust_ac_sanc_lmt")){
      setdr_cust_ac_sanc_lmtb74f7({...dr_cust_ac_sanc_lmtb74f7,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("dr_cust_ac_balance")){
      setdr_cust_ac_balance753dd({...dr_cust_ac_balance753dd,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  return (
    ...
    >
        ...
        {allowedControls.includes("dr_cust_ac_sanc_lmt") ?<TextInputdr_cust_ac_sanc_lmt   /* b74f7 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
        {allowedControls.includes("dr_cust_ac_balance") ?<TextInputdr_cust_ac_balance   /* 753dd */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
