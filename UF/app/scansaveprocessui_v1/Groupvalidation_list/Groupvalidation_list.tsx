...
import GroupvaldnListTable  from "../GroupvaldnListTable/GroupvaldnListTable";
...
const Groupvalidation_list = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "65746b2ebd784ac3b5ab0418204ae827");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setvalidation_listae827Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("valdnlisttable")){
      setvaldnlisttable17ec7({...valdnlisttable17ec7,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const validation_listae827Ref = useRef<any>(null);
  const handleClearSearch = () => {
    validation_listae827Ref.current?.setSearchParams();
    validation_listae827Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(validation_listae827) && Object.keys(validation_listae827)?.length>0)
      {
        setvalidation_listae827({})
      }
    }else 
      prevRefreshRef.current= true
  }, [validation_listae827Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("valdnlisttable")  &&<GroupvaldnListTable  
...
export default Groupvalidation_list
