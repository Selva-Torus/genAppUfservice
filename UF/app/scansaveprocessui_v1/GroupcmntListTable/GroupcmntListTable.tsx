...
import TablecmntListTable  from './TablecmntListTable';  
...
const GroupcmntListTable = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "bfcb81a3220f484494becea016e02d0e");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcmntlisttable02d0eProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cmnts")){
      setcmnts11ffa({...cmnts11ffa,isDisabled:true});
    }
  //////////////
  }
  ...
  const cmntlisttable02d0eRef = useRef<any>(null);
  const handleClearSearch = () => {
    cmntlisttable02d0eRef.current?.setSearchParams();
    cmntlisttable02d0eRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(cmntlisttable02d0e) && Object.keys(cmntlisttable02d0e)?.length>0)
      {
        setcmntlisttable02d0e({})
      }
    }else 
      prevRefreshRef.current= true
  }, [cmntlisttable02d0eProps?.refresh,token])
  ...
  return (
    ...
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<TablecmntListTable headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={cmntlisttable02d0eRef} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
...
export default GroupcmntListTable
