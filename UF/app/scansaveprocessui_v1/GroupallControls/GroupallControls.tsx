...
import GroupcommonInfo  from "../GroupcommonInfo/GroupcommonInfo";
import GroupbasicInfo  from "../GroupbasicInfo/GroupbasicInfo";
import GroupadditionalInfo  from "../GroupadditionalInfo/GroupadditionalInfo";
import Grouplistgroup  from "../Grouplistgroup/Grouplistgroup";
...
const GroupallControls = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "ee86ba2a12eb4d2ea59c747abe171c54");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setallcontrols71c54Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("commoninfo")){
      setcommoninfof4607({...commoninfof4607,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("basicinfo")){
      setbasicinfo3d198({...basicinfo3d198,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("additionalinfo")){
      setadditionalinfod2894({...additionalinfod2894,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("listgroup")){
      setlistgroupdcdbd({...listgroupdcdbd,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const allcontrols71c54Ref = useRef<any>(null);
  const handleClearSearch = () => {
    allcontrols71c54Ref.current?.setSearchParams();
    allcontrols71c54Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(allcontrols71c54) && Object.keys(allcontrols71c54)?.length>0)
      {
        setallcontrols71c54({})
      }
    }else 
      prevRefreshRef.current= true
  }, [allcontrols71c54Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("commoninfo")  &&<GroupcommonInfo  
        ...
        {allowedComponent.includes("basicinfo")  &&<GroupbasicInfo  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedComponent.includes("additionalinfo")  &&<GroupadditionalInfo  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
          ...
          groupData={groupData}
          controlData={controlData}        />}
        {allowedComponent.includes("listgroup")  &&<Grouplistgroup  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
...
export default GroupallControls
