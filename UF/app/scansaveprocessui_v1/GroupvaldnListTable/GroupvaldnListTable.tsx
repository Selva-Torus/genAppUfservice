...
import TablevaldnListTable  from './TablevaldnListTable';  
...
const GroupvaldnListTable = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "6b332e1a71dd48d3a5b440a432017ec7");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setvaldnlisttable17ec7Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("vldcode")){
      setvldcode0c0ce({...vldcode0c0ce,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("vldreason")){
      setvldreason2ef16({...vldreason2ef16,isDisabled:true});
    }
  //////////////
  }
  ...
  const valdnlisttable17ec7Ref = useRef<any>(null);
  const handleClearSearch = () => {
    valdnlisttable17ec7Ref.current?.setSearchParams();
    valdnlisttable17ec7Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(valdnlisttable17ec7) && Object.keys(valdnlisttable17ec7)?.length>0)
      {
        setvaldnlisttable17ec7({})
      }
    }else 
      prevRefreshRef.current= true
  }, [valdnlisttable17ec7Props?.refresh,token])
  ...
  return (
    ...
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<TablevaldnListTable headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={valdnlisttable17ec7Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
...
export default GroupvaldnListTable
