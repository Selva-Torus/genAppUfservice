...
import TimeLinesuccess_queue_journey  from "./TimeLinesuccess_queue_journey";
...
const Groupsuccess_queue_journey_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "2fa3079c690343a68140ea8d7ff755eb");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setsuccess_queue_journey_group755ebProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("success_queue_journey")){
      setsuccess_queue_journey68ac9({...success_queue_journey68ac9,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['success_queue_journey'] = success_queue_journey68ac9,
        codeStates['setsuccess_queue_journey'] = setsuccess_queue_journey68ac9,
  ...
  const success_queue_journey_group755ebRef = useRef<any>(null);
  const handleClearSearch = () => {
    success_queue_journey_group755ebRef.current?.setSearchParams();
    success_queue_journey_group755ebRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(success_queue_journey_group755eb) && Object.keys(success_queue_journey_group755eb)?.length>0)
      {
        setsuccess_queue_journey_group755eb({})
      }
    }else 
      prevRefreshRef.current= true
  }, [success_queue_journey_group755ebProps?.refresh,token])
  ...
  return (
    ...
    >
        {allowedControls.includes("success_queue_journey") ?<TimeLinesuccess_queue_journey   /* 68ac9 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupsuccess_queue_journey_group
