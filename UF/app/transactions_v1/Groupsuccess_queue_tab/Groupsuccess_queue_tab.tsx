...
import Groupsuccess_queue_table  from "../Groupsuccess_queue_table/Groupsuccess_queue_table";
import Groupsuccess_queue_journey_group  from "../Groupsuccess_queue_journey_group/Groupsuccess_queue_journey_group";
...
const Groupsuccess_queue_tab = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setsuccess_queue_tabef582Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    ...
    if(orchestrationData?.data?.readableControls.includes("success_queue_journey_group")){
      setsuccess_queue_journey_group755eb({...success_queue_journey_group755eb,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const handleClearSearch = () => {
    ...
    success_queue_tabef582Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(success_queue_tabef582) && Object.keys(success_queue_tabef582)?.length>0)
      {
        setsuccess_queue_tabef582({})
      }
    }else 
      prevRefreshRef.current= true
  }, [success_queue_tabef582Props?.refresh,token])
  ...
  return (
    ...
    >
        ...
        {allowedComponent.includes("success_queue_journey_group")  &&<Groupsuccess_queue_journey_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
...
export default Groupsuccess_queue_tab
