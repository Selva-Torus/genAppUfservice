...
const TimeLineoperational_pending_journey = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing}:any) => { 
...
const statusMap: Record<string, { icon: any; color: string }> = {
  ...
  const handleStepClick = async (step: Record<string, any>, index: number) => {
    ...
    let filterData2 = await getFilterProps(filterProps2,operational_pending_journey_group63667);
  ...
  const handleMapper = async () => {
    // If operational_pending_journey_group63667 data exists, use it directly without API calls
    if (Array.isArray(operational_pending_journey_group63667?.vgphstm_uuid) && operational_pending_journey_group63667?.vgphstm_uuid.length > 0) {
      setSteps(operational_pending_journey_group63667.vgphstm_uuid)
      setHasMore(false)
    ...
    let orchestrationBody : any = {
      ...
      componentId: "e99523c5df50475e83c05552ec463667",
      controlId: "bd86178cd9cb4f569011ec99c431a1a5",
      isTable: false,
  ...
  const fetchPagination = async (key: string, page: number, count: number) => {
    if (Array.isArray(operational_pending_journey_group63667?.vgphstm_uuid) && operational_pending_journey_group63667?.vgphstm_uuid.length > 0) {
      const journeyData = operational_pending_journey_group63667.vgphstm_uuid
    ...
    const api_paginationBody: api_paginationDto = {
      ...
      page: page || 1,
      count: count,
      searchFilter: {"vgphstm_uuid":operational_pending_journey_group63667?.uuid},
    }
    if (encryptionFlagCont) {
  ...
  //       setoperational_pending_journey_group63667((pre:any)=>({...pre,vgphstm_uuid:dfd_journey_v1Props[0]?.vgphstm_uuid}));
  //   }
  //// }, [dfd_journey_v1Props]);

  useEffect(() => {
    if (!false && !operational_pending_journey_group63667?.uuid) {
      setSteps([]);
      setHasMore(false);
      return;
    }
    ...
    handleMapper();
    },[ operational_pending_journey_group63667?.uuid])

    useEffect(() => {
    if (operational_pending_journey_group63667?.vgphstm_uuid && operational_pending_journey_group63667?.vgphstm_uuid.length > 0) {
      setSteps(operational_pending_journey_group63667.vgphstm_uuid);
      setHasMore(false);
      setIsInitialLoaded(true);
    }
  },[ operational_pending_journey_group63667?.vgphstm_uuid])

  return (
    <div className="" style={{gridColumn: `1 / 25`,gridRow: `1 / 191`, gap:``, height: `100%`, overflow: 'hidden'}} >
      <Modal 
...
export default TimeLineoperational_pending_journey
