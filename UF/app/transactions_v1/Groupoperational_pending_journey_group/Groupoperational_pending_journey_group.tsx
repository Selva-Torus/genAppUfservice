...
import TimeLineoperational_pending_journey  from "./TimeLineoperational_pending_journey";
...
const Groupoperational_pending_journey_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const {operational_pending_journey1a1a5, setoperational_pending_journey1a1a5}= useContext(TotalContext) as TotalContextProps;
  ...
  const data: any = await fetchBatchData(
    ...
    [user],
    'GroupOperationalPendingJourneyGroup',
    token
  );
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "e99523c5df50475e83c05552ec463667");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setoperational_pending_journey_group63667Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("operational_pending_journey")){
      setoperational_pending_journey1a1a5({...operational_pending_journey1a1a5,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['operational_pending_journey'] = operational_pending_journey1a1a5,
        codeStates['setoperational_pending_journey'] = setoperational_pending_journey1a1a5,
  ...
  const operational_pending_journey_group63667Ref = useRef<any>(null);
  const handleClearSearch = () => {
    operational_pending_journey_group63667Ref.current?.setSearchParams();
    operational_pending_journey_group63667Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(operational_pending_journey_group63667) && Object.keys(operational_pending_journey_group63667)?.length>0)
      {
        setoperational_pending_journey_group63667({})
      }
    }else 
      prevRefreshRef.current= true
  }, [operational_pending_journey_group63667Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedControls.includes("operational_pending_journey") ?<TimeLineoperational_pending_journey   /* 1a1a5 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupoperational_pending_journey_group
