...
import TimeLinetechnical_pending_journey  from "./TimeLinetechnical_pending_journey";
...
const Grouptechnical_pending_journey_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  const data: any = await fetchBatchData(
    ...
    [user],
    'GroupTechnicalPendingJourneyGroup',
    token
  );
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "c7d2a9c0c8474e86a5fc7d3544be4f03");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    settechnical_pending_journey_groupe4f03Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("technical_pending_journey")){
      settechnical_pending_journey6601c({...technical_pending_journey6601c,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['technical_pending_journey'] = technical_pending_journey6601c,
        codeStates['settechnical_pending_journey'] = settechnical_pending_journey6601c,
  ...
  const technical_pending_journey_groupe4f03Ref = useRef<any>(null);
  const handleClearSearch = () => {
    technical_pending_journey_groupe4f03Ref.current?.setSearchParams();
    technical_pending_journey_groupe4f03Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(technical_pending_journey_groupe4f03) && Object.keys(technical_pending_journey_groupe4f03)?.length>0)
      {
        settechnical_pending_journey_groupe4f03({})
      }
    }else 
      prevRefreshRef.current= true
  }, [technical_pending_journey_groupe4f03Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedControls.includes("technical_pending_journey") ?<TimeLinetechnical_pending_journey   /* 6601c */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Grouptechnical_pending_journey_group
