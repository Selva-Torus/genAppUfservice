...
const statusMap: Record<string, { icon: any; color: string }> = {
  ...
  const handleStepClick = async (step: Record<string, any>, index: number) => {
    ...
    let filterData2 = await getFilterProps(filterProps2,view_all_journey_group67ce4);
  ...
  const fetchPagination = async (key: string, page: number, count: number) => {
    ...
    const api_paginationBody: api_paginationDto = {
      ...
      page: page || 1,
      count: count,
      searchFilter: {"vgphstm_uuid":view_all_journey_group67ce4?.uuid},
    }
    if (encryptionFlagCont) {
  ...
  useEffect(() => {
    if (!false && !view_all_journey_group67ce4?.uuid) {
      setSteps([]);
      setHasMore(false);
      return;
    }
    ...
    handleMapper();
    },[ view_all_journey_group67ce4?.uuid])
