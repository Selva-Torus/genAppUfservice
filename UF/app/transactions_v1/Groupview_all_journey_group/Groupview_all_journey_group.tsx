...
import TimeLineview_all_journey  from "./TimeLineview_all_journey";
...
const Groupview_all_journey_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "2d439c34493f4559866a2ed55ea67ce4");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setview_all_journey_group67ce4Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("view_all_journey")){
      setview_all_journeyd3ae9({...view_all_journeyd3ae9,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['view_all_journey'] = view_all_journeyd3ae9,
        codeStates['setview_all_journey'] = setview_all_journeyd3ae9,
  ...
  const view_all_journey_group67ce4Ref = useRef<any>(null);
  const handleClearSearch = () => {
    view_all_journey_group67ce4Ref.current?.setSearchParams();
    view_all_journey_group67ce4Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(view_all_journey_group67ce4) && Object.keys(view_all_journey_group67ce4)?.length>0)
      {
        setview_all_journey_group67ce4({})
      }
    }else 
      prevRefreshRef.current= true
  }, [view_all_journey_group67ce4Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedControls.includes("view_all_journey") ?<TimeLineview_all_journey   /* d3ae9 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupview_all_journey_group
