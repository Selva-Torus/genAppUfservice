...
const Buttondownload = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "2e0bbfd05cfa4a23a2320b6369fcb505"
      );
  ...
  useEffect(()=>{
    if (!downloadcb505?.trigger) return;
      if(downloadcb505?.trigger){
      (async()=>{
        await handleClick();
      })();
      setdownloadcb505((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
        pendingAutoSearch.current = true;
      })();
    }
  },[downloadcb505?.trigger])

  useEffect(()=>{
    if(downloadcb505?.refresh){
    (async()=>{
      await handleMapper();
      pendingAutoSearch.current = true;
    })();
    }
  },[downloadcb505?.refresh])
  ...
  const handleClick=async()=>{
    try{  
      ...
      settran_tab_group08b64((prev: any) => ({ ...prev, download: false }));
 ...
 if (downloadcb505?.isHidden) {
    return <></>
  }

  return (
    <div
      style={{gridColumn: `19 / 21`,gridRow: `2 / 11`, gap:``, height: `100%`, overflow: 'auto'}} 
      >
        {showFlag && <Button 
          ref={buttonRef}
          ...
          onClick={handleClick}
          view='flat-info'
          disabled= {downloadcb505?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
