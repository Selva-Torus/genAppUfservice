...
import TimeLinefailure_queue_journey  from "./TimeLinefailure_queue_journey";
...
const Groupfailure_queue_journey_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "1d92ad18583447e2a1b3f3f7a1436aba");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setfailure_queue_journey_group36abaProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("failure_queue_journey")){
      setfailure_queue_journeyc8638({...failure_queue_journeyc8638,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['failure_queue_journey'] = failure_queue_journeyc8638,
        codeStates['setfailure_queue_journey'] = setfailure_queue_journeyc8638,
  ...
  const failure_queue_journey_group36abaRef = useRef<any>(null);
  const handleClearSearch = () => {
    failure_queue_journey_group36abaRef.current?.setSearchParams();
    failure_queue_journey_group36abaRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(failure_queue_journey_group36aba) && Object.keys(failure_queue_journey_group36aba)?.length>0)
      {
        setfailure_queue_journey_group36aba({})
      }
    }else 
      prevRefreshRef.current= true
  }, [failure_queue_journey_group36abaProps?.refresh,token])
  ...
  return (
    ...
    >
        {allowedControls.includes("failure_queue_journey") ?<TimeLinefailure_queue_journey   /* c8638 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}

export default Groupfailure_queue_journey_group
