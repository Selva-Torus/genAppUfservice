...
import Groupoperational_pending_table  from "../Groupoperational_pending_table/Groupoperational_pending_table";
import Groupoperational_pending_journey_group  from "../Groupoperational_pending_journey_group/Groupoperational_pending_journey_group";
...
const Groupoperational_pending_tab = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "b205119371ec4ea5adeee2f7afa67331");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setoperational_pending_tab67331Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("operational_pending_table")){
      setoperational_pending_table0a253({...operational_pending_table0a253,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("operational_pending_journey_group")){
      setoperational_pending_journey_group63667({...operational_pending_journey_group63667,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const operational_pending_tab67331Ref = useRef<any>(null);
  const handleClearSearch = () => {
    operational_pending_tab67331Ref.current?.setSearchParams();
    operational_pending_tab67331Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(operational_pending_tab67331) && Object.keys(operational_pending_tab67331)?.length>0)
      {
        setoperational_pending_tab67331({})
      }
    }else 
      prevRefreshRef.current= true
  }, [operational_pending_tab67331Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("operational_pending_table")  &&<Groupoperational_pending_table  
        ...
        {allowedComponent.includes("operational_pending_journey_group")  &&<Groupoperational_pending_journey_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
...
export default Groupoperational_pending_tab
