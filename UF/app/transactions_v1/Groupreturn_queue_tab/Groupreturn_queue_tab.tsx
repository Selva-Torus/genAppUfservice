...
import Groupreturn_queue_table  from "../Groupreturn_queue_table/Groupreturn_queue_table";
import Groupreturn_queue_journey_group  from "../Groupreturn_queue_journey_group/Groupreturn_queue_journey_group";
...
const Groupreturn_queue_tab = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "ce119568f7cb4e1fa40131efbca5611e");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setreturn_queue_tab5611eProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("return_queue_table")){
      setreturn_queue_table267f0({...return_queue_table267f0,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("return_queue_journey_group")){
      setreturn_queue_journey_group92c55({...return_queue_journey_group92c55,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const return_queue_tab5611eRef = useRef<any>(null);
  const handleClearSearch = () => {
    return_queue_tab5611eRef.current?.setSearchParams();
    return_queue_tab5611eRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(return_queue_tab5611e) && Object.keys(return_queue_tab5611e)?.length>0)
      {
        setreturn_queue_tab5611e({})
      }
    }else 
      prevRefreshRef.current= true
  }, [return_queue_tab5611eProps?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("return_queue_table")  &&<Groupreturn_queue_table  
        ...
        {allowedComponent.includes("return_queue_journey_group")  &&<Groupreturn_queue_journey_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          tableData={tableData}
          setTableData={setTableData}
...
export default Groupreturn_queue_tab
