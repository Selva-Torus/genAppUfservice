...
const TimeLinereturn_queue_journey = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing}:any) => { 
...
const statusMap: Record<string, { icon: any; color: string }> = {
  ...
  const handleStepClick = async (step: Record<string, any>, index: number) => {
    ...
    let filterData2 = await getFilterProps(filterProps2,return_queue_journey_group92c55);
  ...
  const handleMapper = async () => {
    // If return_queue_journey_group92c55 data exists, use it directly without API calls
    if (Array.isArray(return_queue_journey_group92c55?.vgphstm_uuid) && return_queue_journey_group92c55?.vgphstm_uuid.length > 0) {
      setSteps(return_queue_journey_group92c55.vgphstm_uuid)
      setHasMore(false)
    ...
    let orchestrationBody : any = {
      ...
      componentId: "60afebff1f5a468fa8427d3070192c55",
      controlId: "a154ca0794ff4aad816079704e7cc9d3",
      isTable: false,
  ...
  const fetchPagination = async (key: string, page: number, count: number) => {
    if (Array.isArray(return_queue_journey_group92c55?.vgphstm_uuid) && return_queue_journey_group92c55?.vgphstm_uuid.length > 0) {
      const journeyData = return_queue_journey_group92c55.vgphstm_uuid
    ...
    const api_paginationBody: api_paginationDto = {
      ...
      page: page || 1,
      count: count,
      searchFilter: {"vgphstm_uuid":return_queue_journey_group92c55?.uuid},
    }
    if (encryptionFlagCont) {
  ...
  //       setreturn_queue_journey_group92c55((pre:any)=>({...pre,vgphstm_uuid:dfd_journey_v1Props[0]?.vgphstm_uuid}));
  //   }
  //// }, [dfd_journey_v1Props]);

  useEffect(() => {
    if (!false && !return_queue_journey_group92c55?.uuid) {
      setSteps([]);
      setHasMore(false);
      return;
    }
    ...
    handleMapper();
    },[ return_queue_journey_group92c55?.uuid])

    useEffect(() => {
    if (return_queue_journey_group92c55?.vgphstm_uuid && return_queue_journey_group92c55?.vgphstm_uuid.length > 0) {
      setSteps(return_queue_journey_group92c55.vgphstm_uuid);
      setHasMore(false);
      setIsInitialLoaded(true);
    }
  },[ return_queue_journey_group92c55?.vgphstm_uuid])

  return (
    <div className="" style={{gridColumn: `1 / 25`,gridRow: `1 / 186`, gap:``, height: `100%`, overflow: 'hidden'}} >
      <Modal 
...
export default TimeLinereturn_queue_journey
