...
const Groupgroup = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT309:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:bindranscreen:AFVK:v1",componentId:"b3999b9e0f6b40feb569a8e5e2dbf5ce",from:"GroupGroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("name")){
      setnamef9057({...namef9057,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const groupbf5ceRef = useRef<any>(null);
  const handleClearSearch = () => {
    groupbf5ceRef.current?.setSearchParams();
    groupbf5ceRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(groupbf5ce) && Object.keys(groupbf5ce)?.length>0)
      {
        setgroupbf5ce({})
      }
    }else 
      prevRefreshRef.current= true
  }, [groupbf5ceProps?.refresh])
  ...
  return (
    ...
    >
        {allowedControls.includes("name") ?<TextInputname   /* f9057 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}
