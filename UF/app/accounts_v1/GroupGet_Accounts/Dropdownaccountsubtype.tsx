...
const Dropdownaccountsubtype = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagCompData}: any) => {
...
//}));           
  ...
  useEffect(() => {
  if(get_accounts1a859?.accountsubtype=="" || get_accounts1a859?.accountsubtype==undefined || get_accounts1a859?.accountsubtype==null ){
    setSelectedItem([]);
  }
  },[get_accounts1a859?.accountsubtype])
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "756131b04d19485aaa6e58ad999b9399",
          isTable: false,
          accessProfile:accessProfile,
          from:"dropdownaccountsubtype"
        },
        {
          headers: {
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setget_accounts1a859((prev: any) => ({ ...prev, accountsubtype: [value]}))
      setIsRequredData(false)
    } else {
      setget_accounts1a859((prev: any) => ({ ...prev, accountsubtype: ''}))
      setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,accountsubtype:undefined}))
  ...
  useEffect(() => {
    if(initialCount!=0)
     setget_accounts1a859((pre:any)=>({...pre,accountsubtype:""}))
    else
      setInitialCount(1)
  },[accountsubtypeb9399?.refresh])

  if (accountsubtypeb9399?.isHidden) {
    return <></>
  }

  return (
    <div 
className="flex flex-col  "      style={{gridColumn: `7 / 13`,gridRow: `64 / 84`, gap:``, height: `100%`, overflow: 'auto'}} >
      <div>
        <Text className="pb-2">Account Sub Type</Text>
      </div>
      <Dropdown
        className=""
        placeholder={keyset("accountsubtype")} 
        filterable={true}
        hasClear={true}
        static={true}
        staticProps={items}
        disabled= {accountsubtypeb9399?.isDisabled ? true : false}
        width = "250"
        value={get_accounts1a859?.accountsubtype ?get_accounts1a859?.accountsubtype : []}
        onChange={handleClick} 
        validationState={validate?.accountsubtype ? "invalid" : undefined}
        errorMessage={error}
      /> 
...
export default Dropdownaccountsubtype;
