...
const DatePickerTransactionToDateTime = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {
...
const handleUpdate = async(date: any) => {
  ...
  setError('')
  setValidate((pre:any)=>({...pre,transactiontodatetime:undefined}))
  setget_accounts1a859((prev: any) => ({ ...prev, transactiontodatetime: isoDate }))
}
...
useEffect(()=>{
 ...
 },[transactiontodatetime00c33?.refresh])

  useEffect(()=>{
      handleBlur()
  },[validateRefetch.value])

if (transactiontodatetime00c33?.isHidden) {
  return <></>
}
return (
  <div 
      className="flex flex-col " 
  style={{gridColumn: `1 / 7`,gridRow: `43 / 63`, gap:``, height: `100%`, overflow: 'auto'}} >
    <div>
    <Text className="pb-2">Transaction To Date Times</Text>
    </div>
    <DatePicker
      className=""
      label={keyset("")}
      value={get_accounts1a859?.transactiontodatetime}
      onUpdate= {handleUpdate}
      onBlur= {()=>handleBlur()} 
      style={{width:'100%'}}     
      readOnly=  {transactiontodatetime00c33?.isDisabled ? true : false}
      disabled= {transactiontodatetime00c33?.isDisabled ? true : false}
      size='m'
      validationState={validate?.transactiontodatetime ? "invalid" : undefined}
      errorMessage={error}
      />
  </div>
  )
...
export default DatePickerTransactionToDateTime
