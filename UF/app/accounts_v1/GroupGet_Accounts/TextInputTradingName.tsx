...
const TextInputTradingName = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  let schemaArray :any =[];  
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'tradingname',type:"text"})
  ...
  const handleChange = async(e: any) => {
    setError('')
    setValidate((pre:any)=>({...pre,tradingname:undefined}))
    if(dynamicStateandType.type=="number"){
    setget_accounts1a859((prev: any) => ({ ...prev, tradingname: +e.target.value }))
    }
    else{
    setget_accounts1a859((prev: any) => ({ ...prev, tradingname: e.target.value }))
    }
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "d99cbe43beb245ccbd7a8f5741822dd3",
          isTable: false,
          from:"TextInputtradingname",
      ...
      if(orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties){
        let type:any={name:'tradingname',type:'text'}
        type={
          name:'tradingname',
          type: orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.tradingname.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.tradingname.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.tradingname.type
        }
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setget_accounts1a859((pre:any)=>({...pre,tradingname:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
  ...
  if (tradingname22dd3?.isHidden) {
    return <></>
  }
  return (   
    <div 
className="flex flex-col "      style={{gridColumn: `1 / 7`,gridRow: `85 / 105`, gap:``, height: `100%`, overflow: 'auto'}} >
        <div>
          <Text className="pb-2">{keyset("Trading Name")}
      ...
      <TextInput
        ...
        className=""
        label={keyset("TradingName")}
        onChange= {handleChange}
        onBlur={handleBlur}
        type={dynamicStateandType.type}
        value={get_accounts1a859?.tradingname||""}
         disabled= {tradingname22dd3?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        readOnly= {tradingname22dd3?.isDisabled ? true : false}
        size='m'      
        view='normal'
        validationState={validate?.tradingname ? "invalid" : undefined}
        errorMessage={error}
      />
    </div> 
  )
...
export default TextInputTradingName
