...
const DropdownUserType = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagCompData}: any) => {
 ...
 /////////////
  ...
  useEffect(() => {
  if(get_accounts1a859?.usertype=="" || get_accounts1a859?.usertype==undefined || get_accounts1a859?.usertype==null ){
    setSelectedItem("");
  }
  },[get_accounts1a859?.usertype])
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "8d9d102a65ff4a0ab4900a6e6a4218a1",
          isTable: false,
          accessProfile:accessProfile,
          from:"dropdownUserType"
        },
        {
          headers: {
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setget_accounts1a859((prev: any) => ({ ...prev, usertype: value}))
      setIsRequredData(false)
    } else {
      setget_accounts1a859((prev: any) => ({ ...prev, usertype: ''}))
      setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,usertype:undefined}))
  ...
  useEffect(() => {
    if(initialCount!=0)
     setget_accounts1a859((pre:any)=>({...pre,usertype:""}))
    else
      setInitialCount(1)
  },[usertype218a1?.refresh])

  if (usertype218a1?.isHidden) {
    return <></>
  }

  return (
    <div 
className="flex flex-col  "      style={{gridColumn: `7 / 13`,gridRow: `148 / 168`, gap:``, height: `100%`, overflow: 'auto'}} >
      <div>
        <Text className="pb-2">User Type</Text>
      </div>
      <Dropdown
        className=""
        placeholder={keyset("UserType")} 
        filterable={true}
        hasClear={true}
        static={true}
        staticProps={items}
        disabled= {usertype218a1?.isDisabled ? true : false}
        width = "250"
        value={get_accounts1a859?.usertype ?get_accounts1a859?.usertype : []}
        onChange={handleClick} 
        validationState={validate?.usertype ? "invalid" : undefined}
        errorMessage={error}
      /> 
      {validate?.usertype && (
