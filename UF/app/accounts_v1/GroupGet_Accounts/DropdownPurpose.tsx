...
const DropdownPurpose = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagCompData}: any) => {
 ...
 /////////////
  ...
  useEffect(() => {
  if(get_accounts1a859?.purpose=="" || get_accounts1a859?.purpose==undefined || get_accounts1a859?.purpose==null ){
    setSelectedItem("");
  }
  },[get_accounts1a859?.purpose])
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "8a982b6fe4a547bd8d5adcd091c3c50a",
          isTable: false,
          accessProfile:accessProfile,
          from:"dropdownPurpose"
        },
        {
          headers: {
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setget_accounts1a859((prev: any) => ({ ...prev, purpose: value}))
      setIsRequredData(false)
    } else {
      setget_accounts1a859((prev: any) => ({ ...prev, purpose: ''}))
  ...
  useEffect(() => {
    if(initialCount!=0)
     setget_accounts1a859((pre:any)=>({...pre,purpose:""}))
    else
      setInitialCount(1)
  },[purpose3c50a?.refresh])

  if (purpose3c50a?.isHidden) {
    return <></>
  }

  return (
    <div 
className="flex flex-col  "      style={{gridColumn: `1 / 7`,gridRow: `169 / 189`, gap:``, height: `100%`, overflow: 'auto'}} >
      <div>
        <Text className="pb-2">Purpose</Text>
      </div>
      <Dropdown
        className=""
        placeholder={keyset("Purpose")} 
        filterable={true}
        hasClear={true}
        static={true}
        staticProps={items}
        disabled= {purpose3c50a?.isDisabled ? true : false}
        width = "250"
        value={get_accounts1a859?.purpose ?get_accounts1a859?.purpose : []}
...
export default DropdownPurpose;
