...
const TextInputLegalName = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  let schemaArray :any =[];  
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'legalname',type:"text"})
  ...
  const handleChange = async(e: any) => {
    setError('')
    setValidate((pre:any)=>({...pre,legalname:undefined}))
    if(dynamicStateandType.type=="number"){
    setget_accounts1a859((prev: any) => ({ ...prev, legalname: +e.target.value }))
    }
    else{
    setget_accounts1a859((prev: any) => ({ ...prev, legalname: e.target.value }))
    }
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "70bf2879ae2c40dd9e6e6f3ee20bccff",
          isTable: false,
          from:"TextInputlegalname",
      ...
      if(orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties){
        let type:any={name:'legalname',type:'text'}
        type={
          name:'legalname',
          type: orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.legalname.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.legalname.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.legalname.type
        }
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setget_accounts1a859((pre:any)=>({...pre,legalname:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
  ...
  if (legalnamebccff?.isHidden) {
    return <></>
  }
  return (   
    <div 
className="flex flex-col "      style={{gridColumn: `7 / 13`,gridRow: `85 / 105`, gap:``, height: `100%`, overflow: 'auto'}} >
        <div>
          <Text className="pb-2">{keyset("Legal Name")}
      ...
      <TextInput
        ...
        className=""
        label={keyset("LegalName")}
        onChange= {handleChange}
        onBlur={handleBlur}
        type={dynamicStateandType.type}
        value={get_accounts1a859?.legalname||""}
         disabled= {legalnamebccff?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        readOnly= {legalnamebccff?.isDisabled ? true : false}
        size='m'      
        view='normal'
        validationState={validate?.legalname ? "invalid" : undefined}
        errorMessage={error}
      />
    </div> 
  )
...
export default TextInputLegalName
