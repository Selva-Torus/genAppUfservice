...
const TextInputUrl = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  let schemaArray :any =[];  
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'url',type:"text"})
  ...
  const handleChange = async(e: any) => {
    setError('')
    setValidate((pre:any)=>({...pre,url:undefined}))
    if(dynamicStateandType.type=="number"){
    setget_accounts1a859((prev: any) => ({ ...prev, url: +e.target.value }))
    }
    else{
    setget_accounts1a859((prev: any) => ({ ...prev, url: e.target.value }))
    }
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "7a5f145461c0450989f905a1d4fe0b3a",
          isTable: false,
          from:"TextInputurl",
      ...
      if(orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties){
        let type:any={name:'url',type:'text'}
        type={
          name:'url',
          type: orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.url.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.url.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.url.type
        }
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setget_accounts1a859((pre:any)=>({...pre,url:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
  ...
  if (urle0b3a?.isHidden) {
    return <></>
  }
  return (   
    <div 
className="flex flex-col "      style={{gridColumn: `7 / 13`,gridRow: `169 / 189`, gap:``, height: `100%`, overflow: 'auto'}} >
        <div>
          <Text className="pb-2">{keyset("Url")}
      ...
      <TextInput
        ...
        value={get_accounts1a859?.url||""}
         disabled= {urle0b3a?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        readOnly= {urle0b3a?.isDisabled ? true : false}
        size='m'      
        view='normal'
        validationState={validate?.url ? "invalid" : undefined}
        errorMessage={error}
      />
    </div> 
  )
