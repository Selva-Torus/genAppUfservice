...
const TextInputAccountId = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  let schemaArray :any =[];  
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'accountid',type:"text"})
  ...
  const handleChange = async(e: any) => {
    setError('')
    setValidate((pre:any)=>({...pre,accountid:undefined}))
    if(dynamicStateandType.type=="number"){
    setget_accounts1a859((prev: any) => ({ ...prev, accountid: +e.target.value }))
    }
    else{
    setget_accounts1a859((prev: any) => ({ ...prev, accountid: e.target.value }))
    }
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "4dbaa621cd704ae9842d066e574b7d92",
          isTable: false,
          from:"TextInputaccountid",
      ...
      if(orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties){
        let type:any={name:'accountid',type:'text'}
        type={
          name:'accountid',
          type: orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.accountid.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.accountid.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.accountid.type
        }
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setget_accounts1a859((pre:any)=>({...pre,accountid:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
  ...
  if (accountidb7d92?.isHidden) {
    return <></>
  }
  return (   
    <div 
className="flex flex-col "      style={{gridColumn: `7 / 13`,gridRow: `43 / 63`, gap:``, height: `100%`, overflow: 'auto'}} >
        <div>
          <Text className="pb-2">{keyset("Account ID")}
      ...
      <TextInput
        ...
        className=""
        label={keyset("AccountId")}
        onChange= {handleChange}
        onBlur={handleBlur}
        type={dynamicStateandType.type}
        value={get_accounts1a859?.accountid||""}
         disabled= {accountidb7d92?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        readOnly= {accountidb7d92?.isDisabled ? true : false}
        size='m'      
        view='normal'
        validationState={validate?.accountid ? "invalid" : undefined}
        errorMessage={error}
      />
    </div> 
  )
...
export default TextInputAccountId
