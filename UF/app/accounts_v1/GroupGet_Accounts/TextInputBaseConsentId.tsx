...
const TextInputBaseConsentId = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  let schemaArray :any =[];  
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'baseconsentid',type:"text"})
  ...
  const handleChange = async(e: any) => {
    setError('')
    setValidate((pre:any)=>({...pre,baseconsentid:undefined}))
    if(dynamicStateandType.type=="number"){
    setget_accounts1a859((prev: any) => ({ ...prev, baseconsentid: +e.target.value }))
    }
    else{
    setget_accounts1a859((prev: any) => ({ ...prev, baseconsentid: e.target.value }))
    }
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "a35bed03152f438f8260a0c8eaf56ba8",
          isTable: false,
          from:"TextInputbaseconsentid",
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setget_accounts1a859((pre:any)=>({...pre,baseconsentid:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
  ...
  if (baseconsentid56ba8?.isHidden) {
    return <></>
  }
  return (   
    <div 
className="flex flex-col "      style={{gridColumn: `7 / 13`,gridRow: `1 / 21`, gap:``, height: `100%`, overflow: 'auto'}} >
        <div>
          <Text className="pb-2">{keyset("Base Consent ID")}
      ...
      <TextInput
        ...
        className=""
        label={keyset("BaseConsentId")}
        onChange= {handleChange}
        onBlur={handleBlur}
        type={dynamicStateandType.type}
        value={get_accounts1a859?.baseconsentid||""}
         disabled= {baseconsentid56ba8?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        readOnly= {baseconsentid56ba8?.isDisabled ? true : false}
        size='m'      
        view='normal'
        validationState={validate?.baseconsentid ? "invalid" : undefined}
        errorMessage={error}
      />
    </div> 
  )
...
export default TextInputBaseConsentId
