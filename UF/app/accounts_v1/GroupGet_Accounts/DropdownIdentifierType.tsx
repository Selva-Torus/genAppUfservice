...
const DropdownIdentifierType = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagCompData}: any) => {
 ...
 /////////////
  ...
  useEffect(() => {
  if(get_accounts1a859?.identifiertype=="" || get_accounts1a859?.identifiertype==undefined || get_accounts1a859?.identifiertype==null ){
    setSelectedItem("");
  }
  },[get_accounts1a859?.identifiertype])
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "e1d6573f389f4c19a5681f5835637db2",
          isTable: false,
          accessProfile:accessProfile,
          from:"dropdownIdentifierType"
        },
        {
          headers: {
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setget_accounts1a859((prev: any) => ({ ...prev, identifiertype: value}))
      setIsRequredData(false)
    } else {
      setget_accounts1a859((prev: any) => ({ ...prev, identifiertype: ''}))
      setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,identifiertype:undefined}))
  ...
  useEffect(() => {
    if(initialCount!=0)
     setget_accounts1a859((pre:any)=>({...pre,identifiertype:""}))
    else
      setInitialCount(1)
  },[identifiertype37db2?.refresh])

  if (identifiertype37db2?.isHidden) {
    return <></>
  }

  return (
    <div 
className="flex flex-col  "      style={{gridColumn: `1 / 7`,gridRow: `106 / 126`, gap:``, height: `100%`, overflow: 'auto'}} >
      <div>
        <Text className="pb-2">Identifier Type</Text>
      </div>
      <Dropdown
        className=""
        placeholder={keyset("IdentifierType")} 
        filterable={true}
        hasClear={true}
        static={true}
        staticProps={items}
        disabled= {identifiertype37db2?.isDisabled ? true : false}
        width = "250"
        value={get_accounts1a859?.identifiertype ?get_accounts1a859?.identifiertype : []}
        onChange={handleClick} 
        validationState={validate?.identifiertype ? "invalid" : undefined}
        errorMessage={error}
      /> 
...
export default DropdownIdentifierType;
