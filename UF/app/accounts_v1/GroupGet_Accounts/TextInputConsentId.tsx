...
const TextInputConsentId = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  let schemaArray :any =[];  
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'consentid',type:"text"})
  ...
  const handleChange = async(e: any) => {
    setError('')
    setValidate((pre:any)=>({...pre,consentid:undefined}))
    if(dynamicStateandType.type=="number"){
    setget_accounts1a859((prev: any) => ({ ...prev, consentid: +e.target.value }))
    }
    else{
    setget_accounts1a859((prev: any) => ({ ...prev, consentid: e.target.value }))
    }
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "059191992b004d5eb1262d21228a3e0f",
          isTable: false,
          from:"TextInputconsentid",
      ...
      if(orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties){
        let type:any={name:'consentid',type:'text'}
        type={
          name:'consentid',
          type: orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.consentid.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.consentid.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.consentid.type
        }
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setget_accounts1a859((pre:any)=>({...pre,consentid:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
  ...
  if (consentida3e0f?.isHidden) {
    return <></>
  }
  return (   
    <div 
className="flex flex-col "      style={{gridColumn: `1 / 7`,gridRow: `127 / 147`, gap:``, height: `100%`, overflow: 'auto'}} >
        <div>
          <Text className="pb-2">{keyset("Consent ID")}
      ...
      <TextInput
        ...
        className=""
        label={keyset("ConsentId")}
        onChange= {handleChange}
        onBlur={handleBlur}
        type={dynamicStateandType.type}
        value={get_accounts1a859?.consentid||""}
         disabled= {consentida3e0f?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        readOnly= {consentida3e0f?.isDisabled ? true : false}
        size='m'      
        view='normal'
        validationState={validate?.consentid ? "invalid" : undefined}
        errorMessage={error}
      />
    </div> 
  )
...
export default TextInputConsentId
