...
const Dropdowntype = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagCompData}: any) => {
 ...
 /////////////
  ...
  useEffect(() => {
  if(get_accounts1a859?.type=="" || get_accounts1a859?.type==undefined || get_accounts1a859?.type==null ){
    setSelectedItem("");
  }
  },[get_accounts1a859?.type])
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "166117b7162f4e13a18d59a7b6e16590",
          isTable: false,
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setget_accounts1a859((prev: any) => ({ ...prev, type: value}))
      setIsRequredData(false)
    } else {
      setget_accounts1a859((prev: any) => ({ ...prev, type: ''}))
      setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,type:undefined}))
  ...
  useEffect(() => {
    if(initialCount!=0)
     setget_accounts1a859((pre:any)=>({...pre,type:""}))
    else
      setInitialCount(1)
  },[type16590?.refresh])

  if (type16590?.isHidden) {
    return <></>
  }

  return (
    <div 
className="flex flex-col  "      style={{gridColumn: `1 / 7`,gridRow: `1 / 21`, gap:``, height: `100%`, overflow: 'auto'}} >
      <div>
        <Text className="pb-2">Types</Text>
      </div>
      <Dropdown
        className=""
        placeholder={keyset("Types")} 
        filterable={true}
        hasClear={true}
        static={true}
        staticProps={items}
        disabled= {type16590?.isDisabled ? true : false}
        width = "250"
        value={get_accounts1a859?.type ?get_accounts1a859?.type : []}
        onChange={handleClick} 
        validationState={validate?.type ? "invalid" : undefined}
        errorMessage={error}
      /> 
      {validate?.type && (
