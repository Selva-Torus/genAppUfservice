...
const TextInputcheque_no = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'cheque_no',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,ITAX_Payment_Details_v1:{...pre?.ITAX_Payment_Details_v1,cheque_no:undefined}}));
    if(dynamicStateandType.type=="number"){
    setpayment_type_cheque_group239dd((prev: any) => ({ ...prev, cheque_no: +e.target.value }));
    }
    else{
    setpayment_type_cheque_group239dd((prev: any) => ({ ...prev, cheque_no: e.target.value }));
    }
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['prn_details_group']  = {...prn_details_group00560,cheque_no:newInputValue},
      codeStates['setprn_details_group'] = setprn_details_group00560,
      codeStates['prn_datails_table']  = {...prn_datails_table2ad52,cheque_no:newInputValue},
      codeStates['setprn_datails_table'] = setprn_datails_table2ad52,
      codeStates['subscreen_group']  = {...subscreen_groupc0414,cheque_no:newInputValue},
      codeStates['setsubscreen_group'] = setsubscreen_groupc0414,
      codeStates['ct010_af_uf_ufws_i001_itax_itax_payment_type_cheque_v1']  = {...ct010_af_uf_ufws_i001_itax_itax_payment_type_cheque_v19ea86,cheque_no:newInputValue},
      ...
      codeStates['payment_type_cheque_group']  = {...payment_type_cheque_group239dd,cheque_no:newInputValue},
      codeStates['setpayment_type_cheque_group'] = setpayment_type_cheque_group239dd,
      codeStates['ct010_af_uf_ufws_i001_itax_itax_payment_type_directtransfer_v1']  = {...ct010_af_uf_ufws_i001_itax_itax_payment_type_directtransfer_v1dd0c7,cheque_no:newInputValue},
      ...
      codeStates['payment_type_dt_group']  = {...payment_type_dt_groupedf52,cheque_no:newInputValue},
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "a788406469b94061bdfb7dd5fa2cda2a",
          isTable: false,
          from:"TextInputcheque_no",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'cheque_no', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'cheque_no',type:'text'};
      //   type={
      //     name:'cheque_no',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.cheque_no.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.cheque_no.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.cheque_no.type
      //   }
      ...
      //   type={
      //     name:'cheque_no',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.cheque_no.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.cheque_no.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.cheque_no.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  if (cheque_nocda2a?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 25`,gridRow: `46 / 56`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={payment_type_cheque_group239dd?.cheque_no||""}
        numberFormat={dynamicStateandType.type === "number" ? "" : "none"}
         disabled= {cheque_nocda2a?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.ITAX_Payment_Details_v1?.cheque_no ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputcheque_no
