...
const TextInputdebit_amount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_Payment_Details:AFVK:v1|6b1dff68ee4848a7a60e2092f2b239dd|12510ed2b65b4832a7be8e1e680f2e0e"
    }
  ],
  "schemaData": {
    "type": "number"
  ...
  const handleChange = async(e: any) => {
    ...
    setpayment_type_cheque_group239dd((prev: any) => ({ ...prev, debit_amount: +e.target.value }));
    }
    else{
    setpayment_type_cheque_group239dd((prev: any) => ({ ...prev, debit_amount: e.target.value }));
    }
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "12510ed2b65b4832a7be8e1e680f2e0e",
          isTable: false,
  ...
  if(dfd_itax_source_tran_dfd_v1Props?.setSearchFilters && dfd_itax_source_tran_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_itax_source_tran_dfd_v1Props.data) && dfd_itax_source_tran_dfd_v1Props.data.length > 0){
      setpayment_type_cheque_group239dd((pre:any)=>({...pre,debit_amount:dfd_itax_source_tran_dfd_v1Props.data[0]?.debit_amount}));
    }
  }
  },[dfd_itax_source_tran_dfd_v1Props?.setSearchFilters])
  if (debit_amountf2e0e?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={payment_type_cheque_group239dd?.debit_amount||""}
        numberFormat={dynamicStateandType.type === "number" ? "" : "none"}
         disabled= {debit_amountf2e0e?.isDisabled ? true : false}
        pin='brick-brick'     
