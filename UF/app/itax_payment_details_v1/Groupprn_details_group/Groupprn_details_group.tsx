...
import Groupprn_datails_table  from "../Groupprn_datails_table/Groupprn_datails_table";
import Groupsubscreen_group  from "../Groupsubscreen_group/Groupsubscreen_group";
...
import Textprn_details  from "./Textprn_details";
import Textpayment_details  from "./Textpayment_details";
import Textpayment_type_label  from "./Textpayment_type_label";
import Dropdownpayment_type_dropdown  from "./Dropdownpayment_type_dropdown";
import Buttonclear  from "./Buttonclear";
import Buttonmake_payment  from "./Buttonmake_payment";
...
const Groupprn_details_group = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_Payment_Details:AFVK:v1",componentId:"4725fb7b4b994db2bb5cd1ad08100560",from:"GroupPrnDetailsGroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("prn_details")){
      setprn_details7320a({...prn_details7320a,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("prn_datails_table")){
      setprn_datails_table2ad52({...prn_datails_table2ad52,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("payment_details")){
      setpayment_details5a762({...payment_details5a762,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("payment_type_label")){
      setpayment_type_labelb5c98({...payment_type_labelb5c98,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("payment_type_dropdown")){
      setpayment_type_dropdownb558f({...payment_type_dropdownb558f,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("subscreen_group")){
      setsubscreen_groupc0414({...subscreen_groupc0414,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("clear")){
      setclear47c6a({...clear47c6a,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("make_payment")){
      setmake_payment3a4e8({...make_payment3a4e8,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const prn_details_group00560Ref = useRef<any>(null);
  const handleClearSearch = () => {
    prn_details_group00560Ref.current?.setSearchParams();
    prn_details_group00560Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(prn_details_group00560) && Object.keys(prn_details_group00560)?.length>0)
      {
        setprn_details_group00560({})
      }
    }else 
      prevRefreshRef.current= true
  }, [prn_details_group00560Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("prn_datails_table")  &&<Groupprn_datails_table  
        ...
        {allowedComponent.includes("subscreen_group")  &&<Groupsubscreen_group  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          primaryTableData={primaryTableData}
          setPrimaryTableData={setPrimaryTableData}
          ...
          paginationDetails={paginationDetails}
          setIsProcessing={setIsProcessing}        />}
          {allowedControls.includes("prn_details") ?<Textprn_details   /* 7320a */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
          {allowedControls.includes("payment_details") ?<Textpayment_details   /* 5a762 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
          {allowedControls.includes("payment_type_label") ?<Textpayment_type_label   /* b5c98 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
        {allowedControls.includes("payment_type_dropdown") ?<Dropdownpayment_type_dropdown   /* b558f */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} lockedData ={lockedData} setLockedData={setLockedData} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
        ...
        {        (("make_payment" in ButtonGoRuleData)?ButtonGoRuleData["make_payment"]:true) && 
          allowedControls.includes("make_payment")  ?            <Buttonmake_payment lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing}/>: <div></div>} 
    </div>
 )
}

export default Groupprn_details_group
