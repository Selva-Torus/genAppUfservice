...
const TextInputtotal_amount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_Payment_Details:AFVK:v1|9f8b1c13bbde4e59b630d88d4acedf52|b79be5028bef4a19aebdce18cd7e161d"
    }
  ],
  "schemaData": {
    "type": "number"
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,ITAX_Payment_Details_v1:{...pre?.ITAX_Payment_Details_v1,total_amount:undefined}}));
    if(dynamicStateandType.type=="number"){
    setpayment_type_dt_groupedf52((prev: any) => ({ ...prev, total_amount: +e.target.value }));
    }
    else{
    setpayment_type_dt_groupedf52((prev: any) => ({ ...prev, total_amount: e.target.value }));
    }
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['prn_details_group']  = {...prn_details_group00560,total_amount:newInputValue},
      codeStates['setprn_details_group'] = setprn_details_group00560,
      codeStates['prn_datails_table']  = {...prn_datails_table2ad52,total_amount:newInputValue},
      codeStates['setprn_datails_table'] = setprn_datails_table2ad52,
      codeStates['subscreen_group']  = {...subscreen_groupc0414,total_amount:newInputValue},
      codeStates['setsubscreen_group'] = setsubscreen_groupc0414,
      codeStates['ct010_af_uf_ufws_i001_itax_itax_payment_type_cheque_v1']  = {...ct010_af_uf_ufws_i001_itax_itax_payment_type_cheque_v19ea86,total_amount:newInputValue},
      ...
      codeStates['payment_type_cheque_group']  = {...payment_type_cheque_group239dd,total_amount:newInputValue},
      codeStates['setpayment_type_cheque_group'] = setpayment_type_cheque_group239dd,
      codeStates['ct010_af_uf_ufws_i001_itax_itax_payment_type_directtransfer_v1']  = {...ct010_af_uf_ufws_i001_itax_itax_payment_type_directtransfer_v1dd0c7,total_amount:newInputValue},
      ...
      codeStates['payment_type_dt_group']  = {...payment_type_dt_groupedf52,total_amount:newInputValue},
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "b79be5028bef4a19aebdce18cd7e161d",
          isTable: false,
          from:"TextInputtotal_amount",
  ...
  if(dfd_itax_source_tran_dfd_v1Props?.setSearchFilters && dfd_itax_source_tran_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_itax_source_tran_dfd_v1Props.data) && dfd_itax_source_tran_dfd_v1Props.data.length > 0){
      setpayment_type_dt_groupedf52((pre:any)=>({...pre,total_amount:dfd_itax_source_tran_dfd_v1Props.data[0]?.total_amount}));
    }
  }
  },[dfd_itax_source_tran_dfd_v1Props?.setSearchFilters])
  if (total_amounte161d?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 25`,gridRow: `24 / 34`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={payment_type_dt_groupedf52?.total_amount||""}
        numberFormat={dynamicStateandType.type === "number" ? "" : "none"}
         disabled= {total_amounte161d?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.ITAX_Payment_Details_v1?.total_amount ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputtotal_amount
