...
const TextInputdebit_amount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT010:FNGK:AF:FNK:DF-DFD:CATK:I001:AFGK:ITAX:AFK:ITAX_Source_Tran_DFD:AFVK:v1|e94f0b0bfd204105bf68851e77eecc7b|properties.debit_amount"
      ],
      "targetKey": "CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_Payment_Details:AFVK:v1|9f8b1c13bbde4e59b630d88d4acedf52|eefa5b5028d34ea3b922628a83dbfd7f"
    }
  ],
  "schemaData": {
    "type": "number"
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'debit_amount',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,ITAX_Payment_Details_v1:{...pre?.ITAX_Payment_Details_v1,debit_amount:undefined}}));
    if(dynamicStateandType.type=="number"){
    setpayment_type_dt_groupedf52((prev: any) => ({ ...prev, debit_amount: +e.target.value }));
    }
    else{
    setpayment_type_dt_groupedf52((prev: any) => ({ ...prev, debit_amount: e.target.value }));
    }
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['prn_details_group']  = {...prn_details_group00560,debit_amount:newInputValue},
      codeStates['setprn_details_group'] = setprn_details_group00560,
      codeStates['prn_datails_table']  = {...prn_datails_table2ad52,debit_amount:newInputValue},
      codeStates['setprn_datails_table'] = setprn_datails_table2ad52,
      codeStates['subscreen_group']  = {...subscreen_groupc0414,debit_amount:newInputValue},
      codeStates['setsubscreen_group'] = setsubscreen_groupc0414,
      codeStates['ct010_af_uf_ufws_i001_itax_itax_payment_type_cheque_v1']  = {...ct010_af_uf_ufws_i001_itax_itax_payment_type_cheque_v19ea86,debit_amount:newInputValue},
      ...
      codeStates['payment_type_cheque_group']  = {...payment_type_cheque_group239dd,debit_amount:newInputValue},
      codeStates['setpayment_type_cheque_group'] = setpayment_type_cheque_group239dd,
      codeStates['ct010_af_uf_ufws_i001_itax_itax_payment_type_directtransfer_v1']  = {...ct010_af_uf_ufws_i001_itax_itax_payment_type_directtransfer_v1dd0c7,debit_amount:newInputValue},
      ...
      codeStates['payment_type_dt_group']  = {...payment_type_dt_groupedf52,debit_amount:newInputValue},
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "eefa5b5028d34ea3b922628a83dbfd7f",
          isTable: false,
          from:"TextInputdebit_amount",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'debit_amount', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'debit_amount',type:'text'};
      //   type={
      //     name:'debit_amount',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.debit_amount.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.debit_amount.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.debit_amount.type
      //   }
      ...
      //   type={
      //     name:'debit_amount',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.debit_amount.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.debit_amount.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.debit_amount.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  if(dfd_itax_source_tran_dfd_v1Props?.setSearchFilters && dfd_itax_source_tran_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_itax_source_tran_dfd_v1Props.data) && dfd_itax_source_tran_dfd_v1Props.data.length > 0){
      setpayment_type_dt_groupedf52((pre:any)=>({...pre,debit_amount:dfd_itax_source_tran_dfd_v1Props.data[0]?.debit_amount}));
    }
  }
  },[dfd_itax_source_tran_dfd_v1Props?.setSearchFilters])
  if (debit_amountbfd7f?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 25`,gridRow: `35 / 45`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={payment_type_dt_groupedf52?.debit_amount||""}
        numberFormat={dynamicStateandType.type === "number" ? "" : "none"}
         disabled= {debit_amountbfd7f?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.ITAX_Payment_Details_v1?.debit_amount ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputdebit_amount
