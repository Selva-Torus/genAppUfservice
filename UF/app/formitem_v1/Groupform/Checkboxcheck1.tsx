...
const Checkboxcheck1 = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
 
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'check1',type:"text"})
 ...
 /////////////
  ...
  const handleChange=async(e:any)=>{
    setformdaeb3((prev: any) => ({ ...prev, check1: e.target.checked}));
  // show as profile 
  let filterProps =  [
  {
    "name": "DFD Key",
  ...
  if (check1238c5?.isHidden) {
    return <></>;
  }

  return (
    <div 
       style={{gridColumn: `3 / 5`,gridRow: `1 / 2`,marginTop: `auto`, gap:`10px`}} >
    <Checkbox 
    className=""
    value={formdaeb3?.check1||false}
    checked={formdaeb3?.check1||false}
      disabled= {check1238c5?.isDisabled ? true : false}
