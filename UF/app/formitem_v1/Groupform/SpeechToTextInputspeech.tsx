...
import { TorusSpeechToTextInput } from '@/app/TorusComponents/SpeechToTextInput';
...
const SpeechToTextInputspeech = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
 const actionDetails :any = {
  ...
  let schemaArray :any =[];  
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'speech',type:""})
  ...
  const handleChange = (e: any) => {
    ...
    setformdaeb3((prev: any) => ({ ...prev, speech: +e.target.value }))
    }
    else{
    setformdaeb3((prev: any) => ({ ...prev, speech: e.target.value }))
    }
  ...
  return (   
    <div 
      ...
      className=""
        ...
        value={formdaeb3?.speech||""}
      disabled= {speechbcd41?.isDisabled ? true : false}
      />
    </div>
        
  )
}
export default SpeechToTextInputspeech