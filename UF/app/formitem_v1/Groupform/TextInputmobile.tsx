...
const TextInputmobile = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
 const actionDetails :any = {
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT003:FNGK:AF:FNK:DF-DFD:CATK:CG:AFGK:TG2:AFK:vesselDFD:AFVK:v1|279177efdf6040008fa93b58c074a266|properties.mobile"
      ],
      "targetKey": "CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:CG:AFGK:TG2:AFK:showProfile:AFVK:v1|e5f95e127d7e42c8a14f50eca18daeb3|78c9f7fc3ea9447491c34e9febd5fccb"
    }
  ],
  "dstData": [],
  "schemaData": {
  ...
  let schemaArray :any =[];  
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'mobile',type:"text"})
  ...
  const handleChange = async(e: any) => {
    ...
    setformdaeb3((prev: any) => ({ ...prev, mobile: +e.target.value }))
    }
    else{
    setformdaeb3((prev: any) => ({ ...prev, mobile: e.target.value }))
    }
  ...
  const handleBlur=async () => {
    ...
    if (mobile5fccb?.isHidden) {
      return <></>
    }

  return (   
    <div 
      style={{gridColumn: `1 / 3`,gridRow: `2 / 3`,marginTop: `auto`, gap:`10px`}} >
      <TorusTextInput
      className=""
        label={keyset("mobile")}
        ...
        value={formdaeb3?.mobile||""}
      disabled= {mobile5fccb?.isDisabled ? true : false}
          readOnly= {mobile5fccb?.isDisabled ? true : false}
      />
    </div>
