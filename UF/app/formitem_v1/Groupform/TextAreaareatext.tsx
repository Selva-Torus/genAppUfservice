...
const Textareaareatext = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  const encryptionFlagCont: boolean = encryptionFlagCompData.flag || false ;
  let encryptionDpd: string = "";
  encryptionDpd = encryptionDpd !=='' ? encryptionDpd: encryptionFlagCompData.dpd
  let encryptionMethod: string = "";
  encryptionMethod  = encryptionMethod !=='' ? encryptionMethod: encryptionFlagCompData.method
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'areatext',type:"text"})
 ...
 /////////////
  ...
  const {transactionsuf_v1Props, settransactionsuf_v1Props}= useContext(TotalContext) as TotalContextProps;
    useEffect(()=>{
      setformdaeb3((pre:any)=>({...pre,areatext:""}))
    },[areatext565ce?.refresh])
  ...
  const handleChange = async(e: any) => {
    if(dynamicStateandType.type=="number"){
      setformdaeb3((prev: any) => ({ ...prev, areatext: +e.value }))
    }
    else{
      setformdaeb3((prev: any) => ({ ...prev, areatext: e.value }))
    }
  ...
  if (areatext565ce?.isHidden) {
    return <></>
  }

return (
  <div 
  style={{gridColumn: `11 / 13`,gridRow: `1 / 2`,marginTop: `auto`, gap:`10px`}} >
  ...
  <TextArea
    ...
    onBlur={handleBlur}
    value = { formdaeb3?.areatext != null && typeof formdaeb3?.areatext =='object' ? Object.keys(formdaeb3?.areatext)?.length ?  JSON.stringify(formdaeb3?.areatext,null ,2):"" : formdaeb3?.areatext||""}
      disabled= {areatext565ce?.isDisabled ? true : false}
        readOnly=  {areatext565ce?.isDisabled ? true : false}
    minRows={0}
    maxRows={0}
  />
  </div>
...
export default Textareaareatext