...
const Grouptab_header_2 = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT309:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:newTab:AFVK:v1",componentId:"468065e296114e2f99e5fb6690214783",from:"GroupTabHeader2",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("textinput")){
      settextinput88309({...textinput88309,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const tab_header_214783Ref = useRef<any>(null);
  const handleClearSearch = () => {
    tab_header_214783Ref.current?.setSearchParams();
    tab_header_214783Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(tab_header_214783) && Object.keys(tab_header_214783)?.length>0)
      {
        settab_header_214783({})
      }
    }else 
      prevRefreshRef.current= true
  }, [tab_header_214783Props?.refresh])
  ...
  return (
    ...
    >
        {allowedControls.includes("textinput") ?<TextInputtextinput   /* 88309 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default Grouptab_header_2
