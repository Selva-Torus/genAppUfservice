...
import Tabsoldtab  from "./Tabsoldtab";
...
const Groupoldtabgroup = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT309:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:newTab:AFVK:v1",componentId:"a12c6162b8d44d718a83c60e2ff527ef",from:"GroupOldtabgroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("oldtab")){
      setoldtaba33e3({...oldtaba33e3,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("tab2")){
      settab29f914({...tab29f914,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const oldtabgroup527efRef = useRef<any>(null);
  const handleClearSearch = () => {
    oldtabgroup527efRef.current?.setSearchParams();
    oldtabgroup527efRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(oldtabgroup527ef) && Object.keys(oldtabgroup527ef)?.length>0)
      {
        setoldtabgroup527ef({})
      }
    }else 
      prevRefreshRef.current= true
  }, [oldtabgroup527efProps?.refresh])
  ...
  return (
    ...
    >
        ...
        {allowedControls.includes("oldtab") ?<Tabsoldtab /* a33e3 */ encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
    </div>
 )
}

export default Groupoldtabgroup
