...
let defaultColumns = [
  {
    "id": "ids",
    "nodeid": "45898d957b884d11b476e858b4024da4",
    "name": "ids",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "name",
    "nodeid": "8818972c280f4155acc111fb4335545c",
    "name": "name",
    "meta": {
      "sort": true
    },
...
const Tabletable2 = ({ lockedData,setLockedData,primaryTableData, setPrimaryTableData,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData }: any)=>{
 ...
 /////////////
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    settable2c0657Props((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          settable2c0657Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
    ...
    try {
      ...
      if (isRulePresent==false) {
        ...
        const api_paginationBody: api_paginationDto = {
          ...
          count: parseInt(pageSize),
          filterDetails: {
            ufKey:'CK:CT309:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:newTab:AFVK:v1:UO', 
            nodeId: 'b901da00e7f54fc5b5c8181aee8c0657',
            elementId: 'b901da00e7f54fc5b5c8181aee8c0657'
          },
          searchFilter: searchParams
        }
        if(encryptionFlagCont) {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        ...
        settable2c0657(uf_paginationDataFilter.data||[])
...
const colurIndicator = (keyValue:any=[], comingValue:any) => {
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(table2c0657Props.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [table2c0657Props.filterProps])
  ...
  const handlePrimaryTable = () => {
    let findData = table2c0657Props?.selectedIds[table2c0657Props?.selectedIds?.length-1]
    if(Array.isArray(table2c0657) && table2c0657.length>0)
    {
      let data = table2c0657[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "id",
        value: data["id"],
  ...
  useEffect(() => {
    if (table2c0657Props?.selectedIds?.length != 0) handlePrimaryTable()
  }, [table2c0657Props?.selectedIds])
    ...
    <div className='w-full h-full'>
          <div className=' w-full flex flex-row h-[80%]'>
            <Table
              ...
              edgePadding={true}
              selectedIds={table2c0657Props?.selectedIds}  
