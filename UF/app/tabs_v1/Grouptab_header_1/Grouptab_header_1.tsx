...
const Grouptab_header_1 = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT309:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:newTab:AFVK:v1",componentId:"871cb96d26f446a881a08c9e9ab2cce3",from:"GroupTabHeader1",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("code")){
      setcode86871({...code86871,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("button")){
      setbutton823f2({...button823f2,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const tab_header_12cce3Ref = useRef<any>(null);
  const handleClearSearch = () => {
    tab_header_12cce3Ref.current?.setSearchParams();
    tab_header_12cce3Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(tab_header_12cce3) && Object.keys(tab_header_12cce3)?.length>0)
      {
        settab_header_12cce3({})
      }
    }else 
      prevRefreshRef.current= true
  }, [tab_header_12cce3Props?.refresh])
...
export default Grouptab_header_1
