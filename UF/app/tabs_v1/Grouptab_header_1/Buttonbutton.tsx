...
const Buttonbutton = ({ lockedData,setLockedData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}: { lockedData:any,setLockedData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,}) => {
 ...
 /////////////
    ...
    try{     
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "a86797c2747641debfaa2905cb6823f2",
          isTable: false,
          from:"Buttonbutton",
  ...
  },[button823f2?.refresh,currentToken])
  ...
  const handleClick=async()=>{
    if(tab_header_12cce3Props?.validation==true && tab_header_12cce3Props?.required==true || tab_header_12cce3Props?.required==true)
    {
 ...
 if (button823f2?.isHidden) {
    return <></>
  }
 
  return (
    <div
      style={{gridColumn: `2 / 8`,gridRow: `86 / 96`, gap:``, height: `100%`, overflow: 'auto'}} 
      >
        {showFlag && <Button 
          ref={buttonRef}
          className=""
          onClick={handleClick}
          view='action'
          disabled= {button823f2?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
        >
          {keyset("button")}
