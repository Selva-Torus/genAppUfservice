...
const TextInputcode = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  let schemaArray :any =[];  
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'code',type:"text"})
  ...
  const {code86871, setcode86871}= useContext(TotalContext) as TotalContextProps;
  const {button823f2, setbutton823f2}= useContext(TotalContext) as TotalContextProps;
  ...
  const handleChange = async(e: any) => {
    ...
    settab_header_12cce3((prev: any) => ({ ...prev, code: +e.target.value }))
    }
    else{
    settab_header_12cce3((prev: any) => ({ ...prev, code: e.target.value }))
    }
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          componentId: "871cb96d26f446a881a08c9e9ab2cce3",
          controlId: "b0bfb72f7b0f4eba9ceee7818cc86871",
          isTable: false,
          from:"TextInputcode",
      ...
      if(orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties){
        let type:any={name:'code',type:'text'}
        type={
          name:'code',
          type: orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.code.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.code.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.code.type
        }
        ...
        type={
          name:'code',
          type: orchestrationData?.data?.schemaData[0].schema.properties.code.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.properties.code.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.properties.code.type
        }
        setDynamicStateandType(type)
      }
      }
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // settab_header_12cce3((pre:any)=>({...pre,code:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
  ...
  return (   
    <div  
      style={{gridColumn: `5 / 11`,gridRow: `9 / 19`, gap:``, height: `100%`, overflow: 'auto'}} >
      ...
      <TextInput
        ...
        value={tab_header_12cce3?.code||""}
         disabled= {code86871?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
        validationState={validate?.code ? "invalid" : undefined}
        errorMessage={error}
      />
    </div> 
  )
