...
import Groupoldtabgroup  from "./Groupoldtabgroup/Groupoldtabgroup";


export default function PageTabsV1() {
  ...
  const {newtab_v1Props, setnewtab_v1Props} = useContext(TotalContext) as TotalContextProps;
  const [checkgroup,setCheckgroup,]=useState(false);
  const [checkoldtabgroup,setCheckoldtabgroup,]=useState(false);
  const [checktable2,setChecktable2,]=useState(false);
  ...
  const [paginationData,setPaginationData]=useState<any>({count:10,page:1})
    ...
    async function mydfddata_v1(pagination:any){
        ...
        if(newtab_v1Props.length > 0){
          let filterData :any[] =[];
          for(let i=0;i< newtab_v1Props.length;i++){
            if(newtab_v1Props[i].DFDkey == "CK:CT309:FNGK:AF:FNK:DF-DFD:CATK:AG001:AFGK:A001:AFK:myDfdData:AFVK:v1"){
              delete newtab_v1Props[i].DFDkey;
              filterData.push(newtab_v1Props[i])
            }           
          }
  ...
  async function securityCheck() {
    const orchestrationData = await AxiosService.post("/UF/Orchestration",{key:"CK:CT309:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:newTab:AFVK:v1",accessProfile:[user],from:"pageTabsV1"},{
      headers: {
        Authorization: `Bearer ${token}`
      }});
  ...
  "events": {}
    ...
    await mydfddata_v1(pagination)
          ...
          allowedGroup.map((nodes:any)=>{
            ...
            if(nodes?.groupName == 'oldtabgroup' && (nodes?.security== 'AA' || nodes?.security == 'ATO'))
            {
              setCheckoldtabgroup(true)
            }
            if(nodes?.groupName == 'table2' && (nodes?.security== 'AA' || nodes?.security == 'ATO'))
            {
              setChecktable2(true)
            }
          })
    ...
    <>
     ...
     style={{
       ...
       // minHeight: '100vh',
        ...
        {checkoldtabgroup && initialLoad &&<Groupoldtabgroup  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          primaryTableData={primaryTableData}
          setPrimaryTableData={setPrimaryTableData}
