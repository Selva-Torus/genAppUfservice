...
import Groupmaingroup  from "./Groupmaingroup/Groupmaingroup";
...
export default function PageSaveV1() {
  ...
  const {savescreen_v1Props, setsavescreen_v1Props} = useContext(TotalContext) as TotalContextProps;
  const [checkmaingroup,setCheckmaingroup,]=useState(false);
  const [checkuserable,setCheckuserable,]=useState(false);
  ...
  const [paginationData,setPaginationData]=useState<any>({count:10,page:1})
    ...
    async function mydfddata_v1(pagination:any){
        ...
        if(savescreen_v1Props.length > 0){
          let filterData :any[] =[];
          for(let i=0;i< savescreen_v1Props.length;i++){
            if(savescreen_v1Props[i].DFDkey == "CK:CT309:FNGK:AF:FNK:DF-DFD:CATK:AG001:AFGK:A001:AFK:myDfdData:AFVK:v1"){
              delete savescreen_v1Props[i].DFDkey;
              filterData.push(savescreen_v1Props[i])
            }           
          }
  ...
  async function securityCheck() {
    const orchestrationData = await AxiosService.post("/UF/Orchestration",{key:"CK:CT309:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:savescreen:AFVK:v1",accessProfile:[user],from:"pageSaveV1"},{
      headers: {
        Authorization: `Bearer ${token}`
      }});
  ...
  "events": {}
    ...
    await mydfddata_v1(pagination)
          ...
          allowedGroup.map((nodes:any)=>{
            if(nodes?.groupName == 'maingroup' && (nodes?.security== 'AA' || nodes?.security == 'ATO'))
            {
              setCheckmaingroup(true)
            }
            if(nodes?.groupName == 'userable' && (nodes?.security== 'AA' || nodes?.security == 'ATO'))
            {
              setCheckuserable(true)
            }
          })
          }
           }catch(err:any)
    ...
    <>
     ...
     style={{
       ...
       // minHeight: '100vh',
        ...
        {checkmaingroup && initialLoad &&<Groupmaingroup  
