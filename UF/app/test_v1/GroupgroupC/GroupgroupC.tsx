...
const GroupgroupC = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:TT407:FNGK:AF:FNK:UF-UFW:CATK:CGFA:AFGK:TG4CGFA:AFK:forPFCheckUF:AFVK:v1",componentId:"ad81b8df07604f66a40f345977559a19",from:"GroupGroupc",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("save")){
      setsave99cfb({...save99cfb,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(groupc59a19) && Object.keys(groupc59a19)?.length>0)
      {
        setgroupc59a19({})
      }
    }else 
      prevRefreshRef.current= true
  }, [groupc59a19Props?.refresh])

  return (
    <div style = {{"gridAutoRows":"50px","display":"grid","gridTemplateColumns":"repeat(12, 1fr)","gridTemplateRows":"repeat(auto-fill, minmax(50px, 1fr))","gridColumn":"2 / 7","gridRow":"9 / 16","height":"100vh","overflow":"auto"}}
