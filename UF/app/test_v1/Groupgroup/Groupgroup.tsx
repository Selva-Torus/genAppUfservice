...
const Groupgroup = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "1917aeed17e846bba0eec1c2417d23af");
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("button")){
        setbuttond4624((pre:any)=>({...pre,isDisabled:true}));

    }else
    {
        if(buttond4624?.isDisabled==null)
      {
        setbuttond4624((pre:any)=>({...pre,isDisabled:false}));
      }
    }
  //////////////
    if (code != '') {
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if (
        !Array.isArray(groupd23af) &&
        Object.keys(groupd23af)?.length > 0
      ) {
        setgroupd23af({})
      }
    } else prevRefreshRef.current = true
  }, [groupd23afProps?.refresh])
    ...
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md  ${isDark ? 'text-white' : 'text-black'}`}
       onClick={(e:any)=>{e.stopPropagation()
        handleOnClick({}, 0);
          settest_v1((pre:any)=>({...pre,_selectedGroup_:"group"}))
        }}
    >
