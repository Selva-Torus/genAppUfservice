...
export default function PageTestV1({ onReady }: { onReady?: () => void } = {}) {
  ...
  async function securityCheck(): Promise<void> {
    ...
    if (token) {
      const basePath = process.env.NEXT_PUBLIC_BASE_PATH ?? "";
      const res = await fetch(`${basePath}/next-api/auth/introspect?key=CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:test:AFVK:v1`)
      if (!res.ok) {
        logout()
        return
      }
  ...
  useEffect(() => {    
    ...
    handleOnload();
    settest_v1((pre:any)=>({...pre,...allRuleData||{}}))
  }, [])
  ...
  useEffect(() => {
    const handleClickOutside = (event:any) => {
      if (parentRef.current && !parentRef.current.contains(event.target)) {
        settest_v1((pre:any)=>({...pre,_selectedGroup_:""}))
      }
    };
