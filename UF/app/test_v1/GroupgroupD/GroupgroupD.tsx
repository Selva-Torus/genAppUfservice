...
const GroupgroupD = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:TT407:FNGK:AF:FNK:UF-UFW:CATK:CGFA:AFGK:TG4CGFA:AFK:forPFCheckUF:AFVK:v1",componentId:"e486baa5a83d4ece8da9390582fe191f",from:"GroupGroupd",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("address")){
      setaddress332cb({...address332cb,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(groupde191f) && Object.keys(groupde191f)?.length>0)
      {
        setgroupde191f({})
      }
    }else 
      prevRefreshRef.current= true
  }, [groupde191fProps?.refresh])

  return (
    <div style = {{"gridAutoRows":"50px","display":"grid","gridTemplateColumns":"repeat(12, 1fr)","gridTemplateRows":"repeat(auto-fill, minmax(50px, 1fr))","gridColumn":"7 / 12","gridRow":"9 / 16","height":"100vh","overflow":"auto"}}
        className=" rounded-md " >
        {allowedControls.includes("address") ?<TextAreaaddress   /* 332cb */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData}/>: <div></div>}
    </div>             
  )
}
