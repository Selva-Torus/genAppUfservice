...
const Textfailuer_process_code = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      if(Array.isArray(dfd_tran_journey_dtl_v1Props) && dfd_tran_journey_dtl_v1Props){
        settran_journey_error_dtl_grouped0e7((pre:any)=>({...pre,failure_process_code:dfd_tran_journey_dtl_v1Props[0]?.failure_process_code}));
      }
    }catch(err){
      console.log(err)
    }
  ...
  useEffect(()=>{
    handleMapperValue()
  },[failuer_process_codee5490?.refresh])
  ...
  if (failuer_process_codee5490?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {isDynamic ? item?.failure_process_code : (tran_journey_error_dtl_grouped0e7?.failure_process_code || "")}
</Text>
  </div>
  )
}
