...
const Texttrs_created_date = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      if(Array.isArray(dfd_tran_journey_dtl_v1Props) && dfd_tran_journey_dtl_v1Props){
        settran_journey_error_dtl_grouped0e7((pre:any)=>({...pre,version:dfd_tran_journey_dtl_v1Props[0]?.version}));
      }
    }catch(err){
      console.log(err)
    }
  ...
  if (trs_created_datee861b?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {isDynamic ? item?.version : (tran_journey_error_dtl_grouped0e7?.version || "")}
</Text>
  </div>
  )
}

export default Texttrs_created_date
