...
import Groupsearch_group  from "./Groupsearch_group/Groupsearch_group";


export default function PageItaxMainScreenSearchV1() {
  ...
  const {itax_main_screen_search_v1Props, setitax_main_screen_search_v1Props} = useContext(TotalContext) as TotalContextProps;
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function itax_source_tran_dfd_v1(pagination:any): Promise<void>{
        ...
        if(itax_main_screen_search_v1Props.length > 0){
          let filterData :any[] =[];
          for(let i=0;i< itax_main_screen_search_v1Props.length;i++){
            if(itax_main_screen_search_v1Props[i].DFDkey == "CK:CT010:FNGK:AF:FNK:DF-DFD:CATK:I001:AFGK:ITAX:AFK:ITAX_Source_Tran_DFD:AFVK:v1"){
              delete itax_main_screen_search_v1Props[i].DFDkey;
              filterData.push(itax_main_screen_search_v1Props[i])
            }           
          }
  ...
  async function securityCheck(): Promise<void> {
    const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_Main_Screen_Search:AFVK:v1",accessProfile:[user],from:"pageItaxMainScreenSearchV1"},{
      headers: {
        Authorization: `Bearer ${token}`
      }});
