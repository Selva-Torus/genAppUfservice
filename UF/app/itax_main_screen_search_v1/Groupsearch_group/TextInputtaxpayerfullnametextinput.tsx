...
const TextInputtaxpayerfullnametextinput = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_Main_Screen_Search:AFVK:v1|67a3af3c7cf8400e86a421b514e9a617|480b8dace8d14e0c8fbf365c54f0ac43"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,ITAX_Main_Screen_Search_v1:{...pre?.ITAX_Main_Screen_Search_v1,tax_payer_full_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setsearch_group9a617((prev: any) => ({ ...prev, tax_payer_full_name: +e.target.value }));
    }
    else{
    setsearch_group9a617((prev: any) => ({ ...prev, tax_payer_full_name: e.target.value }));
    }
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['search_group']  = {...search_group9a617,tax_payer_full_name:newInputValue},
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "480b8dace8d14e0c8fbf365c54f0ac43",
          isTable: false,
          from:"TextInputtaxpayerfullnametextinput",
  ...
  if(dfd_itax_source_tran_dfd_v1Props?.setSearchFilters && dfd_itax_source_tran_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_itax_source_tran_dfd_v1Props.data) && dfd_itax_source_tran_dfd_v1Props.data.length > 0){
      setsearch_group9a617((pre:any)=>({...pre,tax_payer_full_name:dfd_itax_source_tran_dfd_v1Props.data[0]?.tax_payer_full_name}));
    }
  }
  },[dfd_itax_source_tran_dfd_v1Props?.setSearchFilters])
  if (taxpayerfullnametextinput0ac43?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `17 / 23`,gridRow: `5 / 26`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={search_group9a617?.tax_payer_full_name||""}
        numberFormat={dynamicStateandType.type === "number" ? "" : "none"}
         disabled= {taxpayerfullnametextinput0ac43?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Tax Payer Full Name"
      errorMessage={error}
        validationState={validate?.ITAX_Main_Screen_Search_v1?.tax_payer_full_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputtaxpayerfullnametextinput
