...
const Texterror_code = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_itax_tran_error_log_dfd_v1Props && !dfd_itax_tran_error_log_dfd_v1Props.hasLogicCenter) {
        ...
        setview_error_detail_group21845((pre: any) => {
          return { ...pre, error_code: api_paginationData.data.records[0]?.error_code }
        })
        }
      }
      else{
      if(Array.isArray(dfd_itax_tran_error_log_dfd_v1Props) && dfd_itax_tran_error_log_dfd_v1Props && !view_error_detail_group21845.error_code){
        setview_error_detail_group21845((pre:any)=>({...pre,error_code:dfd_itax_tran_error_log_dfd_v1Props[0]?.error_code}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[error_codeba00c?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_itax_tran_error_log_dfd_v1Props) && !view_error_detail_group21845.error_code){
    setview_error_detail_group21845((pre:any)=>({...pre,error_code:dfd_itax_tran_error_log_dfd_v1Props[0]?.error_code}));
  }
  },[dfd_itax_tran_error_log_dfd_v1Props])

  if (error_codeba00c?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 13`,gridRow: `32 / 42`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className=""
  variant="subheader-2"
  color="primary"
>
      {keyset(isDynamic ? item?.error_code : (view_error_detail_group21845?.error_code || ""))}
</Text>
  </div>
  )
}

export default Texterror_code
