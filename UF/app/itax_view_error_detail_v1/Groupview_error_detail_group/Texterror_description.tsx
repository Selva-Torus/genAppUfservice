...
const Texterror_description = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_itax_tran_error_log_dfd_v1Props && !dfd_itax_tran_error_log_dfd_v1Props.hasLogicCenter) {
        ...
        setview_error_detail_group21845((pre: any) => {
          return { ...pre, error_desc: api_paginationData.data.records[0]?.error_desc }
        })
        }
      }
      else{
      if(Array.isArray(dfd_itax_tran_error_log_dfd_v1Props) && dfd_itax_tran_error_log_dfd_v1Props && !view_error_detail_group21845.error_desc){
        setview_error_detail_group21845((pre:any)=>({...pre,error_desc:dfd_itax_tran_error_log_dfd_v1Props[0]?.error_desc}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[error_description64756?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_itax_tran_error_log_dfd_v1Props) && !view_error_detail_group21845.error_desc){
    setview_error_detail_group21845((pre:any)=>({...pre,error_desc:dfd_itax_tran_error_log_dfd_v1Props[0]?.error_desc}));
  }
  },[dfd_itax_tran_error_log_dfd_v1Props])

  if (error_description64756?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `13 / 25`,gridRow: `32 / 42`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className=""
  variant="subheader-2"
  color="primary"
>
      {keyset(isDynamic ? item?.error_desc : (view_error_detail_group21845?.error_desc || ""))}
</Text>
  </div>
  )
}

export default Texterror_description
