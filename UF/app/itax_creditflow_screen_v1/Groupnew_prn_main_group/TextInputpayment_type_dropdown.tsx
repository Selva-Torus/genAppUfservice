...
const TextInputpayment_type_dropdown = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'payment_type_dropdown',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,ITAX_CreditFlow_Screen_v1:{...pre?.ITAX_CreditFlow_Screen_v1,payment_type_dropdown:undefined}}));
    if(dynamicStateandType.type=="number"){
    setnew_prn_main_group21910((prev: any) => ({ ...prev, payment_type_dropdown: +e.target.value }));
    }
    else{
    setnew_prn_main_group21910((prev: any) => ({ ...prev, payment_type_dropdown: e.target.value }));
    }
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['new_prn_main_group']  = {...new_prn_main_group21910,payment_type_dropdown:newInputValue},
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "9c3668a393a741c6aeb7fe247995d344",
          isTable: false,
          from:"TextInputpayment_type_dropdown",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'payment_type_dropdown', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'payment_type_dropdown',type:'text'};
      //   type={
      //     name:'payment_type_dropdown',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.payment_type_dropdown.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.payment_type_dropdown.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.payment_type_dropdown.type
      //   }
      ...
      //   type={
      //     name:'payment_type_dropdown',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.payment_type_dropdown.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.payment_type_dropdown.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.payment_type_dropdown.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  if (payment_type_dropdown5d344?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 25`,gridRow: `23 / 33`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={new_prn_main_group21910?.payment_type_dropdown||""}
        numberFormat={dynamicStateandType.type === "number" ? "" : "none"}
         disabled= {payment_type_dropdown5d344?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.ITAX_CreditFlow_Screen_v1?.payment_type_dropdown ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputpayment_type_dropdown
