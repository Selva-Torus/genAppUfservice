...
const Textauth_memo_label = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
  ...
  useEffect(()=>{
    handleMapperValue()
  },[auth_memo_labelf0a0e?.refresh])

  if (auth_memo_labelf0a0e?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 13`,gridRow: `78 / 88`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className=""
  variant="subheader-3"
  color="primary"
>
      {keyset("Authorization Memo")}
</Text>
  </div>
  )
}

export default Textauth_memo_label
