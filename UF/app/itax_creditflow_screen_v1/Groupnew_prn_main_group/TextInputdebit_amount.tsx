...
const TextInputdebit_amount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_CreditFlow_Screen:AFVK:v1|f1099583e1124434b28d0c4b0be21910|957e524daf7e43dc9563eec16d0bbf1f"
    }
  ],
  "schemaData": {
    "type": "number"
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,ITAX_CreditFlow_Screen_v1:{...pre?.ITAX_CreditFlow_Screen_v1,debit_amount:undefined}}));
    if(dynamicStateandType.type=="number"){
    setnew_prn_main_group21910((prev: any) => ({ ...prev, debit_amount: +e.target.value }));
    }
    else{
    setnew_prn_main_group21910((prev: any) => ({ ...prev, debit_amount: e.target.value }));
    }
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['new_prn_main_group']  = {...new_prn_main_group21910,debit_amount:newInputValue},
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "957e524daf7e43dc9563eec16d0bbf1f",
          isTable: false,
  ...
  if(dfd_itax_source_tran_dfd_v1Props?.setSearchFilters && dfd_itax_source_tran_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_itax_source_tran_dfd_v1Props.data) && dfd_itax_source_tran_dfd_v1Props.data.length > 0){
      setnew_prn_main_group21910((pre:any)=>({...pre,debit_amount:dfd_itax_source_tran_dfd_v1Props.data[0]?.debit_amount}));
    }
  }
  },[dfd_itax_source_tran_dfd_v1Props?.setSearchFilters])
  if (debit_amountbbf1f?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 25`,gridRow: `56 / 66`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={new_prn_main_group21910?.debit_amount||""}
        numberFormat={dynamicStateandType.type === "number" ? "" : "none"}
         disabled= {debit_amountbbf1f?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.ITAX_CreditFlow_Screen_v1?.debit_amount ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
