...
const Textfilename = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
  ...
  useEffect(()=>{
    handleMapperValue()
  },[filename4f410?.refresh])

  if (filename4f410?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `13 / 25`,gridRow: `78 / 88`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className=""
  variant="subheader-2"
  color="primary"
>
      {keyset(isDynamic ? item?.filename : (new_prn_main_group21910?.filename || ""))}
</Text>
  </div>
  )
}

export default Textfilename
