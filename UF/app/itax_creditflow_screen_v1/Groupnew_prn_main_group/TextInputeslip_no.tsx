...
const TextInputeslip_no = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,ITAX_CreditFlow_Screen_v1:{...pre?.ITAX_CreditFlow_Screen_v1,eslip_no:undefined}}));
    if(dynamicStateandType.type=="number"){
    setnew_prn_main_group21910((prev: any) => ({ ...prev, eslip_no: +e.target.value }));
    }
    else{
    setnew_prn_main_group21910((prev: any) => ({ ...prev, eslip_no: e.target.value }));
    }
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['new_prn_main_group']  = {...new_prn_main_group21910,eslip_no:newInputValue},
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "5a94af4b39ef4ff28ac288c0b6ee1f20",
          isTable: false,
          from:"TextInputeslip_no",
  ...
  if (eslip_noe1f20?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 25`,gridRow: `12 / 22`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={new_prn_main_group21910?.eslip_no||""}
        numberFormat={dynamicStateandType.type === "number" ? "" : "none"}
         disabled= {eslip_noe1f20?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.ITAX_CreditFlow_Screen_v1?.eslip_no ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputeslip_no
