...
const TextInputdebit_account_no = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,ITAX_CreditFlow_Screen_v1:{...pre?.ITAX_CreditFlow_Screen_v1,debit_account_no:undefined}}));
    if(dynamicStateandType.type=="number"){
    setnew_prn_main_group21910((prev: any) => ({ ...prev, debit_account_no: +e.target.value }));
    }
    else{
    setnew_prn_main_group21910((prev: any) => ({ ...prev, debit_account_no: e.target.value }));
    }
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['new_prn_main_group']  = {...new_prn_main_group21910,debit_account_no:newInputValue},
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "43dfe14492ca43ab8dfce1b686d9ec5d",
          isTable: false,
  ...
  if (debit_account_no9ec5d?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 25`,gridRow: `34 / 44`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={new_prn_main_group21910?.debit_account_no||""}
        numberFormat={dynamicStateandType.type === "number" ? "" : "none"}
         disabled= {debit_account_no9ec5d?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.ITAX_CreditFlow_Screen_v1?.debit_account_no ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
