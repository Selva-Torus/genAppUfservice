...
const TextInputtax_payer_full_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,ITAX_CreditFlow_Screen_v1:{...pre?.ITAX_CreditFlow_Screen_v1,tax_payer_full_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setnew_prn_main_group21910((prev: any) => ({ ...prev, tax_payer_full_name: +e.target.value }));
    }
    else{
    setnew_prn_main_group21910((prev: any) => ({ ...prev, tax_payer_full_name: e.target.value }));
    }
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['new_prn_main_group']  = {...new_prn_main_group21910,tax_payer_full_name:newInputValue},
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "08ceb7840b8e4bffb3455888f4a8bf4d",
          isTable: false,
          from:"TextInputtax_payer_full_name",
  ...
  if (tax_payer_full_name8bf4d?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 25`,gridRow: `45 / 55`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={new_prn_main_group21910?.tax_payer_full_name||""}
        numberFormat={dynamicStateandType.type === "number" ? "" : "none"}
         disabled= {tax_payer_full_name8bf4d?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        contentAlign={"left"}
      errorMessage={error}
        validationState={validate?.ITAX_CreditFlow_Screen_v1?.tax_payer_full_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputtax_payer_full_name
