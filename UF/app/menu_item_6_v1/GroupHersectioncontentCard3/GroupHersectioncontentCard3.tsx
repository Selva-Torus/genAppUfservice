...
import IconcardIcon3  from "./IconcardIcon3";
import TextcardText3  from "./TextcardText3";
import TextcardSubtext3  from "./TextcardSubtext3";
...
const GroupHersectioncontentCard3 = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const {cardicon39c7df, setcardicon39c7df}= useContext(TotalContext) as TotalContextProps;
  const {cardtext3d6a42, setcardtext3d6a42}= useContext(TotalContext) as TotalContextProps;
  const {cardsubtext39c9b7, setcardsubtext39c9b7}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:New_HomeScreen:AFVK:v1",componentId:"60f08ce5f1f64b609ef73bc9a4b9a879",from:"GroupHersectioncontentcard3",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cardicon3")){
      setcardicon39c7df({...cardicon39c7df,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("cardtext3")){
      setcardtext3d6a42({...cardtext3d6a42,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("cardsubtext3")){
      setcardsubtext39c9b7({...cardsubtext39c9b7,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const hersectioncontentcard39a879Ref = useRef<any>(null);
  const handleClearSearch = () => {
    hersectioncontentcard39a879Ref.current?.setSearchParams();
    hersectioncontentcard39a879Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(hersectioncontentcard39a879) && Object.keys(hersectioncontentcard39a879)?.length>0)
      {
        sethersectioncontentcard39a879({})
      }
    }else 
      prevRefreshRef.current= true
  }, [hersectioncontentcard39a879Props?.refresh])
  ...
  return (
    ...
    >
        {allowedControls.includes("cardicon3")?<IconcardIcon3 /* 9c7df */ encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
          {allowedControls.includes("cardtext3") ?<TextcardText3   /* d6a42 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
          {allowedControls.includes("cardsubtext3") ?<TextcardSubtext3   /* 9c9b7 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default GroupHersectioncontentCard3
