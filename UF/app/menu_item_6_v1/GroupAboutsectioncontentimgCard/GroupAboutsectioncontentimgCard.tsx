...
import TextAboutsectioncontentimgCardtxt1  from "./TextAboutsectioncontentimgCardtxt1";
import TextAboutsectioncontentimgCardtxt2  from "./TextAboutsectioncontentimgCardtxt2";
...
const GroupAboutsectioncontentimgCard = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const {aboutsectioncontentimgcardtxt1d3325, setaboutsectioncontentimgcardtxt1d3325}= useContext(TotalContext) as TotalContextProps;
  const {aboutsectioncontentimgcardtxt2e6ca9, setaboutsectioncontentimgcardtxt2e6ca9}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:New_HomeScreen:AFVK:v1",componentId:"628ca07625d641e59bd3b2c12061df84",from:"GroupAboutsectioncontentimgcard",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("aboutsectioncontentimgcardtxt1")){
      setaboutsectioncontentimgcardtxt1d3325({...aboutsectioncontentimgcardtxt1d3325,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("aboutsectioncontentimgcardtxt2")){
      setaboutsectioncontentimgcardtxt2e6ca9({...aboutsectioncontentimgcardtxt2e6ca9,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const aboutsectioncontentimgcard1df84Ref = useRef<any>(null);
  const handleClearSearch = () => {
    aboutsectioncontentimgcard1df84Ref.current?.setSearchParams();
    aboutsectioncontentimgcard1df84Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(aboutsectioncontentimgcard1df84) && Object.keys(aboutsectioncontentimgcard1df84)?.length>0)
      {
        setaboutsectioncontentimgcard1df84({})
      }
    }else 
      prevRefreshRef.current= true
  }, [aboutsectioncontentimgcard1df84Props?.refresh])
  ...
  return (
    <div 
      ...
      className={clsx("rounded-md shadow-md",
        "rounded-md",
        isDark ? "bg-gray-800 text-white" : "bg-white text-black"
      )}
    >
          {allowedControls.includes("aboutsectioncontentimgcardtxt1") ?<TextAboutsectioncontentimgCardtxt1   /* d3325 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
          {allowedControls.includes("aboutsectioncontentimgcardtxt2") ?<TextAboutsectioncontentimgCardtxt2   /* e6ca9 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default GroupAboutsectioncontentimgCard
