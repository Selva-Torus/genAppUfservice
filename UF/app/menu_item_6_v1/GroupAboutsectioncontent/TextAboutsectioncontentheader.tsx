...
const TextAboutsectioncontentheader = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
  ...
  useEffect(()=>{
    handleMapperValue()
  },[aboutsectioncontentheaderbda1f?.refresh])

  if (aboutsectioncontentheaderbda1f?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `7 / 13`,gridRow: `15 / 43`,gap:``, height: `100%`, overflow: 'auto'}} >
    <Text 
      className=""
      variant ="display-1"
      color ="primary"
    >
    {isDynamic ? item?.aboutsectioncontentheader : (aboutsectioncontent3ada4?.aboutsectioncontentheader || "")}
    </Text>
  </div>
  )
}

export default TextAboutsectioncontentheader
