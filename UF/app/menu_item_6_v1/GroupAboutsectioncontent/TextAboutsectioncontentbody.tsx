...
const TextAboutsectioncontentbody = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
  ...
  useEffect(()=>{
    handleMapperValue()
  },[aboutsectioncontentbody90f38?.refresh])

  if (aboutsectioncontentbody90f38?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `7 / 13`,gridRow: `43 / 75`,gap:``, height: `100%`, overflow: 'auto'}} >
    <Text 
      className="text-center"
      variant ="body-2"
      color ="primary"
    >
    {isDynamic ? item?.aboutsectioncontentbody : (aboutsectioncontent3ada4?.aboutsectioncontentbody || "")}
    </Text>
  </div>
  )
}

export default TextAboutsectioncontentbody
