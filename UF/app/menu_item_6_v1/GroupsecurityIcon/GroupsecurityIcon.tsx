...
import GroupiconWrapper  from "../GroupiconWrapper/GroupiconWrapper";
...
const GroupsecurityIcon = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const {99d63, set99d63}= useContext(TotalContext) as TotalContextProps;
  const {e8ad7, sete8ad7}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:New_HomeScreen:AFVK:v1",componentId:"fc6d93f0d54c464faa319371b623bde5",from:"GroupSecurityicon",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("iconwrapper")){
      seticonwrapper5dce2({...iconwrapper5dce2,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("")){
      set99d63({...99d63,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("")){
      sete8ad7({...e8ad7,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const securityicon3bde5Ref = useRef<any>(null);
  const handleClearSearch = () => {
    securityicon3bde5Ref.current?.setSearchParams();
    securityicon3bde5Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(securityicon3bde5) && Object.keys(securityicon3bde5)?.length>0)
      {
        setsecurityicon3bde5({})
      }
    }else 
      prevRefreshRef.current= true
  }, [securityicon3bde5Props?.refresh])
  ...
  return (
    <div 
      ...
      className={clsx("rounded-xl shadow-md",
        "rounded-md",
        isDark ? "bg-gray-800 text-white" : "bg-white text-black"
      )}
    >
        {allowedComponent.includes("iconwrapper")  &&<GroupiconWrapper  
          ...
          {allowedControls.includes("") ?<Text   /* 99d63 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
        {allowedControls.includes("")  ?<Button lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData}/>: <div></div>}          
    </div>
 )
}

export default GroupsecurityIcon
