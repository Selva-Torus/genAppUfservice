...
import TextAboutsectioncontentCardsprojecthead  from "./TextAboutsectioncontentCardsprojecthead";
import TextAboutsectioncontentCardsprojectsub  from "./TextAboutsectioncontentCardsprojectsub";
...
const GroupAboutsectioncontentCardsproject = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const {aboutsectioncontentcardsprojectheadef54b, setaboutsectioncontentcardsprojectheadef54b}= useContext(TotalContext) as TotalContextProps;
  const {aboutsectioncontentcardsprojectsubefc2a, setaboutsectioncontentcardsprojectsubefc2a}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:New_HomeScreen:AFVK:v1",componentId:"babb9e283b7145de89ebab91b5139023",from:"GroupAboutsectioncontentcardsproject",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("aboutsectioncontentcardsprojecthead")){
      setaboutsectioncontentcardsprojectheadef54b({...aboutsectioncontentcardsprojectheadef54b,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("aboutsectioncontentcardsprojectsub")){
      setaboutsectioncontentcardsprojectsubefc2a({...aboutsectioncontentcardsprojectsubefc2a,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const aboutsectioncontentcardsproject39023Ref = useRef<any>(null);
  const handleClearSearch = () => {
    aboutsectioncontentcardsproject39023Ref.current?.setSearchParams();
    aboutsectioncontentcardsproject39023Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(aboutsectioncontentcardsproject39023) && Object.keys(aboutsectioncontentcardsproject39023)?.length>0)
      {
        setaboutsectioncontentcardsproject39023({})
      }
    }else 
      prevRefreshRef.current= true
  }, [aboutsectioncontentcardsproject39023Props?.refresh])
  ...
  return (
    ...
    >
          {allowedControls.includes("aboutsectioncontentcardsprojecthead") ?<TextAboutsectioncontentCardsprojecthead   /* ef54b */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
          {allowedControls.includes("aboutsectioncontentcardsprojectsub") ?<TextAboutsectioncontentCardsprojectsub   /* efc2a */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default GroupAboutsectioncontentCardsproject
