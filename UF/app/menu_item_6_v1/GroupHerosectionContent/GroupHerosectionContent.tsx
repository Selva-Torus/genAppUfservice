...
import GroupHerSectioncontentCard  from "../GroupHerSectioncontentCard/GroupHerSectioncontentCard";
...
import ButtonAwardWinningAgency  from "./ButtonAwardWinningAgency";
import TextHersectionheading  from "./TextHersectionheading";
import TextHersectionbody  from "./TextHersectionbody";
import ButtonGetStartedToday  from "./ButtonGetStartedToday";
import ButtonViewOurWork  from "./ButtonViewOurWork";
...
const GroupHerosectionContent = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const {awardwinningagencyf5a9b, setawardwinningagencyf5a9b}= useContext(TotalContext) as TotalContextProps;
  const {hersectionheadingf316a, sethersectionheadingf316a}= useContext(TotalContext) as TotalContextProps;
  const {hersectionbodyf3ff1, sethersectionbodyf3ff1}= useContext(TotalContext) as TotalContextProps;
  const {getstartedtoday048fa, setgetstartedtoday048fa}= useContext(TotalContext) as TotalContextProps;
  const {viewourwork7b0db, setviewourwork7b0db}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:New_HomeScreen:AFVK:v1",componentId:"0971eb6db00949e6ab37b41fc3f0b52c",from:"GroupHerosectioncontent",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("awardwinningagency")){
      setawardwinningagencyf5a9b({...awardwinningagencyf5a9b,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("hersectionheading")){
      sethersectionheadingf316a({...hersectionheadingf316a,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("hersectionbody")){
      sethersectionbodyf3ff1({...hersectionbodyf3ff1,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("getstartedtoday")){
      setgetstartedtoday048fa({...getstartedtoday048fa,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("viewourwork")){
      setviewourwork7b0db({...viewourwork7b0db,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("hersectioncontentcard")){
      sethersectioncontentcard68df4({...hersectioncontentcard68df4,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const herosectioncontent0b52cRef = useRef<any>(null);
  const handleClearSearch = () => {
    herosectioncontent0b52cRef.current?.setSearchParams();
    herosectioncontent0b52cRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(herosectioncontent0b52c) && Object.keys(herosectioncontent0b52c)?.length>0)
      {
        setherosectioncontent0b52c({})
      }
    }else 
      prevRefreshRef.current= true
  }, [herosectioncontent0b52cProps?.refresh])
  ...
  return (
    <div 
      ...
      className={clsx("max-w-[1200px]",
        "rounded-md",
        isDark ? "bg-gray-800 text-white" : "bg-white text-black"
      )}
    >
        {allowedComponent.includes("hersectioncontentcard")  &&<GroupHerSectioncontentCard  
        ...
        {allowedControls.includes("awardwinningagency")  ?<ButtonAwardWinningAgency lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData}/>: <div></div>}          
          {allowedControls.includes("hersectionheading") ?<TextHersectionheading   /* f316a */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
          {allowedControls.includes("hersectionbody") ?<TextHersectionbody   /* f3ff1 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
        {allowedControls.includes("getstartedtoday")  ?<ButtonGetStartedToday lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData}/>: <div></div>}          
        {allowedControls.includes("viewourwork")  ?<ButtonViewOurWork lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData}/>: <div></div>}          
    </div>
 )
}

export default GroupHerosectionContent
