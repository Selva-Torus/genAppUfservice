...
import TextAboutsectioncontentCardsteamhead  from "./TextAboutsectioncontentCardsteamhead";
import TextAboutsectioncontentCardsteamsub  from "./TextAboutsectioncontentCardsteamsub";
...
const GroupAboutsectioncontentCardsteam = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const {aboutsectioncontentcardsteamheadcc537, setaboutsectioncontentcardsteamheadcc537}= useContext(TotalContext) as TotalContextProps;
  const {aboutsectioncontentcardsteamsub8c9ee, setaboutsectioncontentcardsteamsub8c9ee}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:New_HomeScreen:AFVK:v1",componentId:"755b52797d4644ffa744d1668fbcea5d",from:"GroupAboutsectioncontentcardsteam",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("aboutsectioncontentcardsteamhead")){
      setaboutsectioncontentcardsteamheadcc537({...aboutsectioncontentcardsteamheadcc537,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("aboutsectioncontentcardsteamsub")){
      setaboutsectioncontentcardsteamsub8c9ee({...aboutsectioncontentcardsteamsub8c9ee,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const aboutsectioncontentcardsteamcea5dRef = useRef<any>(null);
  const handleClearSearch = () => {
    aboutsectioncontentcardsteamcea5dRef.current?.setSearchParams();
    aboutsectioncontentcardsteamcea5dRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(aboutsectioncontentcardsteamcea5d) && Object.keys(aboutsectioncontentcardsteamcea5d)?.length>0)
      {
        setaboutsectioncontentcardsteamcea5d({})
      }
    }else 
      prevRefreshRef.current= true
  }, [aboutsectioncontentcardsteamcea5dProps?.refresh])
  ...
  return (
    ...
    >
          {allowedControls.includes("aboutsectioncontentcardsteamhead") ?<TextAboutsectioncontentCardsteamhead   /* cc537 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
          {allowedControls.includes("aboutsectioncontentcardsteamsub") ?<TextAboutsectioncontentCardsteamsub   /* 8c9ee */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default GroupAboutsectioncontentCardsteam
