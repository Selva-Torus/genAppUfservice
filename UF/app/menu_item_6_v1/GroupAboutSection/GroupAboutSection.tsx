...
import GroupAboutsectioncontent  from "../GroupAboutsectioncontent/GroupAboutsectioncontent";
...
const GroupAboutSection = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:New_HomeScreen:AFVK:v1",componentId:"24c1fa0673de44faa2436d1e6743edbb",from:"GroupAboutsection",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("aboutsectioncontent")){
      setaboutsectioncontent3ada4({...aboutsectioncontent3ada4,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const aboutsection3edbbRef = useRef<any>(null);
  const handleClearSearch = () => {
    aboutsection3edbbRef.current?.setSearchParams();
    aboutsection3edbbRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(aboutsection3edbb) && Object.keys(aboutsection3edbb)?.length>0)
      {
        setaboutsection3edbb({})
      }
    }else 
      prevRefreshRef.current= true
  }, [aboutsection3edbbProps?.refresh])
  ...
  return (
    ...
    >
        {allowedComponent.includes("aboutsectioncontent")  &&<GroupAboutsectioncontent  
