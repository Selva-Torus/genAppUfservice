...
import IconcardIcon1  from "./IconcardIcon1";
import TextcardText1  from "./TextcardText1";
import TextcardSubtext1  from "./TextcardSubtext1";
...
const GroupHersectioncontentCard1 = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const {cardicon15581a, setcardicon15581a}= useContext(TotalContext) as TotalContextProps;
  const {cardtext15e070, setcardtext15e070}= useContext(TotalContext) as TotalContextProps;
  const {cardsubtext17f94b, setcardsubtext17f94b}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:New_HomeScreen:AFVK:v1",componentId:"f403accf7b6f4338b83df8029cfb9c4a",from:"GroupHersectioncontentcard1",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cardicon1")){
      setcardicon15581a({...cardicon15581a,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("cardtext1")){
      setcardtext15e070({...cardtext15e070,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("cardsubtext1")){
      setcardsubtext17f94b({...cardsubtext17f94b,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const hersectioncontentcard1b9c4aRef = useRef<any>(null);
  const handleClearSearch = () => {
    hersectioncontentcard1b9c4aRef.current?.setSearchParams();
    hersectioncontentcard1b9c4aRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(hersectioncontentcard1b9c4a) && Object.keys(hersectioncontentcard1b9c4a)?.length>0)
      {
        sethersectioncontentcard1b9c4a({})
      }
    }else 
      prevRefreshRef.current= true
  }, [hersectioncontentcard1b9c4aProps?.refresh])
  ...
  return (
    ...
    >
        {allowedControls.includes("cardicon1")?<IconcardIcon1 /* 5581a */ encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
          {allowedControls.includes("cardtext1") ?<TextcardText1   /* 5e070 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
          {allowedControls.includes("cardsubtext1") ?<TextcardSubtext1   /* 7f94b */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default GroupHersectioncontentCard1
