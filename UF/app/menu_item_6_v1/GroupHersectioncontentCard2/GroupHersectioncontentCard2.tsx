...
import IconcardIcon2  from "./IconcardIcon2";
import TextcardText2  from "./TextcardText2";
import TextcardSubtext2  from "./TextcardSubtext2";
...
const GroupHersectioncontentCard2 = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:New_HomeScreen:AFVK:v1",componentId:"654e84826ada4e4aa926755bac9cd13f",from:"GroupHersectioncontentcard2",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cardicon2")){
      setcardicon20cb06({...cardicon20cb06,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("cardtext2")){
      setcardtext23d68e({...cardtext23d68e,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("cardsubtext2")){
      setcardsubtext23c206({...cardsubtext23c206,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const hersectioncontentcard2cd13fRef = useRef<any>(null);
  const handleClearSearch = () => {
    hersectioncontentcard2cd13fRef.current?.setSearchParams();
    hersectioncontentcard2cd13fRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(hersectioncontentcard2cd13f) && Object.keys(hersectioncontentcard2cd13f)?.length>0)
      {
        sethersectioncontentcard2cd13f({})
      }
    }else 
      prevRefreshRef.current= true
  }, [hersectioncontentcard2cd13fProps?.refresh])
  ...
  return (
    ...
    >
        {allowedControls.includes("cardicon2")?<IconcardIcon2 /* 0cb06 */ encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
          {allowedControls.includes("cardtext2") ?<TextcardText2   /* 3d68e */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
          {allowedControls.includes("cardsubtext2") ?<TextcardSubtext2   /* 3c206 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default GroupHersectioncontentCard2
