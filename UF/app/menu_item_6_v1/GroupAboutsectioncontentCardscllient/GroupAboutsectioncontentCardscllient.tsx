...
import TextAboutsectioncontentCardscllienthead  from "./TextAboutsectioncontentCardscllienthead";
import TextAboutsectioncontentCardscllient_sub  from "./TextAboutsectioncontentCardscllient_sub";
...
const GroupAboutsectioncontentCardscllient = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const {aboutsectioncontentcardscllientheadbf743, setaboutsectioncontentcardscllientheadbf743}= useContext(TotalContext) as TotalContextProps;
  const {aboutsectioncontentcardscllient_sube3c0b, setaboutsectioncontentcardscllient_sube3c0b}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:New_HomeScreen:AFVK:v1",componentId:"ac75af517ab64ae391b5869294a4f691",from:"GroupAboutsectioncontentcardscllient",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("aboutsectioncontentcardscllienthead")){
      setaboutsectioncontentcardscllientheadbf743({...aboutsectioncontentcardscllientheadbf743,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("aboutsectioncontentcardscllient_sub")){
      setaboutsectioncontentcardscllient_sube3c0b({...aboutsectioncontentcardscllient_sube3c0b,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const aboutsectioncontentcardscllient4f691Ref = useRef<any>(null);
  const handleClearSearch = () => {
    aboutsectioncontentcardscllient4f691Ref.current?.setSearchParams();
    aboutsectioncontentcardscllient4f691Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(aboutsectioncontentcardscllient4f691) && Object.keys(aboutsectioncontentcardscllient4f691)?.length>0)
      {
        setaboutsectioncontentcardscllient4f691({})
      }
    }else 
      prevRefreshRef.current= true
  }, [aboutsectioncontentcardscllient4f691Props?.refresh])
  ...
  return (
    ...
    >
          {allowedControls.includes("aboutsectioncontentcardscllienthead") ?<TextAboutsectioncontentCardscllienthead   /* bf743 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
          {allowedControls.includes("aboutsectioncontentcardscllient_sub") ?<TextAboutsectioncontentCardscllient_sub   /* e3c0b */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default GroupAboutsectioncontentCardscllient
