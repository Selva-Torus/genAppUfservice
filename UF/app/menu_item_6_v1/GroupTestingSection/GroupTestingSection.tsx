...
const GroupTestingSection = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:New_HomeScreen:AFVK:v1",componentId:"52051c83bc6f4f01a13033d8e97fd8df",from:"GroupTestingsection",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("")){
      seta3963({...a3963,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const testingsectionfd8dfRef = useRef<any>(null);
  const handleClearSearch = () => {
    testingsectionfd8dfRef.current?.setSearchParams();
    testingsectionfd8dfRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(testingsectionfd8df) && Object.keys(testingsectionfd8df)?.length>0)
      {
        settestingsectionfd8df({})
      }
    }else 
      prevRefreshRef.current= true
  }, [testingsectionfd8dfProps?.refresh])
...
export default GroupTestingSection
