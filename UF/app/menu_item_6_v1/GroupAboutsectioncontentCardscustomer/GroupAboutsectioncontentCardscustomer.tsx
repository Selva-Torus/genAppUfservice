...
import TextAboutsectioncontentCardscustomerhead  from "./TextAboutsectioncontentCardscustomerhead";
import TextAboutsectioncontentCardscustomersub  from "./TextAboutsectioncontentCardscustomersub";
...
const GroupAboutsectioncontentCardscustomer = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const {aboutsectioncontentcardscustomerhead56ea2, setaboutsectioncontentcardscustomerhead56ea2}= useContext(TotalContext) as TotalContextProps;
  const {aboutsectioncontentcardscustomersub0e559, setaboutsectioncontentcardscustomersub0e559}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:New_HomeScreen:AFVK:v1",componentId:"da3fce3e7f9a44d89ba55fae6812cf01",from:"GroupAboutsectioncontentcardscustomer",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("aboutsectioncontentcardscustomerhead")){
      setaboutsectioncontentcardscustomerhead56ea2({...aboutsectioncontentcardscustomerhead56ea2,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("aboutsectioncontentcardscustomersub")){
      setaboutsectioncontentcardscustomersub0e559({...aboutsectioncontentcardscustomersub0e559,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const aboutsectioncontentcardscustomer2cf01Ref = useRef<any>(null);
  const handleClearSearch = () => {
    aboutsectioncontentcardscustomer2cf01Ref.current?.setSearchParams();
    aboutsectioncontentcardscustomer2cf01Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(aboutsectioncontentcardscustomer2cf01) && Object.keys(aboutsectioncontentcardscustomer2cf01)?.length>0)
      {
        setaboutsectioncontentcardscustomer2cf01({})
      }
    }else 
      prevRefreshRef.current= true
  }, [aboutsectioncontentcardscustomer2cf01Props?.refresh])
  ...
  return (
    ...
    >
          {allowedControls.includes("aboutsectioncontentcardscustomerhead") ?<TextAboutsectioncontentCardscustomerhead   /* 56ea2 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
          {allowedControls.includes("aboutsectioncontentcardscustomersub") ?<TextAboutsectioncontentCardscustomersub   /* 0e559 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default GroupAboutsectioncontentCardscustomer
