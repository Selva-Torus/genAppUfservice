...
const GroupiconWrapper = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const {5b28a, set5b28a}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:New_HomeScreen:AFVK:v1",componentId:"3aaf3d46-394c-4e45-bc97-b0e223a5dce2",from:"GroupIconwrapper",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("")){
      set5b28a({...5b28a,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const iconwrapper5dce2Ref = useRef<any>(null);
  const handleClearSearch = () => {
    iconwrapper5dce2Ref.current?.setSearchParams();
    iconwrapper5dce2Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(iconwrapper5dce2) && Object.keys(iconwrapper5dce2)?.length>0)
      {
        seticonwrapper5dce2({})
      }
    }else 
      prevRefreshRef.current= true
  }, [iconwrapper5dce2Props?.refresh])
...
",
    ...
    >
        {allowedControls.includes("")?<Icon /* 5b28a */ encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
    </div>
 )
}

export default GroupiconWrapper
