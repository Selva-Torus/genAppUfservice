...
import Group  from "../Group/Group";
import GroupsecurityIcon  from "../GroupsecurityIcon/GroupsecurityIcon";
import Group  from "../Group/Group";
import Group  from "../Group/Group";
import Group  from "../Group/Group";
...
const GroupServiceSectionContent = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const {a379b, seta379b}= useContext(TotalContext) as TotalContextProps;
  ...
  const {f6751, setf6751}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:New_HomeScreen:AFVK:v1",componentId:"42be15460bb5441681044c717e029fb3",from:"GroupServicesectioncontent",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("")){
      seta379b({...a379b,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("")){
      setdd2e0({...dd2e0,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("")){
      setf6751({...f6751,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("")){
      setd29fb({...d29fb,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("securityicon")){
      setsecurityicon3bde5({...securityicon3bde5,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("")){
      setc5619({...c5619,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("")){
      set4a692({...4a692,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("")){
      set1d2f0({...1d2f0,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const servicesectioncontent29fb3Ref = useRef<any>(null);
  const handleClearSearch = () => {
    servicesectioncontent29fb3Ref.current?.setSearchParams();
    servicesectioncontent29fb3Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(servicesectioncontent29fb3) && Object.keys(servicesectioncontent29fb3)?.length>0)
      {
        setservicesectioncontent29fb3({})
      }
    }else 
      prevRefreshRef.current= true
  }, [servicesectioncontent29fb3Props?.refresh])
  ...
  return (
    ...
    >
        ...
        {allowedComponent.includes("")  &&<Group  
          ...
          encryptionFlagPageData={encryptionFlagPageData}
          paginationDetails={paginationDetails}        />}
        {allowedComponent.includes("securityicon")  &&<GroupsecurityIcon  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          primaryTableData={primaryTableData}
          setPrimaryTableData={setPrimaryTableData}
        ...
        {allowedComponent.includes("")  &&<Group  
          ...
          encryptionFlagPageData={encryptionFlagPageData}
          paginationDetails={paginationDetails}        />}
          {allowedControls.includes("") ?<Text   /* a379b */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
          {allowedControls.includes("") ?<Text   /* f6751 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default GroupServiceSectionContent
