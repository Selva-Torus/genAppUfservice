...
import GroupHerosectionContent  from "../GroupHerosectionContent/GroupHerosectionContent";
...
const GroupHeroSection = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:New_HomeScreen:AFVK:v1",componentId:"7f68a76b1bde48a18815bffe06d5b0f3",from:"GroupHerosection",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("herosectioncontent")){
      setherosectioncontent0b52c({...herosectioncontent0b52c,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const herosection5b0f3Ref = useRef<any>(null);
  const handleClearSearch = () => {
    herosection5b0f3Ref.current?.setSearchParams();
    herosection5b0f3Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(herosection5b0f3) && Object.keys(herosection5b0f3)?.length>0)
      {
        setherosection5b0f3({})
      }
    }else 
      prevRefreshRef.current= true
  }, [herosection5b0f3Props?.refresh])
  ...
  return (
    ...
    >
        {allowedComponent.includes("herosectioncontent")  &&<GroupHerosectionContent  
