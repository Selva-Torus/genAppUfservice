...
import TextAboutsectioncontentheading  from "./TextAboutsectioncontentheading";
...
const GroupAboutsectioncontentheadingwrapper = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const {aboutsectioncontentheadingce067, setaboutsectioncontentheadingce067}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:New_HomeScreen:AFVK:v1",componentId:"6775a05def7441a88c9bc01116082b26",from:"GroupAboutsectioncontentheadingwrapper",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("aboutsectioncontentheading")){
      setaboutsectioncontentheadingce067({...aboutsectioncontentheadingce067,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const aboutsectioncontentheadingwrapper82b26Ref = useRef<any>(null);
  const handleClearSearch = () => {
    aboutsectioncontentheadingwrapper82b26Ref.current?.setSearchParams();
    aboutsectioncontentheadingwrapper82b26Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(aboutsectioncontentheadingwrapper82b26) && Object.keys(aboutsectioncontentheadingwrapper82b26)?.length>0)
      {
        setaboutsectioncontentheadingwrapper82b26({})
      }
    }else 
      prevRefreshRef.current= true
  }, [aboutsectioncontentheadingwrapper82b26Props?.refresh])
  ...
  return (
    ...
    >
          {allowedControls.includes("aboutsectioncontentheading") ?<TextAboutsectioncontentheading   /* ce067 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default GroupAboutsectioncontentheadingwrapper
