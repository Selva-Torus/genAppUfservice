...
const Group = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:New_HomeScreen:AFVK:v1",componentId:"edf6ebea7efd47c89a27a8d0b4aa3963",from:"Group",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("")){
      seta0ca9({...a0ca9,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("")){
      set84e59({...84e59,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("")){
      setad050({...ad050,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("")){
      set04eb2({...04eb2,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("")){
      setacdf4({...acdf4,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("")){
      setf377d({...f377d,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("")){
      set2c7d1({...2c7d1,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("")){
      set2d4f2({...2d4f2,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("")){
      setb38f9({...b38f9,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("")){
      set13f42({...13f42,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const a3963Ref = useRef<any>(null);
  const handleClearSearch = () => {
    a3963Ref.current?.setSearchParams();
    a3963Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(a3963) && Object.keys(a3963)?.length>0)
      {
        seta3963({})
      }
    }else 
