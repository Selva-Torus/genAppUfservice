...
import Labelmsg_info_label  from "./Labelmsg_info_label";
...
const Groupmsg_group = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT261:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:VMC_Msg_Info:AFVK:v1",componentId:"b4383d4431504c189353bca8bec609be",from:"GroupMsgGroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("msg_info_label")){
      setmsg_info_label41b13({...msg_info_label41b13,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const msg_group609beRef = useRef<any>(null);
  const handleClearSearch = () => {
    msg_group609beRef.current?.setSearchParams();
    msg_group609beRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(msg_group609be) && Object.keys(msg_group609be)?.length>0)
      {
        setmsg_group609be({})
      }
    }else 
      prevRefreshRef.current= true
  }, [msg_group609beProps?.refresh])
  ...
  return (
    ...
    >
        {allowedControls.includes("msg_info_label")?<Labelmsg_info_label   /* 41b13 */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default Groupmsg_group
