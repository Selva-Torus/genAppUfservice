...
const Cardtotal_calls = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  const handleMapperDetails=async()=>{
    try{
    let code:any;
    const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT261:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:VMC_Msg_Info:AFVK:v1",  componentId:"a6d7927c980b49688fb350316df8d62c",controlId:"0f187e7342c44b298599e10fb09fcfdc",isTable:false,accessProfile:accessProfile,from:"cardTotal Calls"},{
      headers: {
    ...
    try{
      ...
      setinfo_summary_groups8d62c((pre:any)=>({...pre,total_calls:dfd_mongo_total_calls_v1Props[0]?.total_calls}));
  }
    }catch(err){
      console.log(err)
    }
...
useEffect(() => {
  ...
  if (total_callsfcfdc?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `1 / 5`,gridRow: `1 / 36`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Card 
      style={style}
      className=""      
      view="filled"
      disabled= {total_callsfcfdc?.isDisabled ? true : false}
      onClick={handleClick}  
      >
...
{info_summary_groups8d62c?.total_calls?info_summary_groups8d62c?.total_calls:"0"}
      </Text>
    </div>
      </Card>
    </div>
