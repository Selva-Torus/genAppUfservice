...
const Carderror_rate = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  const handleMapperDetails=async()=>{
    try{
    let code:any;
    const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT261:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:VMC_Msg_Info:AFVK:v1",  componentId:"a6d7927c980b49688fb350316df8d62c",controlId:"15e66d79621e445ca2b581b5661ad931",isTable:false,accessProfile:accessProfile,from:"cardError Rate"},{
      headers: {
    ...
    try{
      ...
      setinfo_summary_groups8d62c((pre:any)=>({...pre,error_rate:dfd_mongo_total_calls_v1Props[0]?.error_rate}));
  }
    }catch(err){
      console.log(err)
    }
...
useEffect(() => {
  ...
  if (error_ratead931?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `9 / 13`,gridRow: `1 / 36`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Card 
      style={style}
      className="
"      
      view="filled"
      disabled= {error_ratead931?.isDisabled ? true : false}
      onClick={handleClick}  
      >
...
{info_summary_groups8d62c?.error_rate?info_summary_groups8d62c?.error_rate:"0"}
      </Text>
    </div>
      </Card>
    </div>
