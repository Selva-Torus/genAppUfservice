...
import Groupinfo_summary_repository_group  from "../Groupinfo_summary_repository_group/Groupinfo_summary_repository_group";
...
const Groupvmc_msg_info = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT261:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:VMC_Msg_Info:AFVK:v1",componentId:"0b4cb68790804760aeda3cd6f2ab41b0",from:"GroupVmcMsgInfo",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("info_summary_repository_group")){
      setinfo_summary_repository_group8106c({...info_summary_repository_group8106c,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("api_process_log")){
      setapi_process_log17839({...api_process_log17839,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const vmc_msg_infob41b0Ref = useRef<any>(null);
  const handleClearSearch = () => {
    vmc_msg_infob41b0Ref.current?.setSearchParams();
    vmc_msg_infob41b0Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(vmc_msg_infob41b0) && Object.keys(vmc_msg_infob41b0)?.length>0)
      {
        setvmc_msg_infob41b0({})
      }
    }else 
      prevRefreshRef.current= true
  }, [vmc_msg_infob41b0Props?.refresh])
  ...
  return (
    ...
    >
        {allowedComponent.includes("info_summary_repository_group")  &&<Groupinfo_summary_repository_group  
...
export default Groupvmc_msg_info
