...
import Groupmsg_group  from "../Groupmsg_group/Groupmsg_group";
import Groupapi_repository_groups  from "../Groupapi_repository_groups/Groupapi_repository_groups";
import Groupinfo_summary_groups  from "../Groupinfo_summary_groups/Groupinfo_summary_groups";
...
const Groupinfo_summary_repository_group = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT261:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:VMC_Msg_Info:AFVK:v1",componentId:"3fbab9dd6ec9455a8ba950db2498106c",from:"GroupInfoSummaryRepositoryGroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("msg_group")){
      setmsg_group609be({...msg_group609be,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("api_repository_groups")){
      setapi_repository_groupsb475a({...api_repository_groupsb475a,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("info_summary_groups")){
      setinfo_summary_groups8d62c({...info_summary_groups8d62c,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const info_summary_repository_group8106cRef = useRef<any>(null);
  const handleClearSearch = () => {
    info_summary_repository_group8106cRef.current?.setSearchParams();
    info_summary_repository_group8106cRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(info_summary_repository_group8106c) && Object.keys(info_summary_repository_group8106c)?.length>0)
      {
        setinfo_summary_repository_group8106c({})
      }
    }else 
      prevRefreshRef.current= true
  }, [info_summary_repository_group8106cProps?.refresh])
  ...
  return (
    ...
    >
        {allowedComponent.includes("msg_group")  &&<Groupmsg_group  
        ...
        {allowedComponent.includes("api_repository_groups")  &&<Groupapi_repository_groups  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          primaryTableData={primaryTableData}
          setPrimaryTableData={setPrimaryTableData}
          ...
          encryptionFlagPageData={encryptionFlagPageData}
          paginationDetails={paginationDetails}        />}
        {allowedComponent.includes("info_summary_groups")  &&<Groupinfo_summary_groups  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          primaryTableData={primaryTableData}
          setPrimaryTableData={setPrimaryTableData}
...
export default Groupinfo_summary_repository_group
