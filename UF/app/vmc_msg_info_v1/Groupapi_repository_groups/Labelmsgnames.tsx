...
const Labelmsgnames = ({encryptionFlagCompData}:any) => {
  ...
  try{
    const orchestrationData: any = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        controlId: "0646e53048e2482da235bde65897a309",
        isTable: false,
        from:"labelMessage Name",
...
useEffect(()=>{
  handleMapperValue();
  setapi_repository_groupsb475a((pre:any)=>({...pre,msgnames:""}));
},[msgnames7a309?.refresh])


const handleClick =async(e:any)=>{
  setapi_repository_groupsb475a((prev: any) => ({ ...prev, msgnames: e.target.value }));
  let code = allCode;
    if (code != '') {
    let codeStates: any = {};
  ...
  if (msgnames7a309?.isHidden) {
    return <></>
  }  

  return (
    <div 
      style={{gridColumn: `1 / 3`,gridRow: `1 / 15`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Label 
        className=""
        disabled= {msgnames7a309?.isDisabled ? true : false}
        theme="normal"
        interactive={false}
      onClick = {handleClick}
      >
