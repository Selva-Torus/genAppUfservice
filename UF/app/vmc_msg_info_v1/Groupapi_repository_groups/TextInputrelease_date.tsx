...
const TextInputrelease_date = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT261:FNGK:AF:FNK:DF-DFD:CATK:AG001:AFGK:A001:AFK:Mongo_Api_Repository:AFVK:v1|bfcdbb21028547269c3c5bde0907e7bc|properties.status"
      ],
      "targetKey": "CK:CT261:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:VMC_Msg_Info:AFVK:v1|862b2b2574e048eca62763b9b23b475a|c722e9f0b0ae423fb1d4f8a478427b40"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  let schemaArray :any =[];  
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'status',type:"text"})
  ...
  const handleChange = async(e: any) => {
    setError('')
    setValidate((pre:any)=>({...pre,status:undefined}))
    if(dynamicStateandType.type=="number"){
    setapi_repository_groupsb475a((prev: any) => ({ ...prev, status: +e.target.value }))
    }
    else{
    setapi_repository_groupsb475a((prev: any) => ({ ...prev, status: e.target.value }))
    }
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "c722e9f0b0ae423fb1d4f8a478427b40",
          isTable: false,
          from:"TextInputrelease_date",
      ...
      if(orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties){
        ...
        type={
          name:'status',
          type: orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.status.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.status.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.status.type
        }
       ...
       if(orchestrationData?.data?.schemaData[0].schema.properties){
        ...
        type={
          name:'status',
          type: orchestrationData?.data?.schemaData[0].schema.properties.status.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.properties.status.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.properties.status.type
        }
        setDynamicStateandType(type)
      }
    }
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setapi_repository_groupsb475a((pre:any)=>({...pre,status:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
  ...
  if (release_date27b40?.isHidden) {
    return <></>
  }
  return (   
    <div 
      style={{gridColumn: `10 / 13`,gridRow: `17 / 30`, gap:``, height: `100%`, overflow: 'auto'}} >
      ...
      <TextInput
        ...
        value={api_repository_groupsb475a?.status||""}
         disabled= {release_date27b40?.isDisabled ? true : false}
        pin='brick-brick'     
        readOnly={true}
        view='normal'
        validationState={validate?.status ? "invalid" : undefined}
        errorMessage={error}
      />
    </div> 
  )
...
export default TextInputrelease_date
