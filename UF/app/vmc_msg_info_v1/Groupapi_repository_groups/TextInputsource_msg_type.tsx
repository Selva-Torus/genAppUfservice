...
const TextInputsource_msg_type = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT261:FNGK:AF:FNK:DF-DFD:CATK:AG001:AFGK:A001:AFK:Mongo_Api_Repository:AFVK:v1|bfcdbb21028547269c3c5bde0907e7bc|properties.source_msg_type"
      ],
      "targetKey": "CK:CT261:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:VMC_Msg_Info:AFVK:v1|862b2b2574e048eca62763b9b23b475a|3d03ce36152c4564998835265c8a3c5a"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  let schemaArray :any =[];  
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'source_msg_type',type:"text"})
  ...
  const handleChange = async(e: any) => {
    setError('')
    setValidate((pre:any)=>({...pre,source_msg_type:undefined}))
    if(dynamicStateandType.type=="number"){
    setapi_repository_groupsb475a((prev: any) => ({ ...prev, source_msg_type: +e.target.value }))
    }
    else{
    setapi_repository_groupsb475a((prev: any) => ({ ...prev, source_msg_type: e.target.value }))
    }
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "3d03ce36152c4564998835265c8a3c5a",
          isTable: false,
          from:"TextInputsource_msg_type",
      ...
      if(orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties){
        let type:any={name:'source_msg_type',type:'text'}
        type={
          name:'source_msg_type',
          type: orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.source_msg_type.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.source_msg_type.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.source_msg_type.type
        }
       ...
       if(orchestrationData?.data?.schemaData[0].schema.properties){
        ...
        type={
          name:'source_msg_type',
          type: orchestrationData?.data?.schemaData[0].schema.properties.source_msg_type.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.properties.source_msg_type.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.properties.source_msg_type.type
        }
        setDynamicStateandType(type)
      }
    }
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setapi_repository_groupsb475a((pre:any)=>({...pre,source_msg_type:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
  ...
  if (source_msg_typea3c5a?.isHidden) {
    return <></>
  }
  return (   
    <div 
      style={{gridColumn: `1 / 3`,gridRow: `16 / 30`, gap:``, height: `100%`, overflow: 'auto'}} >
      ...
      <TextInput
        ...
        value={api_repository_groupsb475a?.source_msg_type||""}
         disabled= {source_msg_typea3c5a?.isDisabled ? true : false}
        pin='brick-brick'     
        readOnly={true}
        view='normal'
        validationState={validate?.source_msg_type ? "invalid" : undefined}
        errorMessage={error}
      />
    </div> 
  )
...
export default TextInputsource_msg_type
