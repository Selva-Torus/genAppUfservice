...
const Labelrelease_dates = ({encryptionFlagCompData}:any) => {
  ...
  try{
    const orchestrationData: any = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        controlId: "1c7b727e7c714c4dae01f982c28b8be1",
        isTable: false,
        from:"labelRelease Date",
...
useEffect(()=>{
  handleMapperValue();
  setapi_repository_groupsb475a((pre:any)=>({...pre,release_dates:""}));
},[release_datesb8be1?.refresh])


const handleClick =async(e:any)=>{
  setapi_repository_groupsb475a((prev: any) => ({ ...prev, release_dates: e.target.value }));
  let code = allCode;
    if (code != '') {
    let codeStates: any = {};
  ...
  if (release_datesb8be1?.isHidden) {
    return <></>
  }  

  return (
    <div 
      style={{gridColumn: `10 / 13`,gridRow: `1 / 11`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Label 
        className=""
        disabled= {release_datesb8be1?.isDisabled ? true : false}
        theme="normal"
        interactive={false}
      onClick = {handleClick}
      >
...
export default Labelrelease_dates
