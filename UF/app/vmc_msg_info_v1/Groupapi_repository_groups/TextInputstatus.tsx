...
const TextInputstatus = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT261:FNGK:AF:FNK:DF-DFD:CATK:AG001:AFGK:A001:AFK:Mongo_Api_Repository:AFVK:v1|bfcdbb21028547269c3c5bde0907e7bc|properties.release_date"
      ],
      "targetKey": "CK:CT261:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:VMC_Msg_Info:AFVK:v1|862b2b2574e048eca62763b9b23b475a|f2b104d2a10f46f0993358cc2d63a35d"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  let schemaArray :any =[];  
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'release_date',type:"text"})
  ...
  const handleChange = async(e: any) => {
    setError('')
    setValidate((pre:any)=>({...pre,release_date:undefined}))
    if(dynamicStateandType.type=="number"){
    setapi_repository_groupsb475a((prev: any) => ({ ...prev, release_date: +e.target.value }))
    }
    else{
    setapi_repository_groupsb475a((prev: any) => ({ ...prev, release_date: e.target.value }))
    }
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "f2b104d2a10f46f0993358cc2d63a35d",
          isTable: false,
      ...
      if(orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties){
        let type:any={name:'release_date',type:'text'}
        type={
          name:'release_date',
          type: orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.release_date.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.release_date.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.release_date.type
        }
       ...
       if(orchestrationData?.data?.schemaData[0].schema.properties){
        ...
        type={
          name:'release_date',
          type: orchestrationData?.data?.schemaData[0].schema.properties.release_date.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.properties.release_date.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.properties.release_date.type
        }
        setDynamicStateandType(type)
      }
    }
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setapi_repository_groupsb475a((pre:any)=>({...pre,release_date:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
  ...
  if (status3a35d?.isHidden) {
    return <></>
  }
  return (   
    <div 
      style={{gridColumn: `7 / 9`,gridRow: `17 / 30`, gap:``, height: `100%`, overflow: 'auto'}} >
      ...
      <TextInput
        ...
        value={api_repository_groupsb475a?.release_date||""}
         disabled= {status3a35d?.isDisabled ? true : false}
        pin='brick-brick'     
        readOnly={true}
        view='normal'
        validationState={validate?.release_date ? "invalid" : undefined}
        errorMessage={error}
      />
    </div> 
  )
