...
const Labelversions = ({encryptionFlagCompData}:any) => {
  ...
  try{
    const orchestrationData: any = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        controlId: "e5e6de4194b74645adb087569b0beb4b",
        isTable: false,
        from:"labelVersion",
...
useEffect(()=>{
  handleMapperValue();
  setapi_repository_groupsb475a((pre:any)=>({...pre,versions:""}));
},[versionsbeb4b?.refresh])


const handleClick =async(e:any)=>{
  setapi_repository_groupsb475a((prev: any) => ({ ...prev, versions: e.target.value }));
  let code = allCode;
    if (code != '') {
    let codeStates: any = {};
  ...
  if (versionsbeb4b?.isHidden) {
    return <></>
  }  

  return (
    <div 
      style={{gridColumn: `4 / 6`,gridRow: `1 / 11`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Label 
        className=""
        disabled= {versionsbeb4b?.isDisabled ? true : false}
        theme="normal"
        interactive={false}
      onClick = {handleClick}
      >
