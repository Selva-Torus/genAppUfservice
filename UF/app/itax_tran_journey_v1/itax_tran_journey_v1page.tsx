...
export default function PageItaxTranJourneyV1() {
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function itax_source_tran_dtl_dfd_v1(pagination:any): Promise<void>{
        ...
        if(itax_tran_journey_v1Props.length > 0){
          let filterData :any[] =[];
          for(let i=0;i< itax_tran_journey_v1Props.length;i++){
            if(itax_tran_journey_v1Props[i].DFDkey == "CK:CT010:FNGK:AF:FNK:DF-DFD:CATK:I001:AFGK:ITAX:AFK:ITAX_Source_Tran_Dtl_DFD:AFVK:v1"){
              delete itax_tran_journey_v1Props[i].DFDkey;
              filterData.push(itax_tran_journey_v1Props[i])
            }           
          }
  ...
  async function securityCheck(): Promise<void> {
    const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_Tran_Journey:AFVK:v1",accessProfile:[user],from:"pageItaxTranJourneyV1"},{
      headers: {
        Authorization: `Bearer ${token}`
      }});
  ...
  "events": {}
    ...
    await itax_source_tran_dtl_dfd_v1(pagination)
        ...
        if (code !="" ) {
          ...
          codeStates['tran_journey_group'] = tran_journey_group30215;
          codeStates['settran_journey_group'] = settran_journey_group30215;
