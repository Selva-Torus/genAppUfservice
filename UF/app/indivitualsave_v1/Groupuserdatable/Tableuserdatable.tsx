...
let defaultColumns = [
  {
    "id": "phone",
    "nodeid": "ece8f3aa0e994c6bbe8fa46008c9cd99",
    "name": "phone",
    "meta": {
      "sort": true
    },
...
const Tableuserdatable = ({ lockedData,setLockedData,primaryTableData, setPrimaryTableData,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData }: any)=>{
 ...
 /////////////
  ...
  const GetTableDetails = async () => {
    const orchestrationData = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        componentId: 'a477833912864b289fca8054131a0a95',
        isTable: true,
        from :"Tableuserdatable",
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setuserdatablea0a95Props((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setuserdatablea0a95Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any) {
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["use_id"]:userdatablea0a95Props?.presetValues?.use_id}
 
    let dstKey=dfKey?.dfKey
    dstKey=dstKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    try {
    if(!userdatablea0a95Props?.presetValues || Object.keys(userdatablea0a95Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
      ...
      if (isRulePresent==false) {
        ...
        const api_paginationBody: api_paginationDto = {
          ...
          count: parseInt(pageSize),
          filterDetails: {
            ufKey:'CK:CT309:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:indivitualsave:AFVK:v1:UO', 
            nodeId: 'a477833912864b289fca8054131a0a95',
            elementId: 'a477833912864b289fca8054131a0a95'
          },
          searchFilter: searchParams
        }
        if(encryptionFlagCont) {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        ...
        setuserdatablea0a95(uf_paginationDataFilter.data||[])
...
const colurIndicator = (keyValue:any=[], comingValue:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(userdatablea0a95))
    {
      GetTableDetails()
    }
  }, [userdatablea0a95])
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(userdatablea0a95Props.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [userdatablea0a95Props.filterProps])
  ...
  const handlePrimaryTable = () => {
    let findData = userdatablea0a95Props?.selectedIds[userdatablea0a95Props?.selectedIds?.length-1]
    if(Array.isArray(userdatablea0a95) && userdatablea0a95.length>0)
    {
      let data = userdatablea0a95[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "id",
        value: data["id"],
  ...
  useEffect(() => {
    if (userdatablea0a95Props?.selectedIds?.length != 0) handlePrimaryTable()
  }, [userdatablea0a95Props?.selectedIds])
    ...
    <div className='w-full h-full'>
          <div className=' w-full flex flex-row h-[80%]'>
            <Table
              ...
              edgePadding={true}
              selectedIds={userdatablea0a95Props?.selectedIds}  
...
export default Tableuserdatable
