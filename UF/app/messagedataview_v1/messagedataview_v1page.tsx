...
export default function PageMessagedataviewV1() {
  ...
  const {messagedataview_v1, setmessagedataview_v1} = useContext(TotalContext) as TotalContextProps;
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function journey_v1DFD(pagination:any): Promise<void>{
        ...
        if(messagedataview_v1Props.length > 0){
          for(let i=0;i< messagedataview_v1Props.length;i++){
            if(messagedataview_v1Props[i].DFDkey == "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:journey:AFVK:v1"){
              // delete messagedataview_v1Props[i].DFDkey;
              let temp=structuredClone(messagedataview_v1Props[i])
              delete temp?.DFDkey
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setmessagedataview_v1({...result,_artfactPFRule_:rule})
  }

  async function securityCheck(): Promise<void> {
    const data: any = await fetchBatchData(
      'CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:messageDataView:AFVK:v1',
      [user],
      'pageMessagedataviewV1',
      token
    )
        ...
        if(encryptionFlagPage){
           introspect = await AxiosService.get("/UF/introspect",{
            ...
            params: {
              ...
              method: encryptionMethod,
              key:"CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:messageDataView:AFVK:v1"
            }
          }) 
        }else{
          introspect = await AxiosService.get("/UF/introspect",{
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setmessagedataview_v1(allRuleData)
  }, [])

  useEffect(()=>{
    if(messagedataview_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
