...
export default function PageIndisaveV1() {
  ...
  const [checkuserdatable,setCheckuserdatable,]=useState(false);
  ...
  const [paginationData,setPaginationData]=useState<any>({count:10,page:1})
    ...
    async function mydfddata_v1(pagination:any){
        ...
        if(indivitualsave_v1Props.length > 0){
          let filterData :any[] =[];
          for(let i=0;i< indivitualsave_v1Props.length;i++){
            if(indivitualsave_v1Props[i].DFDkey == "CK:CT309:FNGK:AF:FNK:DF-DFD:CATK:AG001:AFGK:A001:AFK:myDfdData:AFVK:v1"){
              delete indivitualsave_v1Props[i].DFDkey;
              filterData.push(indivitualsave_v1Props[i])
            }           
          }
  ...
  "events": {}
    ...
    await mydfddata_v1(pagination)
          ...
          allowedGroup.map((nodes:any)=>{
            ...
            if(nodes?.groupName == 'userdatable' && (nodes?.security== 'AA' || nodes?.security == 'ATO'))
            {
              setCheckuserdatable(true)
            }
          })
          }
           }catch(err:any)
    ...
    <>
     ...
     style={{
       ...
       // minHeight: '100vh',
        ...
        {checkuserdatable && initialLoad &&<Groupuserdatable  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          primaryTableData={primaryTableData}
          setPrimaryTableData={setPrimaryTableData}
