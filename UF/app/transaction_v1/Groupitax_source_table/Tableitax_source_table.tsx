...
let defaultColumns = [
  {
    "id": "itaxst_id",
    "nodeid": "13f5de2bc0684e478371fa0c49b95c9b",
    "name": "Itaxst Id",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "eslip_no",
    "nodeid": "4a7319bb10be44b5bf15618f18fde051",
    "name": "PRN",
    "meta": {
      "sort": true
    },
  ...
  },
  {
    ...
    "nodeid": "8258b3b59e684815a29ec1fdfba99bf8",
  ...
  },
  {
    ...
    "nodeid": "eb5fc901991447fa903342bbaa642330",
  ...
  {
    "id": "tax_payer_pin",
    "nodeid": "66c3740542944d649765294f83b6f022",
    "name": "Tax Payer Pin",
    "meta": {
      "sort": true
    },
  ...
  },
  {
    ...
    "nodeid": "1fdf0ad79b874189946ae0744c50bab4",
  ...
  {
    "id": "total_amount",
    "nodeid": "5a28a7b165e24fb1a9e4b3847cc6ff13",
    "name": "Total Amount",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "currency",
    "nodeid": "30bd4af0d0be402c99fbc2aac8d925d5",
    "name": "Currency",
    "meta": {
      "sort": true
    },
  ...
  },
  {
    ...
    "nodeid": "be5e4c57bc724155a4ffaea3d4a3d5ac",
    "name": "Status",
    "meta": {
      "sort": true
    },
...
const RowActionComponent = React.memo(({index, allData, setRefetch,lockedData,setLockedData,primaryTableData,setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,encryptionFlagCompData,setIsProcessing,security=[],goRuleData={},decodedTokenObj}: any) => {
  ...
  useEffect(() => {
    async function loadPopupData() {
      ...
      let log:any = await handleSecurity("log") || false
      let payment:any = await handleSecurity("payment") || false
      const content = (
        ...
        {
        log&&
        security.find((cols:any)=>(((cols?.controlType+cols?.id ).toLowerCase())=='buttonlog'))&&(<Buttonlog mainData={flattenKeepInner(filteredData)} lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing}/>)}
        {
        payment&&
        security.find((cols:any)=>(((cols?.controlType+cols?.id ).toLowerCase())=='buttonpayment'))&&(<Buttonpayment mainData={flattenKeepInner(filteredData)} lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing}/>)}
        </div>
      );
...
const Tableitax_source_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing}: any)=>{
 ...
 /////////////
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setitax_source_table1afd6Props((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setitax_source_table1afd6Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: 'ed904c9f9554459cb9951c83c141afd6',
                elementId: 'ed904c9f9554459cb9951c83c141afd6'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( itax_source_table1afd6Props?.presetValues&&Object.keys(itax_source_table1afd6Props?.presetValues).length > 0) {
          filtertedData = [itax_source_table1afd6Props?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setitax_source_table1afd6(uf_paginationDataFilter.data||[])
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(itax_source_table1afd6Props.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [itax_source_table1afd6Props.filterProps])
  ...
  useEffect(() => {
    if (itax_source_table1afd6Props?.clearData === true) {
      setitax_source_table1afd6([]);
      setAllDataObject([]);
      setAllData([]);
      setSelectedPaginationData([]);
      setLockedData((pre:any) => ({...pre, data:[]}));
      setitax_source_table1afd6Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [itax_source_table1afd6Props?.clearData])
  useEffect(() => {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [itax_source_table1afd6Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = itax_source_table1afd6Props?.selectedIds[itax_source_table1afd6Props?.selectedIds?.length-1]
    if(Array.isArray(itax_source_table1afd6) && itax_source_table1afd6.length>0)
    {
      let data = itax_source_table1afd6[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "itaxst_id",
  ...
  useEffect(() => {
    if (itax_source_table1afd6Props?.selectedIds?.length != 0) handlePrimaryTable()
  }, [itax_source_table1afd6Props?.selectedIds])
  ...
  if (itax_source_table1afd6?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div className=' w-full h-full flex flex-row'>
            <Table
              ...
              edgePadding={true}
              tableActions={true}
              selectedIds={itax_source_table1afd6Props?.selectedIds}  
...
export default Tableitax_source_table
