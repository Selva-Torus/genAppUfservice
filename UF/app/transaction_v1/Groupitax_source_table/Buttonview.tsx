...
import PageItaxViewDetailspage2 from '@/app/itax_view_details_v1/itax_view_details_v1page';
import PageItaxPrnApprovalDetailspage10 from '@/app/itax_prn_approval_details_v1/itax_prn_approval_details_v1page';
...
const Buttonview = ({ mainData,lockedData,setLockedData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing}: { mainData:any,lockedData:any,setLockedData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any}) => {
 ...
 /////////////
  ...
  const {itax_view_details_v1Props, setitax_view_details_v1Props}= useContext(TotalContext) as TotalContextProps;
  ...
  const {itax_prn_approval_details_v1Props, setitax_prn_approval_details_v1Props}= useContext(TotalContext) as TotalContextProps;
    ...
    try{     
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "47250b8f99cf455dabaef1c096ab80f4",
          isTable: false,
          from:"ButtonView",
  ...
  },[viewb80f4?.refresh,currentToken])
  ...
  const handleClick=async()=>{
    ...
    setitax_view_details_v1Props([...filterData2 ]);
    if(eventDecisionTable({conditionalKey:"selectedAccessProfile",conditionalValue:"Branch Officer, Credit Approver"},{...decodedTokenObj,...mainData})==false){
    setShowProfileAsModalOpen2(true);
    }
    //bindTran
    // For group or table
      setview_detail_group73f21(mainData||{})
      setview_detail_group73f21Props({...view_detail_group73f21Props,presetValues:mainData||{}})  
  // copyFormData
    setprn_no_datails_tablefc106((pre:any)=>({...pre,...mainData}));
    setprn_no_datails_tablefc106Props({...prn_no_datails_tablefc106Props,presetValues:mainData});
  // copyFormData
    setprn_no_datails_tablefc106((pre:any)=>({...pre,...mainData}));
    setprn_no_datails_tablefc106Props({...prn_no_datails_tablefc106Props,presetValues:mainData});
  // copyFormData
    setview_detail_group73f21((pre:any)=>({...pre,...mainData}));
    setview_detail_group73f21Props({...view_detail_group73f21Props,presetValues:mainData});
    // showArtifactAsModal
    let filterProps10:any =  [];
    let filterData10 = await getFilterProps(filterProps10,mainData);
    setitax_prn_approval_details_v1Props([...filterData10 ]);
    if(eventDecisionTable({conditionalKey:"selectedAccessProfile",conditionalValue:"Branch Manager"},{...decodedTokenObj,...mainData})==false){
    setShowProfileAsModalOpen10(true);
    }
    //bindTran
    // For group or table
      setviewed_details_grp73f21(mainData||{})
      setviewed_details_grp73f21Props({...viewed_details_grp73f21Props,presetValues:mainData||{}})  
  // copyFormData
    setprn_no_details_tablefc106((pre:any)=>({...pre,...mainData}));
    setprn_no_details_tablefc106Props({...prn_no_details_tablefc106Props,presetValues:mainData});
  // copyFormData
    setprn_no_details_tablefc106((pre:any)=>({...pre,...mainData}));
    setprn_no_details_tablefc106Props({...prn_no_details_tablefc106Props,presetValues:mainData});
  // copyFormData
    setviewed_details_grp73f21((pre:any)=>({...pre,...mainData}));
    setviewed_details_grp73f21Props({...viewed_details_grp73f21Props,presetValues:mainData});
  ...
  return (
    <div>
      <Modal 
        ...
        showOverlay = {true}
        position = {"center"}
        modalName = "itax_view_details"
        className='w-[] h-[] bg-gray-50 overflow-auto'
      >
        <PageItaxViewDetailspage2/>
      </Modal>
      <Modal 
        ...
        showOverlay = {true}
        position = {"center"}
        modalName = "itax_prn_approval_details"
        className='w-[] h-[] bg-gray-50 overflow-auto'
      >
        <PageItaxPrnApprovalDetailspage10/>
      </Modal>
        {showFlag && <Button 
          ref={buttonRef}
          className=""
          onClick={handleClick}
          view='action'
          disabled= {viewb80f4?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
        >
          {keyset("View")}
