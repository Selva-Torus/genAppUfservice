...
import Tableitax_source_table  from './Tableitax_source_table';  
...
const Groupitax_source_table = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_KEDTB_Main_Screen:AFVK:v1",componentId:"ed904c9f9554459cb9951c83c141afd6",from:"GroupItaxSourceTable",isTable : true,accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("itaxst_id")){
      setitaxst_id95c9b({...itaxst_id95c9b,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("eslip_no")){
      seteslip_node051({...eslip_node051,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("slip_payment_code")){
      setslip_payment_code99bf8({...slip_payment_code99bf8,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("payment_advice_date")){
      setpayment_advice_date42330({...payment_advice_date42330,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("tax_payer_pin")){
      settax_payer_pin6f022({...tax_payer_pin6f022,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("tax_payer_full_name")){
      settax_payer_full_name0bab4({...tax_payer_full_name0bab4,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("total_amount")){
      settotal_amount6ff13({...total_amount6ff13,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("view")){
      setviewb80f4({...viewb80f4,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("log")){
      setlogfd488({...logfd488,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("currency")){
      setcurrency925d5({...currency925d5,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("payment")){
      setpayment2954d({...payment2954d,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("trs_event_process_status")){
      settrs_event_process_status3d5ac({...trs_event_process_status3d5ac,isDisabled:true});
    }
  //////////////
  }
  ...
  const itax_source_table1afd6Ref = useRef<any>(null);
  const handleClearSearch = () => {
    itax_source_table1afd6Ref.current?.setSearchParams();
    itax_source_table1afd6Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(itax_source_table1afd6) && Object.keys(itax_source_table1afd6)?.length>0)
      {
        setitax_source_table1afd6({})
      }
    }else 
      prevRefreshRef.current= true
  }, [itax_source_table1afd6Props?.refresh,token])
  ...
  return (
    ...
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<Tableitax_source_table headerButtonsRenders={renderBUttons}
        lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={itax_source_table1afd6Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing}/>}
      </div>
      </div>
    </div>
 )
...
export default Groupitax_source_table
