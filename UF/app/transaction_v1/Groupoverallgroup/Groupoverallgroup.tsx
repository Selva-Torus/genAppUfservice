...
import Groupitax_main_tab_group  from "../Groupitax_main_tab_group/Groupitax_main_tab_group";
...
const Groupoverallgroup = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_KEDTB_Main_Screen:AFVK:v1",componentId:"9e5ed184b2fa46c18a79edc06f84d9a0",from:"GroupOverallgroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("itax_main_tab_group")){
      setitax_main_tab_group216b3({...itax_main_tab_group216b3,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const overallgroup4d9a0Ref = useRef<any>(null);
  const handleClearSearch = () => {
    overallgroup4d9a0Ref.current?.setSearchParams();
    overallgroup4d9a0Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(overallgroup4d9a0) && Object.keys(overallgroup4d9a0)?.length>0)
      {
        setoverallgroup4d9a0({})
      }
    }else 
      prevRefreshRef.current= true
  }, [overallgroup4d9a0Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("itax_main_tab_group")  &&<Groupitax_main_tab_group  
