...
const Groupview_all_tab = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:VGPH001:AFK:Transaction:AFVK:v1",componentId:"b1f469bbda264cb5b084c62eb6c71a07",from:"GroupViewAllTab",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("view_all_table")){
      setview_all_table648c4({...view_all_table648c4,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const view_all_tab71a07Ref = useRef<any>(null);
  const handleClearSearch = () => {
    view_all_tab71a07Ref.current?.setSearchParams();
    view_all_tab71a07Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(view_all_tab71a07) && Object.keys(view_all_tab71a07)?.length>0)
      {
        setview_all_tab71a07({})
      }
    }else 
      prevRefreshRef.current= true
  }, [view_all_tab71a07Props?.refresh])
