...
import PageItaxCreditApprovalScreenpage2 from '@/app/itax_credit_approval_screen_v1/itax_credit_approval_screen_v1page';
...
const Buttoncp_view = ({ mainData,lockedData,setLockedData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing}: { mainData:any,lockedData:any,setLockedData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any}) => {
 ...
 /////////////
    ...
    try{     
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "1fc2cb7308104f5d9100a73911907f80",
          isTable: false,
          from:"ButtonView",
  ...
  },[cp_view07f80?.refresh,currentToken])
  ...
  const handleClick=async()=>{
    ...
    setitax_credit_approval_screen_v1Props([...filterData2 ]);
    if(eventDecisionTable({conditionalKey:"selectedAccessProfile",conditionalValue:"Credit Approver"},{...decodedTokenObj,...mainData})==false){
    setShowProfileAsModalOpen2(true);
    }
  // copyFormData
    setprndetails_group881d8((pre:any)=>({...pre,...mainData}));
    setprndetails_group881d8Props({...prndetails_group881d8Props,presetValues:mainData});
    //bindTran
    // For group or table
      setapprove_tableafbb9(mainData||{})
      setapprove_tableafbb9Props({...approve_tableafbb9Props,presetValues:mainData||{}})  
  // copyFormData
    setchecker_approval_main_groupff981((pre:any)=>({...pre,...mainData}));
    setchecker_approval_main_groupff981Props({...checker_approval_main_groupff981Props,presetValues:mainData});
  // copyFormData
    setchecker_approval_main_groupff981((pre:any)=>({...pre,...mainData}));
    setchecker_approval_main_groupff981Props({...checker_approval_main_groupff981Props,presetValues:mainData});
  // copyFormData
    setdocumentviewer_group0a3fb((pre:any)=>({...pre,...mainData}));
    setdocumentviewer_group0a3fbProps({...documentviewer_group0a3fbProps,presetValues:mainData});
  ...
  return (
    <div>
      <Modal 
        ...
        showOverlay = {true}
        position = {"center"}
        modalName = "itax_credit_approval_screen"
        className='w-[80%] h-[] bg-gray-50 overflow-auto'
      >
        <PageItaxCreditApprovalScreenpage2/>
      </Modal>
        {showFlag && <Button 
          ref={buttonRef}
          className=""
          onClick={handleClick}
          view='action'
          disabled= {cp_view07f80?.isDisabled ? true : false}
          pin='brick-brick'
          contentAlign={"center"}
        >
          {keyset("View")}
