...
import Buttonvp_view  from './Buttonvp_view'
import Buttonvp_log  from './Buttonvp_log'
import Buttonvp_payment  from './Buttonvp_payment'
...
let defaultColumns = [
  {
    "id": "itaxst_id",
    "nodeid": "7ab178df2a4d463995e87cca9fcda14c",
    "name": "itaxst_id",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "prn",
    "nodeid": "2e683db782b74f7087f77695433af781",
    "name": "PRN",
    "meta": {
      "sort": true
    },
  ...
  },
  {
    ...
    "nodeid": "8d73a38d87d14e17bd57f21812966d40",
  ...
  },
  {
    ...
    "nodeid": "6f06fa447ffe4e07be99bb4be841d75f",
  ...
  {
    "id": "tax_payer_pin",
    "nodeid": "5194d6854f3e40cc977c7dc5aa772747",
    "name": "Tax Payer Pin",
    "meta": {
      "sort": true
    },
  ...
  },
  {
    ...
    "nodeid": "e3424d38799b4457be3f531e1e9a3d32",
  ...
  {
    "id": "total_amount",
    "nodeid": "45cf96d3b0034f0e8b4b2e9cf64b9286",
    "name": "Total Amount",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "currency",
    "nodeid": "21d3cddf26ca45508dac669d40f90f00",
    "name": "Currency",
    "meta": {
      "sort": true
    },
  ...
  },
  {
    ...
    "nodeid": "d297f8360e374a78993db363aa1a505f",
    "name": "Status",
    "meta": {
      "sort": true
    },
...
const RowActionComponent = React.memo(({index, allData, setRefetch,lockedData,setLockedData,primaryTableData,setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,encryptionFlagCompData,setIsProcessing,security=[],goRuleData={},decodedTokenObj}: any) => {
  ...
  useEffect(() => {
    async function loadPopupData() {
      let vp_view:any = await handleSecurity("vp_view") || false
      let vp_log:any = await handleSecurity("vp_log") || false
      let vp_payment:any = await handleSecurity("vp_payment") || false
      const content = (
        <div className='flex flex-col gap-1'>
        {
        vp_view&&
        security.find((cols:any)=>(((cols?.controlType+cols?.id ).toLowerCase())=='buttonvp_view'))&&(<Buttonvp_view mainData={flattenKeepInner(filteredData)} lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing}/>)}
        {
        vp_log&&
        security.find((cols:any)=>(((cols?.controlType+cols?.id ).toLowerCase())=='buttonvp_log'))&&(<Buttonvp_log mainData={flattenKeepInner(filteredData)} lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing}/>)}
        {
        vp_payment&&
        security.find((cols:any)=>(((cols?.controlType+cols?.id ).toLowerCase())=='buttonvp_payment'))&&(<Buttonvp_payment mainData={flattenKeepInner(filteredData)} lockedData={lockedData} setLockedData={setLockedData} primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing}/>)}
        </div>
      );
...
const Tableview_processed_prn_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing}: any)=>{
 ...
 /////////////
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setview_processed_prn_table8f5a5Props((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setview_processed_prn_table8f5a5Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: 'eda9c0f4ccfa4354930fddf45338f5a5',
                elementId: 'eda9c0f4ccfa4354930fddf45338f5a5'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( view_processed_prn_table8f5a5Props?.presetValues&&Object.keys(view_processed_prn_table8f5a5Props?.presetValues).length > 0) {
          filtertedData = [view_processed_prn_table8f5a5Props?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setview_processed_prn_table8f5a5(uf_paginationDataFilter.data||[])
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(view_processed_prn_table8f5a5Props.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [view_processed_prn_table8f5a5Props.filterProps])
  ...
  useEffect(() => {
    if (view_processed_prn_table8f5a5Props?.clearData === true) {
      setview_processed_prn_table8f5a5([]);
      setAllDataObject([]);
      setAllData([]);
      setSelectedPaginationData([]);
      setLockedData((pre:any) => ({...pre, data:[]}));
      setview_processed_prn_table8f5a5Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [view_processed_prn_table8f5a5Props?.clearData])
  useEffect(() => {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [view_processed_prn_table8f5a5Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = view_processed_prn_table8f5a5Props?.selectedIds[view_processed_prn_table8f5a5Props?.selectedIds?.length-1]
    if(Array.isArray(view_processed_prn_table8f5a5) && view_processed_prn_table8f5a5.length>0)
    {
      let data = view_processed_prn_table8f5a5[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "itaxst_id",
  ...
  useEffect(() => {
    if (view_processed_prn_table8f5a5Props?.selectedIds?.length != 0) handlePrimaryTable()
  }, [view_processed_prn_table8f5a5Props?.selectedIds])
  ...
  if (view_processed_prn_table8f5a5?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div className=' w-full h-full flex flex-row'>
            <Table
              ...
              edgePadding={true}
              tableActions={true}
              selectedIds={view_processed_prn_table8f5a5Props?.selectedIds}  
...
export default Tableview_processed_prn_table
