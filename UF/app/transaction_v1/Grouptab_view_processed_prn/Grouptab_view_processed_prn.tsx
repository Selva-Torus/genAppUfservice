...
import Groupview_processed_prn_table  from "../Groupview_processed_prn_table/Groupview_processed_prn_table";
...
const Grouptab_view_processed_prn = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_KEDTB_Main_Screen:AFVK:v1",componentId:"ba006335512e42658882e38f25629a93",from:"GroupTabViewProcessedPrn",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("view_processed_prn_table")){
      setview_processed_prn_table8f5a5({...view_processed_prn_table8f5a5,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const tab_view_processed_prn29a93Ref = useRef<any>(null);
  const handleClearSearch = () => {
    tab_view_processed_prn29a93Ref.current?.setSearchParams();
    tab_view_processed_prn29a93Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(tab_view_processed_prn29a93) && Object.keys(tab_view_processed_prn29a93)?.length>0)
      {
        settab_view_processed_prn29a93({})
      }
    }else 
      prevRefreshRef.current= true
  }, [tab_view_processed_prn29a93Props?.refresh,token])
  ...
  return (
    <div 
      style={{          
        gridColumn: 'undefined / null',
        gridRow: 'undefined / null',
    ...
    >
        {allowedComponent.includes("view_processed_prn_table")  &&<Groupview_processed_prn_table  
...
export default Grouptab_view_processed_prn
