...
import Groupcredit_process_table  from "../Groupcredit_process_table/Groupcredit_process_table";
...
const Grouptab_credit_process_group = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_KEDTB_Main_Screen:AFVK:v1",componentId:"0a613e277d2f454d953b994a1ef546cc",from:"GroupTabCreditProcessGroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("credit_process_table")){
      setcredit_process_table0cd4c({...credit_process_table0cd4c,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const tab_credit_process_group546ccRef = useRef<any>(null);
  const handleClearSearch = () => {
    tab_credit_process_group546ccRef.current?.setSearchParams();
    tab_credit_process_group546ccRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(tab_credit_process_group546cc) && Object.keys(tab_credit_process_group546cc)?.length>0)
      {
        settab_credit_process_group546cc({})
      }
    }else 
      prevRefreshRef.current= true
  }, [tab_credit_process_group546ccProps?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("credit_process_table")  &&<Groupcredit_process_table  
...
export default Grouptab_credit_process_group
