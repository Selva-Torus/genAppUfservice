...
import Cardpayment_completed  from "./Cardpayment_completed";
...
const Groupgrp_payment_completed = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const {payment_completedeae7f, setpayment_completedeae7f}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_Dashboard:AFVK:v1",componentId:"f0087c1317a3471886d0764991134dec",from:"GroupGrpPaymentCompleted",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("payment_completed")){
      setpayment_completedeae7f({...payment_completedeae7f,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const grp_payment_completed34decRef = useRef<any>(null);
  const handleClearSearch = () => {
    grp_payment_completed34decRef.current?.setSearchParams();
    grp_payment_completed34decRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(grp_payment_completed34dec) && Object.keys(grp_payment_completed34dec)?.length>0)
      {
        setgrp_payment_completed34dec({})
      }
    }else 
      prevRefreshRef.current= true
  }, [grp_payment_completed34decProps?.refresh,token])
  ...
  return (
    <div 
      style={{          
        ...
        columnGap: '0px',
        backgroundColor:'',
        backgroundImage:"url('https://cdns3dfsdev.toruslowcode.com/torus/9.1/CT010/resources/images/new_voilet3.png')",
        backgroundPosition: '',
        backgroundSize: '',
        backgroundRepeat: '',
    ...
    >
        {allowedControls.includes("payment_completed") ?<Cardpayment_completed  /* eae7f */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
    </div>
 )
}

export default Groupgrp_payment_completed
