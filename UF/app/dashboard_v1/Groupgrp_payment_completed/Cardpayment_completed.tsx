...
const Cardpayment_completed = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  const handleMapperDetails=async():Promise<void>=>{
    ...
    try{
      ...
      setgrp_payment_completed34dec((pre:any)=>({...pre,payment_completed:dfd_itax_dashboard_cards_v1Props[0]?.payment_completed}));
  }
    }catch(err){
      console.log(err);
    }
...
},[payment_completedeae7f?.refresh])
...
useEffect(() => {
  ...
  if (payment_completedeae7f?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `1 / 25`,gridRow: `1 / 34`, gap:``, height: `100%`, overflow: 'auto'}} >
      ...
      label={keyset("Payment Completed")}
      disabled= {payment_completedeae7f?.isDisabled ? true : false}
      onClick={handleClick} 
      contentAlign={"center"}
      >
      {grp_payment_completed34dec?.payment_completed?grp_payment_completed34dec?.payment_completed:"0"}
      </Card>
    </div>
  )
}

export default Cardpayment_completed
