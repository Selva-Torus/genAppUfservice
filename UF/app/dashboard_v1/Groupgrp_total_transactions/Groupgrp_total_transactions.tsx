...
import Cardtotal_transactions  from "./Cardtotal_transactions";
...
const Groupgrp_total_transactions = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const {total_transactionse8381, settotal_transactionse8381}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_Dashboard:AFVK:v1",componentId:"34342cd27778487692fd10a0f22e00c2",from:"GroupGrpTotalTransactions",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("total_transactions")){
      settotal_transactionse8381({...total_transactionse8381,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const grp_total_transactionse00c2Ref = useRef<any>(null);
  const handleClearSearch = () => {
    grp_total_transactionse00c2Ref.current?.setSearchParams();
    grp_total_transactionse00c2Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(grp_total_transactionse00c2) && Object.keys(grp_total_transactionse00c2)?.length>0)
      {
        setgrp_total_transactionse00c2({})
      }
    }else 
      prevRefreshRef.current= true
  }, [grp_total_transactionse00c2Props?.refresh,token])
  ...
  return (
    <div 
      style={{          
        ...
        columnGap: '0px',
        backgroundColor:'',
        backgroundImage:"url('https://cdns3dfsdev.toruslowcode.com/torus/9.1/CT010/resources/images/new_blue3.png')",
        backgroundPosition: '',
        backgroundSize: '',
        backgroundRepeat: '',
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !rounded-xl ${isDark ? 'text-white' : 'text-black'}`}
    >
        {allowedControls.includes("total_transactions") ?<Cardtotal_transactions  /* e8381 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
    </div>
 )
}

export default Groupgrp_total_transactions
