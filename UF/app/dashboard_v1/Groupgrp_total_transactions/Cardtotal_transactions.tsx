...
const Cardtotal_transactions = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  const handleMapperDetails=async():Promise<void>=>{
    ...
    try{
      ...
      setgrp_total_transactionse00c2((pre:any)=>({...pre,total_transactions:dfd_itax_dashboard_cards_v1Props[0]?.total_transactions}));
  }
    }catch(err){
      console.log(err);
    }
...
},[total_transactionse8381?.refresh])
...
useEffect(() => {
  ...
  if (total_transactionse8381?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `1 / 25`,gridRow: `1 / 34`, gap:``, height: `100%`, overflow: 'auto'}} >
      ...
      label={keyset("Total Transactions")}
      disabled= {total_transactionse8381?.isDisabled ? true : false}
      onClick={handleClick} 
      contentAlign={"center"}
      >
      {grp_total_transactionse00c2?.total_transactions?grp_total_transactionse00c2?.total_transactions:"0"}
      </Card>
    </div>
  )
}

export default Cardtotal_transactions
