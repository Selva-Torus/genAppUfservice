...
import Cardprn_approved  from "./Cardprn_approved";
...
const Groupgrp_prn_approved = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_Dashboard:AFVK:v1",componentId:"77aa4a68fb384b1ba6b788dad85b95cb",from:"GroupGrpPrnApproved",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("prn_approved")){
      setprn_approvedff8ef({...prn_approvedff8ef,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const grp_prn_approvedb95cbRef = useRef<any>(null);
  const handleClearSearch = () => {
    grp_prn_approvedb95cbRef.current?.setSearchParams();
    grp_prn_approvedb95cbRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(grp_prn_approvedb95cb) && Object.keys(grp_prn_approvedb95cb)?.length>0)
      {
        setgrp_prn_approvedb95cb({})
      }
    }else 
      prevRefreshRef.current= true
  }, [grp_prn_approvedb95cbProps?.refresh,token])
  ...
  return (
    <div 
      style={{          
        ...
        columnGap: '0px',
        backgroundColor:'',
        backgroundImage:"url('https://cdns3dfsdev.toruslowcode.com/torus/9.1/CT010/resources/images/New_Yellow3.png')",
        backgroundPosition: '',
        backgroundSize: '',
        backgroundRepeat: '',
    ...
    >
        {allowedControls.includes("prn_approved") ?<Cardprn_approved  /* ff8ef */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
    </div>
 )
}

export default Groupgrp_prn_approved
