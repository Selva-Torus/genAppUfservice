...
import PieChartspie_chart  from "./PieChartspie_chart";
...
const Groupgrp_pie_chart = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_Dashboard:AFVK:v1",componentId:"fc1b6ebba3034adc97af59cfffa2415d",from:"GroupGrpPieChart",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("pie_chart")){
      setpie_chartd26f3({...pie_chartd26f3,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const grp_pie_chart2415dRef = useRef<any>(null);
  const handleClearSearch = () => {
    grp_pie_chart2415dRef.current?.setSearchParams();
    grp_pie_chart2415dRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(grp_pie_chart2415d) && Object.keys(grp_pie_chart2415d)?.length>0)
      {
        setgrp_pie_chart2415d({})
      }
    }else 
      prevRefreshRef.current= true
  }, [grp_pie_chart2415dProps?.refresh,token])
  ...
  return (
    ...
    >
        {allowedControls.includes("pie_chart") ?<PieChartspie_chart /* d26f3 */ encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default Groupgrp_pie_chart
