...
export default function BarChartsbar_chart({ 
  ...
  const handleMapperDetails=async()=>{
    ...
    '/UF/Orchestration',
      {
        ...
        componentId: "69d66a48cbd7491790fe477712802e16",
        controlId: "e46562d0b02c4789834f8b39f0f386dd",
        isTable: false,
    ...
    let code:string= orchestrationData?.data?.code;
      ...
      if(Array.isArray(dfd_itax_bar_chart_dfd_v1Props) && dfd_itax_bar_chart_dfd_v1Props?.length > 0){
        setData(dfd_itax_bar_chart_dfd_v1Props)
        setgrp_bar_chart02e16((pre:any)=>({...pre,name:dfd_itax_bar_chart_dfd_v1Props[0]?.name}));
      }
      if(Array.isArray(dfd_itax_bar_chart_dfd_v1Props)){
        return
      }
    }catch(err){
      console.log(err);
  ...
  return (
    ...
    >
      <BarChart
        data={data}
        title  = {`${keyset("Branch-wise Transaction")}`}
        className = ""
        fillContainer={true}
        contentAlign="left"
      />
