...
const Cardprn_initiated = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  const handleMapperDetails=async():Promise<void>=>{
    try{
    let code:string;
    const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_Dashboard:AFVK:v1",  componentId:"7dcd01f1f061439abce4157f6b42f421",controlId:"c28aa863923b4e999e5090f86b60750d",isTable:false,accessProfile:accessProfile,from:"cardPRN Initiated"},{
      headers: {
    ...
    try{
      ...
      setgrp_prn_initiated2f421((pre:any)=>({...pre,prn_initiated:dfd_itax_dashboard_cards_v1Props[0]?.prn_initiated}));
  }
    }catch(err){
      console.log(err);
    }
...
useEffect(() => {
  ...
  if (prn_initiated0750d?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `1 / 25`,gridRow: `1 / 34`, gap:``, height: `100%`, overflow: 'auto'}} >
      ...
      label={keyset("PRN Initiated")}
      disabled= {prn_initiated0750d?.isDisabled ? true : false}
      onClick={handleClick} 
      contentAlign={"center"}
      >
      {grp_prn_initiated2f421?.prn_initiated?grp_prn_initiated2f421?.prn_initiated:"0"}
      </Card>
    </div>
  )
}

export default Cardprn_initiated
