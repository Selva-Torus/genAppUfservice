...
import Cardprn_initiated  from "./Cardprn_initiated";
...
const Groupgrp_prn_initiated = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const {prn_initiated0750d, setprn_initiated0750d}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_Dashboard:AFVK:v1",componentId:"7dcd01f1f061439abce4157f6b42f421",from:"GroupGrpPrnInitiated",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("prn_initiated")){
      setprn_initiated0750d({...prn_initiated0750d,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const grp_prn_initiated2f421Ref = useRef<any>(null);
  const handleClearSearch = () => {
    grp_prn_initiated2f421Ref.current?.setSearchParams();
    grp_prn_initiated2f421Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(grp_prn_initiated2f421) && Object.keys(grp_prn_initiated2f421)?.length>0)
      {
        setgrp_prn_initiated2f421({})
      }
    }else 
      prevRefreshRef.current= true
  }, [grp_prn_initiated2f421Props?.refresh,token])
  ...
  return (
    <div 
      style={{          
        ...
        columnGap: '0px',
        backgroundColor:'',
        backgroundImage:"url('https://cdns3dfsdev.toruslowcode.com/torus/9.1/CT010/resources/images/new_mildgreen3.png')",
        backgroundPosition: '',
        backgroundSize: '',
        backgroundRepeat: '',
    ...
    >
        {allowedControls.includes("prn_initiated") ?<Cardprn_initiated  /* 0750d */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
    </div>
 )
}

export default Groupgrp_prn_initiated
