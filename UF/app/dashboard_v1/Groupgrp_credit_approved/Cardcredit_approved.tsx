...
const Cardcredit_approved = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  const handleMapperDetails=async():Promise<void>=>{
    ...
    try{
      ...
      setgrp_credit_approved7e3bd((pre:any)=>({...pre,credit_approved:dfd_itax_dashboard_cards_v1Props[0]?.credit_approved}));
  }
    }catch(err){
      console.log(err);
    }
...
useEffect(() => {
  ...
  if (credit_approved179f5?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `1 / 25`,gridRow: `1 / 34`, gap:``, height: `100%`, overflow: 'auto'}} >
      ...
      label={keyset("Credit Approved")}
      disabled= {credit_approved179f5?.isDisabled ? true : false}
      onClick={handleClick} 
      contentAlign={"center"}
      >
      {grp_credit_approved7e3bd?.credit_approved?grp_credit_approved7e3bd?.credit_approved:"0"}
      </Card>
    </div>
  )
}

export default Cardcredit_approved
