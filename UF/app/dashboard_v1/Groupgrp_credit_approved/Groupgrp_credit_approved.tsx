...
import Cardcredit_approved  from "./Cardcredit_approved";
...
const Groupgrp_credit_approved = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const {credit_approved179f5, setcredit_approved179f5}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_Dashboard:AFVK:v1",componentId:"5d3b888ebd9d4c88b0aed1e124a7e3bd",from:"GroupGrpCreditApproved",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("credit_approved")){
      setcredit_approved179f5({...credit_approved179f5,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const grp_credit_approved7e3bdRef = useRef<any>(null);
  const handleClearSearch = () => {
    grp_credit_approved7e3bdRef.current?.setSearchParams();
    grp_credit_approved7e3bdRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(grp_credit_approved7e3bd) && Object.keys(grp_credit_approved7e3bd)?.length>0)
      {
        setgrp_credit_approved7e3bd({})
      }
    }else 
      prevRefreshRef.current= true
  }, [grp_credit_approved7e3bdProps?.refresh,token])
  ...
  return (
    <div 
      style={{          
        ...
        columnGap: '0px',
        backgroundColor:'',
        backgroundImage:"url('https://cdns3dfsdev.toruslowcode.com/torus/9.1/CT010/resources/images/new_green3.png')",
        backgroundPosition: '',
        backgroundSize: '',
        backgroundRepeat: '',
    ...
    >
        {allowedControls.includes("credit_approved") ?<Cardcredit_approved  /* 179f5 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
    </div>
 )
}

export default Groupgrp_credit_approved
