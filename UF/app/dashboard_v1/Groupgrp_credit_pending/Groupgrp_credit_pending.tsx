...
import Cardcredit_pending  from "./Cardcredit_pending";
...
const Groupgrp_credit_pending = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_Dashboard:AFVK:v1",componentId:"797736feff48416f830e6e0a947fe0e2",from:"GroupGrpCreditPending",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("credit_pending")){
      setcredit_pendingdce75({...credit_pendingdce75,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const grp_credit_pendingfe0e2Ref = useRef<any>(null);
  const handleClearSearch = () => {
    grp_credit_pendingfe0e2Ref.current?.setSearchParams();
    grp_credit_pendingfe0e2Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(grp_credit_pendingfe0e2) && Object.keys(grp_credit_pendingfe0e2)?.length>0)
      {
        setgrp_credit_pendingfe0e2({})
      }
    }else 
      prevRefreshRef.current= true
  }, [grp_credit_pendingfe0e2Props?.refresh,token])
  ...
  return (
    <div 
      style={{          
        ...
        columnGap: '0px',
        backgroundColor:'',
        backgroundImage:"url('https://cdns3dfsdev.toruslowcode.com/torus/9.1/CT010/resources/images/new_pink3.png')",
        backgroundPosition: '',
        backgroundSize: '',
        backgroundRepeat: '',
    ...
    >
        {allowedControls.includes("credit_pending") ?<Cardcredit_pending  /* dce75 */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData}  />: <div></div>}
    </div>
 )
}

export default Groupgrp_credit_pending
