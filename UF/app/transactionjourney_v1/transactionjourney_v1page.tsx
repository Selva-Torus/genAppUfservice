...
export default function PageTransactionjourneyV1() {
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function journey_v1DFD(pagination:any): Promise<void>{
        ...
        if(transactionjourney_v1Props.length > 0){
          for(let i=0;i< transactionjourney_v1Props.length;i++){
            if(transactionjourney_v1Props[i].DFDkey == "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:VGPH:AFK:journey:AFVK:v1"){
              // delete transactionjourney_v1Props[i].DFDkey;
              let temp=structuredClone(transactionjourney_v1Props[i])
              delete temp?.DFDkey
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    settransactionjourney_v1({...result,_artfactPFRule_:rule})
  }

  async function securityCheck(): Promise<void> {
    const data: any = await fetchBatchData(
      'CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:VGPH:AFK:transactionJourney:AFVK:v1',
      [user],
      'pageTransactionjourneyV1',
      token
    )
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    settransactionjourney_v1(allRuleData)
  }, [])

  useEffect(()=>{
    if(transactionjourney_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
