...
const Textprocessed_by = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_journey_v1Props && !dfd_journey_v1Props.hasLogicCenter) {
        ...
        setjourney_details_groupd9a0e((pre: any) => {
          return { ...pre, processing_system: api_paginationData.data.records[0]?.processing_system }
        })
        }
      }
      else{
      if(Array.isArray(dfd_journey_v1Props) && dfd_journey_v1Props && !journey_details_groupd9a0e.processing_system){
        setjourney_details_groupd9a0e((pre:any)=>({...pre,processing_system:dfd_journey_v1Props[0]?.processing_system}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[processed_byd2b69?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_journey_v1Props) && !journey_details_groupd9a0e.processing_system){
    setjourney_details_groupd9a0e((pre:any)=>({...pre,processing_system:dfd_journey_v1Props[0]?.processing_system}));
  }
  },[dfd_journey_v1Props])

  if (processed_byd2b69?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 13`,gridRow: `40 / 48`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className="!bg-[#f7f8f8]"
  variant="code-1"
  color="primary"
>
      {keyset(isDynamic ? item?.processing_system : (journey_details_groupd9a0e?.processing_system || ""))}
</Text>
  </div>
  )
}

export default Textprocessed_by
