...
const Texttransaction_reference = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_transaction_v1Props && !dfd_transaction_v1Props.hasLogicCenter) {
        ...
        setjourney_details_groupd9a0e((pre: any) => {
          return { ...pre, tran_reference: api_paginationData.data.records[0]?.tran_reference }
        })
        }
      }
      else{
      if(Array.isArray(dfd_transaction_v1Props) && dfd_transaction_v1Props && !journey_details_groupd9a0e.tran_reference){
        setjourney_details_groupd9a0e((pre:any)=>({...pre,tran_reference:dfd_transaction_v1Props[0]?.tran_reference}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[transaction_reference500d6?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_transaction_v1Props) && !journey_details_groupd9a0e.tran_reference){
    setjourney_details_groupd9a0e((pre:any)=>({...pre,tran_reference:dfd_transaction_v1Props[0]?.tran_reference}));
  }
  },[dfd_transaction_v1Props])

  if (transaction_reference500d6?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `13 / 25`,gridRow: `74 / 82`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className="!bg-[#f7f8f8]"
  variant="code-1"
  color="primary"
>
      {keyset(isDynamic ? item?.tran_reference : (journey_details_groupd9a0e?.tran_reference || ""))}
</Text>
  </div>
  )
}

export default Texttransaction_reference
