...
const Textcurrency = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_transaction_v1Props && !dfd_transaction_v1Props.hasLogicCenter) {
        ...
        setjourney_details_groupd9a0e((pre: any) => {
          return { ...pre, dr_currency: api_paginationData.data.records[0]?.dr_currency }
        })
        }
      }
      else{
      if(Array.isArray(dfd_transaction_v1Props) && dfd_transaction_v1Props && !journey_details_groupd9a0e.dr_currency){
        setjourney_details_groupd9a0e((pre:any)=>({...pre,dr_currency:dfd_transaction_v1Props[0]?.dr_currency}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[currency9c8a2?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_transaction_v1Props) && !journey_details_groupd9a0e.dr_currency){
    setjourney_details_groupd9a0e((pre:any)=>({...pre,dr_currency:dfd_transaction_v1Props[0]?.dr_currency}));
  }
  },[dfd_transaction_v1Props])

  if (currency9c8a2?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 13`,gridRow: `57 / 65`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className="!bg-[#f7f8f8]"
  variant="code-1"
  color="primary"
>
      {keyset(isDynamic ? item?.dr_currency : (journey_details_groupd9a0e?.dr_currency || ""))}
</Text>
  </div>
  )
}
