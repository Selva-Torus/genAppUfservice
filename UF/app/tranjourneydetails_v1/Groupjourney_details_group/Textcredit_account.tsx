...
const Textcredit_account = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_transaction_v1Props && !dfd_transaction_v1Props.hasLogicCenter) {
        ...
        setjourney_details_groupd9a0e((pre: any) => {
          return { ...pre, cr_account: api_paginationData.data.records[0]?.cr_account }
        })
        }
      }
      else{
      if(Array.isArray(dfd_transaction_v1Props) && dfd_transaction_v1Props && !journey_details_groupd9a0e.cr_account){
        setjourney_details_groupd9a0e((pre:any)=>({...pre,cr_account:dfd_transaction_v1Props[0]?.cr_account}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[credit_account0d1f4?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_transaction_v1Props) && !journey_details_groupd9a0e.cr_account){
    setjourney_details_groupd9a0e((pre:any)=>({...pre,cr_account:dfd_transaction_v1Props[0]?.cr_account}));
  }
  },[dfd_transaction_v1Props])

  if (credit_account0d1f4?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `13 / 25`,gridRow: `57 / 65`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className="!bg-[#f7f8f8]"
  variant="code-1"
  color="primary"
>
      {keyset(isDynamic ? item?.cr_account : (journey_details_groupd9a0e?.cr_account || ""))}
</Text>
  </div>
  )
}

export default Textcredit_account
