...
const Texttransaction_date_time = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_journey_v1Props) && dfd_journey_v1Props && !journey_details_groupd9a0e.trs_created_date){
        setjourney_details_groupd9a0e((pre:any)=>({...pre,trs_created_date:dfd_journey_v1Props[0]?.trs_created_date}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(() => {
  if(Array.isArray(dfd_journey_v1Props) && !journey_details_groupd9a0e.trs_created_date){
    setjourney_details_groupd9a0e((pre:any)=>({...pre,trs_created_date:dfd_journey_v1Props[0]?.trs_created_date}));
  }
  },[dfd_journey_v1Props])
