...
const Texttransaction_date_time = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_transaction_v1Props && !dfd_transaction_v1Props.hasLogicCenter) {
        ...
        setjourney_details_groupd9a0e((pre: any) => {
          return { ...pre, trs_created_date: api_paginationData.data.records[0]?.trs_created_date }
        })
        }
      }
      else{
      if(Array.isArray(dfd_transaction_v1Props) && dfd_transaction_v1Props && !journey_details_groupd9a0e.trs_created_date){
        setjourney_details_groupd9a0e((pre:any)=>({...pre,trs_created_date:dfd_transaction_v1Props[0]?.trs_created_date}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[transaction_date_time14856?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_transaction_v1Props) && !journey_details_groupd9a0e.trs_created_date){
    setjourney_details_groupd9a0e((pre:any)=>({...pre,trs_created_date:dfd_transaction_v1Props[0]?.trs_created_date}));
  }
  },[dfd_transaction_v1Props])

  if (transaction_date_time14856?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 13`,gridRow: `23 / 31`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className="!bg-[#f7f8f8]"
  variant="code-1"
  color="primary"
>
      {keyset(isDynamic ? item?.trs_created_date : (journey_details_groupd9a0e?.trs_created_date || ""))}
</Text>
  </div>
  )
}

export default Texttransaction_date_time
