...
const Textstatus = ({encryptionFlagCompData,isDynamic,item,index,setIsProcessing}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_journey_v1Props && !dfd_journey_v1Props.hasLogicCenter) {
        ...
        setjourney_details_groupd9a0e((pre: any) => {
          return { ...pre, result: api_paginationData.data.records[0]?.result }
        })
        }
      }
      else{
      if(Array.isArray(dfd_journey_v1Props) && dfd_journey_v1Props && !journey_details_groupd9a0e.result){
        setjourney_details_groupd9a0e((pre:any)=>({...pre,result:dfd_journey_v1Props[0]?.result}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(() => {
  if(Array.isArray(dfd_journey_v1Props) && !journey_details_groupd9a0e.result){
    setjourney_details_groupd9a0e((pre:any)=>({...pre,result:dfd_journey_v1Props[0]?.result}));
  }
  },[dfd_journey_v1Props])

  if (status88bc7?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `13 / 25`,gridRow: `23 / 31`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className="!bg-[#f7f8f8]"
  variant="code-1"
  color="primary"
>
      {keyset(isDynamic ? item?.result : (journey_details_groupd9a0e?.result || ""))}
</Text>
  </div>
  )
}
