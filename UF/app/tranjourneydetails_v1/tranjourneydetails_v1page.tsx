...
import Groupjourney_details_group  from "./Groupjourney_details_group/Groupjourney_details_group";


export default function PageTranjourneydetailsV1() {
    ...
    async function transaction_v1DFD(pagination:any): Promise<void>{
        ...
        if(tranjourneydetails_v1Props.length > 0){
          for(let i=0;i< tranjourneydetails_v1Props.length;i++){
            if(tranjourneydetails_v1Props[i].DFDkey == "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:transaction:AFVK:v1"){
  ...
  useEffect(()=>{
    ...
    async function journey_v1DFD(pagination:any): Promise<void>{
        ...
        if(tranjourneydetails_v1Props.length > 0){
          for(let i=0;i< tranjourneydetails_v1Props.length;i++){
            if(tranjourneydetails_v1Props[i].DFDkey == "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:journey:AFVK:v1"){
              // delete tranjourneydetails_v1Props[i].DFDkey;
              let temp=structuredClone(tranjourneydetails_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
        ...
        if(encryptionFlagPage){
           introspect = await AxiosService.get("/UF/introspect",{
            ...
            params: {
              ...
              method: encryptionMethod,
              key:"CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:tranJourneyDetails:AFVK:v1"
            }
          }) 
        }else{
          introspect = await AxiosService.get("/UF/introspect",{
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    settranjourneydetails_v1(allRuleData)
  }, [])
