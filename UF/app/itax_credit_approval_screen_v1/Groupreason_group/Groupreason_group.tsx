...
import Textreason_label  from "./Textreason_label";
...
const Groupreason_group = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_Credit_Approval_Screen:AFVK:v1",componentId:"2aff41bf95344ac0baa4346848839480",from:"GroupReasonGroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("reason_label")){
      setreason_label97548({...reason_label97548,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("reason")){
      setreasone20ad({...reasone20ad,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  function handleConfirmOnLoad(){
    ...
    const handleOnload=()=>{
      ...
      setdocumentviewer_group0a3fb((prev:any) => ({ ...prev, ...reason_group39480 }));
  }
  const handleOnChange=()=>{

  }
  const reason_group39480Ref = useRef<any>(null);
  const handleClearSearch = () => {
    reason_group39480Ref.current?.setSearchParams();
    reason_group39480Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(reason_group39480) && Object.keys(reason_group39480)?.length>0)
      {
        setreason_group39480({})
      }
    }else 
      prevRefreshRef.current= true
  }, [reason_group39480Props?.refresh,token])
  ...
  return (
    ...
    >
          {allowedControls.includes("reason_label") ?<Textreason_label   /* 97548 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
        {allowedControls.includes("reason") ?<TextAreareason   /* e20ad */ checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData}/>: <div></div>}
    </div>
 )
}

export default Groupreason_group
