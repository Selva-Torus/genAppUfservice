...
import Groupprndetails_group  from "../Groupprndetails_group/Groupprndetails_group";
...
import Textprndetails_label  from "./Textprndetails_label";
...
const Groupoverall_group = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const {prndetails_label38a32, setprndetails_label38a32}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_Credit_Approval_Screen:AFVK:v1",componentId:"49985cf6cdc747e9baf60d0c08c1e6a4",from:"GroupOverallGroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("prndetails_label")){
      setprndetails_label38a32({...prndetails_label38a32,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("prndetails_group")){
      setprndetails_group881d8({...prndetails_group881d8,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const overall_group1e6a4Ref = useRef<any>(null);
  const handleClearSearch = () => {
    overall_group1e6a4Ref.current?.setSearchParams();
    overall_group1e6a4Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(overall_group1e6a4) && Object.keys(overall_group1e6a4)?.length>0)
      {
        setoverall_group1e6a4({})
      }
    }else 
      prevRefreshRef.current= true
  }, [overall_group1e6a4Props?.refresh,token])
  ...
  return (
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-2 !border !rounded ${isDark ? 'text-white' : 'text-black'}`}
    >
        {allowedComponent.includes("prndetails_group")  &&<Groupprndetails_group  
          ...
          {allowedControls.includes("prndetails_label") ?<Textprndetails_label   /* 38a32 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}
