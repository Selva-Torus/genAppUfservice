...
const Textesip_no = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_itax_source_tran_dfd_v1Props) && dfd_itax_source_tran_dfd_v1Props && !prndetails_group881d8.eslip_no){
        setprndetails_group881d8((pre:any)=>({...pre,eslip_no:dfd_itax_source_tran_dfd_v1Props[0]?.eslip_no}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[esip_no6f361?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_itax_source_tran_dfd_v1Props) && !prndetails_group881d8.eslip_no){
    setprndetails_group881d8((pre:any)=>({...pre,eslip_no:dfd_itax_source_tran_dfd_v1Props[0]?.eslip_no}));
  }
  },[dfd_itax_source_tran_dfd_v1Props])

  if (esip_no6f361?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.eslip_no : (prndetails_group881d8?.eslip_no || ""))}
</Text>
  </div>
  )
}

export default Textesip_no
