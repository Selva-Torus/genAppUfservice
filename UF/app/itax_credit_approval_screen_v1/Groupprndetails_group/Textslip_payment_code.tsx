...
const Textslip_payment_code = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_itax_source_tran_dfd_v1Props && !dfd_itax_source_tran_dfd_v1Props.hasLogicCenter) {
        ...
        setprndetails_group881d8((pre: any) => {
          return { ...pre, slip_payment_code: api_paginationData.data.records[0]?.slip_payment_code }
        })
        }
      }
      else{
      if(Array.isArray(dfd_itax_source_tran_dfd_v1Props) && dfd_itax_source_tran_dfd_v1Props && !prndetails_group881d8.slip_payment_code){
        setprndetails_group881d8((pre:any)=>({...pre,slip_payment_code:dfd_itax_source_tran_dfd_v1Props[0]?.slip_payment_code}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[slip_payment_code983fc?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_itax_source_tran_dfd_v1Props) && !prndetails_group881d8.slip_payment_code){
    setprndetails_group881d8((pre:any)=>({...pre,slip_payment_code:dfd_itax_source_tran_dfd_v1Props[0]?.slip_payment_code}));
  }
  },[dfd_itax_source_tran_dfd_v1Props])

  if (slip_payment_code983fc?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `13 / 25`,gridRow: `13 / 23`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className=""
  variant="subheader-2"
  color="primary"
>
      {keyset(isDynamic ? item?.slip_payment_code : (prndetails_group881d8?.slip_payment_code || ""))}
</Text>
  </div>
  )
}

export default Textslip_payment_code
