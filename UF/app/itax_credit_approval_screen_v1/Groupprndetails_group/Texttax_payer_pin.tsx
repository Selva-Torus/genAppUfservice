...
const Texttax_payer_pin = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_itax_source_tran_dfd_v1Props && !dfd_itax_source_tran_dfd_v1Props.hasLogicCenter) {
        ...
        setprndetails_group881d8((pre: any) => {
          return { ...pre, tax_payer_pin: api_paginationData.data.records[0]?.tax_payer_pin }
        })
        }
      }
      else{
      if(Array.isArray(dfd_itax_source_tran_dfd_v1Props) && dfd_itax_source_tran_dfd_v1Props && !prndetails_group881d8.tax_payer_pin){
        setprndetails_group881d8((pre:any)=>({...pre,tax_payer_pin:dfd_itax_source_tran_dfd_v1Props[0]?.tax_payer_pin}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[tax_payer_pin2328b?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_itax_source_tran_dfd_v1Props) && !prndetails_group881d8.tax_payer_pin){
    setprndetails_group881d8((pre:any)=>({...pre,tax_payer_pin:dfd_itax_source_tran_dfd_v1Props[0]?.tax_payer_pin}));
  }
  },[dfd_itax_source_tran_dfd_v1Props])

  if (tax_payer_pin2328b?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `13 / 25`,gridRow: `37 / 47`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className=""
  variant="subheader-2"
  color="primary"
>
      {keyset(isDynamic ? item?.tax_payer_pin : (prndetails_group881d8?.tax_payer_pin || ""))}
</Text>
  </div>
  )
}

export default Texttax_payer_pin
