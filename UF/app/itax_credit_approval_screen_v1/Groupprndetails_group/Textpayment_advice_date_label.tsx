...
const Textpayment_advice_date_label = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
  ...
  useEffect(()=>{
    handleMapperValue()
  },[payment_advice_date_label0a7cd?.refresh])

  if (payment_advice_date_label0a7cd?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 13`,gridRow: `25 / 35`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className=""
  variant="subheader-3"
  color="primary"
>
      {keyset("Payment Advice Date")}
</Text>
  </div>
  )
}

export default Textpayment_advice_date_label
