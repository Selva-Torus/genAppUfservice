...
const Textitaxst_id = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_itax_source_tran_dfd_v1Props) && dfd_itax_source_tran_dfd_v1Props && !prndetails_group881d8.itaxst_id){
        setprndetails_group881d8((pre:any)=>({...pre,itaxst_id:dfd_itax_source_tran_dfd_v1Props[0]?.itaxst_id}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[itaxst_idd3e56?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_itax_source_tran_dfd_v1Props) && !prndetails_group881d8.itaxst_id){
    setprndetails_group881d8((pre:any)=>({...pre,itaxst_id:dfd_itax_source_tran_dfd_v1Props[0]?.itaxst_id}));
  }
  },[dfd_itax_source_tran_dfd_v1Props])

  if (itaxst_idd3e56?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `1 / 12`,gridRow: `59 / 64`, gap:``, height: `100%`}} >
<Text
  contentAlign={"center"}
  className=""
  variant="subheader-3"
  color="primary"
>
      {keyset(isDynamic ? item?.itaxst_id : (prndetails_group881d8?.itaxst_id || ""))}
</Text>
  </div>
  )
}
