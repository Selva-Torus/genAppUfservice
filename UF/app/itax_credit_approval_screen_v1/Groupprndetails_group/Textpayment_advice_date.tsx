...
const Textpayment_advice_date = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      if ("hasLogicCenter" in dfd_itax_source_tran_dfd_v1Props && !dfd_itax_source_tran_dfd_v1Props.hasLogicCenter) {
        ...
        setprndetails_group881d8((pre: any) => {
          return { ...pre, payment_advice_date: api_paginationData.data.records[0]?.payment_advice_date }
        })
        }
      }
      else{
      if(Array.isArray(dfd_itax_source_tran_dfd_v1Props) && dfd_itax_source_tran_dfd_v1Props && !prndetails_group881d8.payment_advice_date){
        setprndetails_group881d8((pre:any)=>({...pre,payment_advice_date:dfd_itax_source_tran_dfd_v1Props[0]?.payment_advice_date}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[payment_advice_datefb2ad?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_itax_source_tran_dfd_v1Props) && !prndetails_group881d8.payment_advice_date){
    setprndetails_group881d8((pre:any)=>({...pre,payment_advice_date:dfd_itax_source_tran_dfd_v1Props[0]?.payment_advice_date}));
  }
  },[dfd_itax_source_tran_dfd_v1Props])

  if (payment_advice_datefb2ad?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `13 / 25`,gridRow: `25 / 35`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className=""
  variant="subheader-2"
  color="primary"
>
      {keyset(isDynamic ? item?.payment_advice_date : (prndetails_group881d8?.payment_advice_date || ""))}
</Text>
  </div>
  )
}

export default Textpayment_advice_date
