...
const Texttax_payer_full_name = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
    ...
    try{
      if ("hasLogicCenter" in dfd_itax_source_tran_dfd_v1Props && !dfd_itax_source_tran_dfd_v1Props.hasLogicCenter) {
        ...
        setprndetails_group881d8((pre: any) => {
          return { ...pre, tax_payer_full_name: api_paginationData.data.records[0]?.tax_payer_full_name }
        })
        }
      }
      else{
      if(Array.isArray(dfd_itax_source_tran_dfd_v1Props) && dfd_itax_source_tran_dfd_v1Props && !prndetails_group881d8.tax_payer_full_name){
        setprndetails_group881d8((pre:any)=>({...pre,tax_payer_full_name:dfd_itax_source_tran_dfd_v1Props[0]?.tax_payer_full_name}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[tax_payer_full_namef6644?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_itax_source_tran_dfd_v1Props) && !prndetails_group881d8.tax_payer_full_name){
    setprndetails_group881d8((pre:any)=>({...pre,tax_payer_full_name:dfd_itax_source_tran_dfd_v1Props[0]?.tax_payer_full_name}));
  }
  },[dfd_itax_source_tran_dfd_v1Props])

  if (tax_payer_full_namef6644?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `13 / 25`,gridRow: `49 / 59`, gap:``, height: `100%`}} >
<Text
  contentAlign={"left"}
  className=""
  variant="subheader-2"
  color="primary"
>
      {keyset(isDynamic ? item?.tax_payer_full_name : (prndetails_group881d8?.tax_payer_full_name || ""))}
</Text>
  </div>
  )
}

export default Texttax_payer_full_name
