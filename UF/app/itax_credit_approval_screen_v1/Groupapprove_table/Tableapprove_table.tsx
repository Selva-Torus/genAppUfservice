...
import Buttonapprove_delete  from './Buttonapprove_delete'
...
let defaultColumns = [
  {
    "id": "approve_doc",
    "nodeid": "c05ffe9feb6149ee829947767821f48f",
    "name": "Document Name",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "itaxstd_id",
    "nodeid": "b515991f467c4bda90b93e074acdb195",
    "name": "itaxstd_id",
    "meta": {
      "sort": true
    },
 ...
 /////////////
  ...
  const [SearchParams,setSearchParams] = useState<any>({})
    ...
    if(needLockingAndRule.lockMode=='Single'){
      ...
      if (ids.length == 0) {
        ...
        setdocumentviewer_group0a3fb(keys)
        keys={}       
        Object.keys(documentviewer_group0a3fbProps?.presetValues).map((item: any) => {
          keys[item] = null
        }) 
        setdocumentviewer_group0a3fbProps((pre:any)=>({...pre.presetValues,...keys}))
        setapprove_tableafbb9Props((pre:any)=>({...pre, selectedIds:[]}))
                  //bindtran
        setdocumentviewer_group0a3fb({})
        setdocumentviewer_group0a3fbProps({...documentviewer_group0a3fbProps,presetValues:{}})
        setLockedData((pre:any)=>({...pre,data:[]}))
        return
      }

      approve_tableafbb9.filter((item:any,id:number)=>{
        if (ids[ids.length - 1] == id.toString()){
          selectedData?.push(allData[id])
          postIds.push(item.itaxstd_id)
      ...
      //////////
                  //bindtran
        setdocumentviewer_group0a3fb(flattenKeepInner(allData[ids]))
        setdocumentviewer_group0a3fbProps({...documentviewer_group0a3fbProps,presetValues:{}})
      //////////
      setapprove_tableafbb9Props((pre:any)=>({...pre, selectedIds:[ids[ids.length-1]]}))      
    }
      ...
      setapprove_tableafbb9Props((pre:any)=>({...pre, selectedIds:ids}))
      if(ids?.length>0)
      {
                  //bindtran
        setdocumentviewer_group0a3fb(flattenKeepInner(allData[ids]))
        setdocumentviewer_group0a3fbProps({...documentviewer_group0a3fbProps,presetValues:{}})
                ...
                //bindtran
      setdocumentviewer_group0a3fb(flattenKeepInner(selectedData[0]||{}))
      setdocumentviewer_group0a3fbProps({...documentviewer_group0a3fbProps,presetValues:selectedData[0]||{}})
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setapprove_tableafbb9Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["itaxst_id"]:approve_tableafbb9Props?.presetValues?.itaxst_id}
 
    let dstKey=dfKey?.dfKey
    dstKey=dstKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    try {
    if(!approve_tableafbb9Props?.presetValues || Object.keys(approve_tableafbb9Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_Credit_Approval_Screen:AFVK:v1:UO', 
                nodeId: '15b5384d5b5641709b3c6558e61afbb9',
                elementId: '15b5384d5b5641709b3c6558e61afbb9'
              },
            }
          }else
          {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setapprove_tableafbb9(uf_paginationDataFilter.data||[])
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(approve_tableafbb9))
    {
      GetTableDetails()
    }
  }, [approve_tableafbb9])
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(approve_tableafbb9Props.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [approve_tableafbb9Props.filterProps])
  ...
  useEffect(() => {
    if (approve_tableafbb9Props?.clearData === true) {
      setapprove_tableafbb9([]);
      setAllDataObject([]);
      setAllData([]);
      setSelectedPaginationData([]);
      setLockedData((pre:any) => ({...pre, data:[]}));
      setapprove_tableafbb9Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [approve_tableafbb9Props?.clearData])
  useEffect(() => {
    setapprove_tableafbb9([])
    setapprove_tableafbb9Props((pre:any)=>({...pre, selectedIds:[]}))
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [approve_tableafbb9Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = approve_tableafbb9Props?.selectedIds[approve_tableafbb9Props?.selectedIds?.length-1]
    if(Array.isArray(approve_tableafbb9) && approve_tableafbb9.length>0)
    {
      let data = approve_tableafbb9[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "itaxstd_id",
  ...
  useEffect(() => {
    if (approve_tableafbb9Props?.selectedIds?.length != 0) handlePrimaryTable()
  }, [approve_tableafbb9Props?.selectedIds])
  ...
  if (approve_tableafbb9?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div className=' w-full h-full flex flex-row'>
            <Table
              ...
              edgePadding={true}
              tableSelection={true}
              selectedIds={approve_tableafbb9Props?.selectedIds}  
...
export default Tableapprove_table
