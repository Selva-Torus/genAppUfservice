...
import Groupapplication_tab_group  from "../Groupapplication_tab_group/Groupapplication_tab_group";
...
const Groupapplication_group = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_Credit_Approval_Screen:AFVK:v1",componentId:"40beebec53f84a64a5057ba342b16335",from:"GroupApplicationGroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("application_tab_group")){
      setapplication_tab_groupf82f4({...application_tab_groupf82f4,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const application_group16335Ref = useRef<any>(null);
  const handleClearSearch = () => {
    application_group16335Ref.current?.setSearchParams();
    application_group16335Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(application_group16335) && Object.keys(application_group16335)?.length>0)
      {
        setapplication_group16335({})
      }
    }else 
      prevRefreshRef.current= true
  }, [application_group16335Props?.refresh,token])
  ...
  return (
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md p-1 !border !rounded ${isDark ? 'text-white' : 'text-black'}`}
    >
        {allowedComponent.includes("application_tab_group")  &&<Groupapplication_tab_group  
...
export default Groupapplication_group
