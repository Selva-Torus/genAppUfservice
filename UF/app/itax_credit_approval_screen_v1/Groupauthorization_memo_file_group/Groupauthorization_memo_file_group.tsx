...
import Textauthorization_memo_file_label  from "./Textauthorization_memo_file_label";
...
const Groupauthorization_memo_file_group = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing}:any)=> {
 ...
 /////////////
  ...
  const {authorization_memo_file_label01fe5, setauthorization_memo_file_label01fe5}= useContext(TotalContext) as TotalContextProps;
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT010:FNGK:AF:FNK:UF-UFW:CATK:I001:AFGK:ITAX:AFK:ITAX_Credit_Approval_Screen:AFVK:v1",componentId:"5b7595e41c2f4b09826b291d0a717228",from:"GroupAuthorizationMemoFileGroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("authorization_memo_file_label")){
      setauthorization_memo_file_label01fe5({...authorization_memo_file_label01fe5,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const authorization_memo_file_group17228Ref = useRef<any>(null);
  const handleClearSearch = () => {
    authorization_memo_file_group17228Ref.current?.setSearchParams();
    authorization_memo_file_group17228Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(authorization_memo_file_group17228) && Object.keys(authorization_memo_file_group17228)?.length>0)
      {
        setauthorization_memo_file_group17228({})
      }
    }else 
      prevRefreshRef.current= true
  }, [authorization_memo_file_group17228Props?.refresh,token])
  ...
  return (
    ...
    >
          {allowedControls.includes("authorization_memo_file_label") ?<Textauthorization_memo_file_label   /* 01fe5 */ isDynamic={false } index={idx} item={item} checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} />: <div></div>}
    </div>
 )
}

export default Groupauthorization_memo_file_group
