...
const Dropdownproduct_code = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagCompData}: any) => {
 ...
 /////////////
  ...
  useEffect(() => {
  if(system_setup_group2af15?.product_code=="" || system_setup_group2af15?.product_code==undefined || system_setup_group2af15?.product_code==null ){
    setSelectedItem("");
  }
  },[system_setup_group2af15?.product_code])
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "8e6af89b624d4b63a0b5cd43484523b7",
          isTable: false,
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setsystem_setup_group2af15((prev: any) => ({ ...prev, product_code: value}))
         setIsRequredData(false)
    } else {
       setsystem_setup_group2af15((prev: any) => ({ ...prev, product_code: ''}))
  ...
  useEffect(() => {
    if(initialCount!=0)
     setsystem_setup_group2af15((pre:any)=>({...pre,product_code:""}))
    else
      setInitialCount(1)
  },[product_code523b7?.refresh])

  if (product_code523b7?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown
        ...
        static={true}
        staticProps={items}
        disabled= {product_code523b7?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={system_setup_group2af15?.product_code ?system_setup_group2af15?.product_code: []}
...
export default Dropdownproduct_code;
