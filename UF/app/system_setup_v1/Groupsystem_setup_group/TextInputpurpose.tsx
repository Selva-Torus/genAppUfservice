...
const TextInputpurpose = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:V001:AFGK:VGPH001:AFK:Master_System_Setup_DFD:AFVK:v1|ea8268fea07e4f9e991a1536c35c8463|properties.purpose"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:VGPH001:AFK:Master_System_Setup:AFVK:v1|723ad64155fe45adba8c526f1ce2af15|2be197908b6c42ba93b3e0c2eb33b7f4"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'purpose',type:"text"})
  ...
  const handleChange = async(e: any) => {
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['system_setup_group']  = {...system_setup_group2af15,purpose:newInputValue},
      codeStates['setsystem_setup_group'] = setsystem_setup_group2af15,
    codeExecution(code,codeStates);
    }  
    setError('');
    setValidate((pre:any)=>({...pre,purpose:undefined}));
    if(dynamicStateandType.type=="number"){
    setsystem_setup_group2af15((prev: any) => ({ ...prev, purpose: +e.target.value }));
    }
    else{
    setsystem_setup_group2af15((prev: any) => ({ ...prev, purpose: e.target.value }));
    }
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "2be197908b6c42ba93b3e0c2eb33b7f4",
          isTable: false,
          from:"TextInputpurpose",
      ...
      if(orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties){
        let type:any={name:'purpose',type:'text'};
        type={
          name:'purpose',
          type: orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.purpose.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.purpose.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.purpose.type
        }
        ...
        type={
          name:'purpose',
          type: orchestrationData?.data?.schemaData[0].schema.properties.purpose.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.properties.purpose.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.properties.purpose.type
        }
        setDynamicStateandType(type);
      }
      }
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setsystem_setup_group2af15((pre:any)=>({...pre,purpose:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
  ...
  if(dfd_master_system_setup_dfd_v1Props?.setSearchFilters && dfd_master_system_setup_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_master_system_setup_dfd_v1Props.data) && dfd_master_system_setup_dfd_v1Props.data.length > 0){
      setsystem_setup_group2af15((pre:any)=>({...pre,purpose:dfd_master_system_setup_dfd_v1Props.data[0]?.purpose}));
    }
  }
  },[dfd_master_system_setup_dfd_v1Props?.setSearchFilters])
  if (purpose3b7f4?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `17 / 25`,gridRow: `26 / 45`, gap:``, height: `100%`, overflow: 'auto'}} >
      <TextInput
        ...
        value={system_setup_group2af15?.purpose||""}
         disabled= {purpose3b7f4?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerPosition='top'
        headerText="Purpose"
        validationState={validate?.purpose ? "invalid" : undefined}
        errorMessage={error}
      />
    </div> 
  )
...
export default TextInputpurpose
