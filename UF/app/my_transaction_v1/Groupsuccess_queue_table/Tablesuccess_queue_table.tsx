...
let defaultColumns:any = [
  {
    "id": "product_code_success_queue",
    "nodeid": "d28d6e27d1d64f28a89a6e3d81e7c209",
    "name": "Product Code",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "channel_name_success_queue",
    "nodeid": "595590e11f0b493a8898af7a463eddaf",
    "name": "Channel",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "uuid_success_queue",
    "nodeid": "cdf1ef1529a94f9d9e65d323337c805b",
    "name": "UUID",
    "meta": {
      "sort": true
    },
    ...
    "isSearch": false,
    "colourIndicator": [],
    "dfdName": "parent_vgphstm_uuid"
  },
  {
    "id": "dr_account_operational_pending",
    "nodeid": "61c62624e3bb467bac88de015b710a49",
  ...
  {
    "id": "dr_amount_success_queue",
    "nodeid": "f3f0482ecb1a4934abd9cc71594da254",
    "name": "Debitor Amount",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "cr_account_success_queue",
    "nodeid": "bbe8cdacd0de4b91b4a1546d87f60480",
  ...
  {
    "id": "cr_Amount_success_queue",
    "nodeid": "2aa66bb6dc954b4ca40f48f61eeb80d4",
  ...
  {
    "id": "remittance_info_success_queue",
    "nodeid": "43467147c2fc4f6ab32956ef3382f950",
  ...
  {
    "id": "status_success_queue",
    "nodeid": "a572277f7389400580571ad2b95019a2",
    "name": "Status",
    "meta": {
      "sort": true
    },
...
const Tablesuccess_queue_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,controlData,groupData}: any)=>{
 ...
 /////////////
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "479889fb285d4c6ba08dce5b04663aae",
      );
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setsuccess_queue_table63aaeProps((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setsuccess_queue_table63aaeProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:VGPH:AFK:transaction:AFVK:v1:UO', 
                nodeId: '479889fb285d4c6ba08dce5b04663aae',
                elementId: '479889fb285d4c6ba08dce5b04663aae'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( success_queue_table63aaeProps?.presetValues&&Object.keys(success_queue_table63aaeProps?.presetValues).length > 0) {
          filtertedData = [success_queue_table63aaeProps?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setsuccess_queue_table63aae(api_pagination?.data?.records||[])
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(success_queue_table63aaeProps.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [success_queue_table63aaeProps.filterProps])
  ...
  useEffect(() => {
    if (success_queue_table63aaeProps?.clearData === true) {
      setsuccess_queue_table63aae([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setsuccess_queue_table63aaeProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [success_queue_table63aaeProps?.clearData])
  useEffect(() => {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [success_queue_table63aaeProps?.refresh])


  const handlePrimaryTable = () => {
    let findData = success_queue_table63aaeProps?.selectedIds[success_queue_table63aaeProps?.selectedIds?.length-1]
    if(Array.isArray(success_queue_table63aae) && success_queue_table63aae.length>0)
    {
      let data = success_queue_table63aae[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "vgphst_id",
  ...
  useEffect(() => {
    if (success_queue_table63aaeProps?.selectedIds?.length != 0) handlePrimaryTable()
    if (success_queue_table63aaeProps?.selectedIds?.length == 0){
      handleOnRowClick({},success_queue_table63aaeProps?.selectedIds)
    }
  }, [success_queue_table63aaeProps?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(success_queue_table63aae?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&success_queue_table63aae?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](success_queue_table63aae?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: success_queue_table63aae?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&success_queue_table63aae?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](success_queue_table63aae?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: success_queue_table63aae?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (success_queue_table63aae?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"479889fb285d4c6ba08dce5b04663aae"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              selectedIds={success_queue_table63aaeProps?.selectedIds}  
...
export default Tablesuccess_queue_table
