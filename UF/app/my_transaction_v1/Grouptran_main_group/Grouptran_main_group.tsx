...
import Grouptran_tab_group  from "../Grouptran_tab_group/Grouptran_tab_group";
...
const Grouptran_main_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData={}, controlData={}}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData = getGroupOrchestrationData(groupData, "9e5a570a2667451fb476775abc71dc7f");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    settran_main_group1dc7fProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("tran_tab_group")){
      settran_tab_group08b64({...tran_tab_group08b64,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const tran_main_group1dc7fRef = useRef<any>(null);
  const handleClearSearch = () => {
    tran_main_group1dc7fRef.current?.setSearchParams();
    tran_main_group1dc7fRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(tran_main_group1dc7f) && Object.keys(tran_main_group1dc7f)?.length>0)
      {
        settran_main_group1dc7f({})
      }
    }else 
      prevRefreshRef.current= true
  }, [tran_main_group1dc7fProps?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("tran_tab_group")  &&<Grouptran_tab_group  
...
export default Grouptran_main_group
