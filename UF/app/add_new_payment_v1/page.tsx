...
import Grouppayment_group  from "./Grouppayment_group/Grouppayment_group";


export default function PageAddNewPaymentV1() {
  ...
  const [checkpayment_group,setCheckpayment_group,]=useState<boolean>(false);
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function get_transaction_dfd_v1(pagination:any): Promise<void>{
        ...
        if(add_new_payment_v1Props.length > 0){
          let filterData :any[] =[];
          for(let i=0;i< add_new_payment_v1Props.length;i++){
            if(add_new_payment_v1Props[i].DFDkey == "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:V001:AFGK:VGPH001:AFK:Get_Transaction_DFD:AFVK:v1"){
              delete add_new_payment_v1Props[i].DFDkey;
              filterData.push(add_new_payment_v1Props[i])
            }           
          }
  ...
  async function securityCheck(): Promise<void> {
    const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:VGPH001:AFK:Add_New_Payment:AFVK:v1",accessProfile:[user],from:"pageAddNewPaymentV1"},{
      headers: {
        Authorization: `Bearer ${token}`
      }});
  ...
  return (
    <>
    <div className='p-2 groupStyle w-[100px] '>
      <Button
          onClick={handleClick}
     ...
     style={{
       ...
       // minHeight: '100vh',
        ...
        {checkpayment_group && initialLoad &&<Grouppayment_group
