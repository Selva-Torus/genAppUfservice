...
const Dropdowntran_category = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagCompData}: any) => {
 ...
 /////////////
  ...
  useEffect(() => {
  if(payment_group1c8a5?.tran_category=="" || payment_group1c8a5?.tran_category==undefined || payment_group1c8a5?.tran_category==null ){
    setSelectedItem("");
  }
  },[payment_group1c8a5?.tran_category])
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "7c4a18d27c81473d871fbabadf981c97",
          isTable: false,
          accessProfile:accessProfile,
          from:"dropdownTran Category"
        },
        {
          headers: {
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setpayment_group1c8a5((prev: any) => ({ ...prev, tran_category: value}))
         setIsRequredData(false)
    } else {
       setpayment_group1c8a5((prev: any) => ({ ...prev, tran_category: ''}))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,tran_category:undefined}))
  ...
  useEffect(() => {
    if(initialCount!=0)
     setpayment_group1c8a5((pre:any)=>({...pre,tran_category:""}))
    else
      setInitialCount(1)
  },[tran_category81c97?.refresh])

  if (tran_category81c97?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown
        className=""
        placeholder={keyset("Tran Category")} 
        filterable={true}
        hasClear={true}
        static={true}
        staticProps={items}
        disabled= {tran_category81c97?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={payment_group1c8a5?.tran_category ?payment_group1c8a5?.tran_category: []}
        onChange={handleClick} 
      /> 
      {validate?.tran_category && (
...
export default Dropdowntran_category;
