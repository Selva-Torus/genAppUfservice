...
const Dropdowncurrency = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagCompData}: any) => {
 ...
 /////////////
  ...
  useEffect(() => {
  if(payment_group1c8a5?.dr_currency=="" || payment_group1c8a5?.dr_currency==undefined || payment_group1c8a5?.dr_currency==null ){
    setSelectedItem("");
  }
  },[payment_group1c8a5?.dr_currency])
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "7165d3f7c408400baf90991363d124c5",
          isTable: false,
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setpayment_group1c8a5((prev: any) => ({ ...prev, dr_currency: value}))
         setIsRequredData(false)
    } else {
       setpayment_group1c8a5((prev: any) => ({ ...prev, dr_currency: ''}))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,dr_currency:undefined}))
  ...
  useEffect(() => {
    if(initialCount!=0)
     setpayment_group1c8a5((pre:any)=>({...pre,dr_currency:""}))
    else
      setInitialCount(1)
  },[currency124c5?.refresh])

  if (currency124c5?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown
        ...
        static={true}
        staticProps={items}
        disabled= {currency124c5?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={payment_group1c8a5?.dr_currency ?payment_group1c8a5?.dr_currency: []}
