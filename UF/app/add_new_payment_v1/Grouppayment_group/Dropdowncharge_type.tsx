...
const Dropdowncharge_type = ({lockedData,setLockedData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagCompData}: any) => {
 ...
 /////////////
  ...
  useEffect(() => {
  if(payment_group1c8a5?.charge_type=="" || payment_group1c8a5?.charge_type==undefined || payment_group1c8a5?.charge_type==null ){
    setSelectedItem("");
  }
  },[payment_group1c8a5?.charge_type])
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "048bb88c91544964a7e6cc63f7a977c5",
          isTable: false,
  ...
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      ...
      setpayment_group1c8a5((prev: any) => ({ ...prev, charge_type: value}))
         setIsRequredData(false)
    } else {
       setpayment_group1c8a5((prev: any) => ({ ...prev, charge_type: ''}))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,charge_type:undefined}))
  ...
  useEffect(() => {
    if(initialCount!=0)
     setpayment_group1c8a5((pre:any)=>({...pre,charge_type:""}))
    else
      setInitialCount(1)
  },[charge_type977c5?.refresh])

  if (charge_type977c5?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      <Dropdown
        ...
        static={true}
        staticProps={items}
        disabled= {charge_type977c5?.isDisabled ? true : false}
        contentAlign={"center"}
        headerPosition='top'
        headerText={
          <>
        ...
        value={payment_group1c8a5?.charge_type ?payment_group1c8a5?.charge_type: []}
...
export default Dropdowncharge_type;
