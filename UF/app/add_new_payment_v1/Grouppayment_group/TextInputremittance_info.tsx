...
const TextInputremittance_info = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:V001:AFGK:VGPH001:AFK:Get_Transaction_DFD:AFVK:v1|5bc8f410f27248d88fc91b7fe01fb9c0|properties.remittance_info"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:VGPH001:AFK:Add_New_Payment:AFVK:v1|4e3a333fdb93472c97f4c7ca2461c8a5|4265794a398144068b7407a0e4f57b4b"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  const handleChange = async(e: any) => {
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['payment_group']  = {...payment_group1c8a5,remittance_info:newInputValue},
      codeStates['setpayment_group'] = setpayment_group1c8a5,
    codeExecution(code,codeStates);
    }  
    setError('');
    setValidate((pre:any)=>({...pre,remittance_info:undefined}));
    if(dynamicStateandType.type=="number"){
    setpayment_group1c8a5((prev: any) => ({ ...prev, remittance_info: +e.target.value }));
    }
    else{
    setpayment_group1c8a5((prev: any) => ({ ...prev, remittance_info: e.target.value }));
    }
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "4265794a398144068b7407a0e4f57b4b",
          isTable: false,
      ...
      if(orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties){
        ...
        type={
          ...
          type: orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.remittance_info.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.remittance_info.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.remittance_info.type
        }
        ...
        type={
          name:'remittance_info',
          type: orchestrationData?.data?.schemaData[0].schema.properties.remittance_info.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.properties.remittance_info.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.properties.remittance_info.type
        }
        setDynamicStateandType(type);
      }
      }
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setpayment_group1c8a5((pre:any)=>({...pre,remittance_info:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
  ...
  if(dfd_get_transaction_dfd_v1Props?.setSearchFilters && dfd_get_transaction_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_get_transaction_dfd_v1Props.data) && dfd_get_transaction_dfd_v1Props.data.length > 0){
      setpayment_group1c8a5((pre:any)=>({...pre,remittance_info:dfd_get_transaction_dfd_v1Props.data[0]?.remittance_info}));
    }
  }
  },[dfd_get_transaction_dfd_v1Props?.setSearchFilters])
  if (remittance_info57b4b?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 7`,gridRow: `61 / 80`, gap:``, height: `100%`, overflow: 'auto'}} >
      <TextInput
        ...
        value={payment_group1c8a5?.remittance_info||""}
         disabled= {remittance_info57b4b?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Remittance Info"
        validationState={validate?.remittance_info ? "invalid" : undefined}
        errorMessage={error}
      />
    </div> 
  )
