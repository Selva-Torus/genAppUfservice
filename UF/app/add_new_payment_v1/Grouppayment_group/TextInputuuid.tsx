...
const TextInputuuid = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:V001:AFGK:VGPH001:AFK:Get_Transaction_DFD:AFVK:v1|5bc8f410f27248d88fc91b7fe01fb9c0|properties.uuid"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:VGPH001:AFK:Add_New_Payment:AFVK:v1|4e3a333fdb93472c97f4c7ca2461c8a5|357ca51f298f4c97a4fc47d537ce86ae"
    }
  ],
  "schemaData": {
    "type": "string"
  ...
  let schemaArray :any =[];  
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'uuid',type:"text"})
  ...
  const handleChange = async(e: any) => {
    ...
    let code:any=allCode
     if (code != '') {
      let codeStates: any = {}
      codeStates['payment_group']  = {...payment_group1c8a5,uuid:newInputValue},
      codeStates['setpayment_group'] = setpayment_group1c8a5,
    codeExecution(code,codeStates)
    }  
    setError('')
    setValidate((pre:any)=>({...pre,uuid:undefined}))
    if(dynamicStateandType.type=="number"){
    setpayment_group1c8a5((prev: any) => ({ ...prev, uuid: +e.target.value }))
    }
    else{
    setpayment_group1c8a5((prev: any) => ({ ...prev, uuid: e.target.value }))
    }
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "357ca51f298f4c97a4fc47d537ce86ae",
          isTable: false,
          from:"TextInputuuid",
      ...
      if(orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties){
        ...
        type={
          name:'uuid',
          type: orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.uuid.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.uuid.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.uuid.type
        }
        ...
        type={
          name:'uuid',
          type: orchestrationData?.data?.schemaData[0].schema.properties.uuid.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.properties.uuid.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.properties.uuid.type
        }
        setDynamicStateandType(type)
      }
      }
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setpayment_group1c8a5((pre:any)=>({...pre,uuid:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
  ...
  if(dfd_get_transaction_dfd_v1Props?.setSearchFilters && dfd_get_transaction_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_get_transaction_dfd_v1Props.data) && dfd_get_transaction_dfd_v1Props.data.length > 0){
      setpayment_group1c8a5((pre:any)=>({...pre,uuid:dfd_get_transaction_dfd_v1Props.data[0]?.uuid}));
    }
  }
  },[dfd_get_transaction_dfd_v1Props?.setSearchFilters])
  if (uuide86ae?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 7`,gridRow: `41 / 60`, gap:``, height: `100%`, overflow: 'auto'}} >
      <TextInput
        ...
        value={payment_group1c8a5?.uuid||""}
         disabled= {uuide86ae?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerPosition='top'
        headerText="UUID"
        validationState={validate?.uuid ? "invalid" : undefined}
        errorMessage={error}
      />
    </div> 
  )
