...
const TextInputamount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:V001:AFGK:VGPH001:AFK:Get_Transaction_DFD:AFVK:v1|5bc8f410f27248d88fc91b7fe01fb9c0|properties.dr_amount"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:VGPH001:AFK:Add_New_Payment:AFVK:v1|4e3a333fdb93472c97f4c7ca2461c8a5|35c4e67b25094df4aee713942a9c2ae9"
    }
  ],
  "schemaData": {
    "type": "number"
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'dr_amount',type:"text"})
  ...
  const handleChange = async(e: any) => {
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
      codeStates['payment_group']  = {...payment_group1c8a5,dr_amount:newInputValue},
      codeStates['setpayment_group'] = setpayment_group1c8a5,
    codeExecution(code,codeStates);
    }  
    setError('');
    setValidate((pre:any)=>({...pre,dr_amount:undefined}));
    if(dynamicStateandType.type=="number"){
    setpayment_group1c8a5((prev: any) => ({ ...prev, dr_amount: +e.target.value }));
    }
    else{
    setpayment_group1c8a5((prev: any) => ({ ...prev, dr_amount: e.target.value }));
    }
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "35c4e67b25094df4aee713942a9c2ae9",
          isTable: false,
          from:"TextInputamount",
      ...
      if(orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties){
        ...
        type={
          name:'dr_amount',
          type: orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.dr_amount.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.dr_amount.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.dr_amount.type
        }
        ...
        type={
          name:'dr_amount',
          type: orchestrationData?.data?.schemaData[0].schema.properties.dr_amount.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.properties.dr_amount.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.properties.dr_amount.type
        }
        setDynamicStateandType(type);
      }
      }
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setpayment_group1c8a5((pre:any)=>({...pre,dr_amount:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
  ...
  if(dfd_get_transaction_dfd_v1Props?.setSearchFilters && dfd_get_transaction_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_get_transaction_dfd_v1Props.data) && dfd_get_transaction_dfd_v1Props.data.length > 0){
      setpayment_group1c8a5((pre:any)=>({...pre,dr_amount:dfd_get_transaction_dfd_v1Props.data[0]?.dr_amount}));
    }
  }
  },[dfd_get_transaction_dfd_v1Props?.setSearchFilters])
  if (amountc2ae9?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 19`,gridRow: `21 / 40`, gap:``, height: `100%`, overflow: 'auto'}} >
      <TextInput
        ...
        value={payment_group1c8a5?.dr_amount||""}
         disabled= {amountc2ae9?.isDisabled ? true : false}
        pin='brick-brick'     
        ...
        headerPosition='top'
        headerText="Amount"
        validationState={validate?.dr_amount ? "invalid" : undefined}
        errorMessage={error}
      />
    </div> 
  )
