...
const TextInputcreditor_accounts = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:V001:AFGK:VGPH001:AFK:Get_Transaction_DFD:AFVK:v1|5bc8f410f27248d88fc91b7fe01fb9c0|properties.cr_account"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:V001:AFGK:VGPH001:AFK:Add_New_Payment:AFVK:v1|4e3a333fdb93472c97f4c7ca2461c8a5|6a37866271a241699f68573065282148"
    }
  ],
  "schemaData": {
    "type": "string"
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "6a37866271a241699f68573065282148",
          isTable: false,
          from:"TextInputcreditor_accounts",
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setpayment_group1c8a5((pre:any)=>({...pre,cr_account:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
  ...
  if(dfd_get_transaction_dfd_v1Props?.setSearchFilters && dfd_get_transaction_dfd_v1Props?.data)
  {
    if(Array.isArray(dfd_get_transaction_dfd_v1Props.data) && dfd_get_transaction_dfd_v1Props.data.length > 0){
      setpayment_group1c8a5((pre:any)=>({...pre,cr_account:dfd_get_transaction_dfd_v1Props.data[0]?.cr_account}));
    }
  }
  },[dfd_get_transaction_dfd_v1Props?.setSearchFilters])
  if (creditor_accounts82148?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `7 / 13`,gridRow: `21 / 40`, gap:``, height: `100%`, overflow: 'auto'}} >
      <TextInput
        ...
        value={payment_group1c8a5?.cr_account||""}
         disabled= {creditor_accounts82148?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Creditor Account"
        validationState={validate?.cr_account ? "invalid" : undefined}
        errorMessage={error}
      />
    </div> 
  )
...
export default TextInputcreditor_accounts
