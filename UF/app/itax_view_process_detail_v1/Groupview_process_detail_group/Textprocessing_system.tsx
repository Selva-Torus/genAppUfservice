...
const Textprocessing_system = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if(Array.isArray(dfd_itax_tran_log_dfd_v1Props) && dfd_itax_tran_log_dfd_v1Props && !view_process_detail_groupe7fe3.processing_system){
        setview_process_detail_groupe7fe3((pre:any)=>({...pre,processing_system:dfd_itax_tran_log_dfd_v1Props[0]?.processing_system}));
      }
      }
    }catch(err){
      console.log(err)
  ...
  useEffect(()=>{
    handleMapperValue()
  },[processing_systemcd502?.refresh])

  useEffect(() => {
  if(Array.isArray(dfd_itax_tran_log_dfd_v1Props) && !view_process_detail_groupe7fe3.processing_system){
    setview_process_detail_groupe7fe3((pre:any)=>({...pre,processing_system:dfd_itax_tran_log_dfd_v1Props[0]?.processing_system}));
  }
  },[dfd_itax_tran_log_dfd_v1Props])

  if (processing_systemcd502?.isHidden) {
    return <></>
  }
...
<Text
  ...
  color="primary"
>
      {keyset(isDynamic ? item?.processing_system : (view_process_detail_groupe7fe3?.processing_system || ""))}
</Text>
  </div>
  )
}

export default Textprocessing_system
