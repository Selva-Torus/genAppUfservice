...
const Buttoncancel = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const handleMapper=async (data?:any) => {
    try{     
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "295325e9712143f0956c9761f02e5af4"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("cancel" in button_group74f3eProps?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(button_group74f3eProps?.dynamicActionRule?.cancel)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("cancel" in scanapproveprocessui_v1?.button_group && scanapproveprocessui_v1?.button_group["cancel"]?.itsHaveArtifact== true)
      {
        setShowFlag(scanapproveprocessui_v1?.button_group["cancel"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(scanapproveprocessui_v1?.button_group?.cancel?.order)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    if (!cancele5af4?.trigger) return;
      if(cancele5af4?.trigger){
      (async()=>{
        await handleClick();
      })();
      setcancele5af4((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
  ...
  const handleClick=async()=>{
    try{  
      ...
      setbutton_group74f3e((prev: any) => ({ ...prev, cancel: true }));
        //onClick
    ...
    setShowProfileAsModalOpen4(true);
      ...
      setbutton_group74f3e((prev: any) => ({ ...prev, cancel: false }));
  ...
  }, [scanapproveprocessui_v1?.button_group?.cancel,button_group74f3e?.dynamicActionRule?.cancel,control_tab_groupbc3e2])

 if (cancele5af4?.isHidden) {
    return <></>
  }
    ...
    <div
      ...
      >
        ...
        {showFlag && <Button 
          ...
          onClick={handleClick}
          view='action'
          disabled= {cancele5af4?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
          icon="MdCancel"
