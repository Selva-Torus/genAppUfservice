...
const Buttonreject = ({ lockedData, setLockedData, tableData, setTableData, primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}: { lockedData:any,setLockedData:any,tableData:any,setTableData:any,checkToAdd:any,setCheckToAdd:any,refetch:any,setRefetch:any,primaryTableData:any,setPrimaryTableData:any,encryptionFlagCompData:any,setIsProcessing:any,controlData:any}) => {
 ...
 /////////////
  ...
  const {approvef2390, setapprovef2390}= useContext(TotalContext) as TotalContextProps;
  const {reject2bf72, setreject2bf72}= useContext(TotalContext) as TotalContextProps;
  const {cancele5af4, setcancele5af4}= useContext(TotalContext) as TotalContextProps;
  ...
  const handleCustomCode=async () => {
    ...
    if (code != '') {
      let codeStates: Record<string, any> = {};
        ...
        codeStates['approve'] = approvef2390,
        codeStates['setapprove'] = setapprovef2390,
        codeStates['reject'] = reject2bf72,
        codeStates['setreject'] = setreject2bf72,
        codeStates['cancel'] = cancele5af4,
        codeStates['setcancel'] = setcancele5af4,
  ...
  const handleMapper=async (data?:any) => {
    try{     
      const orchestrationData : any = getControlOrchestrationData(
        ...
        "1606eaecab464786b4cc3669d5a2bf72"
      );
    ...
    if(orchestrationData?.data?.rule?.nodes?.length > 0){
      ...
      if("reject" in button_group74f3eProps?.dynamicActionRule){
        setShowFlag(true)
        setStyleSate({...getGridPositionFromOrder(button_group74f3eProps?.dynamicActionRule?.reject)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }
      else
      {
        setShowFlag(false)   
      ...
      if("reject" in scanapproveprocessui_v1?.button_group && scanapproveprocessui_v1?.button_group["reject"]?.itsHaveArtifact== true)
      {
        setShowFlag(scanapproveprocessui_v1?.button_group["reject"]?.show||false)
        
        setStyleSate({...getGridPositionFromOrder(scanapproveprocessui_v1?.button_group?.reject?.order)||{}, gap:`12px`, height: `100%`, overflow: 'auto'})
      }else{
        setShowFlag(false)
      }
    }
  ...
  useEffect(()=>{
    if (!reject2bf72?.trigger) return;
      if(reject2bf72?.trigger){
      (async()=>{
        await handleClick();
      })();
      setreject2bf72((prev:any) => ({...prev, trigger: !prev?.trigger}));
      (async()=>{
        await handleMapper();
  ...
  const handleClick=async()=>{
    try{  
      ...
      setbutton_group74f3e((prev: any) => ({ ...prev, reject: true }));
        //onClick
    ...
    let filterData4 = await getFilterProps(filterProps4,button_group74f3e);
    setrejectpopupui_v1Props([...filterData4 ]);
    setShowProfileAsModalOpen4(true);
      await handleCustomCode();
    }catch (err: any) {
      setIsProcessing(false);
      setbutton_group74f3e((prev: any) => ({ ...prev, reject: false }));
  ...
  }, [scanapproveprocessui_v1?.button_group?.reject,button_group74f3e?.dynamicActionRule?.reject,control_tab_groupbc3e2])

 if (reject2bf72?.isHidden) {
    return <></>
  }
  ...
  return (
    <div
      ...
      >
        <PageRejectpopupuipage4/>
      </Modal>
        {showFlag && <Button 
          ref={buttonRef}
          className=" p-1  !bg-red-300 !text-red-800 !rounded-md"
          onClick={handleClick}
          view='action'
          disabled= {reject2bf72?.isDisabled ? true : false}
          pin='circle-circle'
          contentAlign={"center"}
