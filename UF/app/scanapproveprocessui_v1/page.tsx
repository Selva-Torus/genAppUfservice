...
export default function PageScanapproveprocessuiV1() {
    ...
    async function scansaveprocessdfd_v1DFD(pagination:any): Promise<void>{
        ...
        if(scanapproveprocessui_v1Props.length > 0){
          for(let i=0;i< scanapproveprocessui_v1Props.length;i++){
            if(scanapproveprocessui_v1Props[i].DFDkey == "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:scanSaveProcessDfd:AFVK:v1"){
  ...
  useEffect(()=>{
    ...
    async function crbankcodedropdowndfd_v1DFD(pagination:any): Promise<void>{
        ...
        if(scanapproveprocessui_v1Props.length > 0){
          for(let i=0;i< scanapproveprocessui_v1Props.length;i++){
            if(scanapproveprocessui_v1Props[i].DFDkey == "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:crBankCodeDropDownDfd:AFVK:v1"){
              // delete scanapproveprocessui_v1Props[i].DFDkey;
              let temp=structuredClone(scanapproveprocessui_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  useEffect(()=>{
    ...
    async function forexcurrencydropdowndfd_v1DFD(pagination:any): Promise<void>{
        ...
        if(scanapproveprocessui_v1Props.length > 0){
          for(let i=0;i< scanapproveprocessui_v1Props.length;i++){
            if(scanapproveprocessui_v1Props[i].DFDkey == "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:forexCurrencyDropDownDfd:AFVK:v1"){
              // delete scanapproveprocessui_v1Props[i].DFDkey;
              let temp=structuredClone(scanapproveprocessui_v1Props[i])
              delete temp?.DFDkey
              filterData.push(temp)
  ...
  const handleArtfactRule=async(rule:any,data:any={},allRuleData:any)=>{
    ...
    setscanapproveprocessui_v1({...result,_artfactPFRule_:rule})
  }
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    setscanapproveprocessui_v1(allRuleData)
  }, [])

  useEffect(()=>{
    if(scanapproveprocessui_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
...
control_tab_group:control_tab_groupbc3e2.control_tab_group,list_tab_group:list_tab_groupd6905.list_tab_group,rtgs_list_tab_grp:rtgs_list_tab_grp024e1.rtgs_list_tab_grp,      }
      handleArtfactRule(scanapproveprocessui_v1?._artfactPFRule_,data,allRuleData)
    }
