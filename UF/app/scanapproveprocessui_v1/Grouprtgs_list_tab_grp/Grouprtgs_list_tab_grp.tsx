...
const Grouprtgs_list_tab_grp = ({lockedData={},setLockedData,primaryTableData={},tableData=[], setTableData ,setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData={}, controlData={}}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "8d116f1e9c354a0d8bf1c9c8ebe024e1"
      );
  ...
  for(let i=0;i<tabOptions?.length;i++){
    ...
    {
      setrtgs_list_tab_grp024e1((pre:any)=>({...pre,rtgs_list_tab_grp:tabOptions[i]?.id}));
      break;
    }
  }   
  /////////////
        setdocumnt_list03a06({...documnt_list03a06,isDisabled:orchestrationData?.data?.readableControls.includes("documnt_list")});
        setvalidtn_lista5b14({...validtn_lista5b14,isDisabled:orchestrationData?.data?.readableControls.includes("validtn_list")});
        setcmnt_listebbbc({...cmnt_listebbbc,isDisabled:orchestrationData?.data?.readableControls.includes("cmnt_list")});
  //////////////
    if (code != '') {
      let codeStates: any = {};
    ...
    const handleOnload=()=>{
     setrtgs_list_tab_grp024e1((pre:any)=>({...pre,rtgs_list_tab_grp:"documnt_list"}));
  }
  const handleOnChange=async(id?:string)=>{
    ...
    setrtgs_list_tab_grp024e1((pre:any)=>({...pre,rtgs_list_tab_grp:id}));

  }
  const rtgs_list_tab_grp024e1Ref = useRef<any>(null);
  const handleClearSearch = () => {
    rtgs_list_tab_grp024e1Ref.current?.setSearchParams();
    rtgs_list_tab_grp024e1Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rtgs_list_tab_grp024e1) && Object.keys(rtgs_list_tab_grp024e1)?.length>0)
      {
        setrtgs_list_tab_grp024e1({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rtgs_list_tab_grp024e1Props?.refresh])
...
let tabHeaderItems : any =[
  ...
  return (
    ...
    <Tabs
      ...
      defaultActiveId={rtgs_list_tab_grp024e1?.rtgs_list_tab_grp || "documnt_list"}
      activeTab={rtgs_list_tab_grp024e1?.rtgs_list_tab_grp || "documnt_list"}
