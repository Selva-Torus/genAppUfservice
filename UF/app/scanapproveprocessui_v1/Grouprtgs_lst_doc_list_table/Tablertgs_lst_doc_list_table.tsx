...
import Buttonaction_rtgs_list  from './Buttonaction_rtgs_list'
...
let defaultColumns:any = [
  {
    "id": "file_name_rtgs_list",
    "nodeid": "20a9cde70dcf481b8a0db52724261a4b",
    "name": "File Name",
    "meta": {
      "sort": true
    },
...
const Tablertgs_lst_doc_list_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const {file_name_rtgs_list61a4b, setfile_name_rtgs_list61a4b}= useContext(TotalContext) as TotalContextProps  
  const {action_rtgs_list543ba, setaction_rtgs_list543ba}= useContext(TotalContext) as TotalContextProps  
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "c7a357981ebb493eba732ca11d1e57bb",
      );
  ...
  const [SearchParams,setSearchParams] = useState<any>({})
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['file_name_rtgs_list'] = file_name_rtgs_list61a4b,
        codeStates['setfile_name_rtgs_list'] = setfile_name_rtgs_list61a4b,
        codeStates['action_rtgs_list'] = action_rtgs_list543ba,
        codeStates['setaction_rtgs_list'] = setaction_rtgs_list543ba,
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setrtgs_lst_doc_list_tablee57bbProps((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setrtgs_lst_doc_list_tablee57bbProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    let filterData :any[] =[];
    if(scanapproveprocessui_v1Props.length > 0){
      for(let i=0;i< scanapproveprocessui_v1Props.length;i++){
        if(scanapproveprocessui_v1Props[i].DFDkey == ""){
          // delete scanapproveprocessui_v1Props[i].DFDkey;
          let temp=structuredClone(scanapproveprocessui_v1Props[i])
          delete temp?.DFDkey
    ...
    try {
        ...
        if(itsFromRefreshHandler)
        {
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ufKey:'CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:scanApproveProcessUi:AFVK:v1:UO', 
                nodeId: 'c7a357981ebb493eba732ca11d1e57bb',
                elementId: 'c7a357981ebb493eba732ca11d1e57bb'
              },
            }
          }else
          {
      ...
      //   key: 'CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:scanApproveProcessUi:AFVK:v1',
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( rtgs_lst_doc_list_tablee57bbProps?.presetValues&&Object.keys(rtgs_lst_doc_list_tablee57bbProps?.presetValues).length > 0) {
          filtertedData = [rtgs_lst_doc_list_tablee57bbProps?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setrtgs_lst_doc_list_tablee57bb(api_pagination?.data?.records||[])
        }
...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    async function handleSecurity(controller: any = '') {
      ...
      }else if (controller in scanapproveprocessui_v1?.rtgs_lst_doc_list_table && scanapproveprocessui_v1?.rtgs_lst_doc_list_table[controller]?.itsHaveArtifact== true)
      {
        if(scanapproveprocessui_v1?.rtgs_lst_doc_list_table[controller])
        {
          let result :any = await getAftfactLevelRule(scanapproveprocessui_v1._artfactPFRule_,{...decodedTokenObj,session:decodedTokenObj,rtgs_lst_doc_list_table:filteredData},{rtgs_lst_doc_list_table:scanapproveprocessui_v1?.rtgs_lst_doc_list_table})
          if(result?.rtgs_lst_doc_list_table?.[controller]?.show==true)
            return true
          else
            return false
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(rtgs_lst_doc_list_tablee57bbProps.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [rtgs_lst_doc_list_tablee57bbProps.filterProps])
  ...
  useEffect(() => {
    if (rtgs_lst_doc_list_tablee57bbProps?.clearData === true) {
      setrtgs_lst_doc_list_tablee57bb([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setrtgs_lst_doc_list_tablee57bbProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [rtgs_lst_doc_list_tablee57bbProps?.clearData])
  useEffect(() => {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [rtgs_lst_doc_list_tablee57bbProps?.refresh])


  const handlePrimaryTable = () => {
    let findData = rtgs_lst_doc_list_tablee57bbProps?.selectedIds[rtgs_lst_doc_list_tablee57bbProps?.selectedIds?.length-1]
    if(Array.isArray(rtgs_lst_doc_list_tablee57bb) && rtgs_lst_doc_list_tablee57bb.length>0)
    {
      let data = rtgs_lst_doc_list_tablee57bb[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "",
        value: data[""],
  ...
  useEffect(() => {
    if (rtgs_lst_doc_list_tablee57bbProps?.selectedIds?.length != 0) handlePrimaryTable()
    if (rtgs_lst_doc_list_tablee57bbProps?.selectedIds?.length == 0){
      handleOnRowClick({},rtgs_lst_doc_list_tablee57bbProps?.selectedIds)
    }
  }, [rtgs_lst_doc_list_tablee57bbProps?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(rtgs_lst_doc_list_tablee57bb?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&rtgs_lst_doc_list_tablee57bb?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](rtgs_lst_doc_list_tablee57bb?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: rtgs_lst_doc_list_tablee57bb?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&rtgs_lst_doc_list_tablee57bb?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](rtgs_lst_doc_list_tablee57bb?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: rtgs_lst_doc_list_tablee57bb?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (rtgs_lst_doc_list_tablee57bb?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"c7a357981ebb493eba732ca11d1e57bb"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              selectedIds={rtgs_lst_doc_list_tablee57bbProps?.selectedIds}  
...
export default Tablertgs_lst_doc_list_table
