...
let defaultColumns:any = [
  {
    "id": "vld_code_rtgs_lst",
    "nodeid": "837ee2fc8ec24082a4a084e0364fc45a",
    "name": "Vld Code",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "vld_reason_rtgs_list",
    "nodeid": "66d2841fc5294392a11e990296bd8f6e",
    "name": "Vld Reason",
    "meta": {
      "sort": true
    },
...
const Tablertgs_list_validtn_table = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const {vld_code_rtgs_lstfc45a, setvld_code_rtgs_lstfc45a}= useContext(TotalContext) as TotalContextProps  
  const {vld_reason_rtgs_listd8f6e, setvld_reason_rtgs_listd8f6e}= useContext(TotalContext) as TotalContextProps  
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "6092fd816dd9472689ae9c2c5cf39a42",
      );
  ...
  const [SearchParams,setSearchParams] = useState<any>({})
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['vld_code_rtgs_lst'] = vld_code_rtgs_lstfc45a,
        codeStates['setvld_code_rtgs_lst'] = setvld_code_rtgs_lstfc45a,
        codeStates['vld_reason_rtgs_list'] = vld_reason_rtgs_listd8f6e,
        codeStates['setvld_reason_rtgs_list'] = setvld_reason_rtgs_listd8f6e,
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setrtgs_list_validtn_table39a42Props((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setrtgs_list_validtn_table39a42Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: '6092fd816dd9472689ae9c2c5cf39a42',
                elementId: '6092fd816dd9472689ae9c2c5cf39a42'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( rtgs_list_validtn_table39a42Props?.presetValues&&Object.keys(rtgs_list_validtn_table39a42Props?.presetValues).length > 0) {
          filtertedData = [rtgs_list_validtn_table39a42Props?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setrtgs_list_validtn_table39a42(api_pagination?.data?.records||[])
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(rtgs_list_validtn_table39a42Props.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [rtgs_list_validtn_table39a42Props.filterProps])
  ...
  useEffect(() => {
    if (rtgs_list_validtn_table39a42Props?.clearData === true) {
      setrtgs_list_validtn_table39a42([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setrtgs_list_validtn_table39a42Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [rtgs_list_validtn_table39a42Props?.clearData])
  useEffect(() => {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [rtgs_list_validtn_table39a42Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = rtgs_list_validtn_table39a42Props?.selectedIds[rtgs_list_validtn_table39a42Props?.selectedIds?.length-1]
    if(Array.isArray(rtgs_list_validtn_table39a42) && rtgs_list_validtn_table39a42.length>0)
    {
      let data = rtgs_list_validtn_table39a42[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "",
        value: data[""],
  ...
  useEffect(() => {
    if (rtgs_list_validtn_table39a42Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (rtgs_list_validtn_table39a42Props?.selectedIds?.length == 0){
      handleOnRowClick({},rtgs_list_validtn_table39a42Props?.selectedIds)
    }
  }, [rtgs_list_validtn_table39a42Props?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(rtgs_list_validtn_table39a42?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&rtgs_list_validtn_table39a42?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](rtgs_list_validtn_table39a42?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: rtgs_list_validtn_table39a42?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&rtgs_list_validtn_table39a42?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](rtgs_list_validtn_table39a42?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: rtgs_list_validtn_table39a42?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (rtgs_list_validtn_table39a42?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"6092fd816dd9472689ae9c2c5cf39a42"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              selectedIds={rtgs_list_validtn_table39a42Props?.selectedIds}  
...
export default Tablertgs_list_validtn_table
