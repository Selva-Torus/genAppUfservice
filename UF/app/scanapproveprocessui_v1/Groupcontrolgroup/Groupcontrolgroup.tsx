...
import Groupcontrol_tab_group  from "../Groupcontrol_tab_group/Groupcontrol_tab_group";
...
const Groupcontrolgroup = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "d724b8884dd74bdcb204739797eda197");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setcontrolgroupda197Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("control_tab_group")){
      setcontrol_tab_groupbc3e2({...control_tab_groupbc3e2,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const controlgroupda197Ref = useRef<any>(null);
  const handleClearSearch = () => {
    controlgroupda197Ref.current?.setSearchParams();
    controlgroupda197Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(controlgroupda197) && Object.keys(controlgroupda197)?.length>0)
      {
        setcontrolgroupda197({})
      }
    }else 
      prevRefreshRef.current= true
  }, [controlgroupda197Props?.refresh,token])
  ...
  return (
    <div 
      ...
      className={`flex flex-col overflow-auto rounded-md !rounded-2xl p-2 ${isDark ? 'text-white' : 'text-black'}`}
    >
        {allowedComponent.includes("control_tab_group")  &&<Groupcontrol_tab_group  
...
export default Groupcontrolgroup
