...
let defaultColumns:any = [
  {
    "id": "cmnts",
    "nodeid": "15d64e2af4994b8ba17e13eb7b011ffa",
    "name": "Comments",
    "meta": {
      "sort": true
    },
...
const TablecmntListTable = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const {cmnts11ffa, setcmnts11ffa}= useContext(TotalContext) as TotalContextProps  
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "bfcb81a3220f484494becea016e02d0e",
      );
  ...
  const [SearchParams,setSearchParams] = useState<any>({})
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['cmnts'] = cmnts11ffa,
        codeStates['setcmnts'] = setcmnts11ffa,
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setcmntlisttable02d0eProps((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setcmntlisttable02d0eProps((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    try {
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: 'bfcb81a3220f484494becea016e02d0e',
                elementId: 'bfcb81a3220f484494becea016e02d0e'
              },
            }
          }else
          {
      ...
      // CopyFromData (Parent table): use presetValues if present, else use pagination-filtered data
        ...
        if ( cmntlisttable02d0eProps?.presetValues&&Object.keys(cmntlisttable02d0eProps?.presetValues).length > 0) {
          filtertedData = [cmntlisttable02d0eProps?.presetValues];
        }else {
          filtertedData = structuredClone(uf_paginationDataFilter.data)||[]
          setcmntlisttable02d0e(api_pagination?.data?.records||[])
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(cmntlisttable02d0eProps.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [cmntlisttable02d0eProps.filterProps])
  ...
  useEffect(() => {
    if (cmntlisttable02d0eProps?.clearData === true) {
      setcmntlisttable02d0e([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setcmntlisttable02d0eProps((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [cmntlisttable02d0eProps?.clearData])
  useEffect(() => {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [cmntlisttable02d0eProps?.refresh])


  const handlePrimaryTable = () => {
    let findData = cmntlisttable02d0eProps?.selectedIds[cmntlisttable02d0eProps?.selectedIds?.length-1]
    if(Array.isArray(cmntlisttable02d0e) && cmntlisttable02d0e.length>0)
    {
      let data = cmntlisttable02d0e[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "",
        value: data[""],
  ...
  useEffect(() => {
    if (cmntlisttable02d0eProps?.selectedIds?.length != 0) handlePrimaryTable()
    if (cmntlisttable02d0eProps?.selectedIds?.length == 0){
      handleOnRowClick({},cmntlisttable02d0eProps?.selectedIds)
    }
  }, [cmntlisttable02d0eProps?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(cmntlisttable02d0e?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&cmntlisttable02d0e?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](cmntlisttable02d0e?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: cmntlisttable02d0e?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&cmntlisttable02d0e?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](cmntlisttable02d0e?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: cmntlisttable02d0e?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (cmntlisttable02d0e?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"bfcb81a3220f484494becea016e02d0e"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              selectedIds={cmntlisttable02d0eProps?.selectedIds}  
...
export default TablecmntListTable
