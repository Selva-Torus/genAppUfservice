...
import Groupcontrolgroup  from "../Groupcontrolgroup/Groupcontrolgroup";
...
const Groupoverallgroup = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "2f07f01a54904a4c93e25f47cda01c61");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setoverallgroup01c61Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("controlgroup")){
      setcontrolgroupda197({...controlgroupda197,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("documentviewer")){
      setdocumentviewer9df1d({...documentviewer9df1d,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  function handleConfirmOnLoad(){
    ...
    const handleOnload=()=>{
      ...
      setbutton_group74f3e((prev:any) => ({ ...prev, ...overallgroup01c61 }));
  }
  ...
  const overallgroup01c61Ref = useRef<any>(null);
  const handleClearSearch = () => {
    overallgroup01c61Ref.current?.setSearchParams();
    overallgroup01c61Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(overallgroup01c61) && Object.keys(overallgroup01c61)?.length>0)
      {
        setoverallgroup01c61({})
      }
    }else 
      prevRefreshRef.current= true
  }, [overallgroup01c61Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("controlgroup")  &&<Groupcontrolgroup  
        ...
        {allowedControls.includes("documentviewer") ?<DocumentViewerdocumentviewer   /* 9df1d */checkToAdd={checkToAdd} setCheckToAdd={setCheckToAdd} encryptionFlagCompData={encryptionFlagCompData} setIsProcessing={setIsProcessing} controlData={controlData} />: <div></div>}
    </div>
 )
}
