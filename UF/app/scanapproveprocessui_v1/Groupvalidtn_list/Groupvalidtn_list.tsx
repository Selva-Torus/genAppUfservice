...
import Grouprtgs_list_validtn_list_grp  from "../Grouprtgs_list_validtn_list_grp/Grouprtgs_list_validtn_list_grp";
...
const Groupvalidtn_list = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "988490a9d5094a4ca1b10336de3a5b14");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setvalidtn_lista5b14Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("rtgs_list_validtn_list_grp")){
      setrtgs_list_validtn_list_grpc5569({...rtgs_list_validtn_list_grpc5569,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const validtn_lista5b14Ref = useRef<any>(null);
  const handleClearSearch = () => {
    validtn_lista5b14Ref.current?.setSearchParams();
    validtn_lista5b14Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(validtn_lista5b14) && Object.keys(validtn_lista5b14)?.length>0)
      {
        setvalidtn_lista5b14({})
      }
    }else 
      prevRefreshRef.current= true
  }, [validtn_lista5b14Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("rtgs_list_validtn_list_grp")  &&<Grouprtgs_list_validtn_list_grp  
...
export default Groupvalidtn_list
