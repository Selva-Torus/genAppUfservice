...
import GroupdocListTable  from "../GroupdocListTable/GroupdocListTable";
...
const Groupdocument_list = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "cb79ff36b9894207b95d18cee0338c6e");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdocument_list38c6eProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("doclisttable")){
      setdoclisttable56e97({...doclisttable56e97,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const document_list38c6eRef = useRef<any>(null);
  const handleClearSearch = () => {
    document_list38c6eRef.current?.setSearchParams();
    document_list38c6eRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(document_list38c6e) && Object.keys(document_list38c6e)?.length>0)
      {
        setdocument_list38c6e({})
      }
    }else 
      prevRefreshRef.current= true
  }, [document_list38c6eProps?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("doclisttable")  &&<GroupdocListTable  
...
export default Groupdocument_list
