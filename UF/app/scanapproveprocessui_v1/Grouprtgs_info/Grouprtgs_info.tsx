...
import GroupallControls  from "../GroupallControls/GroupallControls";
...
const Grouprtgs_info = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "4adea4ad64844942a8ab9198a5ffd0aa");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrtgs_infofd0aaProps((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("allcontrols")){
      setallcontrols71c54({...allcontrols71c54,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const rtgs_infofd0aaRef = useRef<any>(null);
  const handleClearSearch = () => {
    rtgs_infofd0aaRef.current?.setSearchParams();
    rtgs_infofd0aaRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rtgs_infofd0aa) && Object.keys(rtgs_infofd0aa)?.length>0)
      {
        setrtgs_infofd0aa({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rtgs_infofd0aaProps?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("allcontrols")  &&<GroupallControls  
