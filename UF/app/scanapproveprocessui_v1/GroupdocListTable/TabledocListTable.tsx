...
let defaultColumns:any = [
  {
    "id": "fileName",
    "nodeid": "512e5f5a301a4512b7d6bf215587c104",
    "name": "File Name",
    "meta": {
      "sort": true
    },
...
const TabledocListTable = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setdoclisttable56e97Props((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setdoclisttable56e97Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(scanapproveprocessui_v1Props.length > 0){
      for(let i=0;i< scanapproveprocessui_v1Props.length;i++){
        if(scanapproveprocessui_v1Props[i].DFDkey == "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:documentListDfd:AFVK:v1"){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["vgphsts_uuid"]:doclisttable56e97Props?.presetValues?.vgphsts_uuid}
 
    let dstKey=dfKey?.dfKey
    dstKey=dstKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    try {
    if(!doclisttable56e97Props?.presetValues || Object.keys(doclisttable56e97Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: '0999fb419263477bb8b19cf211256e97',
                elementId: '0999fb419263477bb8b19cf211256e97'
              },
            }
          }else
          {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setdoclisttable56e97(api_pagination?.data?.records||[])
...
////////////////////////////////
  const RowAction = async({item,index,nodeName}: any) => {
    ...
    async function handleSecurity(controller: any = '') {
      ...
      }else if (controller in scanapproveprocessui_v1?.doclisttable && scanapproveprocessui_v1?.doclisttable[controller]?.itsHaveArtifact== true)
      {
        if(scanapproveprocessui_v1?.doclisttable[controller])
        {
          let result :any = await getAftfactLevelRule(scanapproveprocessui_v1._artfactPFRule_,{...decodedTokenObj,session:decodedTokenObj,doclisttable:filteredData},{doclisttable:scanapproveprocessui_v1?.doclisttable})
          if(result?.doclisttable?.[controller]?.show==true)
            return true
          else
            return false
        }
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(doclisttable56e97))
    {
      GetTableDetails()
    }
  }, [doclisttable56e97])
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(doclisttable56e97Props.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [doclisttable56e97Props.filterProps])
  ...
  useEffect(() => {
    if (doclisttable56e97Props?.clearData === true) {
      setdoclisttable56e97([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setdoclisttable56e97Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [doclisttable56e97Props?.clearData])
  useEffect(() => {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [doclisttable56e97Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = doclisttable56e97Props?.selectedIds[doclisttable56e97Props?.selectedIds?.length-1]
    if(Array.isArray(doclisttable56e97) && doclisttable56e97.length>0)
    {
      let data = doclisttable56e97[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "attachment_id",
  ...
  useEffect(() => {
    if (doclisttable56e97Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (doclisttable56e97Props?.selectedIds?.length == 0){
      handleOnRowClick({},doclisttable56e97Props?.selectedIds)
    }
  }, [doclisttable56e97Props?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(doclisttable56e97?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&doclisttable56e97?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](doclisttable56e97?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: doclisttable56e97?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&doclisttable56e97?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](doclisttable56e97?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: doclisttable56e97?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"0999fb419263477bb8b19cf211256e97"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              selectedIds={doclisttable56e97Props?.selectedIds}  
...
export default TabledocListTable
