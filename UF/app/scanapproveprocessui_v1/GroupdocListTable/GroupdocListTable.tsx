...
import TabledocListTable  from './TabledocListTable';  
...
const GroupdocListTable = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "0999fb419263477bb8b19cf211256e97");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setdoclisttable56e97Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("filename")){
      setfilename7c104({...filename7c104,isDisabled:true});
    }
    if(orchestrationData?.data?.readableControls.includes("action")){
      setactionf530a({...actionf530a,isDisabled:true});
    }
  //////////////
  }
  ...
  const doclisttable56e97Ref = useRef<any>(null);
  const handleClearSearch = () => {
    doclisttable56e97Ref.current?.setSearchParams();
    doclisttable56e97Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(doclisttable56e97) && Object.keys(doclisttable56e97)?.length>0)
      {
        setdoclisttable56e97({})
      }
    }else 
      prevRefreshRef.current= true
  }, [doclisttable56e97Props?.refresh,token])
  ...
  return (
    ...
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<TabledocListTable headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={doclisttable56e97Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
...
export default GroupdocListTable
