...
const TextInputdr_cust_ac_sanc_lmt = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:scanSaveProcessDfd:AFVK:v1|2e406af65f3a4e38bfad9e92c2647a4c|properties.product_basic.properties.dr_cust_ac_sanc_lmt"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:scanApproveProcessUi:AFVK:v1|409b134cde0449b5a031a7686df3d198|c433d4e33fac41a29a01830c87c955a9"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'dr_cust_ac_sanc_lmt',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,scanApproveProcessUi_v1:{...pre?.scanApproveProcessUi_v1,dr_cust_ac_sanc_lmt:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasicinfo3d198((prev: any) => ({ ...prev, dr_cust_ac_sanc_lmt: +e.target.value }));
    ...
    let code:string=allCode;
     if (code != '') {
      let codeStates: any = {};
        ...
        codeStates['dr_cust_ac_balance'] = dr_cust_ac_balance3be3f,
        codeStates['setdr_cust_ac_balance'] = setdr_cust_ac_balance3be3f,
        codeStates['dr_cust_ac_sanc_lmt'] = dr_cust_ac_sanc_lmt955a9,
        codeStates['setdr_cust_ac_sanc_lmt'] = setdr_cust_ac_sanc_lmt955a9,
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(basicinfo3d198?.dr_cust_ac_sanc_lmt == "" || basicinfo3d198?.dr_cust_ac_sanc_lmt == undefined){
      basicinfo3d198.dr_cust_ac_sanc_lmt = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, basicinfo3d198?.dr_cust_ac_sanc_lmt);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,scanApproveProcessUi_v1:{...pre?.scanApproveProcessUi_v1,dr_cust_ac_sanc_lmt:"invalid"}}));
        }
    }else if(basicinfo3d198?.dr_cust_ac_sanc_lmt !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +basicinfo3d198?.dr_cust_ac_sanc_lmt);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "c433d4e33fac41a29a01830c87c955a9"
      );
      ...
      //     controlId: "c433d4e33fac41a29a01830c87c955a9",
      //     isTable: false,
      //     from:"TextInputdr_cust_ac_sanc_lmt",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'dr_cust_ac_sanc_lmt', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'dr_cust_ac_sanc_lmt',type:'text'};
      //   type={
      //     name:'dr_cust_ac_sanc_lmt',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.dr_cust_ac_sanc_lmt.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.dr_cust_ac_sanc_lmt.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.dr_cust_ac_sanc_lmt.type
      //   }
      ...
      //   type={
      //     name:'dr_cust_ac_sanc_lmt',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.dr_cust_ac_sanc_lmt.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.dr_cust_ac_sanc_lmt.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.dr_cust_ac_sanc_lmt.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!basicinfo3d198?.dr_cust_ac_sanc_lmt)
      { 
  ...
  if(dfd_scansaveprocessdfd_v1Props?.setSearchFilters && dfd_scansaveprocessdfd_v1Props?.data)
  {
    if(Array.isArray(dfd_scansaveprocessdfd_v1Props.data) && dfd_scansaveprocessdfd_v1Props.data.length > 0){
      setbasicinfo3d198((pre:any)=>({...pre,dr_cust_ac_sanc_lmt:dfd_scansaveprocessdfd_v1Props.data[0]?.dr_cust_ac_sanc_lmt}));
    }
  }
  },[dfd_scansaveprocessdfd_v1Props?.setSearchFilters])
  if (dr_cust_ac_sanc_lmt955a9?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `7 / 13`,gridRow: `11 / 29`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      <div style={{ flex: 1, minHeight: 0 }}>
      <TextInput
        require={isRequredData}
        className="!rounded-xl"
        label={keyset("Dr Cust Ac Sanc Lmt")}
        onChange= {handleChange}
        onBlur={handleBlur}
        itsHaveCurrency={false}
        type={dynamicStateandType.type}
        value={basicinfo3d198?.dr_cust_ac_sanc_lmt||""}
        disabled={ true }
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Dr Cust Ac Sanc Lmt"
      errorMessage={error}
        validationState={validate?.scanApproveProcessUi_v1?.dr_cust_ac_sanc_lmt ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputdr_cust_ac_sanc_lmt
