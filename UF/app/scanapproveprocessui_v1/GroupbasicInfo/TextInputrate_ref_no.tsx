...
const TextInputrate_ref_no = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:scanSaveProcessDfd:AFVK:v1|2e406af65f3a4e38bfad9e92c2647a4c|properties.product_basic.properties.rate_ref_no"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:scanApproveProcessUi:AFVK:v1|409b134cde0449b5a031a7686df3d198|62b60509a8084b77bfa223a118a82399"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'rate_ref_no',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,scanApproveProcessUi_v1:{...pre?.scanApproveProcessUi_v1,rate_ref_no:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasicinfo3d198((prev: any) => ({ ...prev, rate_ref_no: +e.target.value }));
    }
    else{
    setbasicinfo3d198((prev: any) => ({ ...prev, rate_ref_no: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "62b60509a8084b77bfa223a118a82399"
      );
      ...
      //     controlId: "62b60509a8084b77bfa223a118a82399",
      //     isTable: false,
      //     from:"TextInputrate_ref_no",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'rate_ref_no', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'rate_ref_no',type:'text'};
      //   type={
      //     name:'rate_ref_no',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.rate_ref_no.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.rate_ref_no.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.rate_ref_no.type
      //   }
      ...
      //   type={
      //     name:'rate_ref_no',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.rate_ref_no.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.rate_ref_no.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.rate_ref_no.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  if(dfd_scansaveprocessdfd_v1Props?.setSearchFilters && dfd_scansaveprocessdfd_v1Props?.data)
  {
    if(Array.isArray(dfd_scansaveprocessdfd_v1Props.data) && dfd_scansaveprocessdfd_v1Props.data.length > 0){
      setbasicinfo3d198((pre:any)=>({...pre,rate_ref_no:dfd_scansaveprocessdfd_v1Props.data[0]?.rate_ref_no}));
    }
  }
  },[dfd_scansaveprocessdfd_v1Props?.setSearchFilters])
  if (rate_ref_no82399?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 7`,gridRow: `29 / 47`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      <div style={{ flex: 1, minHeight: 0 }}>
      <TextInput
        require={isRequredData}
        className="!rounded-xl"
        label={keyset("Rate Ref No")}
        onChange= {handleChange}
        onBlur={handleBlur}
        itsHaveCurrency={false}
        type={dynamicStateandType.type}
        value={basicinfo3d198?.rate_ref_no||""}
        disabled={ true }
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Rate Ref No"
      errorMessage={error}
        validationState={validate?.scanApproveProcessUi_v1?.rate_ref_no ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputrate_ref_no
