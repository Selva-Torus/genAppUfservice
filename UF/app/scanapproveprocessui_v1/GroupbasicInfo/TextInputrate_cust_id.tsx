...
const TextInputrate_cust_id = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:scanSaveProcessDfd:AFVK:v1|2e406af65f3a4e38bfad9e92c2647a4c|properties.product_basic.properties.rate_cust_id"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:scanApproveProcessUi:AFVK:v1|409b134cde0449b5a031a7686df3d198|b661bda49225438eb99e8b0118ead42a"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'rate_cust_id',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,scanApproveProcessUi_v1:{...pre?.scanApproveProcessUi_v1,rate_cust_id:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasicinfo3d198((prev: any) => ({ ...prev, rate_cust_id: +e.target.value }));
    }
    else{
    setbasicinfo3d198((prev: any) => ({ ...prev, rate_cust_id: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "b661bda49225438eb99e8b0118ead42a"
      );
      ...
      //     controlId: "b661bda49225438eb99e8b0118ead42a",
      //     isTable: false,
      //     from:"TextInputrate_cust_id",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'rate_cust_id', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'rate_cust_id',type:'text'};
      //   type={
      //     name:'rate_cust_id',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.rate_cust_id.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.rate_cust_id.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.rate_cust_id.type
      //   }
      ...
      //   type={
      //     name:'rate_cust_id',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.rate_cust_id.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.rate_cust_id.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.rate_cust_id.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  if(dfd_scansaveprocessdfd_v1Props?.setSearchFilters && dfd_scansaveprocessdfd_v1Props?.data)
  {
    if(Array.isArray(dfd_scansaveprocessdfd_v1Props.data) && dfd_scansaveprocessdfd_v1Props.data.length > 0){
      setbasicinfo3d198((pre:any)=>({...pre,rate_cust_id:dfd_scansaveprocessdfd_v1Props.data[0]?.rate_cust_id}));
    }
  }
  },[dfd_scansaveprocessdfd_v1Props?.setSearchFilters])
  if (rate_cust_idad42a?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `7 / 13`,gridRow: `29 / 47`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      <div style={{ flex: 1, minHeight: 0 }}>
      <TextInput
        require={isRequredData}
        className="!rounded-xl"
        label={keyset("Rate Cust Id")}
        onChange= {handleChange}
        onBlur={handleBlur}
        itsHaveCurrency={false}
        type={dynamicStateandType.type}
        value={basicinfo3d198?.rate_cust_id||""}
        disabled={ true }
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Rate Cust Id"
      errorMessage={error}
        validationState={validate?.scanApproveProcessUi_v1?.rate_cust_id ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputrate_cust_id
