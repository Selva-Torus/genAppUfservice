...
const TextInputcr_bank_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      ...
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:scanApproveProcessUi:AFVK:v1|409b134cde0449b5a031a7686df3d198|eca51347c40c453caca735963ee434eb"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'cr_bank_name',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,scanApproveProcessUi_v1:{...pre?.scanApproveProcessUi_v1,cr_bank_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setbasicinfo3d198((prev: any) => ({ ...prev, cr_bank_name: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(basicinfo3d198?.cr_bank_name == "" || basicinfo3d198?.cr_bank_name == undefined){
      basicinfo3d198.cr_bank_name = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, basicinfo3d198?.cr_bank_name);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,scanApproveProcessUi_v1:{...pre?.scanApproveProcessUi_v1,cr_bank_name:"invalid"}}));
        }
    }else if(basicinfo3d198?.cr_bank_name !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +basicinfo3d198?.cr_bank_name);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'cr_bank_name', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'cr_bank_name',type:'text'};
      //   type={
      //     name:'cr_bank_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.cr_bank_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.cr_bank_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.cr_bank_name.type
      //   }
      ...
      //   type={
      //     name:'cr_bank_name',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.cr_bank_name.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.cr_bank_name.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.cr_bank_name.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!basicinfo3d198?.cr_bank_name)
      { 
  ...
  if(dfd_crbankcodedropdowndfd_v1Props?.setSearchFilters && dfd_crbankcodedropdowndfd_v1Props?.data)
  {
    if(Array.isArray(dfd_crbankcodedropdowndfd_v1Props.data) && dfd_crbankcodedropdowndfd_v1Props.data.length > 0){
      setbasicinfo3d198((pre:any)=>({...pre,cr_bank_name:dfd_crbankcodedropdowndfd_v1Props.data[0]?.cr_bank_name}));
    }
  }
  ...
  return (   
    <div  
      ...
      <TextInput
        ...
        value={basicinfo3d198?.cr_bank_name||""}
        disabled={ true }
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.scanApproveProcessUi_v1?.cr_bank_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
