...
import Tablertgs_list_cmnts_list  from './Tablertgs_list_cmnts_list';  
...
const Grouprtgs_list_cmnts_list = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "53c3156dbeb24f39bc1fc71d25615716");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrtgs_list_cmnts_list15716Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cmnts_rtgs_list")){
      setcmnts_rtgs_list1952c({...cmnts_rtgs_list1952c,isDisabled:true});
    }
  //////////////
  }
  ...
  const rtgs_list_cmnts_list15716Ref = useRef<any>(null);
  const handleClearSearch = () => {
    rtgs_list_cmnts_list15716Ref.current?.setSearchParams();
    rtgs_list_cmnts_list15716Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rtgs_list_cmnts_list15716) && Object.keys(rtgs_list_cmnts_list15716)?.length>0)
      {
        setrtgs_list_cmnts_list15716({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rtgs_list_cmnts_list15716Props?.refresh,token])
  ...
  return (
    ...
    >
      <div className='flex flex-col h-full w-full min-w-0 overflow-auto'>
       ...
       {<Tablertgs_list_cmnts_list headerButtonsRenders={renderBUttons}
        tableData={tableData} setTableData={setTableData} lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={rtgs_list_cmnts_list15716Ref} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData} setIsProcessing={setIsProcessing} groupData={groupData} controlData={controlData}/>}
      </div>
      </div>
    </div>
 )
...
export default Grouprtgs_list_cmnts_list
