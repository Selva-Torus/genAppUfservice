...
const TextInputforex_amount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:scanSaveProcessDfd:AFVK:v1|2e406af65f3a4e38bfad9e92c2647a4c|properties.product_common.properties.cr_amount"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:scanApproveProcessUi:AFVK:v1|f2dd7aceaf454c72bdb1327c439f4607|9aa6d2edc1ce46309fc399ac39d0f335"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'cr_amount',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,scanApproveProcessUi_v1:{...pre?.scanApproveProcessUi_v1,cr_amount:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcommoninfof4607((prev: any) => ({ ...prev, cr_amount: +e.target.value }));
    }
    else{
    setcommoninfof4607((prev: any) => ({ ...prev, cr_amount: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "9aa6d2edc1ce46309fc399ac39d0f335"
      );
      ...
      //     controlId: "9aa6d2edc1ce46309fc399ac39d0f335",
      //     isTable: false,
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'cr_amount', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'cr_amount',type:'text'};
      //   type={
      //     name:'cr_amount',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.cr_amount.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.cr_amount.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.cr_amount.type
      //   }
      ...
      //   type={
      //     name:'cr_amount',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.cr_amount.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.cr_amount.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.cr_amount.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  if(dfd_scansaveprocessdfd_v1Props?.setSearchFilters && dfd_scansaveprocessdfd_v1Props?.data)
  {
    if(Array.isArray(dfd_scansaveprocessdfd_v1Props.data) && dfd_scansaveprocessdfd_v1Props.data.length > 0){
      setcommoninfof4607((pre:any)=>({...pre,cr_amount:dfd_scansaveprocessdfd_v1Props.data[0]?.cr_amount}));
    }
  }
  },[dfd_scansaveprocessdfd_v1Props?.setSearchFilters])
  if (forex_amount0f335?.isHidden) {
    return <></>
  }
  return (   
    <div  
      ...
      <TextInput
        ...
        value={commoninfof4607?.cr_amount||""}
        disabled={ true }
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.scanApproveProcessUi_v1?.cr_amount ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
