...
const TextInputbase_currency = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:scanSaveProcessDfd:AFVK:v1|2e406af65f3a4e38bfad9e92c2647a4c|properties.product_common.properties.dr_currency"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:scanApproveProcessUi:AFVK:v1|f2dd7aceaf454c72bdb1327c439f4607|9549a13b6e484a73ae4865515e4b386d"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'dr_currency',type:"text"})
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,scanApproveProcessUi_v1:{...pre?.scanApproveProcessUi_v1,dr_currency:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcommoninfof4607((prev: any) => ({ ...prev, dr_currency: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(commoninfof4607?.dr_currency == "" || commoninfof4607?.dr_currency == undefined){
      commoninfof4607.dr_currency = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, commoninfof4607?.dr_currency);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,scanApproveProcessUi_v1:{...pre?.scanApproveProcessUi_v1,dr_currency:"invalid"}}));
        }
    }else if(commoninfof4607?.dr_currency !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +commoninfof4607?.dr_currency);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "9549a13b6e484a73ae4865515e4b386d"
      );
      ...
      //     controlId: "9549a13b6e484a73ae4865515e4b386d",
      //     isTable: false,
      //     from:"TextInputbase_currency",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'dr_currency', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'dr_currency',type:'text'};
      //   type={
      //     name:'dr_currency',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.dr_currency.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.dr_currency.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.dr_currency.type
      //   }
      ...
      //   type={
      //     name:'dr_currency',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.dr_currency.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.dr_currency.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.dr_currency.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!commoninfof4607?.dr_currency)
      { 
  ...
  if(dfd_scansaveprocessdfd_v1Props?.setSearchFilters && dfd_scansaveprocessdfd_v1Props?.data)
  {
    if(Array.isArray(dfd_scansaveprocessdfd_v1Props.data) && dfd_scansaveprocessdfd_v1Props.data.length > 0){
      setcommoninfof4607((pre:any)=>({...pre,dr_currency:dfd_scansaveprocessdfd_v1Props.data[0]?.dr_currency}));
    }
  }
  },[dfd_scansaveprocessdfd_v1Props?.setSearchFilters])
  if (base_currencyb386d?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `13 / 19`,gridRow: `10 / 28`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      <div style={{ flex: 1, minHeight: 0 }}>
      <TextInput
        require={isRequredData}
        className="!rounded-xl"
        label={keyset("Base Currency")}
        onChange= {handleChange}
        onBlur={handleBlur}
        itsHaveCurrency={false}
        type={dynamicStateandType.type}
        value={commoninfof4607?.dr_currency||""}
        disabled={ true }
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
      ...
      errorMessage={error}
        validationState={validate?.scanApproveProcessUi_v1?.dr_currency ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputbase_currency
