...
const TextInputremittance_info = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:scanSaveProcessDfd:AFVK:v1|2e406af65f3a4e38bfad9e92c2647a4c|properties.product_common.properties.remittance_info"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:scanApproveProcessUi:AFVK:v1|f2dd7aceaf454c72bdb1327c439f4607|065a092daea64bb18d5c16b208564004"
    }
  ],
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'remittance_info',type:"text"})
  ...
  "v.regex(/^[A-Za-z\\s]+$/, 'Only alphabets and spaces are allowed.')"
] ;
  ...
  const handleChange = async(e: any) => {
    if(e.target.value=="")
    {
      ...
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,scanApproveProcessUi_v1:{...pre?.scanApproveProcessUi_v1,remittance_info:undefined}}));
    if(dynamicStateandType.type=="number"){
    setcommoninfof4607((prev: any) => ({ ...prev, remittance_info: +e.target.value }));
  ...
  const handleValidate=async (e?:any) => {
      let validate:any
      if(commoninfof4607?.remittance_info == "" || commoninfof4607?.remittance_info == undefined){
      commoninfof4607.remittance_info = "";
     if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, NaN);
        }
        else{
          validate = v.safeParse(schema, commoninfof4607?.remittance_info);
        }
        if(!validate.success){
          setError(validate?.issues[0]?.message);
          setValidate((pre:any)=>({...pre,scanApproveProcessUi_v1:{...pre?.scanApproveProcessUi_v1,remittance_info:"invalid"}}));
        }
    }else if(commoninfof4607?.remittance_info !== ""){
   if(dynamicStateandType.type=="number"){
          validate = v.safeParse(schema, +commoninfof4607?.remittance_info);
        }
        else{
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "065a092daea64bb18d5c16b208564004"
      );
      ...
      //     controlId: "065a092daea64bb18d5c16b208564004",
      //     isTable: false,
      //     from:"TextInputremittance_info",
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'remittance_info', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'remittance_info',type:'text'};
      //   type={
      //     name:'remittance_info',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.remittance_info.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.remittance_info.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.remittance_info.type
      //   }
      ...
      //   type={
      //     name:'remittance_info',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.remittance_info.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.remittance_info.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.remittance_info.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  useEffect(()=>{
      handleMapperValue();
      if(!commoninfof4607?.remittance_info)
      { 
  ...
  if(dfd_scansaveprocessdfd_v1Props?.setSearchFilters && dfd_scansaveprocessdfd_v1Props?.data)
  {
    if(Array.isArray(dfd_scansaveprocessdfd_v1Props.data) && dfd_scansaveprocessdfd_v1Props.data.length > 0){
      setcommoninfof4607((pre:any)=>({...pre,remittance_info:dfd_scansaveprocessdfd_v1Props.data[0]?.remittance_info}));
    }
  }
  },[dfd_scansaveprocessdfd_v1Props?.setSearchFilters])
  if (remittance_info64004?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `7 / 13`,gridRow: `48 / 66`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      <div style={{ flex: 1, minHeight: 0 }}>
      <TextInput
        require={isRequredData}
        className="!rounded-xl"
        label={keyset("Remittance Information")}
        onChange= {handleChange}
        onBlur={handleBlur}
        itsHaveCurrency={false}
        type={dynamicStateandType.type}
        value={commoninfof4607?.remittance_info||""}
        disabled={ true }
        pin='brick-brick'     
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Remittance Information"
      errorMessage={error}
        validationState={validate?.scanApproveProcessUi_v1?.remittance_info ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputremittance_info
