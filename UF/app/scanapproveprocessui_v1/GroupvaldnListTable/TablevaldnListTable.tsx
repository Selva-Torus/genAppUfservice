...
let defaultColumns:any = [
  {
    "id": "vldCode",
    "nodeid": "c200c748b79c40a4a2b1a69390c0c0ce",
    "name": "Vld Code",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "vldReason",
    "nodeid": "3900d3290ef242ecb243f68c9a72ef16",
    "name": "Vld Reason",
    "meta": {
      "sort": true
    },
...
const TablevaldnListTable = ({ headerButtonsRenders=()=>{return<></>},headerPosition="",headerText="",lockedData,setLockedData,tableData, setTableData,primaryTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData,setIsProcessing,groupData,controlData}: any)=>{
 ...
 /////////////
  ...
  const {vldcode0c0ce, setvldcode0c0ce}= useContext(TotalContext) as TotalContextProps  
  const {vldreason2ef16, setvldreason2ef16}= useContext(TotalContext) as TotalContextProps  
  ...
  const GetTableDetails = async () => {
    const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "6b332e1a71dd48d3a5b440a432017ec7",
      );
  ...
  const [SearchParams,setSearchParams] = useState<any>({})
    ...
    if (allCode != '') {
      let codeStates: any = {};
        ...
        codeStates['vldcode'] = vldcode0c0ce,
        codeStates['setvldcode'] = setvldcode0c0ce,
        codeStates['vldreason'] = vldreason2ef16,
        codeStates['setvldreason'] = setvldreason2ef16,
  ...
  const handleUpdate = (page:any, pageSize:any) =>{
    ...
    setvaldnlisttable17ec7Props((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setvaldnlisttable17ec7Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any,itsFromRefreshHandler:any=false){
    ...
    if(scanapproveprocessui_v1Props.length > 0){
      for(let i=0;i< scanapproveprocessui_v1Props.length;i++){
        if(scanapproveprocessui_v1Props[i].DFDkey == "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:errorListDfd:AFVK:v1"){
    ...
    if(searchFilterFlag===true){
      ...
      searchParams={...searchParams,["vgphsts_id"]:valdnlisttable17ec7Props?.presetValues?.vgphsts_id}
 
    let dstKey=dfKey?.dfKey
    dstKey=dstKey?.replace(":AFC:",":AFCP:").replace(":AF:",":AFP:").replace(":DF-DFD:",":DF-DST:");
    try {
    if(!valdnlisttable17ec7Props?.presetValues || Object.keys(valdnlisttable17ec7Props?.presetValues).length === 0){
      setAllDataObject([])
      return
    }
          ...
          if(DFkeyAndRule?.isRulePresent==true)
          {
            api_paginationBody = {
              ...
              filterDetails: {
                ...
                nodeId: '6b332e1a71dd48d3a5b440a432017ec7',
                elementId: '6b332e1a71dd48d3a5b440a432017ec7'
              },
            }
          }else
          {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        let filtertedData:any;
          ...
          setvaldnlisttable17ec7(api_pagination?.data?.records||[])
...
const colurIndicator = (keyValue:any=[], comingValue:any,ColourIndicatorType:any) => {
  ...
  useEffect(() => {
    if(!Array.isArray(valdnlisttable17ec7))
    {
      GetTableDetails()
    }
  }, [valdnlisttable17ec7])
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(valdnlisttable17ec7Props.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [valdnlisttable17ec7Props.filterProps])
  ...
  useEffect(() => {
    if (valdnlisttable17ec7Props?.clearData === true) {
      setvaldnlisttable17ec7([]);
      setAllDataObject([]);
      setAllData([]);
      setTableData([]);
      ...
      setvaldnlisttable17ec7Props((pre:any) => ({...pre, clearData: false, selectedIds: []}));
      setPaginationData((pre:any) => ({...pre, total: 0}));
    }
  }, [valdnlisttable17ec7Props?.clearData])
  useEffect(() => {
    ...
    setSelectedPaginationData([])
    setAllDataObject([])
  }, [valdnlisttable17ec7Props?.refresh])


  const handlePrimaryTable = () => {
    let findData = valdnlisttable17ec7Props?.selectedIds[valdnlisttable17ec7Props?.selectedIds?.length-1]
    if(Array.isArray(valdnlisttable17ec7) && valdnlisttable17ec7.length>0)
    {
      let data = valdnlisttable17ec7[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "vgphtelm_id",
  ...
  useEffect(() => {
    if (valdnlisttable17ec7Props?.selectedIds?.length != 0) handlePrimaryTable()
    if (valdnlisttable17ec7Props?.selectedIds?.length == 0){
      handleOnRowClick({},valdnlisttable17ec7Props?.selectedIds)
    }
  }, [valdnlisttable17ec7Props?.selectedIds])
  ...
  const bindPreviousAndNext=(currectIndex:any,forEvent:any,setData:string,parentTrigger:any="",currectPage:any)=>{ 
    ...
    {
      currectIndex=-1
      if(valdnlisttable17ec7?.length==0)
      {
        eventBus.emit(parentTrigger, { message: "no data" });
        return
      }
      if(forEvent=="next"&&valdnlisttable17ec7?.at((+currectIndex)+1))
      {
        const newIndex = (+currectIndex)+1;
        allState[setData](valdnlisttable17ec7?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: valdnlisttable17ec7?.at(newIndex) ,currectPage:paginationData?.page});
        return newIndex
      }
      if(forEvent=="previous"&&((+currectIndex)-1)>=0&&valdnlisttable17ec7?.at((+currectIndex)-1))
      {
        const newIndex = (+currectIndex)-1;
        allState[setData](valdnlisttable17ec7?.at(newIndex))
        eventBus.emit(parentTrigger, { index: newIndex, data: valdnlisttable17ec7?.at(newIndex) ,currectPage:paginationData?.page});
  ...
  if (valdnlisttable17ec7?.isHidden) {
    return <></>
  }
  return(
    <div className='w-full h-full'>
          <div
            ...
            id={"6b332e1a71dd48d3a5b440a432017ec7"}  onClick={(e:any) => {
          ...
          >
            <Table
              ...
              edgePadding={true}
              selectedIds={valdnlisttable17ec7Props?.selectedIds}  
...
export default TablevaldnListTable
