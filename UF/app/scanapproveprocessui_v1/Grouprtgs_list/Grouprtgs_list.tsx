...
import Grouprtgs_list_grp  from "../Grouprtgs_list_grp/Grouprtgs_list_grp";
...
const Grouprtgs_list = ({lockedData={},setLockedData,primaryTableData={},tableData=[],setTableData, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData: groupDataProp={}, controlData: controlDataProp={}}:any)=> {
 ...
 /////////////
  ...
  let orchestrationData:any = getGroupOrchestrationData(currentGroupData, "68003189e60c4bd196ad08795d5a0a19");
  ...
  if(orchestrationData?.data?.rule?.nodes?.length > 0){
    ...
    setrtgs_lista0a19Props((pre:any)=>({...pre,isHaveRule:true}))
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("rtgs_list_grp")){
      setrtgs_list_grpcf7d8({...rtgs_list_grpcf7d8,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const rtgs_lista0a19Ref = useRef<any>(null);
  const handleClearSearch = () => {
    rtgs_lista0a19Ref.current?.setSearchParams();
    rtgs_lista0a19Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(rtgs_lista0a19) && Object.keys(rtgs_lista0a19)?.length>0)
      {
        setrtgs_lista0a19({})
      }
    }else 
      prevRefreshRef.current= true
  }, [rtgs_lista0a19Props?.refresh,token])
  ...
  return (
    ...
    >
        {allowedComponent.includes("rtgs_list_grp")  &&<Grouprtgs_list_grp  
