...
const Groupcontrol_tab_group = ({lockedData={},setLockedData,primaryTableData={},tableData=[], setTableData ,setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false,setIsProcessing, groupData={}, controlData={}}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = getGroupOrchestrationData(
        groupData,
        "9aad8dd647f145e9bb8eeba7082bc3e2"
      );
  ...
  for(let i=0;i<tabOptions?.length;i++){
    ...
    {
      setcontrol_tab_groupbc3e2((pre:any)=>({...pre,control_tab_group:tabOptions[i]?.id}));
      break;
    }
  }   
  /////////////
        setbutton_group74f3e({...button_group74f3e,isDisabled:orchestrationData?.data?.readableControls.includes("button_group")});
        setrtgs_infofd0aa({...rtgs_infofd0aa,isDisabled:orchestrationData?.data?.readableControls.includes("rtgs_info")});
        setrtgs_lista0a19({...rtgs_lista0a19,isDisabled:orchestrationData?.data?.readableControls.includes("rtgs_list")});
  //////////////
    if (code != '') {
      let codeStates: any = {};
    ...
    const handleOnload=()=>{
     setcontrol_tab_groupbc3e2((pre:any)=>({...pre,control_tab_group:"rtgs_info"}));
  }
  const handleOnChange=async(id?:string)=>{
    ...
    setcontrol_tab_groupbc3e2((pre:any)=>({...pre,control_tab_group:id}));

  }
  const control_tab_groupbc3e2Ref = useRef<any>(null);
  const handleClearSearch = () => {
    control_tab_groupbc3e2Ref.current?.setSearchParams();
    control_tab_groupbc3e2Ref.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(control_tab_groupbc3e2) && Object.keys(control_tab_groupbc3e2)?.length>0)
      {
        setcontrol_tab_groupbc3e2({})
      }
    }else 
      prevRefreshRef.current= true
  }, [control_tab_groupbc3e2Props?.refresh])
...
let tabHeaderItems : any =[
  ...
  return (
    ...
    <Tabs
      ...
      defaultActiveId={control_tab_groupbc3e2?.control_tab_group || "rtgs_info"}
      activeTab={control_tab_groupbc3e2?.control_tab_group || "rtgs_info"}
