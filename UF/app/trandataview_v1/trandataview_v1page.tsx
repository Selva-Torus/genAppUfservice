...
export default function PageTrandataviewV1() {
  ...
  const [checktran_data_group,setChecktran_data_group,]=useState<boolean>(false);
  const [checkreq_data_group,setCheckreq_data_group,]=useState<boolean>(false);
  const [checkres_data_group,setCheckres_data_group,]=useState<boolean>(false);
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    ...
    async function journey_v1DFD(pagination:any): Promise<void>{
        ...
        if(trandataview_v1Props.length > 0){
          for(let i=0;i< trandataview_v1Props.length;i++){
            if(trandataview_v1Props[i].DFDkey == "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:journey:AFVK:v1"){
              // delete trandataview_v1Props[i].DFDkey;
              let temp=structuredClone(trandataview_v1Props[i])
              delete temp?.DFDkey
        ...
        if(encryptionFlagPage){
           introspect = await AxiosService.get("/UF/introspect",{
            ...
            params: {
              ...
              method: encryptionMethod,
              key:"CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:tranDataView:AFVK:v1"
            }
          }) 
        }else{
          introspect = await AxiosService.get("/UF/introspect",{
  ...
  "events": {}
    ...
    await journey_v1DFD(pagination)
          ...
          allowedGroup.map((nodes:AllowedGroupNode)=>{
            if(nodes?.groupName == 'tran_data_group' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setChecktran_data_group(true)
            }
            if(nodes?.groupName == 'req_data_group' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setCheckreq_data_group(true)
            }
            if(nodes?.groupName == 'res_data_group' && (nodes?.security== 'AA' || nodes?.security == 'ATO' || nodes?.security == 'RA'))
            {
              setCheckres_data_group(true)
            }
          })
          }
           }catch(err:any)
  ...
  useEffect(() => {    
    ...
    securityCheck();
    handleOnload();
    settrandataview_v1(allRuleData)
  }, [])

  useEffect(()=>{
    if(trandataview_v1?._artfactPFRule_)
    {
      let data:any ={
        ...decodedTokenObj,
    ...
    <>
     ...
     style={{
      ...
      {isProcessing && (
        ...
        {checktran_data_group && initialLoad &&<Grouptran_data_group
