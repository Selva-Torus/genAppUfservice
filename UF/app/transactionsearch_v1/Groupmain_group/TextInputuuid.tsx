...
const TextInputuuid = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:VGPH:AFK:transaction:AFVK:v1|c2a320f6a30140a487ed20c46f1763dd|properties.parent_vgphstm_uuid"
      ],
  ...
  "schemaData": {
    ...
    "x-expression": "parent_vgphstm_uuid"
  },
  "dataType": "string"
}
  ...
  const [dynamicStateandType,setDynamicStateandType]=useState<Record<string, any>>({name:'parent_vgphstm_uuid',type:"text"})
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,transactionSearch_v1:{...pre?.transactionSearch_v1,parent_vgphstm_uuid:undefined}}));
    if(dynamicStateandType.type=="number"){
    setmain_group9066f((prev: any) => ({ ...prev, parent_vgphstm_uuid: +e.target.value }));
    }
    else{
    setmain_group9066f((prev: any) => ({ ...prev, parent_vgphstm_uuid: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      ...
      if (orchestrationData?.data?.dataType ==='integer' || orchestrationData?.data?.dataType ==='number') {
        setDynamicStateandType({name:'parent_vgphstm_uuid', type: 'number'});
      }
      // if(orchestrationData?.data?.schemaData?.at(0)?.nodeType=='apinode'){
      // if(orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties){
      //   let type:any={name:'parent_vgphstm_uuid',type:'text'};
      //   type={
      //     name:'parent_vgphstm_uuid',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.parent_vgphstm_uuid.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.parent_vgphstm_uuid.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.responses["200"].content["application/json"].schema.items.properties.parent_vgphstm_uuid.type
      //   }
      ...
      //   type={
      //     name:'parent_vgphstm_uuid',
      //     type: orchestrationData?.data?.schemaData?.at(0)?.schema.properties.parent_vgphstm_uuid.type == 'string' ? 'text' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.parent_vgphstm_uuid.type =='integer' ? 'number' : orchestrationData?.data?.schemaData?.at(0)?.schema.properties.parent_vgphstm_uuid.type
      //   }
      //   setDynamicStateandType(type);
      // }
      // }
  ...
  if(dfd_transaction_v1Props?.setSearchFilters && dfd_transaction_v1Props?.data)
  {
    if(Array.isArray(dfd_transaction_v1Props.data) && dfd_transaction_v1Props.data.length > 0){
      setmain_group9066f((pre:any)=>({...pre,parent_vgphstm_uuid:dfd_transaction_v1Props.data[0]?.parent_vgphstm_uuid}));
    }
  }
  ...
  return (   
    <div  
      style={{gridColumn: `13 / 19`,gridRow: `29 / 47`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={main_group9066f?.parent_vgphstm_uuid||""}
        ...
        headerText="UUID"
      errorMessage={error}
        validationState={validate?.transactionSearch_v1?.parent_vgphstm_uuid ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
