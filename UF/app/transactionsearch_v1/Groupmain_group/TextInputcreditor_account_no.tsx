...
const TextInputcreditor_account_no = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:transaction:AFVK:v1|c2a320f6a30140a487ed20c46f1763dd|properties.cr_account"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:transactionSearch:AFVK:v1|526f0e58d5454270aca67c481a99066f|33aedd45e57047308cb6e699151ca692"
    }
  ],
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,transactionSearch_v1:{...pre?.transactionSearch_v1,cr_account:undefined}}));
    if(dynamicStateandType.type=="number"){
    setmain_group9066f((prev: any) => ({ ...prev, cr_account: +e.target.value }));
    }
    else{
    setmain_group9066f((prev: any) => ({ ...prev, cr_account: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "33aedd45e57047308cb6e699151ca692"
      );
      ...
      //     controlId: "33aedd45e57047308cb6e699151ca692",
      //     isTable: false,
      //     from:"TextInputcreditor_account_no",
  ...
  if(dfd_transaction_v1Props?.setSearchFilters && dfd_transaction_v1Props?.data)
  {
    if(Array.isArray(dfd_transaction_v1Props.data) && dfd_transaction_v1Props.data.length > 0){
      setmain_group9066f((pre:any)=>({...pre,cr_account:dfd_transaction_v1Props.data[0]?.cr_account}));
    }
  }
  },[dfd_transaction_v1Props?.setSearchFilters])
  if (creditor_account_noca692?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `1 / 9`,gridRow: `34 / 52`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={main_group9066f?.cr_account||""}
         disabled= {creditor_account_noca692?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Creditor Account No"
      errorMessage={error}
        validationState={validate?.transactionSearch_v1?.cr_account ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputcreditor_account_no
