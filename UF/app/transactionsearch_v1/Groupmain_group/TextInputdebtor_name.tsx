...
const TextInputdebtor_name = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:transaction:AFVK:v1|c2a320f6a30140a487ed20c46f1763dd|properties.dr_name"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:transactionSearch:AFVK:v1|526f0e58d5454270aca67c481a99066f|02ea918896784a32a595a828d70e2d9f"
    }
  ],
  "dfdKey": "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:transaction:AFVK:v1:",
  "schemaData": {
    "type": "string",
    "x-expression": "dr_name"
  },
  "dataType": "string"
}
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,transactionSearch_v1:{...pre?.transactionSearch_v1,dr_name:undefined}}));
    if(dynamicStateandType.type=="number"){
    setmain_group9066f((prev: any) => ({ ...prev, dr_name: +e.target.value }));
    }
    else{
    setmain_group9066f((prev: any) => ({ ...prev, dr_name: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "02ea918896784a32a595a828d70e2d9f"
      );
      ...
      //     controlId: "02ea918896784a32a595a828d70e2d9f",
      //     isTable: false,
  ...
  if(dfd_transaction_v1Props?.setSearchFilters && dfd_transaction_v1Props?.data)
  {
    if(Array.isArray(dfd_transaction_v1Props.data) && dfd_transaction_v1Props.data.length > 0){
      setmain_group9066f((pre:any)=>({...pre,dr_name:dfd_transaction_v1Props.data[0]?.dr_name}));
    }
  }
  },[dfd_transaction_v1Props?.setSearchFilters])
  if (debtor_namee2d9f?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `17 / 25`,gridRow: `15 / 33`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={main_group9066f?.dr_name||""}
         disabled= {debtor_namee2d9f?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        view='normal'
        contentAlign={"left"}
        headerPosition='top'
        headerText="Debtor Name"
      errorMessage={error}
        validationState={validate?.transactionSearch_v1?.dr_name ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
