...
const TextInputpayment_amount = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData,setIsProcessing,controlData}:any) => {  
  ...
  "mapper": [
    {
      "sourceKey": [
        "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:transaction:AFVK:v1|c2a320f6a30140a487ed20c46f1763dd|properties.dr_amount"
      ],
      "targetKey": "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:transactionSearch:AFVK:v1|526f0e58d5454270aca67c481a99066f|9a3236945d3940fa946c124e7c7042b1"
    }
  ],
  "dfdKey": "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:GSS:AFGK:RTGS:AFK:transaction:AFVK:v1:",
  "schemaData": {
    "type": "number",
  ...
  const handleChange = async(e: any) => {
      let validate:any;    
      setError('');
      setValidate((pre:any)=>({...pre,transactionSearch_v1:{...pre?.transactionSearch_v1,dr_amount:undefined}}));
    if(dynamicStateandType.type=="number"){
    setmain_group9066f((prev: any) => ({ ...prev, dr_amount: +e.target.value }));
    }
    else{
    setmain_group9066f((prev: any) => ({ ...prev, dr_amount: e.target.value }));
    }
  ...
  const handleMapperValue=async()=>{
    try{
      const orchestrationData:any = getControlOrchestrationData(
        ...
        "9a3236945d3940fa946c124e7c7042b1"
      );
      // const orchestrationData: any = await AxiosService.post(
      //   '/UF/Orchestration',
      //   {
      //     key: "CK:CT005:FNGK:AF:FNK:UF-UFW:CATK:GSS:AFGK:RTGS:AFK:transactionSearch:AFVK:v1",
      //     componentId: "526f0e58d5454270aca67c481a99066f",
      //     controlId: "9a3236945d3940fa946c124e7c7042b1",
      //     isTable: false,
      //     from:"TextInputpayment_amount",
  ...
  if(dfd_transaction_v1Props?.setSearchFilters && dfd_transaction_v1Props?.data)
  {
    if(Array.isArray(dfd_transaction_v1Props.data) && dfd_transaction_v1Props.data.length > 0){
      setmain_group9066f((pre:any)=>({...pre,dr_amount:dfd_transaction_v1Props.data[0]?.dr_amount}));
    }
  }
  },[dfd_transaction_v1Props?.setSearchFilters])
  if (payment_amount042b1?.isHidden) {
    return <></>
  }
  return (   
    <div  
      style={{gridColumn: `17 / 25`,gridRow: `34 / 52`, gap:``, height: `100%`, overflow: 'auto', display: 'flex', flexDirection: 'column'}} >
      ...
      <TextInput
        ...
        value={main_group9066f?.dr_amount||""}
         disabled= {payment_amount042b1?.isDisabled ? true : false}
        pin='brick-brick'     
      ...
      errorMessage={error}
        validationState={validate?.transactionSearch_v1?.dr_amount ? "invalid" : undefined}
      />
      </div>
    </div> 
  )
...
export default TextInputpayment_amount
