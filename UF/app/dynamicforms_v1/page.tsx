...
import Groupgroupfordynamicbutton  from "./Groupgroupfordynamicbutton/Groupgroupfordynamicbutton";


export default function PageDynamicformsV1() {
  ...
  const screenName:string = "dynamicforms";
  ...
  const [checkgroupfordynamicbutton,setCheckgroupfordynamicbutton,]=useState(false);
  ...
  const [paginationData,setPaginationData]=useState<any>({count:10,page:1})
    ...
    async function mydfddata_v1(pagination:any){
        ...
        if(dynamicforms_v1Props.length > 0){
          let filterData :any[] =[];
          for(let i=0;i< dynamicforms_v1Props.length;i++){
            if(dynamicforms_v1Props[i].DFDkey == "CK:CT309:FNGK:AF:FNK:DF-DFD:CATK:AG001:AFGK:A001:AFK:myDfdData:AFVK:v1"){
              delete dynamicforms_v1Props[i].DFDkey;
              filterData.push(dynamicforms_v1Props[i])
            }           
          }
  ...
  async function securityCheck() {
    const orchestrationData = await AxiosService.post("/UF/Orchestration",{key:"CK:CT309:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:dynamicforms:AFVK:v1",accessProfile:[user],from:"pageDynamicformsV1"},{
      headers: {
        Authorization: `Bearer ${token}`
      }});
  ...
  "events": {}
    ...
    await mydfddata_v1(pagination)
          ...
          allowedGroup.map((nodes:any)=>{
            if(nodes?.groupName == 'parent' && (nodes?.security== 'AA' || nodes?.security == 'ATO'))
            {
              setCheckparent(true)
            }
            ...
            if(nodes?.groupName == 'groupfordynamicbutton' && (nodes?.security== 'AA' || nodes?.security == 'ATO'))
            {
              setCheckgroupfordynamicbutton(true)
            }
            if(nodes?.groupName == 'buttons' && (nodes?.security== 'AA' || nodes?.security == 'ATO'))
            {
    ...
    <>
     ...
     style={{
       ...
       // minHeight: '100vh',
        ...
        {checkparent && initialLoad &&<Groupparent  
        ...
        {checkgroupfordynamicbutton && initialLoad &&<Groupgroupfordynamicbutton  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          primaryTableData={primaryTableData}
          setPrimaryTableData={setPrimaryTableData}
