...
import Grouptablegroup  from "./Grouptablegroup/Grouptablegroup";


export default function PageTablecheckV1() {
  ...
  const {tablecheck_v1Props, settablecheck_v1Props} = useContext(TotalContext) as TotalContextProps;
  const [checkusertable,setCheckusertable,]=useState(false);
  const [checktablegroup,setChecktablegroup,]=useState(false);
  const [checktexttable,setChecktexttable,]=useState(false);
  ...
  const [paginationData,setPaginationData]=useState<any>({count:10,page:1})
    ...
    async function mydfddata_v1(pagination:any){
        ...
        if(tablecheck_v1Props.length > 0){
          let filterData :any[] =[];
          for(let i=0;i< tablecheck_v1Props.length;i++){
            if(tablecheck_v1Props[i].DFDkey == "CK:CT309:FNGK:AF:FNK:DF-DFD:CATK:AG001:AFGK:A001:AFK:myDfdData:AFVK:v1"){
              delete tablecheck_v1Props[i].DFDkey;
              filterData.push(tablecheck_v1Props[i])
            }           
          }
  ...
  async function securityCheck() {
    const orchestrationData = await AxiosService.post("/UF/Orchestration",{key:"CK:CT309:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:tablecheck:AFVK:v1",accessProfile:[user],from:"pageTablecheckV1"},{
      headers: {
        Authorization: `Bearer ${token}`
      }});
  ...
  "events": {}
    ...
    await mydfddata_v1(pagination)
          ...
          allowedGroup.map((nodes:any)=>{
            ...
            if(nodes?.groupName == 'tablegroup' && (nodes?.security== 'AA' || nodes?.security == 'ATO'))
            {
              setChecktablegroup(true)
            }
            if(nodes?.groupName == 'texttable' && (nodes?.security== 'AA' || nodes?.security == 'ATO'))
            {
              setChecktexttable(true)
            }
          })
          }
           }catch(err:any)
    ...
    <>
     ...
     style={{
       ...
       // minHeight: '100vh',
        ...
        {checktablegroup && initialLoad &&<Grouptablegroup  
          lockedData={lockedData} 
          setLockedData={setLockedData} 
          primaryTableData={primaryTableData}
          setPrimaryTableData={setPrimaryTableData}
