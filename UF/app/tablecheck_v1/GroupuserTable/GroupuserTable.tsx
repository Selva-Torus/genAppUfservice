...
const GroupuserTable = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT309:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:A001:AFK:tablecheck:AFVK:v1",componentId:"a57c0891dab94e4bb69afec9979e2c3b",from:"GroupUsertable",isTable : true,accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("name")){
      setnameffb02({...nameffb02,isDisabled:true});
    }
  //////////////
  }
  ...
  const usertablee2c3bRef = useRef<any>(null);
  const handleClearSearch = () => {
    usertablee2c3bRef.current?.setSearchParams();
    usertablee2c3bRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(usertablee2c3b) && Object.keys(usertablee2c3b)?.length>0)
      {
        setusertablee2c3b({})
      }
    }else 
  ...
  return (
    ...
    >
       ...
       {<TableuserTable lockedData={lockedData} setLockedData={setLockedData}  primaryTableData={primaryTableData} setPrimaryTableData={setPrimaryTableData}  refetch={refetch} setRefetch={setRefetch} encryptionFlagCompData={encryptionFlagCompData} paginationDetails={paginationDetails} open={open} setOpen={setOpen} ref={usertablee2c3bRef} ButtonGoRuleData={ButtonGoRuleData} setButtonGoRuleData ={setButtonGoRuleData}/>}
      </div>
        </div>
      </CommonHeaderAndTooltip>
    </div>
