...
let defaultColumns = [
  {
    "id": "id",
    "nodeid": "c6643a677c024e869f58e38c68c2c392",
    "name": "id",
    "meta": {
      "sort": true
    },
  ...
  {
    "id": "names",
    "nodeid": "46e91cedc7564471adc3855fa720c3b9",
    "name": "names",
    "meta": {
      "sort": true
    },
...
const Tabletexttable = ({ lockedData,setLockedData,primaryTableData, setPrimaryTableData,refetch, setRefetch,setData,encryptionFlagCompData,paginationDetails,open, setOpen, ref, ButtonGoRuleData, setButtonGoRuleData }: any)=>{
  ...
  const GetTableDetails = async () => {
    const orchestrationData = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        componentId: '44373986eabb49a0b5aec6b3864badf1',
        isTable: true,
        from :"Tabletexttable",
    ...
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          settexttablebadf1Props((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
  ...
  async function fetchData(page:any = 1, pageSize:any = 10, searchParams = {},dfKey:any,isRulePresent:any=false,isOnLoad = false,filterProps?:any) {
    ...
    try {
      ...
      if (isRulePresent==false) {
        ...
        const api_paginationBody: api_paginationDto = {
          ...
          filterDetails: {
            ...
            nodeId: '44373986eabb49a0b5aec6b3864badf1',
            elementId: '44373986eabb49a0b5aec6b3864badf1'
          },
          searchFilter: searchParams
        }
        if(encryptionFlagCont) {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        ...
        settexttablebadf1(uf_paginationDataFilter.data||[])
...
const colurIndicator = (keyValue:any=[], comingValue:any) => {
  ...
  useEffect(() => {
    if (prevRefreshRef.current) {
      UpdatedDataHandle(texttablebadf1Props.filterProps)
    }else 
      prevRefreshRef.current= true
  }, [texttablebadf1Props.filterProps])
  ...
  const handlePrimaryTable = () => {
    let findData = texttablebadf1Props?.selectedIds[texttablebadf1Props?.selectedIds?.length-1]
    if(Array.isArray(texttablebadf1) && texttablebadf1.length>0)
    {
      let data = texttablebadf1[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "ids",
        value: data["ids"],
  ...
  useEffect(() => {
    if (texttablebadf1Props?.selectedIds?.length != 0) handlePrimaryTable()
