...
export default function PageTranJourneyDtlV1() {
  ...
  const [paginationData,setPaginationData]=useState<PaginationData>({count:10,page:1})
    const prevRefreshRef = useRef<any>({
      tran_journey_dtl_v1:false,
    });
    async function tran_journey_dtl_v1(pagination:any): Promise<void>{
        let tran_journey_dtl_v1Body:te_refreshDto={
          key: "CK:CT005:FNGK:AF:FNK:DF-DFD:CATK:V001:AFGK:VGPH001:AFK:Tran_Journey_Dtl:AFVK:v1"+":",
          refreshFlag: "Y",
        ...
        const tran_journey_dtl_v1Data:any=await AxiosService.post("/te/eventEmitter",tran_journey_dtl_v1Body,{
          headers: {
            Authorization: `Bearer ${token}`
          }
        })

        if (tran_journey_dtl_v1Data?.data?.dataset) {
          setdfd_tran_journey_dtl_v1Props(tran_journey_dtl_v1Data?.data?.dataset?.data || []);
        }else{
         //////////////
        let dstKey:string=tran_journey_dtl_v1Body?.key || ""
        ...
        setdfd_tran_journey_dtl_v1Props(api_paginationData?.data?.records || []);
        }
      }
  useEffect(()=>{
    if (prevRefreshRef?.current?.tran_journey_dtl_v1) {
      tran_journey_dtl_v1(paginationData)
    }else 
      prevRefreshRef.current.tran_journey_dtl_v1= true
