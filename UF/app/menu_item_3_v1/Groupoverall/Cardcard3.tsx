...
const Cardcard3 = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  const handleMapperDetails=async()=>{
    try{
    let code:any;
    const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:OpenBanking:AFVK:v1",  componentId:"f0f6a573e6b64b268dd38ea18e005a6d",controlId:"4a12688f313d495db9f0577a5ac1630c",isTable:false,accessProfile:accessProfile,from:"cardStar Bank"},{
      headers: {
...
useEffect(() => {
    setoverall05a6d((pre:any)=>({...pre,card3:""}));
  },[card31630c?.refresh])
  ...
  if (card31630c?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `6 / 8`,gridRow: `61 / 112`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Card 
      style={style}
      className=""      
      size="l"
      theme="utility"
      view="filled"
      disabled= {card31630c?.isDisabled ? true : false}
      onClick={handleClick}  
      >
...
{overall05a6d?.card3?overall05a6d?.card3:"0"}
      </Text>
    </div>
      </Card>
    </div>
