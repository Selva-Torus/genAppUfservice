...
const Cardcard5 = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  const handleMapperDetails=async()=>{
    try{
    let code:any;
    const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:AG001:AFGK:oprmatrix:AFK:OpenBanking:AFVK:v1",  componentId:"f0f6a573e6b64b268dd38ea18e005a6d",controlId:"f47b9185ab0246d2819bb4370b0e0759",isTable:false,accessProfile:accessProfile,from:"cardAdd Account"},{
      headers: {
...
useEffect(() => {
    setoverall05a6d((pre:any)=>({...pre,card5:""}));
  },[card5e0759?.refresh])
  ...
  if (card5e0759?.isHidden) {
    return <></>
  }  
  return (
    <div 
    style={{gridColumn: `11 / 13`,gridRow: `61 / 112`, gap:``, height: `100%`, overflow: 'auto'}} >
      <Card 
      style={style}
      className=""      
      size="l"
      theme="utility"
      view="filled"
      disabled= {card5e0759?.isDisabled ? true : false}
      onClick={handleClick}  
      >
      
      {/* <div className="my-4 w-3/4 border-1 border-gray-300"></div> */}
      <div className=' flex justify-center space-x-2  '>
        <span className=" flex justify-center "><Icon size={25} data="MdAdd" /></span>
...
{overall05a6d?.card5?overall05a6d?.card5:"0"}
      </Text>
    </div>
      </Card>
    </div>
