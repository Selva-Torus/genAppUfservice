...
const TextInputRelease_Date = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  const handleChange = (e: any) => {
    ...
    setInfo_Groupaab7f((prev: any) => ({ ...prev, release_date: +e.target.value }))
    }
    else{
    setInfo_Groupaab7f((prev: any) => ({ ...prev, release_date: e.target.value }))
    }
  ...
  const handleMapperValue=async()=>{
    try{
      let orchestrationBody:any = {
          ...
          controlId: "cda8bcf1f9bb4e4e8f4d7e52f06731a0",
          isTable: false,
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setInfo_Groupaab7f((pre:any)=>({...pre,release_date:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
    ...
    useEffect(()=>{
      handleMapperValue()
          setDisable((prev: any) => ({ ...prev, textinputRelease_Date731a0:false }));
    },[refresh.textinputRelease_Date731a0])

    useEffect(()=>{
        handleBlur()
    },[validateRefetch.value])

    if (hide.textinputRelease_Date731a0) {
      return <></>
    }

  return (   
    <div className="col-start-9 col-end-11 row-start-3 row-end-3 gap-10px" >
      <TorusTextInput
        label={keyset("")}
        onChange= {handleChange}
        onBlur={()=>handleBlur()}
        type={dynamicStateandType.type}
        value={Info_Groupaab7f?.release_date||""}
          disabled={disable.textinputRelease_Date731a0||false}
        readOnly={true} 
      />
    </div>
