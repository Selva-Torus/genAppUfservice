...
const TextInputApi_Category = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  let schemaArray :any =[];  
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'api_category',type:"text"})
  ...
  const handleChange = (e: any) => {
    ...
    setInfo_Groupaab7f((prev: any) => ({ ...prev, api_category: +e.target.value }))
    }
    else{
    setInfo_Groupaab7f((prev: any) => ({ ...prev, api_category: e.target.value }))
    }
  ...
  const handleMapperValue=async()=>{
    try{
      let orchestrationBody:any = {
          ...
          controlId: "24c428c42e9a4d7dafc23d7dd67b5688",
          isTable: false,
          from:"TextInputApi_Category",
      ...
      if(orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties){
        let type:any={name:'api_category',type:'text'}
        type={
          name:'api_category',
          type: orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.api_category.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.api_category.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.api_category.type
      }
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setInfo_Groupaab7f((pre:any)=>({...pre,api_category:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
    ...
    useEffect(()=>{
      handleMapperValue()
          setDisable((prev: any) => ({ ...prev, textinputApi_Categoryb5688:false }));
    },[refresh.textinputApi_Categoryb5688])

    useEffect(()=>{
        handleBlur()
    },[validateRefetch.value])

    if (hide.textinputApi_Categoryb5688) {
      return <></>
    }

  return (   
    <div className="col-start-7 col-end-9 row-start-3 row-end-3 gap-10px" >
      <TorusTextInput
        label={keyset("")}
        onChange= {handleChange}
        onBlur={()=>handleBlur()}
        type={dynamicStateandType.type}
        value={Info_Groupaab7f?.api_category||""}
          disabled={disable.textinputApi_Categoryb5688||false}
        readOnly={true} 
      />
    </div>
...
export default TextInputApi_Category