...
const TextInputStatus = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  const handleChange = (e: any) => {
    ...
    setInfo_Groupaab7f((prev: any) => ({ ...prev, status: +e.target.value }))
    }
    else{
    setInfo_Groupaab7f((prev: any) => ({ ...prev, status: e.target.value }))
    }
  ...
  const handleMapperValue=async()=>{
    try{
      let orchestrationBody:any = {
          ...
          controlId: "5e41122b03fc4b51ac5a59f803b0a91d",
          isTable: false,
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setInfo_Groupaab7f((pre:any)=>({...pre,status:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
    ...
    useEffect(()=>{
      handleMapperValue()
          setDisable((prev: any) => ({ ...prev, textinputStatus0a91d:false }));
    },[refresh.textinputStatus0a91d])

    useEffect(()=>{
        handleBlur()
    },[validateRefetch.value])

    if (hide.textinputStatus0a91d) {
      return <></>
    }

  return (   
    <div className="col-start-5 col-end-7 row-start-3 row-end-3 gap-10px" >
      <TorusTextInput
        label={keyset("")}
        onChange= {handleChange}
        onBlur={()=>handleBlur()}
        type={dynamicStateandType.type}
        value={Info_Groupaab7f?.status||""}
          disabled={disable.textinputStatus0a91d||false}
        readOnly={true} 
      />
    </div>
