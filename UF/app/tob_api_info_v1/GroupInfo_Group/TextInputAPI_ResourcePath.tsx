...
const TextInputAPI_ResourcePath = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  let schemaArray :any =[];  
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'api_resourcepath',type:"text"})
  ...
  const handleChange = (e: any) => {
    ...
    setInfo_Groupaab7f((prev: any) => ({ ...prev, api_resourcepath: +e.target.value }))
    }
    else{
    setInfo_Groupaab7f((prev: any) => ({ ...prev, api_resourcepath: e.target.value }))
    }
  ...
  const handleMapperValue=async()=>{
    try{
      let orchestrationBody:any = {
          ...
          controlId: "5add8ce14c2e44c0b685ac09b2c1fa90",
          isTable: false,
          from:"TextInputAPI_ResourcePath",
      ...
      if(orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties){
        let type:any={name:'api_resourcepath',type:'text'}
        type={
          name:'api_resourcepath',
          type: orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.api_resourcepath.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.api_resourcepath.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.api_resourcepath.type
      }
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setInfo_Groupaab7f((pre:any)=>({...pre,api_resourcepath:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
    ...
    useEffect(()=>{
      handleMapperValue()
          setDisable((prev: any) => ({ ...prev, textinputAPI_ResourcePath1fa90:false }));
    },[refresh.textinputAPI_ResourcePath1fa90])

    useEffect(()=>{
        handleBlur()
    },[validateRefetch.value])

    if (hide.textinputAPI_ResourcePath1fa90) {
      return <></>
    }
  ...
  return (   
    <div className="col-start-11 col-end-13 row-start-3 row-end-3 gap-10px" >
      <TorusTextInput
        ...
        value={Info_Groupaab7f?.api_resourcepath||""}
          disabled={disable.textinputAPI_ResourcePath1fa90||false}
        readOnly={true} 
      />
    </div>
...
export default TextInputAPI_ResourcePath