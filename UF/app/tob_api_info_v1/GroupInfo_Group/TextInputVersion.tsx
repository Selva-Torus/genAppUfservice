...
const TextInputVersion = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  const handleChange = (e: any) => {
    ...
    setInfo_Groupaab7f((prev: any) => ({ ...prev, version: +e.target.value }))
    }
    else{
    setInfo_Groupaab7f((prev: any) => ({ ...prev, version: e.target.value }))
    }
  ...
  const handleMapperValue=async()=>{
    try{
      let orchestrationBody:any = {
          ...
          controlId: "1bd96a4e5acc4f1db0fcf933f38f9dea",
          isTable: false,
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setInfo_Groupaab7f((pre:any)=>({...pre,version:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
    ...
    useEffect(()=>{
      handleMapperValue()
          setDisable((prev: any) => ({ ...prev, textinputVersionf9dea:false }));
    },[refresh.textinputVersionf9dea])

    useEffect(()=>{
        handleBlur()
    },[validateRefetch.value])

    if (hide.textinputVersionf9dea) {
      return <></>
    }

  return (   
    <div className="col-start-3 col-end-5 row-start-3 row-end-3 gap-10px" >
      <TorusTextInput
        label={keyset("")}
        onChange= {handleChange}
        onBlur={()=>handleBlur()}
        type={dynamicStateandType.type}
        value={Info_Groupaab7f?.version||""}
          disabled={disable.textinputVersionf9dea||false}
        readOnly={true} 
      />
    </div>
