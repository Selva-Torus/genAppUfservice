...
const Carderror_rate = ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
  ...
  const handleMapperDetails=async()=>{
    ...
    let orchestrationDataBody:any = {
      ...
      controlId:"9a67c03d7dc64af5bdbac01c6da4859e",
      isTable:false,
    ...
    code = orchestrationData?.data?.code
      ...
      if(Array.isArray(orchestrationData?.data?.dstData)&&orchestrationData?.data?.dstData?.length>0){
        setSummary_Table98cb0((pre:any)=>({...pre,error_rate:orchestrationData?.data?.dstData[0]?.error_rate}))
      } 
    }catch(err){
      console.log(err)
    }
  ...
  useEffect(() => {
    handleMapperDetails()
        setDisable((prev: any) => ({ ...prev, carderror_rate4859e:false }));
  }, [refresh.carderror_rate4859e])
  ...
  if (hide.carderror_rate4859e) {
    return <></>
  }  
  return (
    <div className="col-start-9 col-end-13 row-start-2 row-end-2 gap-10px" >
      <TorusCard 
      style={style}
      className="rounded-lg shadow-lg p-6 bg-white"
      disabled={disable.carderror_rate4859e || false}
      >
      ...
      {Summary_Table98cb0?.error_rate?Summary_Table98cb0?.error_rate:"0"}
      </Text>
    </div>
      </TorusCard>
    </div>
