...
const TableAPI_Process_Log_Table=({ lockedData,setLockedData,primaryTableData, setPrimaryTableData,refetch, setRefetch,setData,encryptionFlagCompData }: any)=>{
...
//another screen
  ...
  const GetTableDetails = async () => {
    let orchestrationBody : any = {
        ...
        isTable: true,
        from :"TableAPI_Process_Log_Table",
  ...
  const handleUpdate: PaginationProps['onUpdate'] = (page, pageSize) =>{
    setAPI_Process_Log_Table4f441ContainValidataion((pre:any)=>({...pre, selectedIds:[]}))
    let checkedData: any = selectedPaginationData
    if (checkedData.length) {
      for (let i = 0; i < checkedData.length; i++) {
        if (checkedData[i].page == page) {
          setAPI_Process_Log_Table4f441ContainValidataion((pre:any)=>({...pre, selectedIds:checkedData[i].data}))
        }
      }
    }
    ...
    try {
      ...
      if (isRulePresent==false) {
        ...
        const api_paginationBody: api_paginationDto = {
          ...
          count: pageSize,
          filterDetails: {
            ufKey:'CK:CT242:FNGK:AF:FNK:UF-UFW:CATK:TOB001:AFGK:TOB002:AFK:TOB_API_Info:AFVK:v1:UO', 
            nodeId: 'c22f5cc4649840d08ebe8074d414f441',
            elementId: 'c22f5cc4649840d08ebe8074d414f441'
          },
          searchFilter: searchParams
        }
        if(encryptionFlagCont) {
      ...
      if (uf_paginationDataFilter.data.length >= 0&&Array.isArray(uf_paginationDataFilter.data)) {
        ...
        setAPI_Process_Log_Table4f441(uf_paginationDataFilter.data||[])
  ...
  useEffect(() => {
    GetTableDetails()
  }, [refresh?.tableAPI_Process_Log_Table4f441])
  ...
  const handlePrimaryTable = () => {
    let findData = isAPI_Process_Log_Table4f441ContainValidataion.selectedIds[isAPI_Process_Log_Table4f441ContainValidataion.selectedIds.length-1]
    if(Array.isArray(API_Process_Log_Table4f441) && API_Process_Log_Table4f441.length>0)
    {
      let data = API_Process_Log_Table4f441[findData]
      setPrimaryTableData({
        ...primaryTableData,
        primaryKey: "id",
        value: data["id"],
  ...
  useEffect(() => {
    if (isAPI_Process_Log_Table4f441ContainValidataion.selectedIds.length != 0) handlePrimaryTable()
  }, [isAPI_Process_Log_Table4f441ContainValidataion.selectedIds])
  ...
  if (hide.tableAPI_Process_Log_Table4f441) {
    return <></>
  }
  return(
        ...
        <Col>
          <Flex direction='column' >
            <MyTable
              ...
              columns={columns}
              edgePadding={true}
              selectedIds={isAPI_Process_Log_Table4f441ContainValidataion.selectedIds}  
