...
@Injectable()
export class Get_Accounts_Consent_Event_Consents_ConsentsService {
  private ajv = new Ajv();    
  ...
  private readonly logger = new Logger(Get_Accounts_Consent_Event_Consents_ConsentsService.name);

  async getGet_Accounts_Consent_Event_Consents_ConsentsProcess(input:PoEvent) {  
      ...
      for (var j = 0; j < poNode.length; j++) {
                      ...
                      for(let a=0;a< customcoderesult.length;a++){ 
                        if(Array.isArray(apires) && apires.length>0){
                          for(let i=0;i< apires.length;i++){
                            Object.assign(apires[i], customcoderesult[a]);
                          }
                        }else{
                           Object.assign(apires, customcoderesult[a]);
                        } 
                    }                   
        ...
        if (nodeType == 'streamnode' && poNode[j].nodeId == nodeId) {
          try {
            ...
            let streamArr:any = [];
            let oprname, oprkey, streamName, fromStreamid, toStreamid, connectorType, storageType, dpdkey, conncectorName,apikey,responseNodeName,fieldName,isStatic,useAsConsumer,consumerName,consumerGroupName;
            let customConfig = ndp[poNode[j].nodeId] 
            let nodeVersion = customConfig?.nodeVersion;
            if (!nodeVersion)
            ...
            if (customConfig) {
              ...
              if(!isStatic && !childInsertArr || childInsertArr?.length == 0 && !textobj) throw new CustomException('Stream Config Mapping was Required', 404);               
              ...
              if (!isStatic) {
                ...
                //  const now = new Date();
                  ...
                  if (startOfToday && endOfToday) {
                    //streamvalue = await this.redisService.getStreamRevRange(streamName, endOfToday, startOfToday, 3);
                    ...
                    if (await redis.call('EXISTS', streamName)) {
                      ...
                      if (streamData != 'No Data available to read') {
                        for (var s = 0; s < streamData.length; s++) {
                          ...
                          let dataObj = streamData[s].data                          
                          let jsondata = await this.convertToKeyValue(dataObj) 
                          streamArr.push(jsondata)
                          //await redis.call('XDEL',streamName,msgid)
                          streamArr = streamArr.flat()
                        }                       
                      } else {
                        throw streamData
                      }
                  ...
                  if(!fieldName) 
                   ...
                   if(childInsertArr?.length>0 && streamName && !textobj){
                    ...
                    streamArr = await redis.xadd(streamName, '*', fieldName, JSON.stringify(childInsertArr[a]));
                   }
                   }else if(textobj && streamName){
                    streamArr = await redis.xadd(streamName, '*', fieldName, JSON.stringify(textobj));
                   }           
          ...
          try {
            ...
            let finalobj = {}
             ...
             // console.log('edges',edges);
                ...
                if ((currentFabric == 'DF-DFD' || (currentFabric == 'PF-PFD' && (methodName == 'post' || methodName == 'patch' || methodName == 'put'|| methodName == 'delete')))) {
                  if (Array.isArray(inputparam) && inputparam.length > 0) {
