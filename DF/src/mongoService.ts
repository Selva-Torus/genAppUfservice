...
export class MongoService {
  ...
  async updateKey(key): Promise<any> {
   ...
   async saveFileToGridFS(bucketName: string, objectName: string, objectData:any, encryptionFlag?: string): Promise<any> {
    try {
      ...
      if(encryptionFlag == 'Y'){
       ...
       return new Promise((resolve, reject) => {
        var uploadStream
        if(encryptionFlag == 'Y'){
          uploadStream = gridfsBucket.openUploadStream(objectName,{metadata: {isEncrypted: true}});
        }else{
          if(objectData?.METADATA){
            uploadStream = gridfsBucket.openUploadStream(objectName,{metadata: objectData.METADATA});
          }else{
            uploadStream = gridfsBucket.openUploadStream(objectName);
          }
        }
 ...
 async readFileFromGridFS(bucketName: string, filename: string, decryptionFlag?: string): Promise<any> {
    ...
    const cursor:any = gridfsBucket.find({filename});
    ...
    for(let f = 0; f < files.length; f++){
        try {
          ...
          const downloadStream:any = gridfsBucket.openDownloadStream(files[f]._id);
...
async readFileFromGridFSWithId(bucketName: string, fileId: any, decryptionFlag?: string): Promise<any> {
    try {
      ...
      const retrievedData: any = await new Promise((resolve, reject) => {
        ...
        const downloadStream:any = gridfsBucket.openDownloadStream(fileObjectId);
  ...
  async readFileFromGridFsWithFilter(bucketName: string, filename: string, filter:object, decryptionFlag?: string): Promise<any> {
    try {
      ...
      if(filter){
        ...
        var files:any = await gridfsBucket.find(metaDataVal).toArray();
      }else{
        var files:any = await gridfsBucket.find({filename}).toArray();
      }
   ...
   async deleteFileFromGridFs(bucketName: string, filename: string, filter?: object): Promise<any> {
    try {
      ...
      if (filter) {
        var files: any = await gridfsBucket.find({ filename, filter }).toArray();
      } else {
        var files: any = await gridfsBucket.find({ filename }).toArray();
      }
      console.log('files', files);
      ...
      if(files?.length > 0){
        for (let f = 0; f < files.length; f++) {
          await gridfsBucket.delete(files[f]._id);
