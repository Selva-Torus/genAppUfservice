...
export class TeService {
   private ajv = new Ajv(); 
    ...
    private readonly CommonService: CommonService, private readonly lockservice: LockService   
   ){}
   private readonly logger = new Logger(TeService.name) 

      
  async DF_EventEmitter(pfdto: pfDto){ 
    let currentFabric = 'DF-DFD'
    const executionCache = new Map<string, any>();
    const page = pfdto.page
    const count = pfdto.count
    const filterdata = pfdto.filterData
    let nodeInfo,processedKey,failureQueue,failureTargetStatus,sourceId, ufname;
    try {
      this.logger.log("DF_Event Emmiter Started....")      

      let event, refflag, pfjson, poJson, dstkey, Ndp, routeArr,pid, pfo;    
      let flg = 0;
      let invalidEventFlg = 0      
      let mergearr = [] 
      let prevres = {};      
      
      // return await this.CommonService.MyAccountForClient(pfdto.token)
        let tokenDecode = this.jwtService.decode(pfdto.token, { json: true })
      if(!tokenDecode || !tokenDecode.loginId) throw new CustomException('Invalid Token',401)

      let fngkKey = await this.CommonService.splitcommonkey(pfdto.key,'FNGK')//pfdto.key.split('FNGK')[1].split(':')[1]
      ...
      let node = await this.securityService.getSecurityTemplate(pfdto.key + 'DO', pfdto.token)
      let logicCenter     
      if (afi && afi.hasOwnProperty('logicCenter')) {
       ...
       if(!logicCenter && pfjson.length>3 )
        throw new CustomException('Nodes length exceed',403)
           
       if(!poJson || !pfjson || !Ndp || !routeArr) throw new CustomException('Invalid Key',401)
      ...
      if(!poNode || poNode.length == 0) throw new CustomException('Nodes not found' ,404)
           ...
           if (pfdto.nodeId == poNode[e].nodeId) {
             if (poNode[e].events.length > 0) {
               for (let k = 0; k < poNode[e].events.length; k++) {
                 if (pfdto.event != poNode[e].events[k].sourceStatus) {
                   eflg++;
                 }
               }
      ...
      pfdto.nodeId = pfdto.nodeId?pfdto.nodeId:poNode[i].nodeId;
        ...
        if (poNode[i].nodeType == 'startnode'|| poNode[i].nodeType == 'schedulernode' || poNode[i].nodeType == 'intervalnode' || poNode[i].nodeType == 'listenernode') {
          ...
          if (poNode[1].events.sourceStatus) {
            ...
            if (Array.isArray(poNode[i].events)) {
                ...
                let getEventInfo = await this.getEventandSourceid(pfdto, poNode[i], event, 'DF-DFD',  sourceId)
          ...
          if (event === srcStatus) {
             ...
             //Setting Source Status
              await this.redisService.setStreamData(srcQueue, 'TASK - ' + pfdto.upId, JSON.stringify({ PID: pfdto.upId, TID: pfdto.nodeId, EVENT: event }));
              let msgPattern = artifact + '_' + poNode[i].nodeId + '_' + event
              eventResponse = await firstValueFrom(this.poClient.send
                (msgPattern,
                  ...
                  new PoEvent(pfdto, event, pfjson, poNode[i], pfo, Ndp, refflag, page, count, targetQueue,failureQueue,pfdto.filterData),
                ));         
            ...
            if (eventResponse.targetStatus)
              eventResponse.targetStatus = eventResponse.targetStatus
            else
              eventResponse.targetStatus = targetStatus
            
            console.log(`${eventResponse.targetStatus} Event emitted successfully by ${poNode[i].nodeName}`);

            //Setting Target Status
            await this.redisService.setStreamData(targetQueue,'TASK - ' + pfdto.upId,JSON.stringify({PID: pfdto.upId,TID: pfdto.nodeId,EVENT: eventResponse.targetStatus}));
        ...
        else if(poNode[i].nodeType == 'datasetnode' && poNode[i].nodeId == pfdto.nodeId ){      
         
            srcStatus = poNode[i].events?.sourceStatus
            srcQueue = poNode[i].events?.sourceQueue 
            targetStatus = poNode[i].events?.pro?.success?.targetStatus
            targetQueue = poNode[i].events.pro?.success?.targetQueue;
            failureQueue = poNode[i].events.pro?.failure?.targetQueue;
            failureTargetStatus = poNode[i].events?.pro?.failure?.targetStatus;
          ...
          if (event === srcStatus) {
            let msgPattern =  ufname + '_' + poNode[i].nodeId + '_' + sourceId + '_' + event;
             //Setting Source Status
              await this.redisService.setStreamData(srcQueue, 'TASK - ' + pfdto.upId, JSON.stringify({ PID: pfdto.upId, TID: pfdto.nodeId, EVENT: event }));
          ...
          if(pfdto.upId){ 
            ...
            if (poNode[i].nodeId == pfdto.nodeId) {
                if (poNode[i].nodeType != 'endnode') {
                  if ((poNode[i].events)) {
                    if (!event) event = pfdto.event          
                    let getEventInfo = await this.getEventandSourceid(pfdto, poNode[i], event, 'DF-DFD', '')
                    ufname = getEventInfo.ufname
                    srcStatus = getEventInfo.srcStatus
                    targetStatus = getEventInfo.targetStatus             
                    srcQueue = getEventInfo.srcQueue
                    sourceId = getEventInfo.sourceId 
                    targetQueue = getEventInfo.targetQueue
                    failureQueue = getEventInfo.failureQueue
                    failureTargetStatus = getEventInfo.failureTargetStatus                     
                  } 
                        ...
                        for (let r = 0; r < routeArray.length; r++) {
                          if (routeArray[r].nodeName == 'End') { 
                              ...
                              if(eventResponse){
                                  ...
                                  //if (!srcQueue) srcQueue = staticQueue   
                                  //this.redisService.setJsonData(dstkey + tokenDecode.loginId+'_DS_Object', JSON.stringify(obj),client)                                  
                                 
                                if(logicCenter){
                                  let keys = await this.redisService.getKeys(dstkey+ tokenDecode.loginId + '_DS_Object',client)
                                if(keys.length>0){                                 
                                  for(let a=0;a< keys.length;a++){                                 
                                    await this.redisService.deleteKey(keys[a],client)
                                  }                                  
                                }
                ...
                if (event == srcStatus) {
                  ...
                  if (mergearr && mergearr.length > 0) {
                    for (let m = 0; m < mergearr.length; m++) {
                      let Confignode: any = JSON.parse(await this.redisService.getJsonDataWithPath(pfdto.key + 'NDP', '.' + poNode[1].nodeId, client))
                      ...
                      eventResponse = await firstValueFrom(this.poClient.send(
                        artifact + '_' + poNode[i].nodeId + '_' + event,                        
                        new PoEvent(pfdto, event, pfjson, poNode[i], pfo,Ndp, refflag, page, count, targetQueue,failureQueue,filterdata),
                      ))
                    ...
                    //await this.CommonService.getTPL(processedKey, pfdto.upId, poNode[i], 'Success', 'targetQueue',pfdto.token, currentFabric, event)//apiUrl,dfoSchema
                  }
                  else {
                       pfdto['logicCenter'] = logicCenter
                      eventResponse = await firstValueFrom(this.poClient.send(
                        artifact + '_' + poNode[i].nodeId + '_' + event,
                        // new PoEvent(pfdto.key, pfdto.upId, event, pfdto.data, pfdto.token, pfdto.nodeId, poNode[i].nodeName, pfdto.nodeType,refflag,page,count, pfdto.filterData),
                        new PoEvent(pfdto, event, pfjson, poNode[i], pfo,Ndp,refflag, page, count,targetQueue,failureQueue,filterdata),
                      ))
                     ...
                     if (eventResponse.targetStatus)
                      ...
                      console.log(`${eventResponse.targetStatus} Event emitted successfully by ${poNode[i].nodeName}`);
                      let eventData = eventResponse?.data
                      // await this.settingRouteArr(processedKey+pfdto.upId,eventData,client)
  ...
  async PF_EventEmitter(pfdto: pfDto){ //PF_EVENTEMITTER
       let currentFabric = await this.CommonService.splitcommonkey(pfdto.key,'FNK')
       
       const page = pfdto.page
       const count = pfdto.count
       let nodeInfo,processedKey,failureQueue,failureTargetStatus
       try {
         this.logger.log("PF_Event Emmiter Started....")         
           const executionCache = new Map<string, any>();
         let event,pid,refflag,ufname,pfjson,poJson,poNode,hlrId,sourceId,routeArr,Ndp,pfo 
         let invalidEventFlg = 0 
         let flg = 0      
         let prevres = {} 
       
         let fngkKey = await this.CommonService.splitcommonkey(pfdto.key,'FNGK')//pfdto.key.split('FNGK')[1].split(':')[1]
         let node = await this.securityService.getSecurityTemplate(pfdto.key + 'PO', pfdto.token)
         if (pfdto.key.includes(fngkKey)) {
           processedKey = pfdto.key.replace(fngkKey, fngkKey + 'P')
         }
         ...
         poNode = poJson?.mappedData?.artifact?.node
         if(!poNode || poNode.length == 0) throw new CustomException('Nodes not found' ,404)          
        ...
        let eflg = 0;
             ...
             if(pfdto.nodeId == poNode[e].nodeId){         
                 if(poNode[e].events.length>0){
                   ...
                   if(eflg == poNode[e].events.length)
                    ...
                    throw new CustomException('events are empty',400)
                  }            
              }else{
                flg++
              }        
         ...
         if(flg == poNode.length){
           throw new CustomException('Invalid nodeId',422)
         }
         ...
         if(pfdto.upId){
            ...
            pid = pfdto.upId 
            prevres = JSON.parse(await this.redisService.getJsonData(processedKey+pfdto.upId+':previousResponse',client)) 
         }      
         ...
         for (var i = 0; i < poNode.length; i++) {
          ...
          nodeInfo = poNode[i]
           ...
           if (poNode[i].nodeType == 'startnode') {
            ...
            this.logger.log(`Human Task node started ${poNode[i].nodeName}`)           
            if (pfdto.upId) {
            ...
            if (sourceId) {
              if (!event) event = pfdto.event 
              let getEventInfo = await this.getEventandSourceid(pfdto, poNode[i], event, 'PF-PFD', sourceId)
              ufname = getEventInfo.ufname
              srcStatus = getEventInfo.srcStatus
              targetStatus = getEventInfo.targetStatus
              srcQueue = getEventInfo.srcQueue
              ...
              if (pfdto.event === srcStatus) {
                ...
                if (pfdto.data) {
                  ...
                  //  throw new CustomException('Data should not be Empty',400)
                // } else {
                  ...
                  await this.redisService.setJsonData(processedKey + pfdto.upId + ':NPV:' + poNode[i].nodeName + '.PRO', JSON.stringify(pfdto.data), client, 'request');
                    ...
                    eventResponse = await this.callEventEmitter(pfdto, poNode[i], processedKey, client, ufname, sourceId, srcQueue, pfjson, Ndp, pfo, refflag, page, count, targetQueue,failureQueue)

                    if (eventResponse == undefined) { throw new CustomException('Event Response is undefined',400)}
                    ...
                    //setting response for next node
                    //  await this.settingRouteArr(processedKey+pfdto.upId,eventResponse.data,routeArr[poNode[i].nodeId]['routeArray'])
            ...
            try {
               ...
               if (poNode[i].nodeId == pfdto.nodeId) {if (Array.isArray(poNode[i].events)) {
              if (!event) event = pfdto.event     
              let getEventInfo = await this.getEventandSourceid(pfdto, poNode[i], event, 'PF-PFD', '')
                 ufname = getEventInfo.ufname
                 sourceId = getEventInfo.sourceId
                 srcStatus = getEventInfo.srcStatus
                 targetStatus = getEventInfo.targetStatus
          ...
          if (poNode[i].nodeId == pfdto.nodeId) {         
            if (Array.isArray(poNode[i].events)) { 
                // console.log("89", pfdto.event) 
              if (!event) event = pfdto.event  
              // console.log("event", event) 
              // console.log("123,pfdto", pfdto)           
                let getEventInfo = await this.getEventandSourceid(pfdto, poNode[i], event, 'PF-PFD', '',)
            ...
            if (event == srcStatus) {
             ...
             this.logger.log(`${poNode[i].nodeType} started ${poNode[i].nodeName}`)

             if (pfdto.upId) {
               if (poNode[i].nodeId == pfdto.nodeId) {
                 if (poNode[i].nodeType != 'endnode') {
                   if (Array.isArray(poNode[i].events) && poNode[i].events.length > 0) {
                    if (!event) event = pfdto.event 
                    let getEventInfo = await this.getEventandSourceid(pfdto, poNode[i], event,'PF-PFD', '')
                     // console.log('ELSA',getEventInfo);
                ...
                if (poNode[i].nodeType == 'endnode') {
                          ...
                          for (let r = 0; r < routeArray.length; r++) {                           
                              if (routeArray[r].nodeName == 'End') {
                                if (!srcQueue) srcQueue = staticQueue
                                  ...
                                  await this.redisService.setStreamData(srcQueue, 'TASK - ' + pfdto.upId, JSON.stringify({ "PID": pfdto.upId, "TID": pfdto.nodeId, "EVENT": event }))
                                  await this.CommonService.getTPL(processedKey, pfdto.upId, poNode[i], 'Success', '', pfdto.token, currentFabric)
                                //if(eventResponse?.data?.[lastnode.nodeName])
                               //eventResponse = eventResponse.data && eventResponse.data[lastnode.nodeName]?eventResponse.data[lastnode.nodeName]:eventResponse
                 ...
                 if (event === srcStatus) {
                   ...
                   // await this.pfPreProcessor(processedKey, poNode[i].nodeName, pfdto.upId, currentFabric);
    ...
    const job: any = new CronJob(interval, async () => {
      try {
        ...
        let sourceStatus, refflag, node, pfjson, poJson, pfo, sourceId, Ndp, currentFabric, processedKey;
        ...
        let pfProcessorResponse: any = await this.pfProcessor(pfdto, sourceStatus, pfjson, poJson, pfo, Ndp, currentFabric, refflag, undefined, undefined, undefined, undefined, undefined, logicCenter)
        if (!pfProcessorResponse?.status && pfProcessorResponse?.status != 200) {
          throw pfProcessorResponse;
        }
        if (pfProcessorResponse?.data) {
          let pfResponseData = pfProcessorResponse.data[Object.keys(pfProcessorResponse.data)[0]]
          if (Array.isArray(pfResponseData) && pfResponseData?.length > 0) {
            pfdto = {}
            for (let p = 0; p < pfResponseData.length; p++) {
              pfdto['key'] = artifactKey
              pfdto['token'] = artifactToken
              pfdto['event'] = pfProcessorResponse.targetStatus
              pfdto['upId'] = upId
              pfdto['data'] = pfResponseData[p]
              ...
              cronResponse = await this.PF_EventEmitter(pfdto);
          ...
          } else if (pfResponseData && Object.keys(pfResponseData).length > 0) {
            pfdto = {}
            pfdto['key'] = artifactKey
            pfdto['token'] = artifactToken
            pfdto['event'] = pfProcessorResponse.targetStatus
            ...
            this.logger.log(`Running dynamic job with data: ${JSON.stringify(pfdto)}`);
            await this.redisService.setJsonData(processedKey + upId + ':NPV:' + ponode[1].nodeName + '.PRO', JSON.stringify(pfResponseData), client, 'response',);
            cronResponse = await this.PF_EventEmitter(pfdto);
            if (cronResponse?.message === 'Success' && cronResponse?.statusCode === 201) {
              pfdto = {};
              pfdto['nodeId'] = ponode[2].nodeId
  ...
  async pfProcessor(pfdto, event, pfjson, poJson, pfo, ndp, currentFabric, flag, page, count, filterData, lockDetails, childtable, logicCenter) {
    ...
    sobj['session.subOrgGrpCode'] = SessionToken?.subOrgGrpCode
    sobj['session.subOrgGrpName'] = SessionToken?.subOrgGrpName
    sobj['session.subOrgCode'] = SessionToken?.subOrgCode
    sobj['session.subOrgName'] = SessionToken?.subOrgName
    ...
    SessionInfo['subOrgGrpName'] = SessionToken?.subOrgGrpName || '';
    SessionInfo['subOrgName'] = SessionToken?.subOrgName || '';
            ...
            if (!isStatic) {
              ...
              if (oprname == 'read') {                  
                ...
                if(streamArr?.length == 0)
                  throw new CustomException('No Data available to read',404)                  
        ...
        try {
          ...
          if (customConfig) {
            ...
            if (internalEdges && internalEdges.hasOwnProperty(poNode[j].nodeId)) {
              ...
              let sourcekey = [];
              let rootarr = [];
              let statickeyword = ["get", "post", "200", '201', '202', '204', '400', '401', '403', '404', '500', "parameters", "requestBody", "responses", "content", "application/json", "application/xml", "schema", "properties", "items", "allOf", "oneOf", "inputschema", "outputschema"] //,"items"
              let numberArr: string[] = Array.from({ length: 101 }, (_, i) => (i).toString());
    ...
    for (let s = 0; s < srcIdArr.length; s++) {
      ...
      for (var h = 0; h < pfo.length; h++) {                              
        if (connectedid == pfo[h].nodeId) {          
          ...
          if(tempArr.length == 0 && inputparam){
            innerpathVal = inputparam
            if(conncectedNodeType == 'api_inputnode'){
              innerpathVal = await this.keysToLowerCaseOnly(innerpathVal)            
            }
            tempArr = await this.combineData(innerpathVal,tempArr)
  ...
  switch (type) {
    ...
    async getEventandSourceid(pfdto, poNode, event,  currentFabric, hsourceid?) {
    let srcStatus, targetStatus, srcQueue, targetQueue, failureQueue, failureTargetStatus, ufname, sourceId
    if(currentFabric == 'PF-PFD'){
      if ((Array.isArray(poNode))){
        for(let po=0; po < poNode.length; po++){
          if (poNode[po].events.length > 0) {
          for (let e = 0; e < poNode[po].events.length; e++) {
            // if (event || pfdto.event == poNode.events[e].source.status) {
            let handlerId
            if (event == poNode[po].events[e].source.status) {
              if (poNode[po].events[e].eventType == 'UEH') {
                let ufkey = (poNode[po].events[e].sourceId).split('|')[0]
                ...
                let sourceid = ((poNode[po].events[e].sourceId).split('|')[2])
                ...
                sourceId = poNode[po].events[e].id.replaceAll('-', '')
              }
              if (pfdto.nodeType == 'humantasknode') {
                if (hsourceid) {
                  if (pfdto.event == poNode[po].events[e].source.status.trim() && hsourceid == handlerId) {
                    srcStatus = poNode[po].events[e].source.status
                    targetStatus = poNode[po].events[e].success.status
                    srcQueue = poNode[po].events[e].source.queue
                    targetQueue = poNode[po].events[e].success.queue;
                    failureQueue = poNode[po].events[e].failure.queue
                    failureTargetStatus = poNode[po].events[e].failure.status
                    break;
                  }
                } else {
                  srcStatus = poNode[po].events[e].source.status
                ...
                srcStatus = poNode[po].events[e].source.status.trim()
                targetStatus = poNode[po].events[e].success.status.trim()
                srcQueue = poNode[po].events[e].source.queue
                targetQueue = poNode[po].events[e].success.queue;
                failureQueue = poNode[po].events[e].failure.queue
                failureTargetStatus = poNode[po].events[e].failure.status
      ...
      for(let po=0; po<poNode.length; po++){
        if (poNode[po].events.length > 0) {
        for (let e = 0; e < poNode[po].events.length; e++) {
          if (event == poNode[po].events[e].sourceStatus) {
            if (pfdto.nodeType == 'humantasknode') {
                srcStatus = poNode[po].events[e].sourceStatus
                targetStatus = poNode[po].events[e].pro.success.targetStatus.trim()
                srcQueue = poNode[po].events[e].sourceQueue
                targetQueue = poNode[po].events[e].pro.success.targetQueue;
                failureQueue = poNode[po].events[e].pro.failure.targetQueue
                failureTargetStatus = poNode[po].events[e].pro.failure.targetStatus
            } else {
              srcStatus = poNode[po].events[e].sourceStatus
              targetStatus = poNode[po].events[e].pro.success.targetStatus.trim()
                srcQueue = poNode[po].events[e].sourceQueue
          ...
          if (event == poNode.events.sourceStatus) {
            if (pfdto.nodeType == 'humantasknode') {
                if (pfdto.event == poNode.events.sourceStatus.trim()) {
                  srcStatus = poNode.events.sourceStatus.trim()
                  targetStatus = poNode.events.pro.success.targetStatus
                  srcQueue = poNode.events.sourceQueue
                  targetQueue = poNode.events.pro.success.targetQueue
                  failureQueue = poNode.events.pro.failure.targetQueue
                  failureTargetStatus = poNode.events.pro.failure.targetStatus
                  return;
                }
            } else {
              srcStatus = poNode.events.sourceStatus
              targetStatus = poNode.events.pro.success.targetStatus.trim()
              srcQueue = poNode.events.sourceQueue
              targetQueue = poNode.events.pro.success.targetQueue
              failureQueue = poNode.events.pro.failure.targetQueue
              failureTargetStatus = poNode.events.pro.failure.targetStatus
    ...
    return { ufname, sourceId, srcStatus, targetStatus, srcQueue, targetQueue, failureQueue, failureTargetStatus }
      ...
      async callEventEmitter(pfdto,poNode,processedKey,client,ufname,sourceId,srcQueue,pfjson,Ndp,pfo,refflag,page,count,targetQueue,failureQueue){
          let nodeObjArr = {
           nodeName: poNode.nodeName,
           nodeId: pfdto.nodeId,
           nodeType: pfdto.nodeType,
        ...
        if (await this.redisService.exist(processedKey + pfdto.upId + ':nodeResponse', client)) {
          ...
          let getnoderesflg = 0
          for(let g=0;g< getnoderesponse.length;g++){
            if(getnoderesponse[g].nodeId != poNode.nodeId){
              getnoderesflg++
            }
          }
          if(getnoderesflg == getnoderesponse.length){                      
       ...
       await this.redisService.setStreamData(srcQueue,'TASK - ' + pfdto.upId,JSON.stringify({PID: pfdto.upId,TID: pfdto.nodeId,EVENT: pfdto.event}));
         ...
         // Event Emmiting logic                   
        let eventResponse = await firstValueFrom(this.poClient.send(
           ufname+'_'+poNode.nodeId+'_'+sourceId+'_'+pfdto.event,
         //  new PoEvent(pfdto, pfdto.event,pfjson,poJson,Ndp,pfo,refflag,page,count),
         new PoEvent(pfdto, pfdto.event,pfjson,poNode,pfo, Ndp,refflag,page,count, targetQueue,failureQueue),
           //new PoEvent(pfdto, pfdto.event,pfjson,poNode,Ndp,pfo,refflag,page,count,targetQueue,failureQueue),
         ))
  ...
  async TEcall(token,key,upId,data,nodeId,nodeName,nodeType,event,sourceId){
    ...
    const requestConfig: AxiosRequestConfig = {
      headers: {
        ...
        formdata =  await this.PF_EventEmitter(pfdto)              
      return formdata
  }catch(err){
    console.log("err",err)
    throw err
       ...
       if(id.length > 0 && data.length>0){
        if( id.length == 1 && data.length == 1){
          if(Object.keys(data).length > 0){
            ...
            throw new CustomException('Data was empty',404)
          }
          ...
          for(var i=0;i< id.length;i++){
            if(id.length == data.length){
              ...
              throw new CustomException('Missing data/id',422)
            }             
          }          
        }
       } else{
        throw new CustomException('data/primarykey is empty',404) 
       }        
      }else{
        throw new CustomException('data/primarykey should be an array',404)
      }
      if(dfkey && upid){
        if(apires?.statusCode){
          if(apires.statusCode == 200){
            ...
            pfdto.token = token 
            pfdto.refreshFlag = 'Y'
           var result = await this.PF_EventEmitter(pfdto)  
           return result         
          } 
         } 
      }
