...
export class TeService {
  ...
  async startCronJob(name: string, interval, pfdto,client) {
    ...
    const job:any = new CronJob(interval, async () => {
      try {
        ...
        //console.log('cronResponse', cronResponse);

        if (cronResponse?.statusCode === 201 && cronResponse?.event === 'ProcessCompleted') {
          pfdto = {};                   
          let sourceStatus
          let ponode = JSON.parse(await this.redisService.getJsonDataWithPath(pfdto.key + 'PO','.mappedData.artifact.node',client))
          if(ponode[1].events.length>0){
          ...
          pfdto['nodeType'] = ponode[1].nodeType
          pfdto['event'] = sourceStatus 
          // pfdto['nodeId'] = temp.nodeId;
          // pfdto['nodeName'] = temp.nodeName
          // pfdto['nodeType'] = temp.nodeType
          // pfdto['event'] = temp.event
        }
