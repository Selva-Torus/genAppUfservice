...
export class TeService implements OnModuleInit, OnModuleDestroy{
  ...
  async startListening(): Promise<void> {   
    ...
    let artifactToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJsb2dpbklkIjoiZ2FtZSIsImNsaWVudCI6IkNUMzA5IiwidHlwZSI6ImMiLCJsb2dUeXBlIjoibW9uZ29kYiIsInNpZCI6IjAxOWVmZGEyLTI3NTUtNGViMy05MjMzLWM2ODA1MjQ1MGU0ZSIsImlhdCI6MTc2ODE5MDkwOCwiZXhwIjoxNzY4MTkyMTA4fQ.KwSQVUDKAu-BiJbvKnOuy1Emf2rvfSOjxDp5P1Pt-Kg';  
          ...
          // let upId = Xid.next();
            ...
            if (pfProcessorResponse?.data) {
              ...
              if (pfResponseData && Object.keys(pfResponseData).length > 0) {
                ...
                console.log(pfResponseData.length);              
      ...
      if(pfdto && (Object.keys(pfdto.data).length >0 || pfdto.data.length > 0)){
        ...
        for(let i=0;i< tempData.length;i++){ 
          let upId =  Xid.next();
            ...
            await this.pfPreProcessor(processedKey, pfs, upId, currentFabric);
            // await this.redisService.setJsonData(key, JSON.stringify(tempData[i]), client, 'response',);
                          ...
                          if (routeArray[r].nodeName == 'End') {
                           ...
                           // await this.redisService.setStreamData(srcQueue, 'TASK - ' + pfdto.upId, JSON.stringify({ PID: pfdto.upId, TID: pfdto.nodeId, EVENT: 'ProcessCompleted' }));
                            ...
                            if (currentFabric == 'PF-PFD' || currentFabric == 'PF-SFD' || currentFabric == 'PF-SCDL') {
                              ...
                              let processedNodes = await this.redisService.getKeys(processedKey + pfdto.upId,client)
                              if(processedNodes?.length>0){
                                for(let n=0;n< processedNodes?.length;n++){                                  
                                  this.redisService.deleteKey(processedNodes[n],client)
                                }
                              } 
                              
                              let processedQueues = await this.redisService.getKeys(client + '_*_ProcessStatus',client)
                              if(processedQueues?.length>0){
                                for(let p=0;p< processedQueues?.length;p++){                                  
                                  this.redisService.deleteKey(processedQueues[p],client)
                                }
                              }  
