...
export class TeService {
     ...
     try {
      ...
      //if(logicCenter || currentFabric == 'PF-PFD')
       ...
       for (let e = 0; e < poNode.length; e++) {
         ...
         if (poNode[e].nodeType != 'startnode' && poNode[e].nodeType != 'endnode' && poNode[e].nodeType != 'schedulernode' && poNode[e].nodeType != 'intervalnode') {
           if (currentFabric == 'PF-PFD' || currentFabric == 'PF-SFD' || currentFabric == 'PF-SCDL') {
             if (poNode[e].events.length > 0) {
               for (let k = 0; k < poNode[e].events.length; k++) {
       ...
       for (var i = 0; i < poNode.length; i++) {
        ...
        if (poNode[i].nodeType == 'startnode' || poNode[i].nodeType == 'schedulernode' || poNode[i].nodeType == 'intervalnode') {
          ...
          if(pfProcessorResponse?.data){
            ...
            if(Array.isArray(pfResponseData) && pfResponseData?.length>0){
              ...
              pfdto = {}
                ...
                pfdto['upId'] = upId
                pfdto['data'] = pfResponseData
                await this.addEventEmitterJob(jobname,pfdto,jobname);
  ...
  async addEventEmitterJob(queueName: string, pfdto: pfDto, jobName: string, options?: JobsOptions) {
      try {
        ...
        if(pfdto && (Object.keys(pfdto.data).length >0 || pfdto.data.length > 0)){
          ...
          for(let i=0;i< tempData.length;i++){   
            ...
            pfdto.data = tempData[i]  

            const job = await queue.add(jobName, pfdto, {
              ...options,
              attempts: 3,
              backoff: {
                type: 'exponential',
              ...
              removeOnComplete: false, // Auto-remove to save memory
              removeOnFail: false,
            });
