...
export class UfController {
  ...
  async getAccessToken(@Body() body: any, @Req() req: any) {
    ...
    const { psCode , selectedAccessProfile , dap ,dpdKey,method, ufClientType } = body;
    ...
    try {
      let result : any = {};
      result["token"] = await this.appService.getAccessToken(token, psCode , selectedAccessProfile , dap , ufClientType);
      if(dpdKey && method){
        result["dpdKey"] = dpdKey
        result["method"] = method
      }
