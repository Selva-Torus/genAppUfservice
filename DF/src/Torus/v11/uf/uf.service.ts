...
export class UfService {
                ...
                if (
                  ...
                  if (
                    ...
                    if (isTable === true) {
                      ...
                      readableControls =
                        componentNameArray.concat(readableControls);
  ...
  async createApiCollection(input, collectionName) {
    try {    
      ...
      if(!tenant || !domain || !collection || !loginId || !schema || !fabric || !endPointCategory) throw 'Invalid Payload';
      var successFlg = 0;
      var methodCount = 0;    
     
      var fabricType = fabric.split('-')[2];
      ...
      if (
        ...
        if (endPointCategory && endPointCategory.length > 0) {
          ...
          for (let i = 0; i < endPointCategory.length; i++) {
            ...
            let replacedEndpointRqst = data?.paths[endPoint][methodName].requestBody
            let replacedEndpointResp = data?.paths[endPoint][methodName].responses

            if(replacedEndpointRqst)
              data.paths[endPoint][methodName].requestBody = JSON.parse((JSON.stringify(replacedEndpointRqst)).toLowerCase())

            if(replacedEndpointResp)
              data.paths[endPoint][methodName].responses = JSON.parse((JSON.stringify(replacedEndpointResp)).toLowerCase())

            let ReqContent = endpointData?.requestBody?.content;
            let ResContent, responseRef;

            if (data?.paths[endPoint][methodName]) {
              var contentJson = JSON.parse(JSON.stringify(data?.paths[endPoint][methodName]));
            ...
            if (fabric && fabric == 'API-APIPD-TORUS') {
              ...
              methodName.toLowerCase() == 'put' ||
              methodName.toLowerCase() == 'delete'
            ) {
              // let apiVersionsArr = await this.apiService.getArtifactVersion(`CK:${tenant}:FNGK:AF:FNK:API-APICD-INT:CATK:${domain}:AFGK:${collection}:AFK:${afkName}`)
              // if(apiVersionsArr?.length>0){
              //   newVersion = Math.max( ...apiVersionsArr.map((item) => parseInt(item.slice(1)))) + 1;
              // }else{
              newVersion = 1;
              //}
              key = `CK:${tenant}:FNGK:AF:FNK:API-APICD-${fabricType}:CATK:${domain}:AFGK:${collection}:AFK:${afkName}:AFVK:v${newVersion}`;
            } else if (methodName.toLowerCase() == 'get') {
              // let apiVersionsArr = await this.apiService.getArtifactVersion(`CK:${tenant}:FNGK:AF:FNK:API-APIPD-INT:CATK:${domain}:AFGK:${collection}:AFK:${afkName}`)
              // if(apiVersionsArr?.length>0){
              //   newVersion = Math.max( ...apiVersionsArr.map((item) => parseInt(item.slice(1)))) + 1;
              // }else{
              newVersion = 1;
              //}
              key = `CK:${tenant}:FNGK:AF:FNK:API-APIPD-${fabricType}:CATK:${domain}:AFGK:${collection}:AFK:${afkName}:AFVK:v${newVersion}`;
            }
                ...
                for (let j = 0; j < propval[i].allOf.length; j++) {
                  if (propval[i].allOf[j].$ref) {                  
                    ...
                    if (data?.components?.[nestedSchema[nestedSchema.length - 2]]?.[nestedSchema[nestedSchema.length - 1]]?.type)
                      dto[requestParameter][propArr[i]] = data?.components?.[nestedSchema[nestedSchema.length - 2]]?.[nestedSchema[nestedSchema.length - 1]]?.type;
                    else 
                      dto[requestParameter][propArr[i]] = 'object';
                ...
                for (let j = 0; j < allOf.length; j++) {
                  ...
                  if (allOf[j].$ref) {
                    ...
                    if (data?.components?.[nestedSchema[nestedSchema.length - 2]]?.[nestedSchema[nestedSchema.length - 1]]?.type){
                      ...
                      ReferenceResponseDto[propertyArr[i]] = data?.components?.[nestedSchema[nestedSchema.length - 2]]?.[nestedSchema[nestedSchema.length - 1]]?.type;
                    }
                    else 
                      ReferenceResponseDto[propertyArr[i]] = 'object';
  ...
  async getResponseModel(
    ...
    try {
      ...
      console.log('getResponseModel Error', error);
      await this.throwCustomException(error);
    }
  }
