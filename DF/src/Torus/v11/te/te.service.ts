...
async EventEmitter(pfdto: pfDto,node?){   
    ...
    try {
      ...
      let dstkey 
          ...
          var currentFabric = await this.teCommonService.splitcommonkey(pfdto.key,'FNK')//pfdto.key.split('FNK')[1].split(':')[1] 
      let fngkKey = await this.teCommonService.splitcommonkey(pfdto.key,'FNGK')//pfdto.key.split('FNGK')[1].split(':')[1]
      ...
      for (var i = 0; i < poNode.length; i++) {
        ...
        let staticQueue = currentFabric == 'PF-PFD' || currentFabric == 'PF-SFD'?'TPH':'TDH'       
        ...
        if (poNode[i].nodeType == 'startnode') {
            ...
            if (poNode[i].nodeType == 'startnode' && pid == undefined) {
              ...
              for (let e = 0; e < poNode[1].events.length; e++) {
                if (poNode[1].events[e].eventType == 'UEH') {
                  ...
                  let sourceid = poNode[1].events[e].sourceId.split('|')[2];
                ...
                if(pfdto.event == poNode[1].events[e].source.status.trim() && sourceId == handlerId){
                  srcStatus = poNode[1].events[e].source.status.trim();
          ...
          if(nodedetails?.status == '200') {             
            if (pfdto.event == null && event == srcStatus) {            
              ...
              //     let parentID = pfjson[x].T_parentId[0]
              //   }
              //  }
              //   for(let y=0;y< pfjson.length;y++){
              //   if(pfjson[y].nodeId == parentID){
              //     let ndname = pfjson[y].nodeName
              //   }
              //  }
              let npvdata;
              ...
              if (pfdto.data) {
                if (Object.keys(pfdto.data).length == 0) {
                  ...
                  //Setting PIDlogs for accessing Data
                   let npvdata = JSON.parse(await this.redisService.getJsonDataWithPath(processedKey + pfdto.upId + ':NPV:'+poNode[i].nodeName+'.PRO','.request',client)) 
                 
                   
                   if(pfdto.data['childData']){
                    await this.redisService.setJsonData(processedKey + pfdto.upId + ':NPV:'+poNode[i].nodeName+'.PRO', JSON.stringify(pfdto.data['childData']), client,'response')
                  ...
                  eventResponse = await firstValueFrom(this.poClient.send(
                    ...
                    new PoEvent(pfdto.key, pfdto.upId, pfdto.event, pfdto.data, pfdto.token, pfdto.nodeId, poNode[i].nodeName, pfdto.nodeType,refflag,page,count),
                  ))
         ...
         // }
          if(currentFabric == 'PF-PFD' || currentFabric == 'PF-SFD'){
                  if (!pfdto.data) {
                    pfdto.data = JSON.parse(await this.redisService.getJsonDataWithPath(processedKey + pfdto.upId + ':NPV:' + poNode[i].nodeName+'_'+poNode[i].nodeId + '.PRO', '.request',client))
                  }else {
                    //if(!(Object.keys(pfdto.data).includes('response'))){
                    await this.redisService.setJsonData(processedKey + pfdto.upId + ':NPV:' + poNode[i].nodeName+'_'+poNode[i].nodeId + '.PRO',JSON.stringify(pfdto.data) ,client,'request')
                  //}
                }
          //}
          }else{
          ...
          if (event == srcstatus) {
            ...
            if (eventResponse == undefined || !eventResponse) { throw 'Event Response is undefined' }
              ...
              if (poNode[i].nodeType != 'endnode') {
                  ...
                  for(let e=0;e < poNode[i].events.length;e++){                  
                    if(event || pfdto.event == poNode[i].events[e].source.status ){
                      ...
                      if(poNode[i].events[e].eventType == 'UEH') {                      
                       ...
                       keyname = ufkey.split(':')  
                       ufname = ((keyname[1]+keyname[5]+keyname[7]+keyname[9]+ keyname[11]+ keyname[13]).replace(/[-_]/g, '')).replace(/\s+/g, '');
                      let handlerid = ((poNode[i].events[e].sourceId).split('|')[2])
                      if(handlerid.includes('/')){
                        sourceId = ((handlerid.split('/'))[handlerid.split('/').length-1]).replaceAll('.','') 
                     }else{
                       sourceId = handlerid.replaceAll('.','')
                ...
                if (event == srcStatus) {
                  ...
                  if(mergearr && mergearr.length > 0 && currentFabric == 'DF-DFD'){  
                    ...
                    await this.teCommonService.getTPL(processedKey,pfdto.upId,poNode[i],'Success',pfdto.token,currentFabric,event)//apiUrl,dfoSchema
                  }
                  else{
                    ...
                    if (currentFabric == 'DF-DFD') { 
                      ...
                      await this.teCommonService.getTPL(processedKey,pfdto.upId,poNode[i],'Success',pfdto.token,currentFabric,event)//apiUrl,dfoSchema
                        ...
                        eventResponse = await firstValueFrom(this.poClient.send(
                          ufname+'_'+poNode[i].nodeId+'_'+sourceId+'_'+event,
                          new PoEvent(pfdto.key, pfdto.upId, event, pfdto.data, pfdto.token, pfdto.nodeId, poNode[i].nodeName, pfdto.nodeType,refflag,page,count),
                        ))
                  ...
                  for (let s = 0; s < getNodeResponse.length; s++) {
                    if (getNodeResponse[s].nodeId == pfdto.nodeId) {
                      console.log('name',processedKey + pfdto.upId + ':nodeResponse');
     ...
     console.log('PO ERROR:', error);     
      if(pfdto.upId){       
        let secError = await this.teCommonService.getTPL(processedKey, pfdto.upId,nodeInfo, 'Failed', pfdto.token,currentFabric, '',pfdto.data,error)    
        console.log('s-logtime', new Date()) 
        //await this.teCommonService.prcLog(tenant+'-'+app+'-TPL')   
        console.log('E-logtime', new Date())      
        // throw new BadRequestException(secError)
        throw new BadRequestException({key:pfdto.key,upId:pfdto.upId,eventError:secError})
      }else{
        
        let tslerror = await this.teCommonService.getTSL(pfdto.key,pfdto.token,error,'')    
        console.log('s-logtime', new Date())  
