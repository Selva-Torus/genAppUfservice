...
@Injectable()
export class vgph_source_corporateService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphsc_id }: {vgphsc_id : bigint} = queryValue;
      ...
      if(vgphsc_id){ 
        query.vgphsc_id = { [queryCondition['vgphsc_id']]: vgphsc_id };
      }
      if(uuid){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_source_corporate.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_source_corporate');
      ...
      this.prismaService.vgph_source_corporate.count({
        where: query,
      }));
...
,vgphsc_id?:number) {
    try{
      ...
      if (vgphsc_id) {
        whereClause.vgphsc_id = vgphsc_id;
      }
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_corporate.findMany({ 
      where: whereClause,
  ...
  async create(createvgph_source_corporateDto: Prisma.vgph_source_corporateCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            vgphsc_id :v.pipe(v.number(),v.maxValue(9999999999999999999999999999999999999999999999999999999999999999 )) , 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_source_corporateDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_source_corporateDto, 'vgph_source_corporate', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_source_corporate.create({
          data: encryptedData,
          select:{vgphsc_id:true,uuid:true,source_category:true,source_reference:true,source_name:true,channel_name:true,channel_reference:true,product_basic:true,product_additional:true,request_data:true,response_data:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_source_corporateDto: Prisma.vgph_source_corporateCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_corporate creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_corporate creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_source_corporate',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphsc_id',
        ...
        return {
          success: true,
          message: 'vgph_source_corporate creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphsc_id:number, updatevgph_source_corporateDto: Prisma.vgph_source_corporateUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_source_corporateDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_source_corporateDto,'vgph_source_corporate','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_corporate.updateMany({
      where: {vgphsc_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgphsc_id:number,
    updatevgph_source_corporateDto: Prisma.vgph_source_corporateUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphsc_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_source_corporate update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_corporate update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_source_corporate update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphsc_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_corporate.deleteMany({
      where: {vgphsc_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_source_corporate');
    let decryptedRes: any = [];
...
vgphsc_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphsc_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_source_corporate deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_corporate deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_source_corporate deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_corporate.findFirst({ 
