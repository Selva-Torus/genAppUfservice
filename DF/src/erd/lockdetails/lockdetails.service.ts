...
@Injectable()
export class lockdetailsService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      const { lockedby }: {lockedby : string} = queryValue;
      const { mode }: {mode : string} = queryValue;
      const { status }: {status : string} = queryValue;
      const { expires }: {expires : string} = queryValue;

      if(id){ 
        query.id = { [queryCondition['id']]: id };
      }
      if(lockedby){ 
        query.lockedby = { [queryCondition['lockedby']]: lockedby };
      }
      if(mode){ 
      ...
      if(expires){ 
        query.expires = { [queryCondition['expires']]: expires };
      }
      const skip = (page - 1) * limit;
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.lockdetails.findMany({
          select:columns,
          where: query,          
        });
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'lockdetails');
      ...
      const totalItems = await this.prismaService.lockdetails.count({
        where: query,
      });
  ...
  async findOne(id:number,token : string) {
    try{
      const res = await this.prismaService.lockdetails.findUnique({ 
      where: {id},
      select: {lockedby:true,mode:true,status:true,expires:true,        trs_created_date:true,
        trs_created_by:true,
    ...
    return  await this.decryptData(res, 'lockdetails');
  } catch (error) {
...
,id?:number) {
    try{
      ...
      const res = await this.prismaService.lockdetails.findMany({ 
      where: whereClause,
      select: {id:true,lockedby:true,mode:true,status:true,expires:true,        trs_created_date:true,
        trs_created_by:true,
        trs_modified_date:true,
        trs_modified_by:true,
        trs_next_status:true,
  ...
  async create(createlockdetailsDto: Prisma.lockdetailsCreateInput,token:string) {
    try{
      ...
      const dataSchema:any =  v.object({
        ...
        let validate : any = v.safeParse(dataSchema,createlockdetailsDto);
        if (!validate.success) {
   ...
   */
  async createMaster(
    createlockdetailsDto: Prisma.lockdetailsCreateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'lockdetails creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'lockdetails creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      const encryptedData = await this.encryptData(createlockdetailsDto, 'lockdetails', 'create');
      ...
      if(role === 'MAKER')
      {
        ...
        return {
          success: true,
          message: 'lockdetails creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(id:number, updatelockdetailsDto: Prisma.lockdetailsUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updatelockdetailsDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const res = await this.prismaService.lockdetails.update({
      where: {id},
      data: await this.encryptData(updatelockdetailsDto,'lockdetails','update'),
      select: {id:true,lockedby:true,mode:true,status:true,expires:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return await this.decryptData(res, 'lockdetails');
    } catch (error) {
...
/**
  ...
  async updateMaster(
id:number,
    updatelockdetailsDto: Prisma.lockdetailsUpdateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'lockdetails update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'lockdetails update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      const existingRecord = await this.prismaService.lockdetails.findUnique({
        where: {id}
      });
      ...
      // Encrypt data if needed
      const encryptedData = await this.encryptData(updatelockdetailsDto, 'lockdetails', 'update');
      ...
      return {
        success: true,
        message: 'lockdetails update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(id:number,token : string) {
    try{
      const res = await this.prismaService.lockdetails.delete({
      where: {id },
      select: {id:true,lockedby:true,mode:true,status:true,expires:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return res;
...
id:number,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'lockdetails deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'lockdetails deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'lockdetails deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.lockdetails.findFirst({ 
