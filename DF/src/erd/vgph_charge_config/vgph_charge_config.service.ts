...
@Injectable()
export class vgph_charge_configService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphcc_id }: {vgphcc_id : bigint} = queryValue;
      const { channel_name }: {channel_name : string} = queryValue;
      const { cust_segment }: {cust_segment : string} = queryValue;
      const { customer_type }: {customer_type : string} = queryValue;
      const { cif_number }: {cif_number : string} = queryValue;
      ...
      const { charge_borne_by }: {charge_borne_by : string} = queryValue;
      const { charge_gl_ac_no }: {charge_gl_ac_no : string} = queryValue;
      const { vat_gl_ac_no }: {vat_gl_ac_no : string} = queryValue;
      const { start_active_date }: {start_active_date :  Date} = queryValue;
      const { end_active_date }: {end_active_date :  Date} = queryValue;
      ...
      if(vgphcc_id){ 
        query.vgphcc_id = { [queryCondition['vgphcc_id']]: vgphcc_id };
      }
      if(channel_name){ 
        query.channel_name = { [queryCondition['channel_name']]: channel_name };
      }
      if(cust_segment){ 
        query.cust_segment = { [queryCondition['cust_segment']]: cust_segment };
      }
      if(customer_type){ 
        query.customer_type = { [queryCondition['customer_type']]: customer_type };
      }
      if(cif_number){ 
        query.cif_number = { [queryCondition['cif_number']]: cif_number };
      }
      ...
      if(charge_borne_by){ 
        query.charge_borne_by = { [queryCondition['charge_borne_by']]: charge_borne_by };
      }
      if(charge_gl_ac_no){ 
        query.charge_gl_ac_no = { [queryCondition['charge_gl_ac_no']]: charge_gl_ac_no };
      }
      if(vat_gl_ac_no){ 
        query.vat_gl_ac_no = { [queryCondition['vat_gl_ac_no']]: vat_gl_ac_no };
      }
      if(start_active_date){ 
        query.start_active_date = { [queryCondition['start_active_date']]: start_active_date };
      }
      if(end_active_date){ 
        query.end_active_date = { [queryCondition['end_active_date']]: end_active_date };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_charge_config.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_charge_config');
      ...
      this.prismaService.vgph_charge_config.count({
        where: query,
      }));
...
,vgphcc_id?:number) {
    try{
      ...
      if (vgphcc_id) {
        whereClause.vgphcc_id = vgphcc_id;
      }
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_charge_config.findMany({ 
      where: whereClause,
  ...
  async create(createvgph_charge_configDto: Prisma.vgph_charge_configCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            vgphcc_id :v.number() , 
            channel_name :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            cust_segment :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            customer_type :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            cif_number :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            customer_account_no :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            message_code :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            charge_code :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            charge_borne_by :  v.optional(v.pipe(v.string(),v.maxLength(8 ))), 
            charge_gl_ac_no :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            vat_gl_ac_no :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            start_active_date :(v.pipe(v.string(),v.isoDate()))  , 
            end_active_date :(v.pipe(v.string(),v.isoDate()))  , 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_charge_configDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_charge_configDto, 'vgph_charge_config', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_charge_config.create({
          data: encryptedData,
          select:{vgphcc_id:true,channel_name:true,cust_segment:true,customer_type:true,cif_number:true,customer_account_no:true,message_code:true,charge_code:true,charge_borne_by:true,charge_gl_ac_no:true,vat_gl_ac_no:true,start_active_date:true,end_active_date:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_charge_configDto: Prisma.vgph_charge_configCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_charge_config creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_charge_config creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_charge_config',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphcc_id',
        ...
        return {
          success: true,
          message: 'vgph_charge_config creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphcc_id:number, updatevgph_charge_configDto: Prisma.vgph_charge_configUpdateInput,token:string) {   
    try{
      ...
      const dataSchema:any =  v.object({
          ...
          charge_gl_ac_no :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
          vat_gl_ac_no :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
          start_active_date :  v.optional((v.any())), 
          end_active_date :  v.optional((v.any())), 
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_charge_configDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_charge_configDto,'vgph_charge_config','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_charge_config.updateMany({
      where: {vgphcc_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgphcc_id:number,
    updatevgph_charge_configDto: Prisma.vgph_charge_configUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphcc_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_charge_config update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_charge_config update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_charge_config update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphcc_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_charge_config.deleteMany({
      where: {vgphcc_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_charge_config');
    let decryptedRes: any = [];
...
vgphcc_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphcc_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_charge_config deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_charge_config deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_charge_config deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_charge_config.findFirst({ 
