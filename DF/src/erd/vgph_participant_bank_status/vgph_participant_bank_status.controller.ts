...
export class vgph_participant_bank_statusController {
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.vgph_participant_bank_statusService.findAllmethod(query, +limit,selectColumns,token);
  }
  ...
  @Post()
  @UsePipes(new PrismaModelValidationPipe('vgph_participant_bank_status'))
  @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    @Body() createvgph_participant_bank_statusDto: Prisma.vgph_participant_bank_statusCreateInput,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_participant_bank_statusService.createMaster(createvgph_participant_bank_statusDto, makerInfo, token);
      return result;
    }

    const result = this.vgph_participant_bank_statusService.create(createvgph_participant_bank_statusDto,token);
    return plainToInstance(vgph_participant_bank_statusEntity, result);
  }
...
@Param('vgphbpbs_id') vgphbpbs_id:number,
    @Body() updatevgph_participant_bank_statusDto: Prisma.vgph_participant_bank_statusUpdateInput,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.vgph_participant_bank_statusService.updateMaster(+vgphbpbs_id,updatevgph_participant_bank_statusDto,makerInfo,token);
      return result;
    }
...
@Param('vgphbpbs_id') vgphbpbs_id:number,
    ...
    if (mcRole && mcUsername) {
      const makerInfo = { role: mcRole, username: mcUsername, remarks: mcRemarks,approvalStatus: mcApprovalStatus };
      const result = await this.vgph_participant_bank_statusService.deleteMaster(+vgphbpbs_id,makerInfo,token);
      return result;
    }
