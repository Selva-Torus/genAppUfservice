...
@Injectable()
export class vgph_participant_bank_statusService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphbpbs_id }: {vgphbpbs_id : bigint} = queryValue;
      const { bank_name }: {bank_name : string} = queryValue;
      const { bank_code }: {bank_code : string} = queryValue;
      const { bank_short_code }: {bank_short_code : string} = queryValue;
      const { bic_code }: {bic_code : string} = queryValue;
      const { availability_flag }: {availability_flag : string} = queryValue;
      const { from_date_time }: {from_date_time :  Date} = queryValue;
      const { to_date_time }: {to_date_time :  Date} = queryValue;
      const { status_code }: {status_code : string} = queryValue;
      const { send_receive }: {send_receive : string} = queryValue;
      const { receive_only }: {receive_only : string} = queryValue;
      const { request_for_pay }: {request_for_pay : string} = queryValue;
      ...
      if(vgphbpbs_id){ 
        query.vgphbpbs_id = { [queryCondition['vgphbpbs_id']]: vgphbpbs_id };
      }
      if(bank_name){ 
        query.bank_name = { [queryCondition['bank_name']]: bank_name };
      }
      if(bank_code){ 
        query.bank_code = { [queryCondition['bank_code']]: bank_code };
      }
      if(bank_short_code){ 
      ...
      if(availability_flag){ 
        query.availability_flag = { [queryCondition['availability_flag']]: availability_flag };
      }
      if(from_date_time){ 
        query.from_date_time = { [queryCondition['from_date_time']]: from_date_time };
      }
      if(to_date_time){ 
        query.to_date_time = { [queryCondition['to_date_time']]: to_date_time };
      }
      if(status_code){ 
        query.status_code = { [queryCondition['status_code']]: status_code };
      }
      if(send_receive){ 
        query.send_receive = { [queryCondition['send_receive']]: send_receive };
      }
      if(receive_only){ 
        query.receive_only = { [queryCondition['receive_only']]: receive_only };
      }
      if(request_for_pay){ 
        query.request_for_pay = { [queryCondition['request_for_pay']]: request_for_pay };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_participant_bank_status.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_participant_bank_status');
      ...
      this.prismaService.vgph_participant_bank_status.count({
        where: query,
      }));
  ...
  async create(createvgph_participant_bank_statusDto: Prisma.vgph_participant_bank_statusCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            vgphbpbs_id :v.pipe(v.number(),v.maxValue(9999999999999999999999999999999999999999999999999999999999999999 )) , 
            bank_name :v.pipe(v.string(),v.maxLength(128 )) , 
            bank_code :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            bank_short_code :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            bic_code :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            availability_flag :  v.optional(v.pipe(v.string(),v.maxLength(1 ))), 
            from_date_time :(v.pipe(v.string(),v.isoDate()))  , 
            to_date_time :(v.pipe(v.string(),v.isoDate()))  , 
            status_code :v.pipe(v.string(),v.maxLength(32 )) , 
            send_receive :v.pipe(v.string(),v.maxLength(1 )) , 
            receive_only :v.pipe(v.string(),v.maxLength(1 )) , 
            request_for_pay :v.pipe(v.string(),v.maxLength(1 )) , 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_participant_bank_statusDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_participant_bank_statusDto, 'vgph_participant_bank_status', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_participant_bank_status.create({
          data: encryptedData,
          select:{vgphbpbs_id:true,bank_name:true,bank_code:true,bank_short_code:true,bic_code:true,availability_flag:true,from_date_time:true,to_date_time:true,status_code:true,send_receive:true,receive_only:true,request_for_pay:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_participant_bank_statusDto: Prisma.vgph_participant_bank_statusCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_participant_bank_status creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_participant_bank_status creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_participant_bank_status',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphbpbs_id',
        ...
        return {
          success: true,
          message: 'vgph_participant_bank_status creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphbpbs_id:number, updatevgph_participant_bank_statusDto: Prisma.vgph_participant_bank_statusUpdateInput,token:string) {   
    try{
      ...
      const dataSchema:any =  v.object({
          ...
          bic_code :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
          availability_flag :  v.optional(v.pipe(v.string(),v.maxLength(1 ))), 
          from_date_time :  v.optional((v.any())), 
          to_date_time :  v.optional((v.any())), 
          status_code :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
          send_receive :  v.optional(v.pipe(v.string(),v.maxLength(1 ))), 
          receive_only :  v.optional(v.pipe(v.string(),v.maxLength(1 ))), 
          request_for_pay :  v.optional(v.pipe(v.string(),v.maxLength(1 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_participant_bank_statusDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_participant_bank_statusDto,'vgph_participant_bank_status','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_participant_bank_status.updateMany({
      where: {vgphbpbs_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgphbpbs_id:number,
    updatevgph_participant_bank_statusDto: Prisma.vgph_participant_bank_statusUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphbpbs_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_participant_bank_status update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_participant_bank_status update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_participant_bank_status update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphbpbs_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_participant_bank_status.deleteMany({
      where: {vgphbpbs_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_participant_bank_status');
    let decryptedRes: any = [];
...
vgphbpbs_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphbpbs_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_participant_bank_status deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_participant_bank_status deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_participant_bank_status deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_participant_bank_status.findFirst({ 
