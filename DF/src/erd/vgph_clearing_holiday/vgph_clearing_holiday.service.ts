...
@Injectable()
export class vgph_clearing_holidayService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgph_chid }: {vgph_chid : bigint} = queryValue;
      const { currency }: {currency : string} = queryValue;
      const { holiday_code }: {holiday_code : string} = queryValue;
      const { holiday_date }: {holiday_date : any } = queryValue;
      const { description }: {description : string} = queryValue;
      const { holiday_flag }: {holiday_flag : string} = queryValue;
      const { additional_info }: {additional_info : any } = queryValue;
      ...
      if(vgph_chid){ 
        query.vgph_chid = { [queryCondition['vgph_chid']]: vgph_chid };
      }
      if(currency){ 
        query.currency = { [queryCondition['currency']]: currency };
      }
      if(holiday_code){ 
        query.holiday_code = { [queryCondition['holiday_code']]: holiday_code };
      }
      if(holiday_date){ 
        query.holiday_date = { [queryCondition['holiday_date']]: holiday_date };
      }
      if(description){ 
        query.description = { [queryCondition['description']]: description };
      }
      if(holiday_flag){ 
        query.holiday_flag = { [queryCondition['holiday_flag']]: holiday_flag };
      }
      if(additional_info){ 
        query.additional_info = { [queryCondition['additional_info']]: additional_info };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_clearing_holiday.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_clearing_holiday');
      ...
      this.prismaService.vgph_clearing_holiday.count({
        where: query,
      }));
  ...
  async create(createvgph_clearing_holidayDto: Prisma.vgph_clearing_holidayCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            vgph_chid :v.pipe(v.number(),v.maxValue(9999999999999999999999999999999999999999999999999999999999999999 )) , 
            currency :  v.optional(v.pipe(v.string(),v.maxLength(3 ))), 
            holiday_code :  v.optional(v.pipe(v.string(),v.maxLength(8 ))), 
            holiday_date :  v.optional(v.pipe(
                  v.string(),
                  v.regex(
            ...
            description :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            holiday_flag :  v.optional(v.pipe(v.string(),v.maxLength(1 ))), 
            additional_info :  v.optional(v.any() ), 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_clearing_holidayDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_clearing_holidayDto, 'vgph_clearing_holiday', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_clearing_holiday.create({
          data: encryptedData,
          select:{vgph_chid:true,currency:true,holiday_code:true,holiday_date:true,description:true,holiday_flag:true,additional_info:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_clearing_holidayDto: Prisma.vgph_clearing_holidayCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_clearing_holiday creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_clearing_holiday creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_clearing_holiday',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgph_chid',
        ...
        return {
          success: true,
          message: 'vgph_clearing_holiday creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgph_chid:number, updatevgph_clearing_holidayDto: Prisma.vgph_clearing_holidayUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_clearing_holidayDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_clearing_holidayDto,'vgph_clearing_holiday','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_clearing_holiday.updateMany({
      where: {vgph_chid},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgph_chid:number,
    updatevgph_clearing_holidayDto: Prisma.vgph_clearing_holidayUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgph_chid;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_clearing_holiday update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_clearing_holiday update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_clearing_holiday update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgph_chid:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_clearing_holiday.deleteMany({
      where: {vgph_chid}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_clearing_holiday');
    let decryptedRes: any = [];
...
vgph_chid:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgph_chid;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_clearing_holiday deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_clearing_holiday deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_clearing_holiday deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_clearing_holiday.findFirst({ 
