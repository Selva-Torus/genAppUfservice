...
@Injectable()
export class vgph_beneficiaryService {
  ...
  async findSchema (token) {
    const data = {
            ...
            beneficiary_common:"json",
            beneficiary_address:"json",
            beneficiary_bank:"json",
            beneficiary_identity:"json",
            beneficiary_compliance:"json",
            beneficiary_preference:"json",
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphb_id }: {vgphb_id : bigint} = queryValue;
      const { beneficiary_category }: {beneficiary_category : string} = queryValue;
      const { beneficiary_id }: {beneficiary_id : string} = queryValue;
      const { beneficiary_common }: {beneficiary_common : any } = queryValue;
      const { beneficiary_address }: {beneficiary_address : any } = queryValue;
      const { beneficiary_bank }: {beneficiary_bank : any } = queryValue;
      const { beneficiary_identity }: {beneficiary_identity : any } = queryValue;
      const { beneficiary_compliance }: {beneficiary_compliance : any } = queryValue;
      const { beneficiary_preference }: {beneficiary_preference : any } = queryValue;
      ...
      if(vgphb_id){ 
        query.vgphb_id = { [queryCondition['vgphb_id']]: vgphb_id };
      }
      if(beneficiary_category){ 
        query.beneficiary_category = { [queryCondition['beneficiary_category']]: beneficiary_category };
      }
      if(beneficiary_id){ 
        query.beneficiary_id = { [queryCondition['beneficiary_id']]: beneficiary_id };
      }
      if(beneficiary_common){ 
        query.beneficiary_common = { [queryCondition['beneficiary_common']]: beneficiary_common };
      }
      if(beneficiary_address){ 
        query.beneficiary_address = { [queryCondition['beneficiary_address']]: beneficiary_address };
      }
      if(beneficiary_bank){ 
        query.beneficiary_bank = { [queryCondition['beneficiary_bank']]: beneficiary_bank };
      }
      if(beneficiary_identity){ 
        query.beneficiary_identity = { [queryCondition['beneficiary_identity']]: beneficiary_identity };
      }
      if(beneficiary_compliance){ 
        query.beneficiary_compliance = { [queryCondition['beneficiary_compliance']]: beneficiary_compliance };
      }
      if(beneficiary_preference){ 
        query.beneficiary_preference = { [queryCondition['beneficiary_preference']]: beneficiary_preference };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_beneficiary.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_beneficiary');
      ...
      this.prismaService.vgph_beneficiary.count({
        where: query,
      }));
  ...
  async create(createvgph_beneficiaryDto: Prisma.vgph_beneficiaryCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            vgphb_id :v.number() , 
            beneficiary_category :v.pipe(v.string(),v.maxLength(32 )) , 
            beneficiary_id :v.pipe(v.string(),v.maxLength(64 )) , 
            beneficiary_common :  v.optional(v.any() ), 
            beneficiary_address :  v.optional(v.any() ), 
            beneficiary_bank :  v.optional(v.any() ), 
            beneficiary_identity :  v.optional(v.any() ), 
            beneficiary_compliance :  v.optional(v.any() ), 
            beneficiary_preference :  v.optional(v.any() ), 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_beneficiaryDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_beneficiaryDto, 'vgph_beneficiary', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_beneficiary.create({
          data: encryptedData,
          select:{vgphb_id:true,beneficiary_category:true,beneficiary_id:true,beneficiary_common:true,beneficiary_address:true,beneficiary_bank:true,beneficiary_identity:true,beneficiary_compliance:true,beneficiary_preference:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_beneficiaryDto: Prisma.vgph_beneficiaryCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_beneficiary creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_beneficiary creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_beneficiary',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphb_id',
        ...
        return {
          success: true,
          message: 'vgph_beneficiary creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphb_id:number, updatevgph_beneficiaryDto: Prisma.vgph_beneficiaryUpdateInput,token:string) {   
    try{

      const dataSchema:any =  v.object({
          beneficiary_category :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
          beneficiary_id :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
          beneficiary_common :  v.optional(v.any()), 
          beneficiary_address :  v.optional(v.any()), 
          beneficiary_bank :  v.optional(v.any()), 
          beneficiary_identity :  v.optional(v.any()), 
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_beneficiaryDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_beneficiaryDto,'vgph_beneficiary','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_beneficiary.updateMany({
      where: {vgphb_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgphb_id:number,
    updatevgph_beneficiaryDto: Prisma.vgph_beneficiaryUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphb_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_beneficiary update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_beneficiary update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_beneficiary update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphb_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_beneficiary.deleteMany({
      where: {vgphb_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_beneficiary');
    let decryptedRes: any = [];
...
vgphb_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphb_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_beneficiary deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_beneficiary deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_beneficiary deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_beneficiary.findFirst({ 
