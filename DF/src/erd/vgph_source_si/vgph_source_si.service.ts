...
@Injectable()
export class vgph_source_siService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphssi_id }: {vgphssi_id : bigint} = queryValue;
      ...
      if(vgphssi_id){ 
        query.vgphssi_id = { [queryCondition['vgphssi_id']]: vgphssi_id };
      }
      if(uuid){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_source_si.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_source_si');
      ...
      this.prismaService.vgph_source_si.count({
        where: query,
      }));
...
,vgphssi_id?:number) {
    try{
      ...
      if (vgphssi_id) {
        whereClause.vgphssi_id = vgphssi_id;
      }
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_si.findMany({ 
      where: whereClause,
  ...
  async create(createvgph_source_siDto: Prisma.vgph_source_siCreateInput,token:string) {
    try{

      enum direction_vgph_source_si{
        OUTBOUND="OUTBOUND",
        INBOUND="INBOUND",
      }
      enum process_type_vgph_source_si{
        OP="OP",
        IR="IR",
        IP="IP",
        OR="OR",
      ...
      const dataSchema:any =  v.object({
            vgphssi_id :v.pipe(v.number(),v.maxValue(9999999999999999999999999999999999999999999999999999999999999999 )) , 
            uuid :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            direction :v.enum(direction_vgph_source_si,"Invalid direction_vgph_source_si enum")  , 
            process_type :v.enum(process_type_vgph_source_si,"Invalid process_type_vgph_source_si enum")  , 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_source_siDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_source_siDto, 'vgph_source_si', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_source_si.create({
          data: encryptedData,
          select:{vgphssi_id:true,uuid:true,direction:true,process_type:true,tran_category:true,source_category:true,source_reference:true,source_name:true,channel_name:true,channel_reference:true,product_basic:true,product_additional:true,request_data:true,response_data:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_source_siDto: Prisma.vgph_source_siCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_si creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_si creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_source_si',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphssi_id',
        ...
        return {
          success: true,
          message: 'vgph_source_si creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphssi_id:number, updatevgph_source_siDto: Prisma.vgph_source_siUpdateInput,token:string) {   
    try{
      ...
      const dataSchema:any =  v.object({
          uuid :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
          direction :  v.optional(v.enum(direction_vgph_source_si,"Invalid direction_vgph_source_si enum")), 
          process_type :  v.optional(v.enum(process_type_vgph_source_si,"Invalid process_type_vgph_source_si enum")), 
          tran_category :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
          source_category :  v.optional(v.pipe(v.string(),v.maxLength(8 ))), 
          source_reference :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
          source_name :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_source_siDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_source_siDto,'vgph_source_si','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_si.updateMany({
      where: {vgphssi_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgphssi_id:number,
    updatevgph_source_siDto: Prisma.vgph_source_siUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphssi_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_source_si update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_si update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_source_si update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphssi_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_si.deleteMany({
      where: {vgphssi_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_source_si');
    let decryptedRes: any = [];
...
vgphssi_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphssi_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_source_si deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_si deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_source_si deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_si.findFirst({ 
