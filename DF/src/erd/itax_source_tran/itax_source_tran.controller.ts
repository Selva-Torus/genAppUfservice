...
import { itax_source_tranService } from './itax_source_tran.service';
...
//import { CreateItaxSourceTranDto } from '../prisma/dto/create-itaxSourceTran.dto';
//import { UpdateItaxSourceTranDto } from '../prisma/dto/update-itaxSourceTran.dto';
import { Createitax_source_tranDto } from './dto/Createitax_source_tran.dto';
import { Updateitax_source_tranDto } from './dto/Updateitax_source_tran.dto';
import { plainToInstance } from 'class-transformer';
import { UfService } from 'src/Torus/v1/uf/uf.service';

 
@Controller('itax_source_tran')
@ApiTags('ERD API')
export class itax_source_tranController {
  constructor(
    private readonly itax_source_tranService: itax_source_tranService,
    private readonly ufservice: UfService
  ) {}

  @Get("/schema")
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: itax_source_tranEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the itax_source_tran table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.itax_source_tranService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: itax_source_tranEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the itax_source_tran table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.itax_source_tranService.findAllmethod(query, +limit,selectColumns,token);
  }

  @Get(':itaxst_id')
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'itaxst_id',type:Number})
  @ApiOkResponse({ type: itax_source_tranEntity })
  @ApiOperation({
    summary: 'Fetch the only one record',
    description: 'Read only one records from the itax_source_tran table',
  })
  
  async findOne(@Headers() authHeader: string,@Param('itaxst_id') itaxst_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = this.itax_source_tranService.findOne(+itaxst_id,token);
    return plainToInstance(itax_source_tranEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the itax_source_tran table',
  })
  ...
  async findAll(@Headers() authHeader: string,@Req() req: any,@Query("trs_created_date") trs_created_date?: Date,@Query("trs_created_by") trs_created_by?: string,@Query("trs_modified_date") trs_modified_date?: Date,@Query("trs_modified_by") trs_modified_by?: string,@Query("trs_process_id") trs_process_id?: string,@Query("trs_access_profile") trs_access_profile?: string,@Query("trs_org_grp_code") trs_org_grp_code?: string,@Query("trs_org_code") trs_org_code?: string,@Query("trs_role_grp_code") trs_role_grp_code?: string,@Query("trs_role_code") trs_role_code?: string,@Query("trs_ps_grp_code") trs_ps_grp_code?: string,@Query("trs_ps_code") trs_ps_code?: string,@Query("trs_sub_org_grp_code") trs_sub_org_grp_code?: string,@Query("trs_sub_org_code") trs_sub_org_code?: string,@Query("trs_locked_by") trs_locked_by?: string,@Query("trs_locked_time") trs_locked_time?: Date,@Query("trs_tenant_id") trs_tenant_id?: string,@Query("trs_app_code") trs_app_code?: string,@Query("trs_product_code") trs_product_code?: string,@Query("trs_event_process_status") trs_event_process_status?: string,@Query("trs_event_status") trs_event_status?: string,@Query("trs_prev_process_code") trs_prev_process_code?: string,@Query("trs_prev_status") trs_prev_status?: string,@Query("trs_prev_process_status") trs_prev_process_status?: string,@Query("trs_process_code") trs_process_code?: string,@Query("trs_status") trs_status?: string,@Query("trs_process_status") trs_process_status?: string,@Query("trs_next_process_code") trs_next_process_code?: string,@Query("trs_next_status") trs_next_status?: string,@Query("trs_next_process_status") trs_next_process_status?: string,@Query() query?: Record<string, any>) {
    ...
    const result = this.itax_source_tranService.findAll(token,trs_created_date,trs_created_by,trs_modified_date,trs_modified_by,trs_process_id,trs_access_profile,trs_org_grp_code,trs_org_code,trs_role_grp_code,trs_role_code,trs_ps_grp_code,trs_ps_code,trs_sub_org_grp_code,trs_sub_org_code,trs_locked_by,trs_locked_time,trs_tenant_id,trs_app_code,trs_product_code,trs_event_process_status,trs_event_status,trs_prev_process_code,trs_prev_status,trs_prev_process_status,trs_process_code,trs_status,trs_process_status,trs_next_process_code,trs_next_status,trs_next_process_status,);
    return plainToInstance(itax_source_tranEntity, result);
  } 
  ...
  @ApiBody({ type: Createitax_source_tranDto })
  @ApiCreatedResponse({ type: itax_source_tranEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the itax_source_tran table',
  })
  ...
  async create(
    ...
    @Body() createitax_source_tranDto: Prisma.itax_source_tranCreateInput,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.itax_source_tranService.createMaster(createitax_source_tranDto, makerInfo, token);
      return result;
    }

    const result = this.itax_source_tranService.create(createitax_source_tranDto,token);
    return plainToInstance(itax_source_tranEntity, result);
  }
  ...
  @ApiHeader({ name: 'xCdcaRemarks', required: false })
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiBody({ type: Updateitax_source_tranDto })
  @ApiOkResponse({ type: itax_source_tranEntity })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the itax_source_tran table',
  })
  ...
  async update(
    ...
    @Headers('xCdcaApprovalStatus') mcApprovalStatus: string,
    @Headers() authHeader: string,
@Param('itaxst_id') itaxst_id:number,
    @Body() updateitax_source_tranDto: Prisma.itax_source_tranUpdateInput,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.itax_source_tranService.updateMaster(+itaxst_id,updateitax_source_tranDto,makerInfo,token);
      return result;
    }

    const result = this.itax_source_tranService.update(+itaxst_id,updateitax_source_tranDto,token);
    return plainToInstance(itax_source_tranEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the itax_source_tran table',
  })
...
@Param('itaxst_id') itaxst_id:number,
    ...
    if (mcRole && mcUsername) {
      const makerInfo = { role: mcRole, username: mcUsername, remarks: mcRemarks,approvalStatus: mcApprovalStatus };
      const result = await this.itax_source_tranService.deleteMaster(+itaxst_id,makerInfo,token);
      return result;
    }

    const result =  this.itax_source_tranService.remove(+itaxst_id,token);
    return plainToInstance(itax_source_tranEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the itax_source_tran table',
  })
  
  async findFirst(@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = this.itax_source_tranService.findFirst(token);
    return plainToInstance(itax_source_tranEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the itax_source_tran table',
  })
  
  async findLast(@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = this.itax_source_tranService.findLast(token);
    return plainToInstance(itax_source_tranEntity, result);
  }
}