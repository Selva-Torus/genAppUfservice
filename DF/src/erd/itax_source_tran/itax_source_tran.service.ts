...
import { itax_source_tranEntity } from './entity/itax_source_tran.entity';
import { CustomException } from 'src/customException';
@Injectable()
export class itax_source_tranService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { itaxst_id }: {itaxst_id : number} = queryValue;
      ...
      if(itaxst_id){ 
        query.itaxst_id = { [queryCondition['itaxst_id']]: itaxst_id };
      }
      if(tran_category){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.itax_source_tran.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'itax_source_tran');
      ...
      this.prismaService.itax_source_tran.count({
        where: query,
      }));
  ...
  async findOne(itaxst_id:number,token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.itax_source_tran.findUnique({ 
      where: {itaxst_id},
      select: {itaxst_id:true,tran_category:true,tran_date:true,tran_reference:true,product_basic:true,itaxs_id :true,            itax_tran_log:{
              select:{
              itaxtl_id:true,              tran_category:true,              processing_system:true,              request_data:true,              response_data:true,              retry_flag:true,              retry_count:true            ,
          ...
          trs_next_process_status:true
            ...
            itax_tran_error_log:{
              select:{
              itaxtel_id:true,              tran_category:true,              error_cateogry:true,              error_code:true,              error_desc:true,              error_data:true            ,
          trs_created_date:true,
          trs_created_by:true,
          trs_modified_date:true,
          trs_modified_by:true,
          ...
          trs_next_process_status:true
            ...
            itax_source_tran_doc:{
              select:{
              itaxstd_id:true,              category:true,              doc_type:true,              doc_group:true,              doc_name:true,              url:true,              doc_additional:true,              doc_content_text:true,              doc_content_byte:true            ,
          trs_created_date:true,
          trs_created_by:true,
          trs_modified_date:true,
          trs_modified_by:true,
          ...
          trs_next_process_status:true
            ...
            itax_source_tran_dtl:{
              select:{
              itaxstd_id:true,              tran_category:true,              tran_date:true,              tran_reference:true,              product_basic:true            ,
          trs_created_date:true,
          trs_created_by:true,
          trs_modified_date:true,
          trs_modified_by:true,
    ...
    return await this.decryptData(await this.commonDecimalDatahandle(res), 'itax_source_tran');
  } catch (error) {
  ...
  async findAll(token : string,trs_created_date?: Date,trs_created_by?: string,trs_modified_date?: Date,trs_modified_by?: string,trs_process_id?: string,trs_access_profile?: string,trs_org_grp_code?: string,trs_org_code?: string,trs_role_grp_code?: string,trs_role_code?: string,trs_ps_grp_code?: string,trs_ps_code?: string,trs_sub_org_grp_code?: string,trs_sub_org_code?: string,trs_locked_by?: string,trs_locked_time?: Date,trs_tenant_id?:string,trs_app_code?:string,trs_product_code?:string,trs_event_process_status?:string,trs_event_status?:string,trs_token_id?:string,trs_prev_process_code?:string,trs_prev_status?:string,trs_prev_process_status?:string,trs_process_code?:string,trs_status?:string,trs_process_status?:string,trs_next_process_code?:string,trs_next_status?:string,trs_next_process_status?:string) {
    try{
      ...
      this.prismaService.itax_source_tran.findMany({ 
      where: whereClause,
      select: {itaxst_id:true,tran_category:true,tran_date:true,tran_reference:true,product_basic:true,          itaxs_id :true,    
          itax_tran_log:{
              select:{
              itaxtl_id:true,              tran_category:true,              processing_system:true,              request_data:true,              response_data:true,              retry_flag:true,              retry_count:true            ,
          trs_created_date:true,
      ...
      for (const indiviual of res) {
        ...
        const decryptedData = await this.decryptData(plain, 'itax_source_tran');
        decryptedRes.push(decryptedData);
      }
      return decryptedRes;
    } catch (error) {
  ...
  async create(createitax_source_tranDto: Prisma.itax_source_tranCreateInput,token:string) {
    try{

      enum tran_category_itax_source_tran{
        Financial="Financial",
        Non_Financial="Non_Financial",
      }
      const dataSchema:any =  v.object({
            tran_category :  v.optional(v.enum(tran_category_itax_source_tran,"Invalid tran_category_itax_source_tran enum")), 
        ...
        let validate : any = v.safeParse(dataSchema,createitax_source_tranDto);
        if (!validate.success) {
   ...
   */
  async createMaster(
    createitax_source_tranDto: Prisma.itax_source_tranCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_source_tran creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_source_tran creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'itax_source_tran',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'itaxst_id',
        ...
        return {
          success: true,
          message: 'itax_source_tran creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(itaxst_id:number, updateitax_source_tranDto: Prisma.itax_source_tranUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updateitax_source_tranDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updateitax_source_tranDto,'itax_source_tran','update');
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.itax_source_tran.update({
      where: {itaxst_id},
      data: encryptedData,
      select: {itaxst_id:true,tran_category:true,tran_date:true,tran_reference:true,product_basic:true,itaxs_id :true,itax_tran_log:true,itax_tran_error_log:true,itax_source_tran_doc:true,itax_source_tran_dtl:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_prev_process_code:true,trs_prev_status:true,trs_prev_process_status:true,trs_process_code:true,trs_status:true,trs_process_status:true,trs_next_process_code:true,trs_next_status:true,trs_next_process_status:true}
    }));
...
/**
  ...
  async updateMaster(
itaxst_id:number,
    updateitax_source_tranDto: Prisma.itax_source_tranUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =itaxst_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'itax_source_tran update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_source_tran update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'itax_source_tran update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(itaxst_id:number,token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.itax_source_tran.delete({
      where: {itaxst_id },
      select: {itaxst_id:true,tran_category:true,tran_date:true,tran_reference:true,product_basic:true,itaxs_id :true,itax_tran_log:true,itax_tran_error_log:true,itax_source_tran_doc:true,itax_source_tran_dtl:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_prev_process_code:true,trs_prev_status:true,trs_prev_process_status:true,trs_process_code:true,trs_status:true,trs_process_status:true,trs_next_process_code:true,trs_next_status:true,trs_next_process_status:true}
    }));
    return await this.decryptData(await this.commonDecimalDatahandle(res), 'itax_source_tran');
...
itaxst_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =itaxst_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'itax_source_tran deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_source_tran deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'itax_source_tran deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.itax_source_tran.findFirst({ 
