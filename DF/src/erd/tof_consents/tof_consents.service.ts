...
@Injectable()
export class tof_consentsService {
  ...
  async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.tof_consents.findMany({
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'tof_consents');
      ...
      const totalItems = await this.prismaService.tof_consents.count({
        where:Object.keys(query).length >0 ?query: undefined,
      });
    ...
    try{
      const res = await this.prismaService.tof_consents.findUnique({ 
      where: {id,},
    ...
    return await this.decryptData(res, 'tof_consents');
  } catch (error) {
  ...
  async findAll(token : string) {
    try{
      const res = await this.prismaService.tof_consents.findMany({ 
      select: {id:true,lfi:true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true}
      });
      let decryptedRes: any = [];
  ...
  async findAllwithquery(token : string,whereClause : Prisma.tof_consentsWhereInput) {
    try{
      whereClause = await this.encryptData(whereClause,'tof_consents','getAll')
      const res = await this.prismaService.tof_consents.findMany({ 
      where: whereClause,
      select: {id:true,lfi:true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true}
      });
  ...
  async create(createtof_consentsDto: Prisma.tof_consentsCreateInput,token:string) {
    try{
      const res = await this.prismaService.tof_consents.create({ 
      data: await this.encryptData(createtof_consentsDto,'tof_consents','create'), 
      select:{id:true,lfi:true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true}
    })
    return await this.decryptData(res, 'tof_consents');
  } catch (error) {
    ...
    async update(id:string,updatetof_consentsDto: Prisma.tof_consentsUpdateInput,token:string) {    
      try{
      const res = await this.prismaService.tof_consents.update({
      where: {id,},
      data: await this.encryptData(updatetof_consentsDto,'tof_consents','update'),
      select: {id:true,lfi:true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true},
    });
    return await this.decryptData(res, 'tof_consents');
    } catch (error) {
    ...
    try{
      const res = await this.prismaService.tof_consents.delete({
      where: {id, },
      select: {id:true,lfi:true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true}
    });
    return res;
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.tof_consents.findFirst({ 
