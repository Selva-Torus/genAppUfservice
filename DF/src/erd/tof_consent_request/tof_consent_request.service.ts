...
@Injectable()
export class tof_consent_requestService {
  ...
  async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { type }: {type : Date} = queryValue;
      const { consent }: {consent : Date} = queryValue;
      const { subscription }: {subscription : Date} = queryValue;

      if(type){ 
        query.type = { [queryCondition['type']]: type };
      }
      if(consent){ 
        query.consent = { [queryCondition['consent']]: consent };
      }
      if(subscription){ 
        query.subscription = { [queryCondition['subscription']]: subscription };
      }
      const skip = (page - 1) * limit;
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.tof_consent_request.findMany({
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'tof_consent_request');
      ...
      const totalItems = await this.prismaService.tof_consent_request.count({
        where:Object.keys(query).length >0 ?query: undefined,
      });
    ...
    try{
      const res = await this.prismaService.tof_consent_request.findUnique({ 
      where: {id,},
      select: {id:true,type:true,consent:true,subscription:true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true}
    });
    return await this.decryptData(res, 'tof_consent_request');
  } catch (error) {
  ...
  async findAll(token : string) {
    try{
      const res = await this.prismaService.tof_consent_request.findMany({ 
      select: {id:true,type:true,consent:true,subscription:true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true}
      });
      let decryptedRes: any = [];
  ...
  async findAllwithquery(token : string,whereClause : Prisma.tof_consent_requestWhereInput) {
    try{
      whereClause = await this.encryptData(whereClause,'tof_consent_request','getAll')
      const res = await this.prismaService.tof_consent_request.findMany({ 
      where: whereClause,
      select: {id:true,type:true,consent:true,subscription:true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true}
      });
  ...
  async create(createtof_consent_requestDto: Prisma.tof_consent_requestCreateInput,token:string) {
    try{
      const res = await this.prismaService.tof_consent_request.create({ 
      data: await this.encryptData(createtof_consent_requestDto,'tof_consent_request','create'), 
      select:{id:true,type:true,consent:true,subscription:true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true}
    })
    return await this.decryptData(res, 'tof_consent_request');
  } catch (error) {
    ...
    async update(id:string,updatetof_consent_requestDto: Prisma.tof_consent_requestUpdateInput,token:string) {    
      try{
      const res = await this.prismaService.tof_consent_request.update({
      where: {id,},
      data: await this.encryptData(updatetof_consent_requestDto,'tof_consent_request','update'),
      select: {id:true,type:true,consent:true,subscription:true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true},
    });
    return await this.decryptData(res, 'tof_consent_request');
    } catch (error) {
    ...
    try{
      const res = await this.prismaService.tof_consent_request.delete({
      where: {id, },
      select: {id:true,type:true,consent:true,subscription:true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true}
    });
    return res;
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.tof_consent_request.findFirst({ 
