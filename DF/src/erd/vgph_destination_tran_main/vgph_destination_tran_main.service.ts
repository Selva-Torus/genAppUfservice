...
import { vgph_destination_tran_mainEntity } from './entity/vgph_destination_tran_main.entity';
import { CustomException } from 'src/customException';
@Injectable()
export class vgph_destination_tran_mainService {
  ...
  async findSchema (token) {
    const data = {
      ...
      vgphdm_uuid:"string",
      vgphstm_uuid:"string",
      vgphstm_id :"number",      vgphdm_id :"number",      trs_created_date:"Date",
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphdtm_id }: {vgphdtm_id : number} = queryValue;
      ...
      const { vgphdm_uuid }: {vgphdm_uuid : string} = queryValue;
      const { vgphstm_uuid }: {vgphstm_uuid : string} = queryValue;

      if(vgphdtm_id){ 
        query.vgphdtm_id = { [queryCondition['vgphdtm_id']]: vgphdtm_id };
      }
      if(tenant_id){ 
      ...
      if(vgphdm_uuid){ 
        query.vgphdm_uuid = { [queryCondition['vgphdm_uuid']]: vgphdm_uuid };
      }
      if(vgphstm_uuid){ 
      ...
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.vgph_destination_tran_main.findMany({
          select:columns,
          where: query,          
        });
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_destination_tran_main');
      ...
      const totalItems = await this.prismaService.vgph_destination_tran_main.count({
        where: query,
      });
  ...
  async findOne(vgphdtm_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_destination_tran_main.findUnique({ 
      where: {vgphdtm_id},
      select: {vgphdtm_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,vgphdm_uuid:true,vgphstm_uuid:true,vgphstm_id :true,vgphdm_id :true,        trs_created_date:true,
        trs_created_by:true,
    ...
    return  await this.decryptData(res, 'vgph_destination_tran_main');
  } catch (error) {
...
,vgphdtm_id?:number) {
    try{
      ...
      if (vgphdtm_id) {
        whereClause.vgphdtm_id = vgphdtm_id;
      }
      const res = await this.prismaService.vgph_destination_tran_main.findMany({ 
      where: whereClause,
      select: {vgphdtm_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,vgphdm_uuid:true,vgphstm_uuid:true,          vgphstm_id :true,    
          vgphdm_id :true,    
  ...
  async create(createvgph_destination_tran_mainDto: Prisma.vgph_destination_tran_mainCreateInput,token:string) {
    try{

      enum direction_vgph_destination_tran_main{
        OUTBOUND="OUTBOUND",
        INBOUND="INBOUND",
      }
      enum process_type_vgph_destination_tran_main{
        OP="OP",
        IR="IR",
        IP="IP",
        OR="OR",
      }
      enum tran_category_vgph_destination_tran_main{
        Financial="Financial",
      ...
      const dataSchema:any =  v.object({
            ...
            direction :v.enum(direction_vgph_destination_tran_main,"Invalid direction_vgph_destination_tran_main enum")  , 
            process_type :v.enum(process_type_vgph_destination_tran_main,"Invalid process_type_vgph_destination_tran_main enum")  , 
            tran_category :v.enum(tran_category_vgph_destination_tran_main,"Invalid tran_category_vgph_destination_tran_main enum")  , 
            vgphdm_uuid :v.pipe(v.string(),v.maxLength(64 )) , 
            vgphstm_uuid :v.pipe(v.string(),v.maxLength(64 )) , 
        });
        let validate : any = v.safeParse(dataSchema,createvgph_destination_tran_mainDto);
        if (!validate.success) {
      ...
      const res = await this.prismaService.vgph_destination_tran_main.create({ 
      data: await this.encryptData(createvgph_destination_tran_mainDto,'vgph_destination_tran_main','create'),
      select:{vgphdtm_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,vgphdm_uuid:true,vgphstm_uuid:true,vgphstm_id :true,vgphdm_id :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
   ...
   */
  async createMaster(
    createvgph_destination_tran_mainDto: Prisma.vgph_destination_tran_mainCreateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_destination_tran_main creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_destination_tran_main creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      const encryptedData = await this.encryptData(createvgph_destination_tran_mainDto, 'vgph_destination_tran_main', 'create');
      ...
      if(role === 'MAKER')
      {
        ...
        const result = await this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_destination_tran_main',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphdtm_id',
        ...
        return {
          success: true,
          message: 'vgph_destination_tran_main creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphdtm_id:number, updatevgph_destination_tran_mainDto: Prisma.vgph_destination_tran_mainUpdateInput,token:string) {   
    try{
      ...
      const dataSchema:any =  v.object({
          ...
          direction :  v.optional(v.enum(direction_vgph_destination_tran_main,"Invalid direction_vgph_destination_tran_main enum")), 
          process_type :  v.optional(v.enum(process_type_vgph_destination_tran_main,"Invalid process_type_vgph_destination_tran_main enum")), 
          tran_category :  v.optional(v.enum(tran_category_vgph_destination_tran_main,"Invalid tran_category_vgph_destination_tran_main enum")), 
          vgphdm_uuid :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
          vgphstm_uuid :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
      });
      let validate : any = v.safeParse(dataSchema,updatevgph_destination_tran_mainDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const res = await this.prismaService.vgph_destination_tran_main.update({
      where: {vgphdtm_id},
      data: await this.encryptData(updatevgph_destination_tran_mainDto,'vgph_destination_tran_main','update'),
      select: {vgphdtm_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,vgphdm_uuid:true,vgphstm_uuid:true,vgphstm_id :true,vgphdm_id :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return await this.decryptData(res, 'vgph_destination_tran_main');
    } catch (error) {
...
/**
  ...
  async updateMaster(
vgphdtm_id:number,
    updatevgph_destination_tran_mainDto: Prisma.vgph_destination_tran_mainUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphdtm_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_destination_tran_main update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_destination_tran_main update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      const existingRecord = await this.prismaService.vgph_destination_tran_main.findUnique({
        where: {vgphdtm_id}
      });
      ...
      // Encrypt data if needed
      const encryptedData = await this.encryptData(updatevgph_destination_tran_mainDto, 'vgph_destination_tran_main', 'update');
      ...
      return {
        success: true,
        message: 'vgph_destination_tran_main update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphdtm_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_destination_tran_main.delete({
      where: {vgphdtm_id },
      select: {vgphdtm_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,vgphdm_uuid:true,vgphstm_uuid:true,vgphstm_id :true,vgphdm_id :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return res;
...
vgphdtm_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphdtm_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_destination_tran_main deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_destination_tran_main deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_destination_tran_main deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.vgph_destination_tran_main.findFirst({ 
