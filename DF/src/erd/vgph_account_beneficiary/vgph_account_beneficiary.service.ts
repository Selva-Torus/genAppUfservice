...
@Injectable()
export class vgph_account_beneficiaryService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphab_id }: {vgphab_id : bigint} = queryValue;
      const { account }: {account : string} = queryValue;
      ...
      if(vgphab_id){ 
        query.vgphab_id = { [queryCondition['vgphab_id']]: vgphab_id };
      }
      if(account){ 
        query.account = { [queryCondition['account']]: account };
      }
      if(beneficiary_id){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_account_beneficiary.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_account_beneficiary');
      ...
      this.prismaService.vgph_account_beneficiary.count({
        where: query,
      }));
  ...
  async create(createvgph_account_beneficiaryDto: Prisma.vgph_account_beneficiaryCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            vgphab_id :v.number() , 
            account :v.pipe(v.string(),v.maxLength(32 )) , 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_account_beneficiaryDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_account_beneficiaryDto, 'vgph_account_beneficiary', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_account_beneficiary.create({
          data: encryptedData,
          select:{vgphab_id:true,account:true,beneficiary_id:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_account_beneficiaryDto: Prisma.vgph_account_beneficiaryCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_account_beneficiary creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_account_beneficiary creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_account_beneficiary',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphab_id',
        ...
        return {
          success: true,
          message: 'vgph_account_beneficiary creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphab_id:number, updatevgph_account_beneficiaryDto: Prisma.vgph_account_beneficiaryUpdateInput,token:string) {   
    try{

      const dataSchema:any =  v.object({
          account :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_account_beneficiaryDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_account_beneficiaryDto,'vgph_account_beneficiary','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_account_beneficiary.updateMany({
      where: {vgphab_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgphab_id:number,
    updatevgph_account_beneficiaryDto: Prisma.vgph_account_beneficiaryUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphab_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_account_beneficiary update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_account_beneficiary update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_account_beneficiary update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphab_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_account_beneficiary.deleteMany({
      where: {vgphab_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_account_beneficiary');
    let decryptedRes: any = [];
...
vgphab_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphab_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_account_beneficiary deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_account_beneficiary deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_account_beneficiary deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_account_beneficiary.findFirst({ 
