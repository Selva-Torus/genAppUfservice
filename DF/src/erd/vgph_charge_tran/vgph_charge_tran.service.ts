...
import { vgph_charge_tranEntity } from './entity/vgph_charge_tran.entity';
import { CustomException } from 'src/customException';
@Injectable()
export class vgph_charge_tranService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphct_id }: {vgphct_id : bigint} = queryValue;
      ...
      const { vgphsm_uuid }: {vgphsm_uuid : string} = queryValue;
      ...
      const { charge_amount }: {charge_amount : any } = queryValue;
      const { charge_additional }: {charge_additional : any } = queryValue;
      const { charge_dr_gl_account }: {charge_dr_gl_account : string} = queryValue;
      const { charge_cr_gl_account }: {charge_cr_gl_account : string} = queryValue;
      const { vat_flag }: {vat_flag : string} = queryValue;
      const { vat_amount }: {vat_amount : any } = queryValue;
      const { vat_dr_gl_account }: {vat_dr_gl_account : string} = queryValue;
      const { vat_cr_gl_account }: {vat_cr_gl_account : string} = queryValue;
      const { posted_flag }: {posted_flag : string} = queryValue;
      const { posted_date }: {posted_date :  Date} = queryValue;
      const { invoice_generation_flag }: {invoice_generation_flag : string} = queryValue;
      ...
      if(vgphct_id){ 
        query.vgphct_id = { [queryCondition['vgphct_id']]: vgphct_id };
      }
      if(charge_group){ 
      ...
      if(vgphsm_uuid){ 
        query.vgphsm_uuid = { [queryCondition['vgphsm_uuid']]: vgphsm_uuid };
      }
      if(vgphss_uuid){ 
      ...
      if(charge_amount){ 
        query.charge_amount = { [queryCondition['charge_amount']]: charge_amount };
      }
      if(charge_additional){ 
        query.charge_additional = { [queryCondition['charge_additional']]: charge_additional };
      }
      if(charge_dr_gl_account){ 
        query.charge_dr_gl_account = { [queryCondition['charge_dr_gl_account']]: charge_dr_gl_account };
      }
      if(charge_cr_gl_account){ 
        query.charge_cr_gl_account = { [queryCondition['charge_cr_gl_account']]: charge_cr_gl_account };
      }
      if(vat_flag){ 
        query.vat_flag = { [queryCondition['vat_flag']]: vat_flag };
      }
      if(vat_amount){ 
        query.vat_amount = { [queryCondition['vat_amount']]: vat_amount };
      }
      if(vat_dr_gl_account){ 
        query.vat_dr_gl_account = { [queryCondition['vat_dr_gl_account']]: vat_dr_gl_account };
      }
      if(vat_cr_gl_account){ 
        query.vat_cr_gl_account = { [queryCondition['vat_cr_gl_account']]: vat_cr_gl_account };
      }
      if(posted_flag){ 
        query.posted_flag = { [queryCondition['posted_flag']]: posted_flag };
      }
      if(posted_date){ 
        query.posted_date = { [queryCondition['posted_date']]: posted_date };
      }
      if(invoice_generation_flag){ 
        query.invoice_generation_flag = { [queryCondition['invoice_generation_flag']]: invoice_generation_flag };
      }
      if(invoice_id){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_charge_tran.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_charge_tran');
      ...
      this.prismaService.vgph_charge_tran.count({
        where: query,
      }));
  ...
  async findOne(vgphct_id:number,token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_charge_tran.findMany({ 
      where: {vgphct_id},
      select: {vgphct_id:true,charge_group:true,charge_description:true,currency:true,vgphsm_uuid:true,vgphss_uuid:true,vgphstm_uuid:true,vgphsts_uuid:true,charge_code:true,charge_amount:true,charge_additional:true,charge_dr_gl_account:true,charge_cr_gl_account:true,vat_flag:true,vat_amount:true,vat_dr_gl_account:true,vat_cr_gl_account:true,posted_flag:true,posted_date:true,invoice_generation_flag:true,invoice_id:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,        }
    }));
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_charge_tran');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'vgph_charge_tran');
      decryptedRes.push(decryptedData);
    }
    return decryptedRes;
  } catch (error:any) {
...
,vgphct_id?:number) {
    try{
      ...
      if (vgphct_id) {
        whereClause.vgphct_id = vgphct_id;
      }
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_charge_tran.findMany({ 
      where: whereClause,
  ...
  async create(createvgph_charge_tranDto: Prisma.vgph_charge_tranCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            vgphct_id :v.number() , 
            charge_group :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            charge_description :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            currency :v.pipe(v.string(),v.maxLength(3 )) , 
            vgphsm_uuid :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            ...
            charge_code :v.pipe(v.string(),v.maxLength(64 )) , 
            charge_amount :v.any()  , 
            charge_additional :  v.optional(v.any() ), 
            charge_dr_gl_account :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            charge_cr_gl_account :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            vat_flag :  v.optional(v.pipe(v.string(),v.maxLength(1 ))), 
            vat_amount :  v.optional(v.any()), 
            vat_dr_gl_account :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            vat_cr_gl_account :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            posted_flag :  v.optional(v.pipe(v.string(),v.maxLength(1 ))), 
            posted_date :  v.optional((v.any())), 
            invoice_generation_flag :  v.optional(v.pipe(v.string(),v.maxLength(1 ))), 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_charge_tranDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_charge_tranDto, 'vgph_charge_tran', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_charge_tran.create({
          data: encryptedData,
          select:{vgphct_id:true,charge_group:true,charge_description:true,currency:true,vgphsm_uuid:true,vgphss_uuid:true,vgphstm_uuid:true,vgphsts_uuid:true,charge_code:true,charge_amount:true,charge_additional:true,charge_dr_gl_account:true,charge_cr_gl_account:true,vat_flag:true,vat_amount:true,vat_dr_gl_account:true,vat_cr_gl_account:true,posted_flag:true,posted_date:true,invoice_generation_flag:true,invoice_id:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_charge_tranDto: Prisma.vgph_charge_tranCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_charge_tran creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_charge_tran creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_charge_tran',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphct_id',
        ...
        return {
          success: true,
          message: 'vgph_charge_tran creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphct_id:number, updatevgph_charge_tranDto: Prisma.vgph_charge_tranUpdateInput,token:string) {   
    try{

      const dataSchema:any =  v.object({
          charge_group :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
          charge_description :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
          currency :  v.optional(v.pipe(v.string(),v.maxLength(3 ))), 
          vgphsm_uuid :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
          vgphss_uuid :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
          vgphstm_uuid :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
          vgphsts_uuid :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
          charge_code :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
          charge_amount :  v.optional(v.number()), 
          charge_additional :  v.optional(v.any()), 
          charge_dr_gl_account :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
          charge_cr_gl_account :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
          vat_flag :  v.optional(v.pipe(v.string(),v.maxLength(1 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_charge_tranDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_charge_tranDto,'vgph_charge_tran','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_charge_tran.updateMany({
      where: {vgphct_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgphct_id:number,
    updatevgph_charge_tranDto: Prisma.vgph_charge_tranUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphct_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_charge_tran update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_charge_tran update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_charge_tran update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphct_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_charge_tran.deleteMany({
      where: {vgphct_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_charge_tran');
    let decryptedRes: any = [];
...
vgphct_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphct_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_charge_tran deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_charge_tran deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_charge_tran deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_charge_tran.findFirst({ 
