...
export class vgph_stepService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphs_id }: {vgphs_id : number} = queryValue;
      ...
      const { sequence }: {sequence : any } = queryValue;
      const { description }: {description : string} = queryValue;
      const { source_process_code }: {source_process_code : string} = queryValue;
      const { result }: {result : string} = queryValue;
      const { destination_process_code }: {destination_process_code : string} = queryValue;
      const { need_process_log }: {need_process_log : string} = queryValue;
      const { need_exception_log }: {need_exception_log : string} = queryValue;
      const { is_final_step }: {is_final_step : string} = queryValue;
      const { param }: {param : any } = queryValue;
      const { version }: {version : string} = queryValue;

      if(vgphs_id){ 
        query.vgphs_id = { [queryCondition['vgphs_id']]: vgphs_id };
      }
      if(tenant_id){ 
      ...
      if(sequence){ 
        query.sequence = { [queryCondition['sequence']]: sequence };
      }
      if(description){ 
        query.description = { [queryCondition['description']]: description };
      }
      if(source_process_code){ 
        query.source_process_code = { [queryCondition['source_process_code']]: source_process_code };
      }
      if(result){ 
        query.result = { [queryCondition['result']]: result };
      }
      if(destination_process_code){ 
        query.destination_process_code = { [queryCondition['destination_process_code']]: destination_process_code };
      }
      if(need_process_log){ 
        query.need_process_log = { [queryCondition['need_process_log']]: need_process_log };
      }
      if(need_exception_log){ 
        query.need_exception_log = { [queryCondition['need_exception_log']]: need_exception_log };
      }
      if(is_final_step){ 
        query.is_final_step = { [queryCondition['is_final_step']]: is_final_step };
      }
      if(param){ 
      ...
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.vgph_step.findMany({
          select:columns,
          where: query,          
        });
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_step');
      ...
      const totalItems = await this.prismaService.vgph_step.count({
        where: query,
      });
  ...
  async findOne(vgphs_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_step.findUnique({ 
      where: {vgphs_id},
      select: {vgphs_id:true,tenant_id:true,product_code:true,flow_code:true,code:true,sequence:true,description:true,source_process_code:true,result:true,destination_process_code:true,need_process_log:true,need_exception_log:true,is_final_step:true,param:true,version:true,        trs_created_date:true,
        trs_created_by:true,
    ...
    return  await this.decryptData(res, 'vgph_step');
  } catch (error) {
...
,vgphs_id?:number) {
    try{
      ...
      if (vgphs_id) {
        whereClause.vgphs_id = vgphs_id;
      }
      const res = await this.prismaService.vgph_step.findMany({ 
      where: whereClause,
      select: {vgphs_id:true,tenant_id:true,product_code:true,flow_code:true,code:true,sequence:true,description:true,source_process_code:true,result:true,destination_process_code:true,need_process_log:true,need_exception_log:true,is_final_step:true,param:true,version:true,        trs_created_date:true,
        trs_created_by:true,
        trs_modified_date:true,
  ...
  async create(createvgph_stepDto: Prisma.vgph_stepCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            tenant_id :v.pipe(v.string(),v.maxLength(16 )) , 
            product_code :v.pipe(v.string(),v.maxLength(32 )) , 
            flow_code :v.pipe(v.string(),v.maxLength(128 )) , 
            code :v.pipe(v.string(),v.maxLength(128 )) , 
            sequence :v.any()  , 
            description :v.pipe(v.string(),v.maxLength(256 )) , 
            source_process_code :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            result :v.pipe(v.string(),v.maxLength(32 )) , 
            destination_process_code :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            need_process_log :  v.optional(v.pipe(v.string(),v.maxLength(1 ))), 
            need_exception_log :  v.optional(v.pipe(v.string(),v.maxLength(1 ))), 
            is_final_step :  v.optional(v.pipe(v.string(),v.maxLength(1 ))), 
            param :  v.optional(v.any() ), 
            version :  v.optional(v.pipe(v.string(),v.maxLength(14 ))), 
        });
        let validate : any = v.safeParse(dataSchema,createvgph_stepDto);
        if (!validate.success) {
      ...
      const res = await this.prismaService.vgph_step.create({ 
      data: await this.encryptData(createvgph_stepDto,'vgph_step','create'),
      select:{vgphs_id:true,tenant_id:true,product_code:true,flow_code:true,code:true,sequence:true,description:true,source_process_code:true,result:true,destination_process_code:true,need_process_log:true,need_exception_log:true,is_final_step:true,param:true,version:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
   ...
   */
  async createMaster(
    createvgph_stepDto: Prisma.vgph_stepCreateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_step creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_step creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      const encryptedData = await this.encryptData(createvgph_stepDto, 'vgph_step', 'create');
      ...
      if(role === 'MAKER')
      {
        ...
        const result = await this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_step',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphs_id',
        ...
        return {
          success: true,
          message: 'vgph_step creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphs_id:number, updatevgph_stepDto: Prisma.vgph_stepUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_stepDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const res = await this.prismaService.vgph_step.update({
      where: {vgphs_id},
      data: await this.encryptData(updatevgph_stepDto,'vgph_step','update'),
      select: {vgphs_id:true,tenant_id:true,product_code:true,flow_code:true,code:true,sequence:true,description:true,source_process_code:true,result:true,destination_process_code:true,need_process_log:true,need_exception_log:true,is_final_step:true,param:true,version:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return await this.decryptData(res, 'vgph_step');
    } catch (error) {
...
/**
  ...
  async updateMaster(
vgphs_id:number,
    updatevgph_stepDto: Prisma.vgph_stepUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphs_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_step update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_step update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      const existingRecord = await this.prismaService.vgph_step.findUnique({
        where: {vgphs_id}
      });
      ...
      // Encrypt data if needed
      const encryptedData = await this.encryptData(updatevgph_stepDto, 'vgph_step', 'update');
      ...
      return {
        success: true,
        message: 'vgph_step update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphs_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_step.delete({
      where: {vgphs_id },
      select: {vgphs_id:true,tenant_id:true,product_code:true,flow_code:true,code:true,sequence:true,description:true,source_process_code:true,result:true,destination_process_code:true,need_process_log:true,need_exception_log:true,is_final_step:true,param:true,version:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return res;
...
vgphs_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphs_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_step deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_step deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_step deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.vgph_step.findFirst({ 
