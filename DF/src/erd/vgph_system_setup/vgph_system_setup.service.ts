...
export class vgph_system_setupService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphssid }: {vgphssid : number} = queryValue;
      ...
      if(vgphssid){ 
        query.vgphssid = { [queryCondition['vgphssid']]: vgphssid };
      }
      if(setup_code){ 
  ...
  async findOne(vgphssid:number,token : string) {
    try{
      const res = await this.prismaService.vgph_system_setup.findUnique({ 
      where: {vgphssid},
      select: {vgphssid:true,setup_code:true,product_code:true,interface_product:true,category:true,sub_category:true,purpose:true,setup_value:true,tenant_id:true,        trs_created_date:true,
        trs_created_by:true,
...
,vgphssid?:number) {
  ...
  async create(createvgph_system_setupDto: Prisma.vgph_system_setupCreateInput,token:string) {
    try{
      ...
      const dataSchema:any =  v.object({
            setup_code :v.pipe(v.string(),v.maxLength(128 )) , 
            product_code :v.pipe(v.string(),v.maxLength(32 )) , 
            interface_product :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            category :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            sub_category :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            purpose :  v.optional(v.pipe(v.string(),v.maxLength(1028 ))), 
   ...
   */
  async createMaster(
    createvgph_system_setupDto: Prisma.vgph_system_setupCreateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_system_setup creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_system_setup creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      const encryptedData = await this.encryptData(createvgph_system_setupDto, 'vgph_system_setup', 'create');
      ...
      if(role === 'MAKER')
      {
        ...
        const result = await this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_system_setup',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphssid',
        ...
        return {
          success: true,
          message: 'vgph_system_setup creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphssid:number, updatevgph_system_setupDto: Prisma.vgph_system_setupUpdateInput,token:string) {   
    try{
...
/**
  ...
  async updateMaster(
vgphssid:number,
    updatevgph_system_setupDto: Prisma.vgph_system_setupUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphssid;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_system_setup update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_system_setup update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      const existingRecord = await this.prismaService.vgph_system_setup.findUnique({
        where: {vgphssid}
      });
      ...
      // Encrypt data if needed
      const encryptedData = await this.encryptData(updatevgph_system_setupDto, 'vgph_system_setup', 'update');
      ...
      return {
        success: true,
        message: 'vgph_system_setup update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphssid:number,token : string) {
    try{
...
vgphssid:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphssid;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_system_setup deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_system_setup deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_system_setup deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
