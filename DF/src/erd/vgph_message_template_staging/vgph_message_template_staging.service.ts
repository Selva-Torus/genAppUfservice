...
@Injectable()
export class vgph_message_template_stagingService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphmts_id }: {vgphmts_id : number} = queryValue;
      ...
      if(vgphmts_id){ 
        query.vgphmts_id = { [queryCondition['vgphmts_id']]: vgphmts_id };
      }
      if(tenant_id){ 
      ...
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.vgph_message_template_staging.findMany({
          select:columns,
          where: query,          
        });
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_message_template_staging');
      ...
      const totalItems = await this.prismaService.vgph_message_template_staging.count({
        where: query,
      });
  ...
  async findOne(vgphmts_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_message_template_staging.findUnique({ 
      where: {vgphmts_id},
      select: {vgphmts_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,type:true,message_code:true,template_value:true,        trs_created_date:true,
        trs_created_by:true,
    ...
    return  await this.decryptData(res, 'vgph_message_template_staging');
  } catch (error) {
  ...
  async findAll(token : string,trs_created_date?: Date,trs_created_by?: string,trs_modified_date?: Date,trs_modified_by?: string,trs_next_status?: string,trs_status?: string,trs_process_id?: string,trs_access_profile?: string,trs_org_grp_code?: string,trs_org_code?: string,trs_role_grp_code?: string,trs_role_code?: string,trs_ps_grp_code?: string,trs_ps_code?: string,trs_sub_org_grp_code?: string,trs_sub_org_code?: string
    ...
    try{
      ...
      const res = await this.prismaService.vgph_message_template_staging.findMany({ 
      where: whereClause,
      select: {vgphmts_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,type:true,message_code:true,template_value:true,        trs_created_date:true,
        trs_created_by:true,
        trs_modified_date:true,
  ...
  async create(createvgph_message_template_stagingDto: Prisma.vgph_message_template_stagingCreateInput,token:string) {
    try{

      enum direction_vgph_message_template_staging{
        OUTBOUND="OUTBOUND",
        INBOUND="INBOUND",
      }
      enum process_type_vgph_message_template_staging{
        OP="OP",
        IR="IR",
        IP="IP",
        OR="OR",
      ...
      const dataSchema:any =  v.object({
            ...
            direction :v.enum(direction_vgph_message_template_staging,"Invalid direction_vgph_message_template_staging enum")  , 
            process_type :v.enum(process_type_vgph_message_template_staging,"Invalid process_type_vgph_message_template_staging enum")  , 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_message_template_stagingDto);
        if (!validate.success) {
   ...
   */
  async createMaster(
    createvgph_message_template_stagingDto: Prisma.vgph_message_template_stagingCreateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_message_template_staging creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_message_template_staging creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      const encryptedData = await this.encryptData(createvgph_message_template_stagingDto, 'vgph_message_template_staging', 'create');
      ...
      if(role === 'MAKER')
      {
        ...
        const result = await this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_message_template_staging',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphmts_id',
        ...
        return {
          success: true,
          message: 'vgph_message_template_staging creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphmts_id:number, updatevgph_message_template_stagingDto: Prisma.vgph_message_template_stagingUpdateInput,token:string) {   
    try{
      ...
      const dataSchema:any =  v.object({
          ...
          direction :  v.optional(v.enum(direction_vgph_message_template_staging,"Invalid direction_vgph_message_template_staging enum")), 
          process_type :  v.optional(v.enum(process_type_vgph_message_template_staging,"Invalid process_type_vgph_message_template_staging enum")), 
          type :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
          message_code :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
          template_value :  v.optional(v.string()), 
      });
      let validate : any = v.safeParse(dataSchema,updatevgph_message_template_stagingDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const res = await this.prismaService.vgph_message_template_staging.update({
      where: {vgphmts_id},
      data: await this.encryptData(updatevgph_message_template_stagingDto,'vgph_message_template_staging','update'),
      select: {vgphmts_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,type:true,message_code:true,template_value:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return await this.decryptData(res, 'vgph_message_template_staging');
    } catch (error) {
...
/**
  ...
  async updateMaster(
vgphmts_id:number,
    updatevgph_message_template_stagingDto: Prisma.vgph_message_template_stagingUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphmts_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_message_template_staging update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_message_template_staging update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      const existingRecord = await this.prismaService.vgph_message_template_staging.findUnique({
        where: {vgphmts_id}
      });
      ...
      // Encrypt data if needed
      const encryptedData = await this.encryptData(updatevgph_message_template_stagingDto, 'vgph_message_template_staging', 'update');
      ...
      return {
        success: true,
        message: 'vgph_message_template_staging update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphmts_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_message_template_staging.delete({
      where: {vgphmts_id },
      select: {vgphmts_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,type:true,message_code:true,template_value:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return res;
...
vgphmts_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphmts_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_message_template_staging deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_message_template_staging deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_message_template_staging deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.vgph_message_template_staging.findFirst({ 
