...
export class vgph_source_stagingService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      const { source_category }: {source_category : string} = queryValue;
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_source_staging.findMany({
          select:columns,
          where: query,          
        }));
      ...
      this.prismaService.vgph_source_staging.count({
        where: query,
      }));
  ...
  async create(createvgph_source_stagingDto: Prisma.vgph_source_stagingCreateInput,token:string) {
    try{
      ...
      const dataSchema:any =  v.object({
            ...
            source_category :  v.optional(v.pipe(v.string(),v.maxLength(8 ))), 
            source_reference :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            source_name :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
      ...
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_source_staging.create({
          data: encryptedData,
          select:{vgphss_id:true,direction:true,process_type:true,tran_category:true,source_category:true,source_reference:true,source_name:true,channel_name:true,channel_reference:true,product_basic:true,product_additional:true,request_data:true,response_data:true,total_count:true,total_amount:true,currency:true,uuid:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,trs_prev_process_code:true,trs_prev_status:true,trs_prev_process_status:true,trs_process_code:true,trs_status:true,trs_process_status:true,trs_next_process_code:true,trs_next_status:true,trs_next_process_status:true,}          
        })
      );
  ...
  async update(vgphss_id:number, updatevgph_source_stagingDto: Prisma.vgph_source_stagingUpdateInput,token:string) {   
    try{
      ...
      this.prismaService.vgph_source_staging.updateMany({
      where: {vgphss_id},
      data: encryptedData
    }));
...
vgphss_id:number,
  ...
  async remove(vgphss_id:number,token : string) {
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_staging.deleteMany({
      where: {vgphss_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_source_staging');
    let decryptedRes: any = [];
...
vgphss_id:number,
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_staging.findFirst({ 
