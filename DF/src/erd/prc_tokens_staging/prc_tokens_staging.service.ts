...
@Injectable()
export class prc_tokens_stagingService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.prc_tokens_staging.findMany({
          select:columns,
          where: query,          
        });
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'prc_tokens_staging');
      ...
      const totalItems = await this.prismaService.prc_tokens_staging.count({
        where: query,
      });
  ...
  async findOne(prcts_id:number,token : string) {
    try{
      const res = await this.prismaService.prc_tokens_staging.findUnique({ 
      where: {prcts_id},
      select: {prcts_id:true,process_name:true,version:true,            vgph_source_tran_staging:{
              select:{
    ...
    return  await this.decryptData(res, 'prc_tokens_staging');
  } catch (error) {
...
,prcts_id?:number) {
    try{
      ...
      if (prcts_id) {
        whereClause.prcts_id = prcts_id;
      }
      const res = await this.prismaService.prc_tokens_staging.findMany({ 
      where: whereClause,
      select: {prcts_id:true,process_name:true,version:true,          vgph_source_tran_staging:{
              select:{
              vgphsts_id:true,              tenant_id:true,              parent_vgphsts_uuid:true,              product_code:true,              direction:true,              process_type:true,              tran_category:true,              message_code:true,              channel_name:true,              channel_reference:true,              tran_date:true,              tran_reference:true,              tran_seq_no:true,              value_date:true,              settlement_date:true,              dr_account:true,              dr_bank_code_type:true,              dr_bank_code:true,              dr_name:true,              dr_amount:true,              dr_currency:true,              cr_account:true,              cr_bank_code_type:true,              cr_bank_code:true,              cr_name:true,              cr_amount:true,              cr_currency:true,              remittance_info:true,              product_basic:true,              product_additional:true,              charge_type:true,              uuid:true,              vgphss_uuid:true            ,
  ...
  async create(createprc_tokens_stagingDto: Prisma.prc_tokens_stagingCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            process_name :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            version :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
        });
        let validate : any = v.safeParse(dataSchema,createprc_tokens_stagingDto);
        if (!validate.success) {
      ...
      const res = await this.prismaService.prc_tokens_staging.create({ 
      data: await this.encryptData(createprc_tokens_stagingDto,'prc_tokens_staging','create'),
      select:{prcts_id:true,process_name:true,version:true,vgph_source_tran_staging:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
   ...
   */
  async createMaster(
    createprc_tokens_stagingDto: Prisma.prc_tokens_stagingCreateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'prc_tokens_staging creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'prc_tokens_staging creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      const encryptedData = await this.encryptData(createprc_tokens_stagingDto, 'prc_tokens_staging', 'create');
      ...
      if(role === 'MAKER')
      {
        ...
        const result = await this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'prc_tokens_staging',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'prcts_id',
        ...
        return {
          success: true,
          message: 'prc_tokens_staging creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(prcts_id:number, updateprc_tokens_stagingDto: Prisma.prc_tokens_stagingUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updateprc_tokens_stagingDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const res = await this.prismaService.prc_tokens_staging.update({
      where: {prcts_id},
      data: await this.encryptData(updateprc_tokens_stagingDto,'prc_tokens_staging','update'),
      select: {prcts_id:true,process_name:true,version:true,vgph_source_tran_staging:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return await this.decryptData(res, 'prc_tokens_staging');
    } catch (error) {
...
/**
  ...
  async updateMaster(
prcts_id:number,
    updateprc_tokens_stagingDto: Prisma.prc_tokens_stagingUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =prcts_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'prc_tokens_staging update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'prc_tokens_staging update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      const existingRecord = await this.prismaService.prc_tokens_staging.findUnique({
        where: {prcts_id}
      });
      ...
      // Encrypt data if needed
      const encryptedData = await this.encryptData(updateprc_tokens_stagingDto, 'prc_tokens_staging', 'update');
      ...
      return {
        success: true,
        message: 'prc_tokens_staging update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(prcts_id:number,token : string) {
    try{
      const res = await this.prismaService.prc_tokens_staging.delete({
      where: {prcts_id },
      select: {prcts_id:true,process_name:true,version:true,vgph_source_tran_staging:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return res;
...
prcts_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =prcts_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'prc_tokens_staging deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'prc_tokens_staging deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'prc_tokens_staging deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.prc_tokens_staging.findFirst({ 
