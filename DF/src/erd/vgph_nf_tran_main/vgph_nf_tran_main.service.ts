...
@Injectable()
export class vgph_nf_tran_mainService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphntm_id }: {vgphntm_id : bigint} = queryValue;
      ...
      if(vgphntm_id){ 
        query.vgphntm_id = { [queryCondition['vgphntm_id']]: vgphntm_id };
      }
      if(vgphstm_uuid){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_nf_tran_main');
      ...
      this.prismaService.vgph_nf_tran_main.count({
        where: query,
      }));
  ...
  async findOne(vgphntm_id:number,token : string) {
    try{
    ...
    for (const indiviual of res) {
      ...
      const decryptedData = await this.decryptData(plain, 'vgph_nf_tran_main');
      decryptedRes.push(decryptedData);
    }
    return decryptedRes;
  } catch (error:any) {
  ...
  async create(createvgph_nf_tran_mainDto: Prisma.vgph_nf_tran_mainCreateInput,token:string) {
    try{
      ...
      enum process_type_vgph_nf_tran_main{
        OP="OP",
        IR="IR",
        IP="IP",
        OR="OR",
      ...
      const dataSchema:any =  v.object({
            vgphntm_id :v.pipe(v.number(),v.maxValue(9999999999999999999999999999999999999999999999999999999999999999 )) , 
            vgphstm_uuid :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            uuid :v.pipe(v.string(),v.maxLength(64 )) , 
            direction :v.enum(direction_vgph_nf_tran_main,"Invalid direction_vgph_nf_tran_main enum")  , 
            process_type :v.enum(process_type_vgph_nf_tran_main,"Invalid process_type_vgph_nf_tran_main enum")  , 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_nf_tran_mainDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_nf_tran_mainDto, 'vgph_nf_tran_main', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_nf_tran_main.create({
          data: encryptedData,
          select:{vgphntm_id:true,vgphstm_uuid:true,uuid:true,direction:true,process_type:true,tran_category:true,message_code:true,channel_name:true,channel_reference:true,tran_date:true,product_common:true,product_basic:true,product_additional:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,trs_prev_process_code:true,trs_prev_status:true,trs_prev_process_status:true,trs_process_code:true,trs_status:true,trs_process_status:true,trs_next_process_code:true,trs_next_status:true,trs_next_process_status:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_nf_tran_mainDto: Prisma.vgph_nf_tran_mainCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_nf_tran_main creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_nf_tran_main creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_nf_tran_main',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphntm_id',
        ...
        return {
          success: true,
          message: 'vgph_nf_tran_main creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphntm_id:number, updatevgph_nf_tran_mainDto: Prisma.vgph_nf_tran_mainUpdateInput,token:string) {   
    try{
      ...
      const dataSchema:any =  v.object({
          ...
          direction :  v.optional(v.enum(direction_vgph_nf_tran_main,"Invalid direction_vgph_nf_tran_main enum")), 
          process_type :  v.optional(v.enum(process_type_vgph_nf_tran_main,"Invalid process_type_vgph_nf_tran_main enum")), 
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_nf_tran_mainDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
...
vgphntm_id:number,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_nf_tran_main update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_nf_tran_main update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_nf_tran_main update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphntm_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_nf_tran_main.deleteMany({
      where: {vgphntm_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_nf_tran_main');
    let decryptedRes: any = [];
...
vgphntm_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphntm_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_nf_tran_main deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_nf_tran_main deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_nf_tran_main deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_nf_tran_main.findFirst({ 
