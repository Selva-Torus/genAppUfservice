...
@Injectable()
export class vgph_handlerService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphh_id }: {vgphh_id : number} = queryValue;
      ...
      const { queue_name }: {queue_name : string} = queryValue;

      if(vgphh_id){ 
        query.vgphh_id = { [queryCondition['vgphh_id']]: vgphh_id };
      }
      if(tenant_id){ 
      ...
      if(queue_name){ 
        query.queue_name = { [queryCondition['queue_name']]: queue_name };
      }
      const skip = (page - 1) * limit;
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.vgph_handler.findMany({
          select:columns,
          where: query,          
        });
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_handler');
      ...
      const totalItems = await this.prismaService.vgph_handler.count({
        where: query,
      });
  ...
  async findOne(vgphh_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_handler.findUnique({ 
      where: {vgphh_id},
      select: {vgphh_id:true,tenant_id:true,product_code:true,type:true,code:true,param:true,queue_name:true,        trs_created_date:true,
        trs_created_by:true,
    ...
    return  await this.decryptData(res, 'vgph_handler');
  } catch (error) {
...
,vgphh_id?:number) {
    try{
      ...
      if (vgphh_id) {
        whereClause.vgphh_id = vgphh_id;
      }
      const res = await this.prismaService.vgph_handler.findMany({ 
      where: whereClause,
      select: {vgphh_id:true,tenant_id:true,product_code:true,type:true,code:true,param:true,queue_name:true,        trs_created_date:true,
        trs_created_by:true,
        trs_modified_date:true,
  ...
  async create(createvgph_handlerDto: Prisma.vgph_handlerCreateInput,token:string) {
    try{
      ...
      const dataSchema:any =  v.object({
            ...
            queue_name :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
        });
        let validate : any = v.safeParse(dataSchema,createvgph_handlerDto);
        if (!validate.success) {
      ...
      const res = await this.prismaService.vgph_handler.create({ 
      data: await this.encryptData(createvgph_handlerDto,'vgph_handler','create'),
      select:{vgphh_id:true,tenant_id:true,product_code:true,type:true,code:true,param:true,queue_name:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
   ...
   */
  async createMaster(
    createvgph_handlerDto: Prisma.vgph_handlerCreateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_handler creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_handler creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      const encryptedData = await this.encryptData(createvgph_handlerDto, 'vgph_handler', 'create');
      ...
      if(role === 'MAKER')
      {
        ...
        const result = await this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_handler',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphh_id',
        ...
        return {
          success: true,
          message: 'vgph_handler creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphh_id:number, updatevgph_handlerDto: Prisma.vgph_handlerUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_handlerDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const res = await this.prismaService.vgph_handler.update({
      where: {vgphh_id},
      data: await this.encryptData(updatevgph_handlerDto,'vgph_handler','update'),
      select: {vgphh_id:true,tenant_id:true,product_code:true,type:true,code:true,param:true,queue_name:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return await this.decryptData(res, 'vgph_handler');
    } catch (error) {
...
/**
  ...
  async updateMaster(
vgphh_id:number,
    updatevgph_handlerDto: Prisma.vgph_handlerUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphh_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_handler update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_handler update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      const existingRecord = await this.prismaService.vgph_handler.findUnique({
        where: {vgphh_id}
      });
      ...
      // Encrypt data if needed
      const encryptedData = await this.encryptData(updatevgph_handlerDto, 'vgph_handler', 'update');
      ...
      return {
        success: true,
        message: 'vgph_handler update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphh_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_handler.delete({
      where: {vgphh_id },
      select: {vgphh_id:true,tenant_id:true,product_code:true,type:true,code:true,param:true,queue_name:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return res;
...
vgphh_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphh_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_handler deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_handler deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_handler deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.vgph_handler.findFirst({ 
