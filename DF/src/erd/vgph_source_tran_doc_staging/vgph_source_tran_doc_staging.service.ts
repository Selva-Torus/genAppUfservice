...
import { vgph_source_tran_doc_stagingEntity } from './entity/vgph_source_tran_doc_staging.entity';
import { CustomException } from 'src/customException';
@Injectable()
export class vgph_source_tran_doc_stagingService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphstds_id }: {vgphstds_id : bigint} = queryValue;
      const { vgphsts_uuid }: {vgphsts_uuid : string} = queryValue;
      const { category }: {category : Date} = queryValue;
      const { doc_type }: {doc_type : Date} = queryValue;
      const { doc_group }: {doc_group : Date} = queryValue;
      const { doc_name }: {doc_name : string} = queryValue;
      const { url }: {url : string} = queryValue;
      const { doc_additional }: {doc_additional : any } = queryValue;
      const { doc_content_text }: {doc_content_text : string} = queryValue;
      const { doc_content_byte }: {doc_content_byte : Date} = queryValue;
      const { doc_size }: {doc_size : string} = queryValue;
      ...
      if(vgphstds_id){ 
        query.vgphstds_id = { [queryCondition['vgphstds_id']]: vgphstds_id };
      }
      if(vgphsts_uuid){ 
      ...
      if(doc_type){ 
        query.doc_type = { [queryCondition['doc_type']]: doc_type };
      }
      if(doc_group){ 
        query.doc_group = { [queryCondition['doc_group']]: doc_group };
      }
      if(doc_name){ 
        query.doc_name = { [queryCondition['doc_name']]: doc_name };
      }
      if(url){ 
        query.url = { [queryCondition['url']]: url };
      }
      if(doc_additional){ 
        query.doc_additional = { [queryCondition['doc_additional']]: doc_additional };
      }
      if(doc_content_text){ 
        query.doc_content_text = { [queryCondition['doc_content_text']]: doc_content_text };
      }
      if(doc_content_byte){ 
        query.doc_content_byte = { [queryCondition['doc_content_byte']]: doc_content_byte };
      }
      if(doc_size){ 
        query.doc_size = { [queryCondition['doc_size']]: doc_size };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_source_tran_doc_staging.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_source_tran_doc_staging');
      ...
      this.prismaService.vgph_source_tran_doc_staging.count({
        where: query,
      }));
  ...
  async findOne(vgphstds_id:number,trs_tenant_id: string,trs_app_code: string,trs_product_code: string,token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_tran_doc_staging.findMany({ 
      where: {vgphstds_id,trs_tenant_id,trs_app_code,trs_product_code},
      select: {vgphstds_id:true,vgphsts_uuid:true,category:true,doc_type:true,doc_group:true,doc_name:true,url:true,doc_additional:true,doc_content_text:true,doc_content_byte:true,doc_size:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,        }
    }));
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_source_tran_doc_staging');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'vgph_source_tran_doc_staging');
      decryptedRes.push(decryptedData);
    }
    return decryptedRes;
  } catch (error:any) {
  ...
  async create(createvgph_source_tran_doc_stagingDto: Prisma.vgph_source_tran_doc_stagingCreateInput,token:string) {
    try{

      enum category_vgph_source_tran_doc_staging{
        front="front",
        back="back",
      }
      enum doc_type_vgph_source_tran_doc_staging{
        tiff="tiff",
        jpg="jpg",
        uv="uv",
        color="color",
      }
      enum doc_group_vgph_source_tran_doc_staging{
        cheque="cheque",
        doc="doc",
        id_proof="id_proof",
      ...
      const dataSchema:any =  v.object({
            vgphstds_id :v.number() , 
            vgphsts_uuid :v.pipe(v.string(),v.maxLength(64 )) , 
            category :v.enum(category_vgph_source_tran_doc_staging,"Invalid category_vgph_source_tran_doc_staging enum")  , 
            doc_type :  v.optional(v.enum(doc_type_vgph_source_tran_doc_staging,"Invalid doc_type_vgph_source_tran_doc_staging enum")), 
            doc_group :  v.optional(v.enum(doc_group_vgph_source_tran_doc_staging,"Invalid doc_group_vgph_source_tran_doc_staging enum")), 
            doc_name :v.pipe(v.string(),v.maxLength(64 )) , 
            url :  v.optional(v.pipe(v.string(),v.maxLength(256 ))), 
            doc_additional :  v.optional(v.any() ), 
            doc_content_text :  v.optional(v.string()), 
            doc_content_byte :  v.optional(v.instance(Buffer),), 
            doc_size :  v.optional(v.string()), 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_source_tran_doc_stagingDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_source_tran_doc_stagingDto, 'vgph_source_tran_doc_staging', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_source_tran_doc_staging.create({
          data: encryptedData,
          select:{vgphstds_id:true,vgphsts_uuid:true,category:true,doc_type:true,doc_group:true,doc_name:true,url:true,doc_additional:true,doc_content_text:true,doc_content_byte:true,doc_size:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_source_tran_doc_stagingDto: Prisma.vgph_source_tran_doc_stagingCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_tran_doc_staging creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_tran_doc_staging creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_source_tran_doc_staging',
        ...
        return {
          success: true,
          message: 'vgph_source_tran_doc_staging creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphstds_id:number, updatevgph_source_tran_doc_stagingDto: Prisma.vgph_source_tran_doc_stagingUpdateInput,token:string) {   
    try{
      ...
      const dataSchema:any =  v.object({
          ...
          category :  v.optional(v.enum(category_vgph_source_tran_doc_staging,"Invalid category_vgph_source_tran_doc_staging enum")), 
          doc_type :  v.optional(v.enum(doc_type_vgph_source_tran_doc_staging,"Invalid doc_type_vgph_source_tran_doc_staging enum")), 
          doc_group :  v.optional(v.enum(doc_group_vgph_source_tran_doc_staging,"Invalid doc_group_vgph_source_tran_doc_staging enum")), 
          doc_name :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
          url :  v.optional(v.pipe(v.string(),v.maxLength(256 ))), 
          doc_additional :  v.optional(v.any()), 
          doc_content_text :  v.optional(v.string()), 
          doc_content_byte :  v.optional(v.instance(Buffer),), 
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_source_tran_doc_stagingDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_source_tran_doc_stagingDto,'vgph_source_tran_doc_staging','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_tran_doc_staging.updateMany({
      where: {vgphstds_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgphstds_id:number,
    updatevgph_source_tran_doc_stagingDto: Prisma.vgph_source_tran_doc_stagingUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphstds_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_source_tran_doc_staging update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_tran_doc_staging update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_source_tran_doc_staging update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphstds_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_tran_doc_staging.deleteMany({
      where: {vgphstds_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_source_tran_doc_staging');
    let decryptedRes: any = [];
...
vgphstds_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphstds_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_source_tran_doc_staging deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_tran_doc_staging deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_source_tran_doc_staging deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_tran_doc_staging.findFirst({ 
