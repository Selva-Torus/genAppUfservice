...
@Injectable()
export class itax_system_setupService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { itaxss_id }: {itaxss_id : number} = queryValue;
      ...
      const { parent_setup_code }: {parent_setup_code : string} = queryValue;

      if(itaxss_id){ 
        query.itaxss_id = { [queryCondition['itaxss_id']]: itaxss_id };
      }
      if(setup_code){ 
      ...
      if(parent_setup_code){ 
        query.parent_setup_code = { [queryCondition['parent_setup_code']]: parent_setup_code };
      }
      const skip = (page - 1) * limit;
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.withConnection(() =>
        this.prismaService.itax_system_setup.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'itax_system_setup');
      ...
      this.prismaService.itax_system_setup.count({
        where: query,
      }));
  ...
  async findOne(itaxss_id:number,token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.itax_system_setup.findUnique({ 
      where: {itaxss_id},
      select: {itaxss_id:true,setup_code:true,interface_product:true,category:true,sub_category:true,purpose:true,setup_value:true,parent_setup_code:true,        trs_created_date:true,
        trs_created_by:true,
  ...
  async findAll(token : string,trs_created_date?: Date,trs_created_by?: string,trs_modified_date?: Date,trs_modified_by?: string,trs_process_id?: string,trs_access_profile?: string,trs_org_grp_code?: string,trs_org_code?: string,trs_role_grp_code?: string,trs_role_code?: string,trs_ps_grp_code?: string,trs_ps_code?: string,trs_sub_org_grp_code?: string,trs_sub_org_code?: string,trs_locked_by?: string,trs_locked_time?: Date,trs_tenant_id?:string,trs_app_code?:string,trs_product_code?:string,trs_event_process_status?:string,trs_event_status?:string,trs_prev_process_code?:string,trs_prev_status?:string,trs_prev_process_status?:string,trs_process_code?:string,trs_status?:string,trs_process_status?:string,trs_next_process_code?:string,trs_next_status?:string,trs_next_process_status?:string,setup_code?:string) {
    try{
      ...
      if (setup_code) {
        whereClause.setup_code = setup_code;
      }
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.itax_system_setup.findMany({ 
      where: whereClause,
      ...
      for (const indiviual of res) {
        ...
        const decryptedData = await this.decryptData(plain, 'itax_system_setup');
        decryptedRes.push(decryptedData);
      }
      return decryptedRes;
    } catch (error) {
  ...
  async create(createitax_system_setupDto: Prisma.itax_system_setupCreateInput,token:string) {
    try{
      ...
      const dataSchema:any =  v.object({
            ...
            parent_setup_code :  v.optional(v.string()), 
        });
        let validate : any = v.safeParse(dataSchema,createitax_system_setupDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createitax_system_setupDto, 'itax_system_setup', 'create');
      const res = await this.prismaService.withConnection(() =>
   ...
   */
  async createMaster(
    createitax_system_setupDto: Prisma.itax_system_setupCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_system_setup creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_system_setup creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'itax_system_setup',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'itaxss_id',
        ...
        return {
          success: true,
          message: 'itax_system_setup creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(itaxss_id:number, updateitax_system_setupDto: Prisma.itax_system_setupUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updateitax_system_setupDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updateitax_system_setupDto,'itax_system_setup','update');
      const res = await this.prismaService.withConnection(() =>
...
/**
  ...
  async updateMaster(
itaxss_id:number,
    updateitax_system_setupDto: Prisma.itax_system_setupUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =itaxss_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'itax_system_setup update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_system_setup update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'itax_system_setup update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(itaxss_id:number,token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
...
itaxss_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =itaxss_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'itax_system_setup deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_system_setup deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'itax_system_setup deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.itax_system_setup.findFirst({ 
