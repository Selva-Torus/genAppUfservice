...
@Injectable()
export class itax_source_tran_paymentService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { itaxstp_id }: {itaxstp_id : number} = queryValue;
      const { eslip_no }: {eslip_no : string} = queryValue;
      const { product_basic }: {product_basic : any } = queryValue;

      if(itaxstp_id){ 
        query.itaxstp_id = { [queryCondition['itaxstp_id']]: itaxstp_id };
      }
      if(eslip_no){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.itax_source_tran_payment.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'itax_source_tran_payment');
      ...
      this.prismaService.itax_source_tran_payment.count({
        where: query,
      }));
  ...
  async findOne(itaxstp_id:number,token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.itax_source_tran_payment.findUnique({ 
      where: {itaxstp_id},
      select: {itaxstp_id:true,eslip_no:true,product_basic:true,itaxs_id :true,        trs_created_date:true,
        trs_created_by:true,
    ...
    return await this.decryptData(await this.commonDecimalDatahandle(res), 'itax_source_tran_payment');
  } catch (error) {
  ...
  async findAll(token : string,trs_created_date?: Date,trs_created_by?: string,trs_modified_date?: Date,trs_modified_by?: string,trs_process_id?: string,trs_access_profile?: string,trs_org_grp_code?: string,trs_org_code?: string,trs_role_grp_code?: string,trs_role_code?: string,trs_ps_grp_code?: string,trs_ps_code?: string,trs_sub_org_grp_code?: string,trs_sub_org_code?: string,trs_locked_by?: string,trs_locked_time?: Date,trs_tenant_id?:string,trs_app_code?:string,trs_product_code?:string,trs_event_process_status?:string,trs_event_status?:string,trs_token_id?:string,trs_prev_process_code?:string,trs_prev_status?:string,trs_prev_process_status?:string,trs_process_code?:string,trs_status?:string,trs_process_status?:string,trs_next_process_code?:string,trs_next_status?:string,trs_next_process_status?:string) {
    try{
      ...
      this.prismaService.itax_source_tran_payment.findMany({ 
      where: whereClause,
      select: {itaxstp_id:true,eslip_no:true,product_basic:true,          itaxs_id :true,    
      ...
      for (const indiviual of res) {
        ...
        const decryptedData = await this.decryptData(plain, 'itax_source_tran_payment');
        decryptedRes.push(decryptedData);
      }
      return decryptedRes;
    } catch (error) {
  ...
  async create(createitax_source_tran_paymentDto: Prisma.itax_source_tran_paymentCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            eslip_no :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            product_basic :  v.optional(v.any() ), 
        });
        let validate : any = v.safeParse(dataSchema,createitax_source_tran_paymentDto);
        if (!validate.success) {
   ...
   */
  async createMaster(
    createitax_source_tran_paymentDto: Prisma.itax_source_tran_paymentCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_source_tran_payment creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_source_tran_payment creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'itax_source_tran_payment',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'itaxstp_id',
        ...
        return {
          success: true,
          message: 'itax_source_tran_payment creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(itaxstp_id:number, updateitax_source_tran_paymentDto: Prisma.itax_source_tran_paymentUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updateitax_source_tran_paymentDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updateitax_source_tran_paymentDto,'itax_source_tran_payment','update');
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.itax_source_tran_payment.update({
      where: {itaxstp_id},
      data: encryptedData,
      select: {itaxstp_id:true,eslip_no:true,product_basic:true,itaxs_id :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_prev_process_code:true,trs_prev_status:true,trs_prev_process_status:true,trs_process_code:true,trs_status:true,trs_process_status:true,trs_next_process_code:true,trs_next_status:true,trs_next_process_status:true}
    }));
...
/**
  ...
  async updateMaster(
itaxstp_id:number,
    updateitax_source_tran_paymentDto: Prisma.itax_source_tran_paymentUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =itaxstp_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'itax_source_tran_payment update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_source_tran_payment update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'itax_source_tran_payment update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(itaxstp_id:number,token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.itax_source_tran_payment.delete({
      where: {itaxstp_id },
      select: {itaxstp_id:true,eslip_no:true,product_basic:true,itaxs_id :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_prev_process_code:true,trs_prev_status:true,trs_prev_process_status:true,trs_process_code:true,trs_status:true,trs_process_status:true,trs_next_process_code:true,trs_next_status:true,trs_next_process_status:true}
    }));
    return await this.decryptData(await this.commonDecimalDatahandle(res), 'itax_source_tran_payment');
...
itaxstp_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =itaxstp_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'itax_source_tran_payment deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_source_tran_payment deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'itax_source_tran_payment deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.itax_source_tran_payment.findFirst({ 
