...
@Injectable()
export class tof_consent_responseService {
  ...
  async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { data }: {data : Date} = queryValue;

      if(data){ 
        query.data = { [queryCondition['data']]: data };
      }
      const skip = (page - 1) * limit;
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.tof_consent_response.findMany({
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'tof_consent_response');
      ...
      const totalItems = await this.prismaService.tof_consent_response.count({
        where:Object.keys(query).length >0 ?query: undefined,
      });
  ...
  async findAll(token : string) {
    try{
      const res = await this.prismaService.tof_consent_response.findMany({ 
      select: {id:true,data:true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true}
      });
      let decryptedRes: any = [];
  ...
  async findAllwithquery(token : string,whereClause : Prisma.tof_consent_responseWhereInput) {
    try{
      whereClause = await this.encryptData(whereClause,'tof_consent_response','getAll')
      const res = await this.prismaService.tof_consent_response.findMany({ 
      where: whereClause,
      select: {id:true,data:true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true}
      });
  ...
  async create(createtof_consent_responseDto: Prisma.tof_consent_responseCreateInput,token:string) {
    try{
      const res = await this.prismaService.tof_consent_response.create({ 
      data: await this.encryptData(createtof_consent_responseDto,'tof_consent_response','create'), 
      select:{id:true,data:true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true}
    })
    return await this.decryptData(res, 'tof_consent_response');
  } catch (error) {
    ...
    async update(id:string,updatetof_consent_responseDto: Prisma.tof_consent_responseUpdateInput,token:string) {    
      try{
      const res = await this.prismaService.tof_consent_response.update({
      where: {id,},
      data: await this.encryptData(updatetof_consent_responseDto,'tof_consent_response','update'),
      select: {id:true,data:true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true},
    });
    return await this.decryptData(res, 'tof_consent_response');
    } catch (error) {
    ...
    try{
      const res = await this.prismaService.tof_consent_response.delete({
      where: {id, },
      select: {id:true,data:true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true}
    });
    return res;
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.tof_consent_response.findFirst({ 
