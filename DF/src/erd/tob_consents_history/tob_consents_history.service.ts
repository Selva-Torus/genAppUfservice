...
@Injectable()
export class tob_consents_historyService {
  ...
  async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      const { requesttimestamp }: {requesttimestamp :  Date} = queryValue;
      const { method }: {method : Date} = queryValue;
      const { baseconsentid }: {baseconsentid : Date} = queryValue;
      const { resourceconsentid }: {resourceconsentid : Date} = queryValue;
      const { consentgroupid }: {consentgroupid : Date} = queryValue;
      const { consenttype }: {consenttype : Date} = queryValue;
      const { requestdata }: {requestdata : any } = queryValue;
      const { responsedata }: {responsedata : any } = queryValue;

      if(requesturl){ 
        query.requesturl = { [queryCondition['requesturl']]: requesturl };
      }
      if(requesttimestamp){ 
        query.requesttimestamp = { [queryCondition['requesttimestamp']]: requesttimestamp };
      }
      if(method){ 
        query.method = { [queryCondition['method']]: method };
      }
      if(baseconsentid){ 
        query.baseconsentid = { [queryCondition['baseconsentid']]: baseconsentid };
      }
      if(resourceconsentid){ 
        query.resourceconsentid = { [queryCondition['resourceconsentid']]: resourceconsentid };
      }
      if(consentgroupid){ 
      ...
      if(requestdata){ 
        query.requestdata = { [queryCondition['requestdata']]: requestdata };
      }
      if(responsedata){ 
        query.responsedata = { [queryCondition['responsedata']]: responsedata };
      }
      const skip = (page - 1) * limit;
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.tob_consents_history.findMany({
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'tob_consents_history');
      ...
      const totalItems = await this.prismaService.tob_consents_history.count({
        where:Object.keys(query).length >0 ?query: undefined,
      });
    ...
    try{
      const res = await this.prismaService.tob_consents_history.findUnique({ 
      where: {id,},
      select: {id:true,requesturl:true,requesttimestamp:true,method:true,baseconsentid:true,resourceconsentid:true,consentgroupid:true,consenttype:true,requestdata:true,responsedata:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true}
    });
    return await this.decryptData(res, 'tob_consents_history');
  } catch (error) {
  ...
  async findAll(token : string) {
    try{
      const res = await this.prismaService.tob_consents_history.findMany({ 
      select: {id:true,requesturl:true,requesttimestamp:true,method:true,baseconsentid:true,resourceconsentid:true,consentgroupid:true,consenttype:true,requestdata:true,responsedata:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true}
      });
      let decryptedRes: any = [];
  ...
  async findAllwithquery(token : string,whereClause : Prisma.tob_consents_historyWhereInput) {
    try{
      whereClause = await this.encryptData(whereClause,'tob_consents_history','getAll')
      const res = await this.prismaService.tob_consents_history.findMany({ 
      where: whereClause,
      select: {id:true,requesturl:true,requesttimestamp:true,method:true,baseconsentid:true,resourceconsentid:true,consentgroupid:true,consenttype:true,requestdata:true,responsedata:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true}
      });
  ...
  async create(createtob_consents_historyDto: Prisma.tob_consents_historyCreateInput,token:string) {
    try{
      const res = await this.prismaService.tob_consents_history.create({ 
      data: await this.encryptData(createtob_consents_historyDto,'tob_consents_history','create'), 
      select:{id:true,requesturl:true,requesttimestamp:true,method:true,baseconsentid:true,resourceconsentid:true,consentgroupid:true,consenttype:true,requestdata:true,responsedata:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true}
    })
    return await this.decryptData(res, 'tob_consents_history');
  } catch (error) {
    ...
    async update(id:string,updatetob_consents_historyDto: Prisma.tob_consents_historyUpdateInput,token:string) {    
      try{
      const res = await this.prismaService.tob_consents_history.update({
      where: {id,},
      data: await this.encryptData(updatetob_consents_historyDto,'tob_consents_history','update'),
      select: {id:true,requesturl:true,requesttimestamp:true,method:true,baseconsentid:true,resourceconsentid:true,consentgroupid:true,consenttype:true,requestdata:true,responsedata:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true},
    });
    return await this.decryptData(res, 'tob_consents_history');
    } catch (error) {
    ...
    try{
      const res = await this.prismaService.tob_consents_history.delete({
      where: {id, },
      select: {id:true,requesturl:true,requesttimestamp:true,method:true,baseconsentid:true,resourceconsentid:true,consentgroupid:true,consenttype:true,requestdata:true,responsedata:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true}
    });
    return res;
