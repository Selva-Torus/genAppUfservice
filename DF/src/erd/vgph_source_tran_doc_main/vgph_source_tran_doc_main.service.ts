...
import { vgph_source_tran_doc_mainEntity } from './entity/vgph_source_tran_doc_main.entity';
import { CustomException } from 'src/customException';
@Injectable()
export class vgph_source_tran_doc_mainService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphstdm_id }: {vgphstdm_id : bigint} = queryValue;
      ...
      if(vgphstdm_id){ 
        query.vgphstdm_id = { [queryCondition['vgphstdm_id']]: vgphstdm_id };
      }
      if(vgphstm_uuid){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_source_tran_doc_main.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_source_tran_doc_main');
      ...
      this.prismaService.vgph_source_tran_doc_main.count({
        where: query,
      }));
  ...
  async create(createvgph_source_tran_doc_mainDto: Prisma.vgph_source_tran_doc_mainCreateInput,token:string) {
    try{

      enum category_vgph_source_tran_doc_main{
        front="front",
        back="back",
      }
      enum doc_type_vgph_source_tran_doc_main{
        tiff="tiff",
        jpg="jpg",
        uv="uv",
        color="color",
      }
      enum doc_group_vgph_source_tran_doc_main{
        cheque="cheque",
        doc="doc",
        id_proof="id_proof",
      ...
      const dataSchema:any =  v.object({
            vgphstdm_id :v.number() , 
            vgphstm_uuid :v.pipe(v.string(),v.maxLength(64 )) , 
            category :v.enum(category_vgph_source_tran_doc_main,"Invalid category_vgph_source_tran_doc_main enum")  , 
            doc_type :  v.optional(v.enum(doc_type_vgph_source_tran_doc_main,"Invalid doc_type_vgph_source_tran_doc_main enum")), 
            doc_group :  v.optional(v.enum(doc_group_vgph_source_tran_doc_main,"Invalid doc_group_vgph_source_tran_doc_main enum")), 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_source_tran_doc_mainDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_source_tran_doc_mainDto, 'vgph_source_tran_doc_main', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_source_tran_doc_main.create({
          data: encryptedData,
          select:{vgphstdm_id:true,vgphstm_uuid:true,category:true,doc_type:true,doc_group:true,doc_name:true,url:true,doc_additional:true,doc_content_text:true,doc_content_byte:true,doc_size:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,trs_prev_process_code:true,trs_prev_status:true,trs_prev_process_status:true,trs_process_code:true,trs_status:true,trs_process_status:true,trs_next_process_code:true,trs_next_status:true,trs_next_process_status:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_source_tran_doc_mainDto: Prisma.vgph_source_tran_doc_mainCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_tran_doc_main creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_tran_doc_main creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_source_tran_doc_main',
        ...
        return {
          success: true,
          message: 'vgph_source_tran_doc_main creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphstdm_id:number, updatevgph_source_tran_doc_mainDto: Prisma.vgph_source_tran_doc_mainUpdateInput,token:string) {   
    try{
      ...
      const dataSchema:any =  v.object({
          ...
          category :  v.optional(v.enum(category_vgph_source_tran_doc_main,"Invalid category_vgph_source_tran_doc_main enum")), 
          doc_type :  v.optional(v.enum(doc_type_vgph_source_tran_doc_main,"Invalid doc_type_vgph_source_tran_doc_main enum")), 
          doc_group :  v.optional(v.enum(doc_group_vgph_source_tran_doc_main,"Invalid doc_group_vgph_source_tran_doc_main enum")), 
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_source_tran_doc_mainDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_source_tran_doc_mainDto,'vgph_source_tran_doc_main','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_tran_doc_main.updateMany({
      where: {vgphstdm_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgphstdm_id:number,
    updatevgph_source_tran_doc_mainDto: Prisma.vgph_source_tran_doc_mainUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphstdm_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_source_tran_doc_main update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_tran_doc_main update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_source_tran_doc_main update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphstdm_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_tran_doc_main.deleteMany({
      where: {vgphstdm_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_source_tran_doc_main');
    let decryptedRes: any = [];
...
vgphstdm_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphstdm_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_source_tran_doc_main deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_tran_doc_main deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_source_tran_doc_main deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_tran_doc_main.findFirst({ 
