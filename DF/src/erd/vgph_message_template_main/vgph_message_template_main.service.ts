...
@Injectable()
export class vgph_message_template_mainService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphmtm_id }: {vgphmtm_id : number} = queryValue;
      ...
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.vgph_message_template_main.findMany({
          select:columns,
          where: query,          
        });
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_message_template_main');
      ...
      const totalItems = await this.prismaService.vgph_message_template_main.count({
        where: query,
      });
  ...
  async findOne(vgphmtm_id:number,token : string) {
    try{
  ...
  async create(createvgph_message_template_mainDto: Prisma.vgph_message_template_mainCreateInput,token:string) {
    try{

      enum direction_vgph_message_template_main{
        OUTBOUND="OUTBOUND",
        INBOUND="INBOUND",
      }
      enum process_type_vgph_message_template_main{
        OP="OP",
        IR="IR",
        IP="IP",
        OR="OR",
      ...
      const dataSchema:any =  v.object({
            ...
            direction :v.enum(direction_vgph_message_template_main,"Invalid direction_vgph_message_template_main enum")  , 
   ...
   */
  async createMaster(
    createvgph_message_template_mainDto: Prisma.vgph_message_template_mainCreateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_message_template_main creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_message_template_main creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      const encryptedData = await this.encryptData(createvgph_message_template_mainDto, 'vgph_message_template_main', 'create');
      ...
      if(role === 'MAKER')
      {
        ...
        const result = await this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_message_template_main',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphmtm_id',
        ...
        return {
          success: true,
          message: 'vgph_message_template_main creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphmtm_id:number, updatevgph_message_template_mainDto: Prisma.vgph_message_template_mainUpdateInput,token:string) {   
    try{
      ...
      const dataSchema:any =  v.object({
          ...
          direction :  v.optional(v.enum(direction_vgph_message_template_main,"Invalid direction_vgph_message_template_main enum")), 
...
/**
  ...
  async updateMaster(
vgphmtm_id:number,
    updatevgph_message_template_mainDto: Prisma.vgph_message_template_mainUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphmtm_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_message_template_main update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_message_template_main update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      const existingRecord = await this.prismaService.vgph_message_template_main.findUnique({
        where: {vgphmtm_id}
      });
      ...
      // Encrypt data if needed
      const encryptedData = await this.encryptData(updatevgph_message_template_mainDto, 'vgph_message_template_main', 'update');
      ...
      return {
        success: true,
        message: 'vgph_message_template_main update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphmtm_id:number,token : string) {
    try{
...
vgphmtm_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphmtm_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_message_template_main deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_message_template_main deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_message_template_main deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.vgph_message_template_main.findFirst({ 
