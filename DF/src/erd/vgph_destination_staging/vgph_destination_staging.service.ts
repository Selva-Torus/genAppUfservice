...
export class vgph_destination_stagingService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphds_id }: {vgphds_id : bigint} = queryValue;
      ...
      const { destination_category }: {destination_category : string} = queryValue;
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_destination_staging.findMany({
          select:columns,
          where: query,          
        }));
      ...
      this.prismaService.vgph_destination_staging.count({
        where: query,
      }));
  ...
  async findOne(vgphds_id:number,trs_tenant_id: string,trs_app_code: string,trs_product_code: string,token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_destination_staging.findMany({ 
      where: {vgphds_id,trs_tenant_id,trs_app_code,trs_product_code},
      select: {vgphds_id:true,direction:true,process_type:true,tran_category:true,message_code:true,destination_category:true,destination_reference:true,destination_name:true,destination_parent_name:true,destionation_status:true,uuid:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,        }
    }));
    //return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_destination_staging');
    let decryptedRes: any = [];
    for (const indiviual of res) {
      const plain = await this.commonDecimalDatahandle(indiviual)
      const decryptedData = await this.decryptData(plain, 'vgph_destination_staging');
      decryptedRes.push(decryptedData);
    }
    return decryptedRes;
  } catch (error:any) {
  ...
  async create(createvgph_destination_stagingDto: Prisma.vgph_destination_stagingCreateInput,token:string) {
    try{
      ...
      const dataSchema:any =  v.object({
            ...
            destination_category :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            destination_reference :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            destination_name :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
      ...
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_destination_staging.create({
          data: encryptedData,
          select:{vgphds_id:true,direction:true,process_type:true,tran_category:true,message_code:true,destination_category:true,destination_reference:true,destination_name:true,destination_parent_name:true,destionation_status:true,uuid:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
  ...
  async update(vgphds_id:number, updatevgph_destination_stagingDto: Prisma.vgph_destination_stagingUpdateInput,token:string) {   
    try{
      ...
      this.prismaService.vgph_destination_staging.updateMany({
      where: {vgphds_id},
      data: encryptedData
    }));
...
vgphds_id:number,
  ...
  async remove(vgphds_id:number,token : string) {
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_destination_staging.deleteMany({
      where: {vgphds_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_destination_staging');
    let decryptedRes: any = [];
...
vgphds_id:number,
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_destination_staging.findFirst({ 
