...
@Injectable()
export class vgph_destination_stagingService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphds_id }: {vgphds_id : number} = queryValue;
      ...
      const { destination_category }: {destination_category : Date} = queryValue;
      const { destination_reference }: {destination_reference : string} = queryValue;
      const { destination_name }: {destination_name : string} = queryValue;
      const { destination_parent_name }: {destination_parent_name : string} = queryValue;
      const { destionation_status }: {destionation_status : string} = queryValue;
      const { uuid }: {uuid : string} = queryValue;

      if(vgphds_id){ 
        query.vgphds_id = { [queryCondition['vgphds_id']]: vgphds_id };
      }
      if(tenant_id){ 
      ...
      if(destination_category){ 
        query.destination_category = { [queryCondition['destination_category']]: destination_category };
      }
      if(destination_reference){ 
        query.destination_reference = { [queryCondition['destination_reference']]: destination_reference };
      }
      if(destination_name){ 
        query.destination_name = { [queryCondition['destination_name']]: destination_name };
      }
      if(destination_parent_name){ 
        query.destination_parent_name = { [queryCondition['destination_parent_name']]: destination_parent_name };
      }
      if(destionation_status){ 
        query.destionation_status = { [queryCondition['destionation_status']]: destionation_status };
      }
      if(uuid){ 
      ...
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.vgph_destination_staging.findMany({
          select:columns,
          where: query,          
        });
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_destination_staging');
      ...
      const totalItems = await this.prismaService.vgph_destination_staging.count({
        where: query,
      });
  ...
  async findOne(vgphds_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_destination_staging.findUnique({ 
      where: {vgphds_id},
      select: {vgphds_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,message_code:true,destination_category:true,destination_reference:true,destination_name:true,destination_parent_name:true,destionation_status:true,uuid:true,            vgph_destination_tran_staging:{
              select:{
    ...
    return  await this.decryptData(res, 'vgph_destination_staging');
  } catch (error) {
...
,vgphds_id?:number) {
    try{
      ...
      if (vgphds_id) {
        whereClause.vgphds_id = vgphds_id;
      }
      const res = await this.prismaService.vgph_destination_staging.findMany({ 
      where: whereClause,
      select: {vgphds_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,message_code:true,destination_category:true,destination_reference:true,destination_name:true,destination_parent_name:true,destionation_status:true,uuid:true,          vgph_destination_tran_staging:{
              select:{
              vgphdts_id:true,              tenant_id:true,              product_code:true,              direction:true,              process_type:true,              tran_category:true,              vgphds_uuid:true,              vgphsts_uuid:true            ,
  ...
  async create(createvgph_destination_stagingDto: Prisma.vgph_destination_stagingCreateInput,token:string) {
    try{

      enum direction_vgph_destination_staging{
        OUTBOUND="OUTBOUND",
        INBOUND="INBOUND",
      }
      enum process_type_vgph_destination_staging{
        OP="OP",
        IR="IR",
        IP="IP",
        OR="OR",
      }
      enum tran_category_vgph_destination_staging{
        Financial="Financial",
        Non_Financial="Non_Financial",
      }
      enum destination_category_vgph_destination_staging{
        QUEUE="QUEUE",
        API="API",
        FILE="FILE",
      }
      const dataSchema:any =  v.object({
            ...
            direction :v.enum(direction_vgph_destination_staging,"Invalid direction_vgph_destination_staging enum")  , 
            process_type :v.enum(process_type_vgph_destination_staging,"Invalid process_type_vgph_destination_staging enum")  , 
            tran_category :v.enum(tran_category_vgph_destination_staging,"Invalid tran_category_vgph_destination_staging enum")  , 
            message_code :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            destination_category :  v.optional(v.enum(destination_category_vgph_destination_staging,"Invalid destination_category_vgph_destination_staging enum")), 
            destination_reference :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            destination_name :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            destination_parent_name :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            destionation_status :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            uuid :v.pipe(v.string(),v.maxLength(64 )) , 
        });
        let validate : any = v.safeParse(dataSchema,createvgph_destination_stagingDto);
        if (!validate.success) {
      ...
      const res = await this.prismaService.vgph_destination_staging.create({ 
      data: await this.encryptData(createvgph_destination_stagingDto,'vgph_destination_staging','create'),
      select:{vgphds_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,message_code:true,destination_category:true,destination_reference:true,destination_name:true,destination_parent_name:true,destionation_status:true,uuid:true,vgph_destination_tran_staging:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
   ...
   */
  async createMaster(
    createvgph_destination_stagingDto: Prisma.vgph_destination_stagingCreateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_destination_staging creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_destination_staging creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      const encryptedData = await this.encryptData(createvgph_destination_stagingDto, 'vgph_destination_staging', 'create');
      ...
      if(role === 'MAKER')
      {
        ...
        const result = await this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_destination_staging',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphds_id',
        ...
        return {
          success: true,
          message: 'vgph_destination_staging creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphds_id:number, updatevgph_destination_stagingDto: Prisma.vgph_destination_stagingUpdateInput,token:string) {   
    try{
      ...
      const dataSchema:any =  v.object({
          ...
          direction :  v.optional(v.enum(direction_vgph_destination_staging,"Invalid direction_vgph_destination_staging enum")), 
          process_type :  v.optional(v.enum(process_type_vgph_destination_staging,"Invalid process_type_vgph_destination_staging enum")), 
          tran_category :  v.optional(v.enum(tran_category_vgph_destination_staging,"Invalid tran_category_vgph_destination_staging enum")), 
          message_code :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
          destination_category :  v.optional(v.enum(destination_category_vgph_destination_staging,"Invalid destination_category_vgph_destination_staging enum")), 
          destination_reference :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
          destination_name :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_destination_stagingDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const res = await this.prismaService.vgph_destination_staging.update({
      where: {vgphds_id},
      data: await this.encryptData(updatevgph_destination_stagingDto,'vgph_destination_staging','update'),
      select: {vgphds_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,message_code:true,destination_category:true,destination_reference:true,destination_name:true,destination_parent_name:true,destionation_status:true,uuid:true,vgph_destination_tran_staging:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return await this.decryptData(res, 'vgph_destination_staging');
    } catch (error) {
...
/**
  ...
  async updateMaster(
vgphds_id:number,
    updatevgph_destination_stagingDto: Prisma.vgph_destination_stagingUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphds_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_destination_staging update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_destination_staging update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      const existingRecord = await this.prismaService.vgph_destination_staging.findUnique({
        where: {vgphds_id}
      });
      ...
      // Encrypt data if needed
      const encryptedData = await this.encryptData(updatevgph_destination_stagingDto, 'vgph_destination_staging', 'update');
      ...
      return {
        success: true,
        message: 'vgph_destination_staging update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphds_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_destination_staging.delete({
      where: {vgphds_id },
      select: {vgphds_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,message_code:true,destination_category:true,destination_reference:true,destination_name:true,destination_parent_name:true,destionation_status:true,uuid:true,vgph_destination_tran_staging:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return res;
...
vgphds_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphds_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_destination_staging deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_destination_staging deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_destination_staging deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.vgph_destination_staging.findFirst({ 
