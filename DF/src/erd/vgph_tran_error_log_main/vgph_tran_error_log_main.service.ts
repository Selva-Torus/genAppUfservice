...
export class vgph_tran_error_log_mainService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphtelm_id }: {vgphtelm_id : number} = queryValue;
      ...
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.vgph_tran_error_log_main.findMany({
          select:columns,
          where: query,          
        });
      ...
      const totalItems = await this.prismaService.vgph_tran_error_log_main.count({
        where: query,
      });
  ...
  async findOne(vgphtelm_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_tran_error_log_main.findUnique({ 
      where: {vgphtelm_id},
      select: {vgphtelm_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,process_code:true,error_cateogry:true,error_code:true,error_desc:true,error_data:true,vgphstm_uuid:true,vgphstm_id :true,        trs_created_date:true,
        trs_created_by:true,
    ...
    return  await this.decryptData(res, 'vgph_tran_error_log_main');
  } catch (error) {
...
,vgphtelm_id?:number) {
    try{
      ...
      const res = await this.prismaService.vgph_tran_error_log_main.findMany({ 
      where: whereClause,
      select: {vgphtelm_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,process_code:true,error_cateogry:true,error_code:true,error_desc:true,error_data:true,vgphstm_uuid:true,          vgphstm_id :true,    
  ...
  async create(createvgph_tran_error_log_mainDto: Prisma.vgph_tran_error_log_mainCreateInput,token:string) {
    try{
      ...
      enum tran_category_vgph_tran_error_log_main{
        Financial="Financial",
        Non_Financial="Non_Financial",
      }
      enum error_cateogry_vgph_tran_error_log_main{
      ...
      const dataSchema:any =  v.object({
            ...
            tran_category :v.enum(tran_category_vgph_tran_error_log_main,"Invalid tran_category_vgph_tran_error_log_main enum")  , 
            process_code :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            error_cateogry :  v.optional(v.enum(error_cateogry_vgph_tran_error_log_main,"Invalid error_cateogry_vgph_tran_error_log_main enum")), 
      ...
      const res = await this.prismaService.vgph_tran_error_log_main.create({ 
      data: await this.encryptData(createvgph_tran_error_log_mainDto,'vgph_tran_error_log_main','create'),
      select:{vgphtelm_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,process_code:true,error_cateogry:true,error_code:true,error_desc:true,error_data:true,vgphstm_uuid:true,vgphstm_id :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
      ...
      if(role === 'MAKER')
      {
        ...
        const result = await this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            ...
            p_record_id      := NULL,
            p_record_id_column := 'vgphtelm_id',
  ...
  async update(vgphtelm_id:number, updatevgph_tran_error_log_mainDto: Prisma.vgph_tran_error_log_mainUpdateInput,token:string) {   
    try{
      ...
      const dataSchema:any =  v.object({
          ...
          tran_category :  v.optional(v.enum(tran_category_vgph_tran_error_log_main,"Invalid tran_category_vgph_tran_error_log_main enum")), 
          process_code :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
          error_cateogry :  v.optional(v.enum(error_cateogry_vgph_tran_error_log_main,"Invalid error_cateogry_vgph_tran_error_log_main enum")), 
          error_code :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
          error_desc :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
      ...
      const res = await this.prismaService.vgph_tran_error_log_main.update({
      where: {vgphtelm_id},
      data: await this.encryptData(updatevgph_tran_error_log_mainDto,'vgph_tran_error_log_main','update'),
      select: {vgphtelm_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,process_code:true,error_cateogry:true,error_code:true,error_desc:true,error_data:true,vgphstm_uuid:true,vgphstm_id :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return await this.decryptData(res, 'vgph_tran_error_log_main');
    } catch (error) {
...
vgphtelm_id:number,
    ...
    try {
      ...
      const existingRecord = await this.prismaService.vgph_tran_error_log_main.findUnique({
        where: {vgphtelm_id}
      });
  ...
  async remove(vgphtelm_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_tran_error_log_main.delete({
      where: {vgphtelm_id },
      select: {vgphtelm_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,process_code:true,error_cateogry:true,error_code:true,error_desc:true,error_data:true,vgphstm_uuid:true,vgphstm_id :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return res;
...
vgphtelm_id:number,
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.vgph_tran_error_log_main.findFirst({ 
