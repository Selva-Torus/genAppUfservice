...
@Injectable()
export class vgph_charge_setupService {
  ...
  async findSchema (token) {
    const data = {
            ...
            charge_type:"string",
            charge_amount:"float",
            min_charge_amt:"float",
            max_charge_amt:"float",
            vat_flag:"string",
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphcs_id }: {vgphcs_id : bigint} = queryValue;
      const { process_category }: {process_category : string} = queryValue;
      const { charge_category }: {charge_category : string} = queryValue;
      const { charge_code }: {charge_code : string} = queryValue;
      const { charge_name }: {charge_name : string} = queryValue;
      const { currency }: {currency : string} = queryValue;
      const { processing_system }: {processing_system : string} = queryValue;
      const { charge_type }: {charge_type : string} = queryValue;
      const { charge_amount }: {charge_amount : any } = queryValue;
      const { min_charge_amt }: {min_charge_amt : any } = queryValue;
      const { max_charge_amt }: {max_charge_amt : any } = queryValue;
      const { vat_flag }: {vat_flag : string} = queryValue;
      const { vat_percentage }: {vat_percentage : any } = queryValue;
      ...
      if(vgphcs_id){ 
        query.vgphcs_id = { [queryCondition['vgphcs_id']]: vgphcs_id };
      }
      if(process_category){ 
        query.process_category = { [queryCondition['process_category']]: process_category };
      }
      if(charge_category){ 
        query.charge_category = { [queryCondition['charge_category']]: charge_category };
      }
      if(charge_code){ 
        query.charge_code = { [queryCondition['charge_code']]: charge_code };
      }
      if(charge_name){ 
        query.charge_name = { [queryCondition['charge_name']]: charge_name };
      }
      if(currency){ 
      ...
      if(min_charge_amt){ 
        query.min_charge_amt = { [queryCondition['min_charge_amt']]: min_charge_amt };
      }
      if(max_charge_amt){ 
        query.max_charge_amt = { [queryCondition['max_charge_amt']]: max_charge_amt };
      }
      if(vat_flag){ 
        query.vat_flag = { [queryCondition['vat_flag']]: vat_flag };
      }
      if(vat_percentage){ 
        query.vat_percentage = { [queryCondition['vat_percentage']]: vat_percentage };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_charge_setup.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_charge_setup');
      ...
      this.prismaService.vgph_charge_setup.count({
        where: query,
      }));
...
,vgphcs_id?:number) {
    try{
      ...
      if (vgphcs_id) {
        whereClause.vgphcs_id = vgphcs_id;
      }
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_charge_setup.findMany({ 
      where: whereClause,
  ...
  async create(createvgph_charge_setupDto: Prisma.vgph_charge_setupCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            vgphcs_id :v.number() , 
            process_category :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            charge_category :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            charge_code :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            charge_name :v.pipe(v.string(),v.maxLength(128 )) , 
            currency :v.pipe(v.string(),v.maxLength(3 )) , 
            processing_system :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            charge_type :v.pipe(v.string(),v.maxLength(16 )) , 
            charge_amount :  v.optional(v.any()), 
            min_charge_amt :  v.optional(v.any()), 
            max_charge_amt :  v.optional(v.any()), 
            vat_flag :  v.optional(v.pipe(v.string(),v.maxLength(1 ))), 
            vat_percentage :  v.optional(v.any()), 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_charge_setupDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_charge_setupDto, 'vgph_charge_setup', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_charge_setup.create({
          data: encryptedData,
          select:{vgphcs_id:true,process_category:true,charge_category:true,charge_code:true,charge_name:true,currency:true,processing_system:true,charge_type:true,charge_amount:true,min_charge_amt:true,max_charge_amt:true,vat_flag:true,vat_percentage:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_charge_setupDto: Prisma.vgph_charge_setupCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_charge_setup creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_charge_setup creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_charge_setup',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphcs_id',
        ...
        return {
          success: true,
          message: 'vgph_charge_setup creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphcs_id:number, updatevgph_charge_setupDto: Prisma.vgph_charge_setupUpdateInput,token:string) {   
    try{
      ...
      const dataSchema:any =  v.object({
          ...
          charge_category :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
          charge_code :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
          charge_name :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
          currency :  v.optional(v.pipe(v.string(),v.maxLength(3 ))), 
          processing_system :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
          charge_type :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
          charge_amount :  v.optional(v.number()), 
          min_charge_amt :  v.optional(v.number()), 
          max_charge_amt :  v.optional(v.number()), 
          vat_flag :  v.optional(v.pipe(v.string(),v.maxLength(1 ))), 
          vat_percentage :  v.optional(v.number()), 
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_charge_setupDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_charge_setupDto,'vgph_charge_setup','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_charge_setup.updateMany({
      where: {vgphcs_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgphcs_id:number,
    updatevgph_charge_setupDto: Prisma.vgph_charge_setupUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphcs_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_charge_setup update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_charge_setup update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_charge_setup update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphcs_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_charge_setup.deleteMany({
      where: {vgphcs_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_charge_setup');
    let decryptedRes: any = [];
...
vgphcs_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphcs_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_charge_setup deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_charge_setup deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_charge_setup deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_charge_setup.findFirst({ 
