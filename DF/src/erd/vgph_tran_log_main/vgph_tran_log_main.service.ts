...
import { vgph_tran_log_mainEntity } from './entity/vgph_tran_log_main.entity';
import { CustomException } from 'src/customException';
@Injectable()
export class vgph_tran_log_mainService {
  ...
  async findSchema (token) {
    const data = {
      ...
      version:"string",
      vgphstm_uuid:"string",
      vgphstm_id :"number",      trs_created_date:"Date",
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphtlm_id }: {vgphtlm_id : number} = queryValue;
      ...
      if(vgphtlm_id){ 
        query.vgphtlm_id = { [queryCondition['vgphtlm_id']]: vgphtlm_id };
      }
      if(tenant_id){ 
      ...
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.vgph_tran_log_main.findMany({
          select:columns,
          where: query,          
        });
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_tran_log_main');
      ...
      const totalItems = await this.prismaService.vgph_tran_log_main.count({
        where: query,
      });
  ...
  async findOne(vgphtlm_id:number,token : string) {
    ...
    return  await this.decryptData(res, 'vgph_tran_log_main');
  } catch (error) {
...
,vgphtlm_id?:number) {
    try{
      ...
      if (vgphtlm_id) {
        whereClause.vgphtlm_id = vgphtlm_id;
      }
      const res = await this.prismaService.vgph_tran_log_main.findMany({ 
      where: whereClause,
      select: {vgphtlm_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,process_category:true,processing_system:true,message_code:true,process_code:true,previous_process_code:true,next_process_code:true,flow_code:true,step_code:true,task_start_time:true,task_end_time:true,source_data:true,request_data:true,response_data:true,message_data:true,process_start_time:true,process_end_time:true,retry_flag:true,retry_count:true,manual_context:true,action_context:true,status:true,action:true,version:true,vgphstm_uuid:true,          vgphstm_id :true,    
  ...
  async create(createvgph_tran_log_mainDto: Prisma.vgph_tran_log_mainCreateInput,token:string) {
    try{

      enum direction_vgph_tran_log_main{
        OUTBOUND="OUTBOUND",
        INBOUND="INBOUND",
      }
      enum process_type_vgph_tran_log_main{
        OP="OP",
        IR="IR",
        IP="IP",
        OR="OR",
      }
      enum tran_category_vgph_tran_log_main{
        Financial="Financial",
      ...
      const dataSchema:any =  v.object({
            ...
            direction :v.enum(direction_vgph_tran_log_main,"Invalid direction_vgph_tran_log_main enum")  , 
            process_type :v.enum(process_type_vgph_tran_log_main,"Invalid process_type_vgph_tran_log_main enum")  , 
            tran_category :v.enum(tran_category_vgph_tran_log_main,"Invalid tran_category_vgph_tran_log_main enum")  , 
            ...
            vgphstm_uuid :v.pipe(v.string(),v.maxLength(64 )) , 
        });
        let validate : any = v.safeParse(dataSchema,createvgph_tran_log_mainDto);
        if (!validate.success) {
      ...
      const res = await this.prismaService.vgph_tran_log_main.create({ 
      data: await this.encryptData(createvgph_tran_log_mainDto,'vgph_tran_log_main','create'),
      select:{vgphtlm_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,process_category:true,processing_system:true,message_code:true,process_code:true,previous_process_code:true,next_process_code:true,flow_code:true,step_code:true,task_start_time:true,task_end_time:true,source_data:true,request_data:true,response_data:true,message_data:true,process_start_time:true,process_end_time:true,retry_flag:true,retry_count:true,manual_context:true,action_context:true,status:true,action:true,version:true,vgphstm_uuid:true,vgphstm_id :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
   ...
   */
  async createMaster(
    createvgph_tran_log_mainDto: Prisma.vgph_tran_log_mainCreateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_tran_log_main creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_tran_log_main creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      const encryptedData = await this.encryptData(createvgph_tran_log_mainDto, 'vgph_tran_log_main', 'create');
      ...
      if(role === 'MAKER')
      {
        ...
        const result = await this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_tran_log_main',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphtlm_id',
        ...
        return {
          success: true,
          message: 'vgph_tran_log_main creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphtlm_id:number, updatevgph_tran_log_mainDto: Prisma.vgph_tran_log_mainUpdateInput,token:string) {   
    try{
      ...
      const dataSchema:any =  v.object({
          ...
          direction :  v.optional(v.enum(direction_vgph_tran_log_main,"Invalid direction_vgph_tran_log_main enum")), 
          process_type :  v.optional(v.enum(process_type_vgph_tran_log_main,"Invalid process_type_vgph_tran_log_main enum")), 
          tran_category :  v.optional(v.enum(tran_category_vgph_tran_log_main,"Invalid tran_category_vgph_tran_log_main enum")), 
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_tran_log_mainDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const res = await this.prismaService.vgph_tran_log_main.update({
      where: {vgphtlm_id},
      data: await this.encryptData(updatevgph_tran_log_mainDto,'vgph_tran_log_main','update'),
      select: {vgphtlm_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,process_category:true,processing_system:true,message_code:true,process_code:true,previous_process_code:true,next_process_code:true,flow_code:true,step_code:true,task_start_time:true,task_end_time:true,source_data:true,request_data:true,response_data:true,message_data:true,process_start_time:true,process_end_time:true,retry_flag:true,retry_count:true,manual_context:true,action_context:true,status:true,action:true,version:true,vgphstm_uuid:true,vgphstm_id :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return await this.decryptData(res, 'vgph_tran_log_main');
    } catch (error) {
...
/**
  ...
  async updateMaster(
vgphtlm_id:number,
    updatevgph_tran_log_mainDto: Prisma.vgph_tran_log_mainUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphtlm_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_tran_log_main update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_tran_log_main update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      const existingRecord = await this.prismaService.vgph_tran_log_main.findUnique({
        where: {vgphtlm_id}
      });
      ...
      // Encrypt data if needed
      const encryptedData = await this.encryptData(updatevgph_tran_log_mainDto, 'vgph_tran_log_main', 'update');
      ...
      return {
        success: true,
        message: 'vgph_tran_log_main update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphtlm_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_tran_log_main.delete({
      where: {vgphtlm_id },
      select: {vgphtlm_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,process_category:true,processing_system:true,message_code:true,process_code:true,previous_process_code:true,next_process_code:true,flow_code:true,step_code:true,task_start_time:true,task_end_time:true,source_data:true,request_data:true,response_data:true,message_data:true,process_start_time:true,process_end_time:true,retry_flag:true,retry_count:true,manual_context:true,action_context:true,status:true,action:true,version:true,vgphstm_uuid:true,vgphstm_id :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return res;
...
vgphtlm_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphtlm_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_tran_log_main deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_tran_log_main deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_tran_log_main deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.vgph_tran_log_main.findFirst({ 
