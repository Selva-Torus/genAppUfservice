...
@Injectable()
export class vgph_holidayService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgph_hid }: {vgph_hid : bigint} = queryValue;
      ...
      if(vgph_hid){ 
        query.vgph_hid = { [queryCondition['vgph_hid']]: vgph_hid };
      }
      if(code){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_holiday.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_holiday');
      ...
      this.prismaService.vgph_holiday.count({
        where: query,
      }));
  ...
  async create(createvgph_holidayDto: Prisma.vgph_holidayCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            vgph_hid :v.pipe(v.number(),v.maxValue(9999999999999999999999999999999999999999999999999999999999999999 )) , 
            code :v.pipe(v.string(),v.maxLength(16 )) , 
            holiday_date :v.pipe(
                  v.string(),
                  v.regex(
            ...
            holiday_flag :v.pipe(v.string(),v.maxLength(1 )) , 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_holidayDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_holidayDto, 'vgph_holiday', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_holiday.create({
          data: encryptedData,
          select:{vgph_hid:true,code:true,holiday_date:true,description:true,holiday_flag:true,additional_info:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_holidayDto: Prisma.vgph_holidayCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_holiday creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_holiday creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_holiday',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgph_hid',
        ...
        return {
          success: true,
          message: 'vgph_holiday creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgph_hid:number, updatevgph_holidayDto: Prisma.vgph_holidayUpdateInput,token:string) {   
    try{

      const dataSchema:any =  v.object({
          code :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_holidayDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_holidayDto,'vgph_holiday','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_holiday.updateMany({
      where: {vgph_hid},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgph_hid:number,
    updatevgph_holidayDto: Prisma.vgph_holidayUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgph_hid;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_holiday update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_holiday update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_holiday update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgph_hid:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_holiday.deleteMany({
      where: {vgph_hid}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_holiday');
    let decryptedRes: any = [];
...
vgph_hid:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgph_hid;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_holiday deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_holiday deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_holiday deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_holiday.findFirst({ 
