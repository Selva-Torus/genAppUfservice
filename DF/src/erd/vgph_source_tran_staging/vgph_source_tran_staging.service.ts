...
export class vgph_source_tran_stagingService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphsts_id }: {vgphsts_id : bigint} = queryValue;
      ...
      const { product_common }: {product_common : any } = queryValue;
      const { vgphss_id }: {vgphss_id : bigint} = queryValue;
      const { prcts_id }: {prcts_id : bigint} = queryValue;
      ...
      if(product_common){ 
        query.product_common = { [queryCondition['product_common']]: product_common };
      }
      if(vgphss_id){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_source_tran_staging.findMany({
          select:columns,
          where: query,          
        }));
      ...
      this.prismaService.vgph_source_tran_staging.count({
        where: query,
      }));
  ...
  async create(createvgph_source_tran_stagingDto: Prisma.vgph_source_tran_stagingCreateInput,token:string) {
    try{
      ...
      const dataSchema:any =  v.object({
            ...
            product_common :  v.optional(v.any() ), 
            vgphss_id :  v.optional(v.number()), 
      ...
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_source_tran_staging.create({
          data: encryptedData,
          select:{vgphsts_id:true,parent_vgphsts_uuid:true,direction:true,process_type:true,tran_category:true,message_code:true,channel_name:true,channel_reference:true,tran_date:true,product_basic:true,product_additional:true,uuid:true,vgphss_uuid:true,product_common:true,vgphss_id:true,prcts_id:true,source_content:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,trs_prev_process_code:true,trs_prev_status:true,trs_prev_process_status:true,trs_process_code:true,trs_status:true,trs_process_status:true,trs_next_process_code:true,trs_next_status:true,trs_next_process_status:true,}          
        })
      );
  ...
  async update(vgphsts_id:number, updatevgph_source_tran_stagingDto: Prisma.vgph_source_tran_stagingUpdateInput,token:string) {   
    try{
      ...
      this.prismaService.vgph_source_tran_staging.updateMany({
      where: {vgphsts_id},
      data: encryptedData
    }));
...
vgphsts_id:number,
  ...
  async remove(vgphsts_id:number,token : string) {
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_tran_staging.deleteMany({
      where: {vgphsts_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_source_tran_staging');
    let decryptedRes: any = [];
...
vgphsts_id:number,
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_tran_staging.findFirst({ 
