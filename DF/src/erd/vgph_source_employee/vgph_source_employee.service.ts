...
@Injectable()
export class vgph_source_employeeService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphse_id }: {vgphse_id : bigint} = queryValue;
      ...
      if(vgphse_id){ 
        query.vgphse_id = { [queryCondition['vgphse_id']]: vgphse_id };
      }
      if(vgphsc_uuid){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_source_employee.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_source_employee');
      ...
      this.prismaService.vgph_source_employee.count({
        where: query,
      }));
...
,vgphse_id?:number) {
    try{
      ...
      if (vgphse_id) {
        whereClause.vgphse_id = vgphse_id;
      }
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_employee.findMany({ 
      where: whereClause,
  ...
  async create(createvgph_source_employeeDto: Prisma.vgph_source_employeeCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            vgphse_id :v.pipe(v.number(),v.maxValue(9999999999999999999999999999999999999999999999999999999999999999 )) , 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_source_employeeDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_source_employeeDto, 'vgph_source_employee', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_source_employee.create({
          data: encryptedData,
          select:{vgphse_id:true,vgphsc_uuid:true,uuid:true,source_category:true,source_reference:true,source_name:true,channel_name:true,channel_reference:true,product_basic:true,product_additional:true,request_data:true,response_data:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_source_employeeDto: Prisma.vgph_source_employeeCreateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_employee creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_employee creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        return {
          success: true,
          message: 'vgph_source_employee creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphse_id:number, updatevgph_source_employeeDto: Prisma.vgph_source_employeeUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_source_employeeDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_source_employeeDto,'vgph_source_employee','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_employee.updateMany({
      where: {vgphse_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgphse_id:number,
    updatevgph_source_employeeDto: Prisma.vgph_source_employeeUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphse_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_source_employee update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_employee update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_source_employee update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphse_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_employee.deleteMany({
      where: {vgphse_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_source_employee');
    let decryptedRes: any = [];
...
vgphse_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphse_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_source_employee deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_employee deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_source_employee deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_employee.findFirst({ 
