...
@Injectable()
export class vgph_interfaceService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphi_id }: {vgphi_id : number} = queryValue;
      ...
      if(vgphi_id){ 
        query.vgphi_id = { [queryCondition['vgphi_id']]: vgphi_id };
      }
      if(tenant_id){ 
      ...
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.vgph_interface.findMany({
          select:columns,
          where: query,          
        });
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_interface');
      ...
      const totalItems = await this.prismaService.vgph_interface.count({
        where: query,
      });
  ...
  async findOne(vgphi_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_interface.findUnique({ 
      where: {vgphi_id},
      select: {vgphi_id:true,tenant_id:true,product_code:true,type:true,code:true,param:true,        trs_created_date:true,
        trs_created_by:true,
    ...
    return  await this.decryptData(res, 'vgph_interface');
  } catch (error) {
...
,vgphi_id?:number) {
    try{
      ...
      if (vgphi_id) {
        whereClause.vgphi_id = vgphi_id;
      }
      const res = await this.prismaService.vgph_interface.findMany({ 
      where: whereClause,
      select: {vgphi_id:true,tenant_id:true,product_code:true,type:true,code:true,param:true,        trs_created_date:true,
        trs_created_by:true,
        trs_modified_date:true,
  ...
  async create(createvgph_interfaceDto: Prisma.vgph_interfaceCreateInput,token:string) {
    try{
      ...
      const dataSchema:any =  v.object({
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_interfaceDto);
        if (!validate.success) {
      ...
      const res = await this.prismaService.vgph_interface.create({ 
      data: await this.encryptData(createvgph_interfaceDto,'vgph_interface','create'),
      select:{vgphi_id:true,tenant_id:true,product_code:true,type:true,code:true,param:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
   ...
   */
  async createMaster(
    createvgph_interfaceDto: Prisma.vgph_interfaceCreateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_interface creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_interface creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      const encryptedData = await this.encryptData(createvgph_interfaceDto, 'vgph_interface', 'create');
      ...
      if(role === 'MAKER')
      {
        ...
        const result = await this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_interface',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphi_id',
        ...
        return {
          success: true,
          message: 'vgph_interface creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphi_id:number, updatevgph_interfaceDto: Prisma.vgph_interfaceUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_interfaceDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const res = await this.prismaService.vgph_interface.update({
      where: {vgphi_id},
      data: await this.encryptData(updatevgph_interfaceDto,'vgph_interface','update'),
      select: {vgphi_id:true,tenant_id:true,product_code:true,type:true,code:true,param:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return await this.decryptData(res, 'vgph_interface');
    } catch (error) {
...
/**
  ...
  async updateMaster(
vgphi_id:number,
    updatevgph_interfaceDto: Prisma.vgph_interfaceUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphi_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_interface update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_interface update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      const existingRecord = await this.prismaService.vgph_interface.findUnique({
        where: {vgphi_id}
      });
      ...
      // Encrypt data if needed
      const encryptedData = await this.encryptData(updatevgph_interfaceDto, 'vgph_interface', 'update');
      ...
      return {
        success: true,
        message: 'vgph_interface update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphi_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_interface.delete({
      where: {vgphi_id },
      select: {vgphi_id:true,tenant_id:true,product_code:true,type:true,code:true,param:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return res;
...
vgphi_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphi_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_interface deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_interface deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_interface deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.vgph_interface.findFirst({ 
