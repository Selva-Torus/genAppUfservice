...
import { vgph_tran_error_log_stagingEntity } from './entity/vgph_tran_error_log_staging.entity';
import { CustomException } from 'src/customException';
@Injectable()
export class vgph_tran_error_log_stagingService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphtels_id }: {vgphtels_id : number} = queryValue;
      ...
      const { error_cateogry }: {error_cateogry : Date} = queryValue;
      ...
      if(vgphtels_id){ 
        query.vgphtels_id = { [queryCondition['vgphtels_id']]: vgphtels_id };
      }
      if(tenant_id){ 
      ...
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.vgph_tran_error_log_staging.findMany({
          select:columns,
          where: query,          
        });
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_tran_error_log_staging');
      ...
      const totalItems = await this.prismaService.vgph_tran_error_log_staging.count({
        where: query,
      });
  ...
  async findOne(vgphtels_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_tran_error_log_staging.findUnique({ 
      where: {vgphtels_id},
      select: {vgphtels_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,process_code:true,error_cateogry:true,error_code:true,error_desc:true,error_data:true,vgphsts_uuid:true,        trs_created_date:true,
        trs_created_by:true,
    ...
    return  await this.decryptData(res, 'vgph_tran_error_log_staging');
  } catch (error) {
...
,vgphtels_id?:number) {
    try{
      ...
      if (vgphtels_id) {
        whereClause.vgphtels_id = vgphtels_id;
      }
      const res = await this.prismaService.vgph_tran_error_log_staging.findMany({ 
      where: whereClause,
      select: {vgphtels_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,process_code:true,error_cateogry:true,error_code:true,error_desc:true,error_data:true,vgphsts_uuid:true,        trs_created_date:true,
        trs_created_by:true,
        trs_modified_date:true,
  ...
  async create(createvgph_tran_error_log_stagingDto: Prisma.vgph_tran_error_log_stagingCreateInput,token:string) {
    try{

      enum direction_vgph_tran_error_log_staging{
        OUTBOUND="OUTBOUND",
        INBOUND="INBOUND",
      }
      enum process_type_vgph_tran_error_log_staging{
        OP="OP",
        IR="IR",
        IP="IP",
        OR="OR",
      }
      enum tran_category_vgph_tran_error_log_staging{
        Financial="Financial",
        Non_Financial="Non_Financial",
      }
      enum error_cateogry_vgph_tran_error_log_staging{
      ...
      const dataSchema:any =  v.object({
            ...
            direction :v.enum(direction_vgph_tran_error_log_staging,"Invalid direction_vgph_tran_error_log_staging enum")  , 
            process_type :v.enum(process_type_vgph_tran_error_log_staging,"Invalid process_type_vgph_tran_error_log_staging enum")  , 
            tran_category :v.enum(tran_category_vgph_tran_error_log_staging,"Invalid tran_category_vgph_tran_error_log_staging enum")  , 
            process_code :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            error_cateogry :  v.optional(v.enum(error_cateogry_vgph_tran_error_log_staging,"Invalid error_cateogry_vgph_tran_error_log_staging enum")), 
            error_code :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            error_desc :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            error_data :  v.optional(v.any() ), 
            vgphsts_uuid :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
        });
        let validate : any = v.safeParse(dataSchema,createvgph_tran_error_log_stagingDto);
        if (!validate.success) {
      ...
      const res = await this.prismaService.vgph_tran_error_log_staging.create({ 
      data: await this.encryptData(createvgph_tran_error_log_stagingDto,'vgph_tran_error_log_staging','create'),
      select:{vgphtels_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,process_code:true,error_cateogry:true,error_code:true,error_desc:true,error_data:true,vgphsts_uuid:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
   ...
   */
  async createMaster(
    createvgph_tran_error_log_stagingDto: Prisma.vgph_tran_error_log_stagingCreateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_tran_error_log_staging creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_tran_error_log_staging creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      const encryptedData = await this.encryptData(createvgph_tran_error_log_stagingDto, 'vgph_tran_error_log_staging', 'create');
      ...
      if(role === 'MAKER')
      {
        ...
        const result = await this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_tran_error_log_staging',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphtels_id',
        ...
        return {
          success: true,
          message: 'vgph_tran_error_log_staging creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphtels_id:number, updatevgph_tran_error_log_stagingDto: Prisma.vgph_tran_error_log_stagingUpdateInput,token:string) {   
    try{
      ...
      const dataSchema:any =  v.object({
          ...
          direction :  v.optional(v.enum(direction_vgph_tran_error_log_staging,"Invalid direction_vgph_tran_error_log_staging enum")), 
          process_type :  v.optional(v.enum(process_type_vgph_tran_error_log_staging,"Invalid process_type_vgph_tran_error_log_staging enum")), 
          tran_category :  v.optional(v.enum(tran_category_vgph_tran_error_log_staging,"Invalid tran_category_vgph_tran_error_log_staging enum")), 
          process_code :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
          error_cateogry :  v.optional(v.enum(error_cateogry_vgph_tran_error_log_staging,"Invalid error_cateogry_vgph_tran_error_log_staging enum")), 
          error_code :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
          error_desc :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_tran_error_log_stagingDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const res = await this.prismaService.vgph_tran_error_log_staging.update({
      where: {vgphtels_id},
      data: await this.encryptData(updatevgph_tran_error_log_stagingDto,'vgph_tran_error_log_staging','update'),
      select: {vgphtels_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,process_code:true,error_cateogry:true,error_code:true,error_desc:true,error_data:true,vgphsts_uuid:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return await this.decryptData(res, 'vgph_tran_error_log_staging');
    } catch (error) {
...
/**
  ...
  async updateMaster(
vgphtels_id:number,
    updatevgph_tran_error_log_stagingDto: Prisma.vgph_tran_error_log_stagingUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphtels_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_tran_error_log_staging update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_tran_error_log_staging update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      const existingRecord = await this.prismaService.vgph_tran_error_log_staging.findUnique({
        where: {vgphtels_id}
      });
      ...
      // Encrypt data if needed
      const encryptedData = await this.encryptData(updatevgph_tran_error_log_stagingDto, 'vgph_tran_error_log_staging', 'update');
      ...
      return {
        success: true,
        message: 'vgph_tran_error_log_staging update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphtels_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_tran_error_log_staging.delete({
      where: {vgphtels_id },
      select: {vgphtels_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,process_code:true,error_cateogry:true,error_code:true,error_desc:true,error_data:true,vgphsts_uuid:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return res;
...
vgphtels_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphtels_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_tran_error_log_staging deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_tran_error_log_staging deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_tran_error_log_staging deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.vgph_tran_error_log_staging.findFirst({ 
