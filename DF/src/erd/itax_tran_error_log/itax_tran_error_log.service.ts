...
@Injectable()
export class itax_tran_error_logService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { itaxtel_id }: {itaxtel_id : number} = queryValue;
      ...
      if(itaxtel_id){ 
        query.itaxtel_id = { [queryCondition['itaxtel_id']]: itaxtel_id };
      }
      if(tran_category){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.itax_tran_error_log.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'itax_tran_error_log');
      ...
      this.prismaService.itax_tran_error_log.count({
        where: query,
      }));
  ...
  async findOne(itaxtel_id:number,token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.itax_tran_error_log.findUnique({ 
      where: {itaxtel_id},
      select: {itaxtel_id:true,tran_category:true,error_cateogry:true,error_code:true,error_desc:true,error_data:true,itaxst_id :true,        trs_created_date:true,
        trs_created_by:true,
    ...
    return await this.decryptData(await this.commonDecimalDatahandle(res), 'itax_tran_error_log');
  } catch (error) {
  ...
  async findAll(token : string,trs_created_date?: Date,trs_created_by?: string,trs_modified_date?: Date,trs_modified_by?: string,trs_process_id?: string,trs_access_profile?: string,trs_org_grp_code?: string,trs_org_code?: string,trs_role_grp_code?: string,trs_role_code?: string,trs_ps_grp_code?: string,trs_ps_code?: string,trs_sub_org_grp_code?: string,trs_sub_org_code?: string,trs_locked_by?: string,trs_locked_time?: Date,trs_tenant_id?:string,trs_app_code?:string,trs_product_code?:string,trs_event_process_status?:string,trs_event_status?:string,trs_prev_process_code?:string,trs_prev_status?:string,trs_prev_process_status?:string,trs_process_code?:string,trs_status?:string,trs_process_status?:string,trs_next_process_code?:string,trs_next_status?:string,trs_next_process_status?:string) {
    try{
      ...
      this.prismaService.itax_tran_error_log.findMany({ 
      where: whereClause,
      select: {itaxtel_id:true,tran_category:true,error_cateogry:true,error_code:true,error_desc:true,error_data:true,          itaxst_id :true,    
      ...
      for (const indiviual of res) {
        ...
        const decryptedData = await this.decryptData(plain, 'itax_tran_error_log');
        decryptedRes.push(decryptedData);
      }
      return decryptedRes;
    } catch (error) {
  ...
  async create(createitax_tran_error_logDto: Prisma.itax_tran_error_logCreateInput,token:string) {
    try{

      enum tran_category_itax_tran_error_log{
        Financial="Financial",
        Non_Financial="Non_Financial",
      }
      enum error_cateogry_itax_tran_error_log{
        VALIDATION="VALIDATION",
        CBS="CBS",
        OTHER="OTHER",
      }
      const dataSchema:any =  v.object({
            tran_category :  v.optional(v.enum(tran_category_itax_tran_error_log,"Invalid tran_category_itax_tran_error_log enum")), 
            error_cateogry :  v.optional(v.enum(error_cateogry_itax_tran_error_log,"Invalid error_cateogry_itax_tran_error_log enum")), 
        ...
        let validate : any = v.safeParse(dataSchema,createitax_tran_error_logDto);
        if (!validate.success) {
   ...
   */
  async createMaster(
    createitax_tran_error_logDto: Prisma.itax_tran_error_logCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_tran_error_log creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_tran_error_log creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'itax_tran_error_log',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'itaxtel_id',
        ...
        return {
          success: true,
          message: 'itax_tran_error_log creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(itaxtel_id:number, updateitax_tran_error_logDto: Prisma.itax_tran_error_logUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updateitax_tran_error_logDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updateitax_tran_error_logDto,'itax_tran_error_log','update');
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.itax_tran_error_log.update({
      where: {itaxtel_id},
      data: encryptedData,
      select: {itaxtel_id:true,tran_category:true,error_cateogry:true,error_code:true,error_desc:true,error_data:true,itaxst_id :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_prev_process_code:true,trs_prev_status:true,trs_prev_process_status:true,trs_process_code:true,trs_status:true,trs_process_status:true,trs_next_process_code:true,trs_next_status:true,trs_next_process_status:true}
    }));
...
/**
  ...
  async updateMaster(
itaxtel_id:number,
    updateitax_tran_error_logDto: Prisma.itax_tran_error_logUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =itaxtel_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'itax_tran_error_log update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_tran_error_log update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'itax_tran_error_log update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(itaxtel_id:number,token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.itax_tran_error_log.delete({
      where: {itaxtel_id },
      select: {itaxtel_id:true,tran_category:true,error_cateogry:true,error_code:true,error_desc:true,error_data:true,itaxst_id :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_prev_process_code:true,trs_prev_status:true,trs_prev_process_status:true,trs_process_code:true,trs_status:true,trs_process_status:true,trs_next_process_code:true,trs_next_status:true,trs_next_process_status:true}
    }));
    return await this.decryptData(await this.commonDecimalDatahandle(res), 'itax_tran_error_log');
...
itaxtel_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =itaxtel_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'itax_tran_error_log deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_tran_error_log deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'itax_tran_error_log deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.itax_tran_error_log.findFirst({ 
