...
export class vgph_flowService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphf_id }: {vgphf_id : number} = queryValue;
      ...
      if(vgphf_id){ 
        query.vgphf_id = { [queryCondition['vgphf_id']]: vgphf_id };
      }
      if(tenant_id){ 
      ...
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.vgph_flow.findMany({
          select:columns,
          where: query,          
        });
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_flow');
      ...
      const totalItems = await this.prismaService.vgph_flow.count({
        where: query,
      });
  ...
  async findOne(vgphf_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_flow.findUnique({ 
      where: {vgphf_id},
      select: {vgphf_id:true,tenant_id:true,product_code:true,type:true,code:true,description:true,version:true,        trs_created_date:true,
        trs_created_by:true,
    ...
    return  await this.decryptData(res, 'vgph_flow');
  } catch (error) {
...
,vgphf_id?:number) {
    try{
      ...
      if (vgphf_id) {
        whereClause.vgphf_id = vgphf_id;
      }
      const res = await this.prismaService.vgph_flow.findMany({ 
      where: whereClause,
      select: {vgphf_id:true,tenant_id:true,product_code:true,type:true,code:true,description:true,version:true,        trs_created_date:true,
        trs_created_by:true,
        trs_modified_date:true,
  ...
  async create(createvgph_flowDto: Prisma.vgph_flowCreateInput,token:string) {
    try{
      ...
      const dataSchema:any =  v.object({
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_flowDto);
        if (!validate.success) {
      ...
      const res = await this.prismaService.vgph_flow.create({ 
      data: await this.encryptData(createvgph_flowDto,'vgph_flow','create'),
      select:{vgphf_id:true,tenant_id:true,product_code:true,type:true,code:true,description:true,version:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
   ...
   */
  async createMaster(
    createvgph_flowDto: Prisma.vgph_flowCreateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_flow creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_flow creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      const encryptedData = await this.encryptData(createvgph_flowDto, 'vgph_flow', 'create');
      ...
      if(role === 'MAKER')
      {
        ...
        const result = await this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_flow',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphf_id',
        ...
        return {
          success: true,
          message: 'vgph_flow creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphf_id:number, updatevgph_flowDto: Prisma.vgph_flowUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_flowDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const res = await this.prismaService.vgph_flow.update({
      where: {vgphf_id},
      data: await this.encryptData(updatevgph_flowDto,'vgph_flow','update'),
      select: {vgphf_id:true,tenant_id:true,product_code:true,type:true,code:true,description:true,version:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return await this.decryptData(res, 'vgph_flow');
    } catch (error) {
...
/**
  ...
  async updateMaster(
vgphf_id:number,
    updatevgph_flowDto: Prisma.vgph_flowUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphf_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_flow update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_flow update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      const existingRecord = await this.prismaService.vgph_flow.findUnique({
        where: {vgphf_id}
      });
      ...
      // Encrypt data if needed
      const encryptedData = await this.encryptData(updatevgph_flowDto, 'vgph_flow', 'update');
      ...
      return {
        success: true,
        message: 'vgph_flow update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphf_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_flow.delete({
      where: {vgphf_id },
      select: {vgphf_id:true,tenant_id:true,product_code:true,type:true,code:true,description:true,version:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return res;
...
vgphf_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphf_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_flow deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_flow deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_flow deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.vgph_flow.findFirst({ 
