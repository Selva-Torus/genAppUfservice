...
import { vgph_process_exception_mainEntity } from './entity/vgph_process_exception_main.entity';
import { CustomException } from 'src/customException';
@Injectable()
export class vgph_process_exception_mainService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphpem_id }: {vgphpem_id : bigint} = queryValue;
      ...
      const { exception_cateogry }: {exception_cateogry : string} = queryValue;
      const { exception_data }: {exception_data : any } = queryValue;
      const { start_date }: {start_date :  Date} = queryValue;
      const { end_date }: {end_date :  Date} = queryValue;
      ...
      if(vgphpem_id){ 
        query.vgphpem_id = { [queryCondition['vgphpem_id']]: vgphpem_id };
      }
      if(vgphstm_uuid){ 
      ...
      if(exception_cateogry){ 
        query.exception_cateogry = { [queryCondition['exception_cateogry']]: exception_cateogry };
      }
      if(exception_data){ 
        query.exception_data = { [queryCondition['exception_data']]: exception_data };
      }
      if(start_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_process_exception_main.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_process_exception_main');
      ...
      this.prismaService.vgph_process_exception_main.count({
        where: query,
      }));
  ...
  async create(createvgph_process_exception_mainDto: Prisma.vgph_process_exception_mainCreateInput,token:string) {
    try{

      enum direction_vgph_process_exception_main{
        INBOUND="INBOUND",
        OUTBOUND="OUTBOUND",
      }
      enum process_type_vgph_process_exception_main{
        OP="OP",
        IR="IR",
        IP="IP",
        OR="OR",
      ...
      const dataSchema:any =  v.object({
            vgphpem_id :v.pipe(v.number(),v.maxValue(9999999999999999999999999999999999999999999999999999999999999999 )) , 
            ...
            direction :  v.optional(v.enum(direction_vgph_process_exception_main,"Invalid direction_vgph_process_exception_main enum")), 
            process_type :  v.optional(v.enum(process_type_vgph_process_exception_main,"Invalid process_type_vgph_process_exception_main enum")), 
            tran_category :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            process_code :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            exception_cateogry :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            exception_data :  v.optional(v.any() ), 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_process_exception_mainDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_process_exception_mainDto, 'vgph_process_exception_main', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_process_exception_main.create({
          data: encryptedData,
          select:{vgphpem_id:true,vgphstm_uuid:true,vgphsts_uuid:true,vgphntm_uuid:true,direction:true,process_type:true,tran_category:true,process_code:true,exception_cateogry:true,exception_data:true,start_date:true,end_date:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,trs_prev_process_code:true,trs_prev_status:true,trs_prev_process_status:true,trs_process_code:true,trs_status:true,trs_process_status:true,trs_next_process_code:true,trs_next_status:true,trs_next_process_status:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_process_exception_mainDto: Prisma.vgph_process_exception_mainCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_process_exception_main creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_process_exception_main creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_process_exception_main',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphpem_id',
        ...
        return {
          success: true,
          message: 'vgph_process_exception_main creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphpem_id:number, updatevgph_process_exception_mainDto: Prisma.vgph_process_exception_mainUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_process_exception_mainDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_process_exception_mainDto,'vgph_process_exception_main','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_process_exception_main.updateMany({
      where: {vgphpem_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgphpem_id:number,
    updatevgph_process_exception_mainDto: Prisma.vgph_process_exception_mainUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphpem_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_process_exception_main update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_process_exception_main update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_process_exception_main update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphpem_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_process_exception_main.deleteMany({
      where: {vgphpem_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_process_exception_main');
    let decryptedRes: any = [];
...
vgphpem_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphpem_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_process_exception_main deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_process_exception_main deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_process_exception_main deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_process_exception_main.findFirst({ 
