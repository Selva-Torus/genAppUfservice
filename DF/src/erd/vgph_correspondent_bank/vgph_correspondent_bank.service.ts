...
@Injectable()
export class vgph_correspondent_bankService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphcb_id }: {vgphcb_id : bigint} = queryValue;
      ...
      const { swift_bic }: {swift_bic : string} = queryValue;
      ...
      if(vgphcb_id){ 
        query.vgphcb_id = { [queryCondition['vgphcb_id']]: vgphcb_id };
      }
      if(currency){ 
      ...
      if(swift_bic){ 
        query.swift_bic = { [queryCondition['swift_bic']]: swift_bic };
      }
      if(comment){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_correspondent_bank.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_correspondent_bank');
      ...
      this.prismaService.vgph_correspondent_bank.count({
        where: query,
      }));
  ...
  async create(createvgph_correspondent_bankDto: Prisma.vgph_correspondent_bankCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            vgphcb_id :v.pipe(v.number(),v.maxValue(9999999999999999999999999999999999999999999999999999999999999999 )) , 
            currency :v.pipe(v.string(),v.maxLength(3 )) , 
            correspondent_bank_name :v.pipe(v.string(),v.maxLength(128 )) , 
            city :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            country :v.pipe(v.string(),v.maxLength(64 )) , 
            swift_bic :v.pipe(v.string(),v.maxLength(32 )) , 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_correspondent_bankDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_correspondent_bankDto, 'vgph_correspondent_bank', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_correspondent_bank.create({
          data: encryptedData,
          select:{vgphcb_id:true,currency:true,correspondent_bank_name:true,city:true,country:true,swift_bic:true,comment:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_correspondent_bankDto: Prisma.vgph_correspondent_bankCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_correspondent_bank creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_correspondent_bank creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_correspondent_bank',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphcb_id',
        ...
        return {
          success: true,
          message: 'vgph_correspondent_bank creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphcb_id:number, updatevgph_correspondent_bankDto: Prisma.vgph_correspondent_bankUpdateInput,token:string) {   
    try{
      ...
      const dataSchema:any =  v.object({
          ...
          city :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
          country :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
          swift_bic :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
          comment :  v.optional(v.pipe(v.string(),v.maxLength(512 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_correspondent_bankDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_correspondent_bankDto,'vgph_correspondent_bank','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_correspondent_bank.updateMany({
      where: {vgphcb_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgphcb_id:number,
    updatevgph_correspondent_bankDto: Prisma.vgph_correspondent_bankUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphcb_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_correspondent_bank update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_correspondent_bank update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_correspondent_bank update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphcb_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_correspondent_bank.deleteMany({
      where: {vgphcb_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_correspondent_bank');
    let decryptedRes: any = [];
...
vgphcb_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphcb_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_correspondent_bank deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_correspondent_bank deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_correspondent_bank deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_correspondent_bank.findFirst({ 
