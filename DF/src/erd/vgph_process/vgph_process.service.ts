...
import { vgph_processEntity } from './entity/vgph_process.entity';
import { CustomException } from 'src/customException';
@Injectable()
export class vgph_processService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphp_id }: {vgphp_id : number} = queryValue;
      ...
      const { mode }: {mode : string} = queryValue;
      const { handler_code }: {handler_code : string} = queryValue;
      const { routing_rule }: {routing_rule : string} = queryValue;
      const { decision_rule }: {decision_rule : string} = queryValue;
      const { override_rule }: {override_rule : string} = queryValue;
      const { failure_process_code }: {failure_process_code : string} = queryValue;
      const { suspicious_process_code }: {suspicious_process_code : string} = queryValue;
      const { error_process_code }: {error_process_code : string} = queryValue;
      const { param }: {param : any } = queryValue;
      const { version }: {version : string} = queryValue;

      if(vgphp_id){ 
        query.vgphp_id = { [queryCondition['vgphp_id']]: vgphp_id };
      }
      if(tenant_id){ 
      ...
      if(mode){ 
        query.mode = { [queryCondition['mode']]: mode };
      }
      if(handler_code){ 
        query.handler_code = { [queryCondition['handler_code']]: handler_code };
      }
      if(routing_rule){ 
        query.routing_rule = { [queryCondition['routing_rule']]: routing_rule };
      }
      if(decision_rule){ 
        query.decision_rule = { [queryCondition['decision_rule']]: decision_rule };
      }
      if(override_rule){ 
        query.override_rule = { [queryCondition['override_rule']]: override_rule };
      }
      if(failure_process_code){ 
        query.failure_process_code = { [queryCondition['failure_process_code']]: failure_process_code };
      }
      if(suspicious_process_code){ 
        query.suspicious_process_code = { [queryCondition['suspicious_process_code']]: suspicious_process_code };
      }
      if(error_process_code){ 
        query.error_process_code = { [queryCondition['error_process_code']]: error_process_code };
      }
      if(param){ 
        query.param = { [queryCondition['param']]: param };
      }
      if(version){ 
      ...
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.vgph_process.findMany({
          select:columns,
          where: query,          
        });
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_process');
      ...
      const totalItems = await this.prismaService.vgph_process.count({
        where: query,
      });
  ...
  async findOne(vgphp_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_process.findUnique({ 
      where: {vgphp_id},
      select: {vgphp_id:true,tenant_id:true,product_code:true,category:true,code:true,description:true,type:true,mode:true,handler_code:true,routing_rule:true,decision_rule:true,override_rule:true,failure_process_code:true,suspicious_process_code:true,error_process_code:true,param:true,version:true,        trs_created_date:true,
        trs_created_by:true,
    ...
    return  await this.decryptData(res, 'vgph_process');
  } catch (error) {
...
,vgphp_id?:number) {
    try{
      ...
      if (vgphp_id) {
        whereClause.vgphp_id = vgphp_id;
      }
      const res = await this.prismaService.vgph_process.findMany({ 
      where: whereClause,
      select: {vgphp_id:true,tenant_id:true,product_code:true,category:true,code:true,description:true,type:true,mode:true,handler_code:true,routing_rule:true,decision_rule:true,override_rule:true,failure_process_code:true,suspicious_process_code:true,error_process_code:true,param:true,version:true,        trs_created_date:true,
        trs_created_by:true,
        trs_modified_date:true,
  ...
  async create(createvgph_processDto: Prisma.vgph_processCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            tenant_id :v.pipe(v.string(),v.maxLength(16 )) , 
            product_code :v.pipe(v.string(),v.maxLength(32 )) , 
            category :v.pipe(v.string(),v.maxLength(64 )) , 
            code :v.pipe(v.string(),v.maxLength(128 )) , 
            description :v.pipe(v.string(),v.maxLength(256 )) , 
            type :v.pipe(v.string(),v.maxLength(64 )) , 
            mode :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            handler_code :v.pipe(v.string(),v.maxLength(128 )) , 
            routing_rule :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            decision_rule :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            override_rule :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            failure_process_code :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            suspicious_process_code :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            error_process_code :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            param :  v.optional(v.any() ), 
            version :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
        });
        let validate : any = v.safeParse(dataSchema,createvgph_processDto);
        if (!validate.success) {
      ...
      const res = await this.prismaService.vgph_process.create({ 
      data: await this.encryptData(createvgph_processDto,'vgph_process','create'),
      select:{vgphp_id:true,tenant_id:true,product_code:true,category:true,code:true,description:true,type:true,mode:true,handler_code:true,routing_rule:true,decision_rule:true,override_rule:true,failure_process_code:true,suspicious_process_code:true,error_process_code:true,param:true,version:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
   ...
   */
  async createMaster(
    createvgph_processDto: Prisma.vgph_processCreateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_process creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_process creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      const encryptedData = await this.encryptData(createvgph_processDto, 'vgph_process', 'create');
      ...
      if(role === 'MAKER')
      {
        ...
        const result = await this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_process',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphp_id',
        ...
        return {
          success: true,
          message: 'vgph_process creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphp_id:number, updatevgph_processDto: Prisma.vgph_processUpdateInput,token:string) {   
    try{

      const dataSchema:any =  v.object({
          tenant_id :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
          product_code :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
          category :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
          ...
          mode :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
          handler_code :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
          routing_rule :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
          decision_rule :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
          override_rule :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
          failure_process_code :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_processDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const res = await this.prismaService.vgph_process.update({
      where: {vgphp_id},
      data: await this.encryptData(updatevgph_processDto,'vgph_process','update'),
      select: {vgphp_id:true,tenant_id:true,product_code:true,category:true,code:true,description:true,type:true,mode:true,handler_code:true,routing_rule:true,decision_rule:true,override_rule:true,failure_process_code:true,suspicious_process_code:true,error_process_code:true,param:true,version:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return await this.decryptData(res, 'vgph_process');
    } catch (error) {
...
/**
  ...
  async updateMaster(
vgphp_id:number,
    updatevgph_processDto: Prisma.vgph_processUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphp_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_process update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_process update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      const existingRecord = await this.prismaService.vgph_process.findUnique({
        where: {vgphp_id}
      });
      ...
      // Encrypt data if needed
      const encryptedData = await this.encryptData(updatevgph_processDto, 'vgph_process', 'update');
      ...
      return {
        success: true,
        message: 'vgph_process update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphp_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_process.delete({
      where: {vgphp_id },
      select: {vgphp_id:true,tenant_id:true,product_code:true,category:true,code:true,description:true,type:true,mode:true,handler_code:true,routing_rule:true,decision_rule:true,override_rule:true,failure_process_code:true,suspicious_process_code:true,error_process_code:true,param:true,version:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return res;
...
vgphp_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphp_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_process deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_process deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_process deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.vgph_process.findFirst({ 
