...
import { itax_tran_logService } from './itax_tran_log.service';
...
//import { CreateItaxTranLogDto } from '../prisma/dto/create-itaxTranLog.dto';
//import { UpdateItaxTranLogDto } from '../prisma/dto/update-itaxTranLog.dto';
import { Createitax_tran_logDto } from './dto/Createitax_tran_log.dto';
import { Updateitax_tran_logDto } from './dto/Updateitax_tran_log.dto';
import { plainToInstance } from 'class-transformer';
import { UfService } from 'src/Torus/v1/uf/uf.service';

 
@Controller('itax_tran_log')
@ApiTags('ERD API')
export class itax_tran_logController {
  constructor(
    private readonly itax_tran_logService: itax_tran_logService,
    private readonly ufservice: UfService
  ) {}

  @Get("/schema")
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: itax_tran_logEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the itax_tran_log table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.itax_tran_logService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: itax_tran_logEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the itax_tran_log table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.itax_tran_logService.findAllmethod(query, +limit,selectColumns,token);
  }

  @Get(':itaxtl_id')
  @ApiBearerAuth('JWT-auth')
  @ApiParam({name: 'itaxtl_id',type:Number})
  @ApiOkResponse({ type: itax_tran_logEntity })
  @ApiOperation({
    summary: 'Fetch the only one record',
    description: 'Read only one records from the itax_tran_log table',
  })
  
  async findOne(@Headers() authHeader: string,@Param('itaxtl_id') itaxtl_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = this.itax_tran_logService.findOne(+itaxtl_id,token);
    return plainToInstance(itax_tran_logEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the itax_tran_log table',
  })
  ...
  @ApiBody({ type: Createitax_tran_logDto })
  @ApiCreatedResponse({ type: itax_tran_logEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the itax_tran_log table',
  })
  ...
  async create(
    ...
    @Body() createitax_tran_logDto: Prisma.itax_tran_logCreateInput,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.itax_tran_logService.createMaster(createitax_tran_logDto, makerInfo, token);
      return result;
    }

    const result = this.itax_tran_logService.create(createitax_tran_logDto,token);
    return plainToInstance(itax_tran_logEntity, result);
  }
  ...
  @ApiHeader({ name: 'xCdcaRemarks', required: false })
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiBody({ type: Updateitax_tran_logDto })
  @ApiOkResponse({ type: itax_tran_logEntity })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the itax_tran_log table',
  })
  ...
  async update(
    ...
    @Headers('xCdcaApprovalStatus') mcApprovalStatus: string,
    @Headers() authHeader: string,
@Param('itaxtl_id') itaxtl_id:number,
    @Body() updateitax_tran_logDto: Prisma.itax_tran_logUpdateInput,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.itax_tran_logService.updateMaster(+itaxtl_id,updateitax_tran_logDto,makerInfo,token);
      return result;
    }

    const result = this.itax_tran_logService.update(+itaxtl_id,updateitax_tran_logDto,token);
    return plainToInstance(itax_tran_logEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the itax_tran_log table',
  })
...
@Param('itaxtl_id') itaxtl_id:number,
    ...
    if (mcRole && mcUsername) {
      const makerInfo = { role: mcRole, username: mcUsername, remarks: mcRemarks,approvalStatus: mcApprovalStatus };
      const result = await this.itax_tran_logService.deleteMaster(+itaxtl_id,makerInfo,token);
      return result;
    }

    const result =  this.itax_tran_logService.remove(+itaxtl_id,token);
    return plainToInstance(itax_tran_logEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the itax_tran_log table',
  })
  
  async findFirst(@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = this.itax_tran_logService.findFirst(token);
    return plainToInstance(itax_tran_logEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the itax_tran_log table',
  })
  
  async findLast(@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = this.itax_tran_logService.findLast(token);
    return plainToInstance(itax_tran_logEntity, result);
  }
}