...
import { itax_tran_logEntity } from './entity/itax_tran_log.entity';
import { CustomException } from 'src/customException';
@Injectable()
export class itax_tran_logService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { itaxtl_id }: {itaxtl_id : number} = queryValue;
      ...
      if(itaxtl_id){ 
        query.itaxtl_id = { [queryCondition['itaxtl_id']]: itaxtl_id };
      }
      if(tran_category){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.itax_tran_log.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'itax_tran_log');
      ...
      this.prismaService.itax_tran_log.count({
        where: query,
      }));
  ...
  async findOne(itaxtl_id:number,token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.itax_tran_log.findUnique({ 
      where: {itaxtl_id},
      select: {itaxtl_id:true,tran_category:true,processing_system:true,request_data:true,response_data:true,retry_flag:true,retry_count:true,itaxst_id :true,        trs_created_date:true,
        trs_created_by:true,
    ...
    return await this.decryptData(await this.commonDecimalDatahandle(res), 'itax_tran_log');
  } catch (error) {
  ...
  async findAll(token : string,trs_created_date?: Date,trs_created_by?: string,trs_modified_date?: Date,trs_modified_by?: string,trs_process_id?: string,trs_access_profile?: string,trs_org_grp_code?: string,trs_org_code?: string,trs_role_grp_code?: string,trs_role_code?: string,trs_ps_grp_code?: string,trs_ps_code?: string,trs_sub_org_grp_code?: string,trs_sub_org_code?: string,trs_locked_by?: string,trs_locked_time?: Date,trs_tenant_id?:string,trs_app_code?:string,trs_product_code?:string,trs_event_process_status?:string,trs_event_status?:string,trs_prev_process_code?:string,trs_prev_status?:string,trs_prev_process_status?:string,trs_process_code?:string,trs_status?:string,trs_process_status?:string,trs_next_process_code?:string,trs_next_status?:string,trs_next_process_status?:string) {
    try{
      ...
      this.prismaService.itax_tran_log.findMany({ 
      where: whereClause,
      select: {itaxtl_id:true,tran_category:true,processing_system:true,request_data:true,response_data:true,retry_flag:true,retry_count:true,          itaxst_id :true,    
      ...
      for (const indiviual of res) {
        ...
        const decryptedData = await this.decryptData(plain, 'itax_tran_log');
        decryptedRes.push(decryptedData);
      }
      return decryptedRes;
    } catch (error) {
  ...
  async create(createitax_tran_logDto: Prisma.itax_tran_logCreateInput,token:string) {
    try{

      enum tran_category_itax_tran_log{
        Financial="Financial",
        Non_Financial="Non_Financial",
      }
      const dataSchema:any =  v.object({
            tran_category :  v.optional(v.enum(tran_category_itax_tran_log,"Invalid tran_category_itax_tran_log enum")), 
        ...
        let validate : any = v.safeParse(dataSchema,createitax_tran_logDto);
        if (!validate.success) {
   ...
   */
  async createMaster(
    createitax_tran_logDto: Prisma.itax_tran_logCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_tran_log creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_tran_log creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'itax_tran_log',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'itaxtl_id',
        ...
        return {
          success: true,
          message: 'itax_tran_log creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(itaxtl_id:number, updateitax_tran_logDto: Prisma.itax_tran_logUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updateitax_tran_logDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updateitax_tran_logDto,'itax_tran_log','update');
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.itax_tran_log.update({
      where: {itaxtl_id},
      data: encryptedData,
      select: {itaxtl_id:true,tran_category:true,processing_system:true,request_data:true,response_data:true,retry_flag:true,retry_count:true,itaxst_id :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_prev_process_code:true,trs_prev_status:true,trs_prev_process_status:true,trs_process_code:true,trs_status:true,trs_process_status:true,trs_next_process_code:true,trs_next_status:true,trs_next_process_status:true}
    }));
...
/**
  ...
  async updateMaster(
itaxtl_id:number,
    updateitax_tran_logDto: Prisma.itax_tran_logUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =itaxtl_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'itax_tran_log update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_tran_log update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'itax_tran_log update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(itaxtl_id:number,token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.itax_tran_log.delete({
      where: {itaxtl_id },
      select: {itaxtl_id:true,tran_category:true,processing_system:true,request_data:true,response_data:true,retry_flag:true,retry_count:true,itaxst_id :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_prev_process_code:true,trs_prev_status:true,trs_prev_process_status:true,trs_process_code:true,trs_status:true,trs_process_status:true,trs_next_process_code:true,trs_next_status:true,trs_next_process_status:true}
    }));
    return await this.decryptData(await this.commonDecimalDatahandle(res), 'itax_tran_log');
...
itaxtl_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =itaxtl_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'itax_tran_log deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_tran_log deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'itax_tran_log deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.itax_tran_log.findFirst({ 
