...
@Injectable()
export class vgph_destination_tran_stagingService {
  ...
  async findSchema (token) {
    const data = {
      ...
      vgphds_uuid:"string",
      vgphsts_uuid:"string",
      vgphsts_id :"number",      vgphds_id :"number",      trs_created_date:"Date",
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphdts_id }: {vgphdts_id : number} = queryValue;
      ...
      const { vgphds_uuid }: {vgphds_uuid : string} = queryValue;
      const { vgphsts_uuid }: {vgphsts_uuid : string} = queryValue;

      if(vgphdts_id){ 
        query.vgphdts_id = { [queryCondition['vgphdts_id']]: vgphdts_id };
      }
      if(tenant_id){ 
      ...
      if(vgphds_uuid){ 
        query.vgphds_uuid = { [queryCondition['vgphds_uuid']]: vgphds_uuid };
      }
      if(vgphsts_uuid){ 
      ...
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.vgph_destination_tran_staging.findMany({
          select:columns,
          where: query,          
        });
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_destination_tran_staging');
      ...
      const totalItems = await this.prismaService.vgph_destination_tran_staging.count({
        where: query,
      });
  ...
  async findOne(vgphdts_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_destination_tran_staging.findUnique({ 
      where: {vgphdts_id},
      select: {vgphdts_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,vgphds_uuid:true,vgphsts_uuid:true,vgphsts_id :true,vgphds_id :true,        trs_created_date:true,
        trs_created_by:true,
    ...
    return  await this.decryptData(res, 'vgph_destination_tran_staging');
  } catch (error) {
...
,vgphdts_id?:number) {
    try{
      ...
      if (vgphdts_id) {
        whereClause.vgphdts_id = vgphdts_id;
      }
      const res = await this.prismaService.vgph_destination_tran_staging.findMany({ 
      where: whereClause,
      select: {vgphdts_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,vgphds_uuid:true,vgphsts_uuid:true,          vgphsts_id :true,    
          vgphds_id :true,    
  ...
  async create(createvgph_destination_tran_stagingDto: Prisma.vgph_destination_tran_stagingCreateInput,token:string) {
    try{

      enum direction_vgph_destination_tran_staging{
        OUTBOUND="OUTBOUND",
        INBOUND="INBOUND",
      }
      enum process_type_vgph_destination_tran_staging{
        OP="OP",
        IR="IR",
        IP="IP",
        OR="OR",
      }
      enum tran_category_vgph_destination_tran_staging{
        Financial="Financial",
      ...
      const dataSchema:any =  v.object({
            ...
            direction :v.enum(direction_vgph_destination_tran_staging,"Invalid direction_vgph_destination_tran_staging enum")  , 
            process_type :v.enum(process_type_vgph_destination_tran_staging,"Invalid process_type_vgph_destination_tran_staging enum")  , 
            tran_category :v.enum(tran_category_vgph_destination_tran_staging,"Invalid tran_category_vgph_destination_tran_staging enum")  , 
            vgphds_uuid :v.pipe(v.string(),v.maxLength(64 )) , 
            vgphsts_uuid :v.pipe(v.string(),v.maxLength(64 )) , 
        });
        let validate : any = v.safeParse(dataSchema,createvgph_destination_tran_stagingDto);
        if (!validate.success) {
      ...
      const res = await this.prismaService.vgph_destination_tran_staging.create({ 
      data: await this.encryptData(createvgph_destination_tran_stagingDto,'vgph_destination_tran_staging','create'),
      select:{vgphdts_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,vgphds_uuid:true,vgphsts_uuid:true,vgphsts_id :true,vgphds_id :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
   ...
   */
  async createMaster(
    createvgph_destination_tran_stagingDto: Prisma.vgph_destination_tran_stagingCreateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_destination_tran_staging creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_destination_tran_staging creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      const encryptedData = await this.encryptData(createvgph_destination_tran_stagingDto, 'vgph_destination_tran_staging', 'create');
      ...
      if(role === 'MAKER')
      {
        ...
        const result = await this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_destination_tran_staging',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphdts_id',
        ...
        return {
          success: true,
          message: 'vgph_destination_tran_staging creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphdts_id:number, updatevgph_destination_tran_stagingDto: Prisma.vgph_destination_tran_stagingUpdateInput,token:string) {   
    try{
      ...
      const dataSchema:any =  v.object({
          ...
          direction :  v.optional(v.enum(direction_vgph_destination_tran_staging,"Invalid direction_vgph_destination_tran_staging enum")), 
          process_type :  v.optional(v.enum(process_type_vgph_destination_tran_staging,"Invalid process_type_vgph_destination_tran_staging enum")), 
          tran_category :  v.optional(v.enum(tran_category_vgph_destination_tran_staging,"Invalid tran_category_vgph_destination_tran_staging enum")), 
          vgphds_uuid :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
          vgphsts_uuid :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
      });
      let validate : any = v.safeParse(dataSchema,updatevgph_destination_tran_stagingDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const res = await this.prismaService.vgph_destination_tran_staging.update({
      where: {vgphdts_id},
      data: await this.encryptData(updatevgph_destination_tran_stagingDto,'vgph_destination_tran_staging','update'),
      select: {vgphdts_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,vgphds_uuid:true,vgphsts_uuid:true,vgphsts_id :true,vgphds_id :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return await this.decryptData(res, 'vgph_destination_tran_staging');
    } catch (error) {
...
/**
  ...
  async updateMaster(
vgphdts_id:number,
    updatevgph_destination_tran_stagingDto: Prisma.vgph_destination_tran_stagingUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphdts_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_destination_tran_staging update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_destination_tran_staging update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      const existingRecord = await this.prismaService.vgph_destination_tran_staging.findUnique({
        where: {vgphdts_id}
      });
      ...
      // Encrypt data if needed
      const encryptedData = await this.encryptData(updatevgph_destination_tran_stagingDto, 'vgph_destination_tran_staging', 'update');
      ...
      return {
        success: true,
        message: 'vgph_destination_tran_staging update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphdts_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_destination_tran_staging.delete({
      where: {vgphdts_id },
      select: {vgphdts_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,vgphds_uuid:true,vgphsts_uuid:true,vgphsts_id :true,vgphds_id :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return res;
...
vgphdts_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphdts_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_destination_tran_staging deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_destination_tran_staging deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_destination_tran_staging deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.vgph_destination_tran_staging.findFirst({ 
