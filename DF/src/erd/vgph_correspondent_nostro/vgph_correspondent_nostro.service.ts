...
@Injectable()
export class vgph_correspondent_nostroService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphcn_id }: {vgphcn_id : bigint} = queryValue;
      const { currency }: {currency : string} = queryValue;
      const { correspondent_bank_name }: {correspondent_bank_name : string} = queryValue;
      const { process_type }: {process_type : string} = queryValue;
      const { nostro_account }: {nostro_account : string} = queryValue;
      const { comment }: {comment : string} = queryValue;
      ...
      if(vgphcn_id){ 
        query.vgphcn_id = { [queryCondition['vgphcn_id']]: vgphcn_id };
      }
      if(currency){ 
        query.currency = { [queryCondition['currency']]: currency };
      }
      if(correspondent_bank_name){ 
        query.correspondent_bank_name = { [queryCondition['correspondent_bank_name']]: correspondent_bank_name };
      }
      if(process_type){ 
        query.process_type = { [queryCondition['process_type']]: process_type };
      }
      if(nostro_account){ 
        query.nostro_account = { [queryCondition['nostro_account']]: nostro_account };
      }
      if(comment){ 
        query.comment = { [queryCondition['comment']]: comment };
      }
      if(charge_amount){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_correspondent_nostro.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_correspondent_nostro');
      ...
      this.prismaService.vgph_correspondent_nostro.count({
        where: query,
      }));
  ...
  async create(createvgph_correspondent_nostroDto: Prisma.vgph_correspondent_nostroCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            vgphcn_id :v.pipe(v.number(),v.maxValue(9999999999999999999999999999999999999999999999999999999999999999 )) , 
            currency :  v.optional(v.pipe(v.string(),v.maxLength(3 ))), 
            correspondent_bank_name :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            process_type :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            nostro_account :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_correspondent_nostroDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_correspondent_nostroDto, 'vgph_correspondent_nostro', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_correspondent_nostro.create({
          data: encryptedData,
          select:{vgphcn_id:true,currency:true,correspondent_bank_name:true,process_type:true,nostro_account:true,comment:true,charge_amount:true,vat_amount:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_correspondent_nostroDto: Prisma.vgph_correspondent_nostroCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_correspondent_nostro creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_correspondent_nostro creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_correspondent_nostro',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphcn_id',
        ...
        return {
          success: true,
          message: 'vgph_correspondent_nostro creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphcn_id:number, updatevgph_correspondent_nostroDto: Prisma.vgph_correspondent_nostroUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_correspondent_nostroDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_correspondent_nostroDto,'vgph_correspondent_nostro','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_correspondent_nostro.updateMany({
      where: {vgphcn_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgphcn_id:number,
    updatevgph_correspondent_nostroDto: Prisma.vgph_correspondent_nostroUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphcn_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_correspondent_nostro update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_correspondent_nostro update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_correspondent_nostro update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphcn_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_correspondent_nostro.deleteMany({
      where: {vgphcn_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_correspondent_nostro');
    let decryptedRes: any = [];
...
vgphcn_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphcn_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_correspondent_nostro deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_correspondent_nostro deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_correspondent_nostro deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_correspondent_nostro.findFirst({ 
