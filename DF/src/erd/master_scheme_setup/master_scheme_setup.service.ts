...
@Injectable()
export class master_scheme_setupService {
  ...
  async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { countrycode }: {countrycode : Date} = queryValue;
      const { schemetypes }: {schemetypes : Date} = queryValue;

      if(countrycode){ 
        query.countrycode = { [queryCondition['countrycode']]: countrycode };
      }
      if(schemetypes){ 
        query.schemetypes = { [queryCondition['schemetypes']]: schemetypes };
      }
      const skip = (page - 1) * limit;
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.master_scheme_setup.findMany({
        ...
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'master_scheme_setup');
      ...
      const totalItems = await this.prismaService.master_scheme_setup.count({
        where:Object.keys(query).length >0 ?query: undefined,
      });
  ...
  async findAll(token : string) {
    try{
      const res = await this.prismaService.master_scheme_setup.findMany({ 
      select: {id:true,countrycode:true,schemetypes:true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
      });
      let decryptedRes: any = [];
  ...
  async findAllwithquery(token : string,whereClause : Prisma.master_scheme_setupWhereInput) {
    try{
      whereClause = await this.encryptData(whereClause,'master_scheme_setup','getAll')
      const res = await this.prismaService.master_scheme_setup.findMany({ 
      where: whereClause,
      select: {id:true,countrycode:true,schemetypes:true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
      });
  ...
  async create(createmaster_scheme_setupDto: Prisma.master_scheme_setupCreateInput,token:string) {
    try{
      const res = await this.prismaService.master_scheme_setup.create({ 
      data: await this.encryptData(createmaster_scheme_setupDto,'master_scheme_setup','create'), 
      select:{id:true,countrycode:true,schemetypes:true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true},
    })
    return await this.decryptData(res, 'master_scheme_setup');
  } catch (error) {
    ...
    async update(id:string,updatemaster_scheme_setupDto: Prisma.master_scheme_setupUpdateInput,token:string) {    
      try{
      const res = await this.prismaService.master_scheme_setup.update({
      where: {id,},
      data: await this.encryptData(updatemaster_scheme_setupDto,'master_scheme_setup','update'),
      select: {id:true,countrycode:true,schemetypes:true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true},
    });
    return await this.decryptData(res, 'master_scheme_setup');
    } catch (error) {
    ...
    try{
      const res = await this.prismaService.master_scheme_setup.delete({
      where: {id, },
      select: {id:true,countrycode:true,schemetypes:true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return res;
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.master_scheme_setup.findFirst({ 
