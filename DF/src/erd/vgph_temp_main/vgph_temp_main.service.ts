...
@Injectable()
export class vgph_temp_mainService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphtm_id }: {vgphtm_id : bigint} = queryValue;
      const { prctm_uuid }: {prctm_uuid : string} = queryValue;
      ...
      if(vgphtm_id){ 
        query.vgphtm_id = { [queryCondition['vgphtm_id']]: vgphtm_id };
      }
      if(prctm_uuid){ 
        query.prctm_uuid = { [queryCondition['prctm_uuid']]: prctm_uuid };
      }
      if(data_json){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_temp_main.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_temp_main');
      ...
      this.prismaService.vgph_temp_main.count({
        where: query,
      }));
  ...
  async create(createvgph_temp_mainDto: Prisma.vgph_temp_mainCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            vgphtm_id :v.number() , 
            prctm_uuid :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            data_json :v.any()  , 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_temp_mainDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_temp_mainDto, 'vgph_temp_main', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_temp_main.create({
          data: encryptedData,
          select:{vgphtm_id:true,prctm_uuid:true,data_json:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_temp_mainDto: Prisma.vgph_temp_mainCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_temp_main creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_temp_main creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_temp_main',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphtm_id',
        ...
        return {
          success: true,
          message: 'vgph_temp_main creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphtm_id:number, updatevgph_temp_mainDto: Prisma.vgph_temp_mainUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_temp_mainDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_temp_mainDto,'vgph_temp_main','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_temp_main.updateMany({
      where: {vgphtm_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgphtm_id:number,
    updatevgph_temp_mainDto: Prisma.vgph_temp_mainUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphtm_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_temp_main update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_temp_main update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_temp_main update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphtm_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_temp_main.deleteMany({
      where: {vgphtm_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_temp_main');
    let decryptedRes: any = [];
...
vgphtm_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphtm_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_temp_main deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_temp_main deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_temp_main deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_temp_main.findFirst({ 
