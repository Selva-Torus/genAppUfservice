...
@Injectable()
export class vgph_tran_dtl_stagingService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphtds_id }: {vgphtds_id : number} = queryValue;
      ...
      const { process_data }: {process_data : any } = queryValue;
      const { vgphss_uuid }: {vgphss_uuid : string} = queryValue;

      if(vgphtds_id){ 
        query.vgphtds_id = { [queryCondition['vgphtds_id']]: vgphtds_id };
      }
      if(tenant_id){ 
      ...
      if(process_data){ 
        query.process_data = { [queryCondition['process_data']]: process_data };
      }
      if(vgphss_uuid){ 
      ...
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.vgph_tran_dtl_staging.findMany({
          select:columns,
          where: query,          
        });
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_tran_dtl_staging');
      ...
      const totalItems = await this.prismaService.vgph_tran_dtl_staging.count({
        where: query,
      });
  ...
  async findOne(vgphtds_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_tran_dtl_staging.findUnique({ 
      where: {vgphtds_id},
      select: {vgphtds_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,process_code:true,flow_code:true,step_code:true,product_basic:true,product_additional:true,process_data:true,vgphss_uuid:true,vgphsts_id :true,        trs_created_date:true,
        trs_created_by:true,
    ...
    return  await this.decryptData(res, 'vgph_tran_dtl_staging');
  } catch (error) {
...
,vgphtds_id?:number) {
    try{
      ...
      if (vgphtds_id) {
        whereClause.vgphtds_id = vgphtds_id;
      }
      const res = await this.prismaService.vgph_tran_dtl_staging.findMany({ 
      where: whereClause,
      select: {vgphtds_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,process_code:true,flow_code:true,step_code:true,product_basic:true,product_additional:true,process_data:true,vgphss_uuid:true,          vgphsts_id :true,    
  ...
  async create(createvgph_tran_dtl_stagingDto: Prisma.vgph_tran_dtl_stagingCreateInput,token:string) {
    try{

      enum direction_vgph_tran_dtl_staging{
        OUTBOUND="OUTBOUND",
        INBOUND="INBOUND",
      }
      enum process_type_vgph_tran_dtl_staging{
        OP="OP",
        IR="IR",
        IP="IP",
        OR="OR",
      }
      enum tran_category_vgph_tran_dtl_staging{
        Financial="Financial",
      ...
      const dataSchema:any =  v.object({
            ...
            direction :v.enum(direction_vgph_tran_dtl_staging,"Invalid direction_vgph_tran_dtl_staging enum")  , 
            process_type :v.enum(process_type_vgph_tran_dtl_staging,"Invalid process_type_vgph_tran_dtl_staging enum")  , 
            tran_category :v.enum(tran_category_vgph_tran_dtl_staging,"Invalid tran_category_vgph_tran_dtl_staging enum")  , 
            process_code :v.pipe(v.string(),v.maxLength(32 )) , 
            flow_code :v.pipe(v.string(),v.maxLength(32 )) , 
            step_code :v.pipe(v.string(),v.maxLength(32 )) , 
            product_basic :  v.optional(v.any() ), 
            product_additional :  v.optional(v.any() ), 
            process_data :  v.optional(v.any() ), 
            vgphss_uuid :v.pipe(v.string(),v.maxLength(64 )) , 
        });
        let validate : any = v.safeParse(dataSchema,createvgph_tran_dtl_stagingDto);
        if (!validate.success) {
      ...
      const res = await this.prismaService.vgph_tran_dtl_staging.create({ 
      data: await this.encryptData(createvgph_tran_dtl_stagingDto,'vgph_tran_dtl_staging','create'),
      select:{vgphtds_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,process_code:true,flow_code:true,step_code:true,product_basic:true,product_additional:true,process_data:true,vgphss_uuid:true,vgphsts_id :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
   ...
   */
  async createMaster(
    createvgph_tran_dtl_stagingDto: Prisma.vgph_tran_dtl_stagingCreateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_tran_dtl_staging creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_tran_dtl_staging creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      const encryptedData = await this.encryptData(createvgph_tran_dtl_stagingDto, 'vgph_tran_dtl_staging', 'create');
      ...
      if(role === 'MAKER')
      {
        ...
        const result = await this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_tran_dtl_staging',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphtds_id',
        ...
        return {
          success: true,
          message: 'vgph_tran_dtl_staging creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphtds_id:number, updatevgph_tran_dtl_stagingDto: Prisma.vgph_tran_dtl_stagingUpdateInput,token:string) {   
    try{
      ...
      const dataSchema:any =  v.object({
          ...
          direction :  v.optional(v.enum(direction_vgph_tran_dtl_staging,"Invalid direction_vgph_tran_dtl_staging enum")), 
          process_type :  v.optional(v.enum(process_type_vgph_tran_dtl_staging,"Invalid process_type_vgph_tran_dtl_staging enum")), 
          tran_category :  v.optional(v.enum(tran_category_vgph_tran_dtl_staging,"Invalid tran_category_vgph_tran_dtl_staging enum")), 
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_tran_dtl_stagingDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const res = await this.prismaService.vgph_tran_dtl_staging.update({
      where: {vgphtds_id},
      data: await this.encryptData(updatevgph_tran_dtl_stagingDto,'vgph_tran_dtl_staging','update'),
      select: {vgphtds_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,process_code:true,flow_code:true,step_code:true,product_basic:true,product_additional:true,process_data:true,vgphss_uuid:true,vgphsts_id :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return await this.decryptData(res, 'vgph_tran_dtl_staging');
    } catch (error) {
...
/**
  ...
  async updateMaster(
vgphtds_id:number,
    updatevgph_tran_dtl_stagingDto: Prisma.vgph_tran_dtl_stagingUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphtds_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_tran_dtl_staging update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_tran_dtl_staging update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      const existingRecord = await this.prismaService.vgph_tran_dtl_staging.findUnique({
        where: {vgphtds_id}
      });
      ...
      // Encrypt data if needed
      const encryptedData = await this.encryptData(updatevgph_tran_dtl_stagingDto, 'vgph_tran_dtl_staging', 'update');
      ...
      return {
        success: true,
        message: 'vgph_tran_dtl_staging update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphtds_id:number,token : string) {
    try{
      const res = await this.prismaService.vgph_tran_dtl_staging.delete({
      where: {vgphtds_id },
      select: {vgphtds_id:true,tenant_id:true,product_code:true,direction:true,process_type:true,tran_category:true,process_code:true,flow_code:true,step_code:true,product_basic:true,product_additional:true,process_data:true,vgphss_uuid:true,vgphsts_id :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return res;
...
vgphtds_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphtds_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_tran_dtl_staging deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_tran_dtl_staging deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_tran_dtl_staging deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.vgph_tran_dtl_staging.findFirst({ 
