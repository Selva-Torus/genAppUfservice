...
@Injectable()
export class vgph_source_tran_mainService {
  ...
  async findSchema (token) {
    const data = {
      ...
      uuid:"string",
      vgphsm_uuid:"string",
      vgphsm_id :"number",      prctm_id :"number",      trs_created_date:"Date",
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphstm_id }: {vgphstm_id : number} = queryValue;
      const { tenant_id }: {tenant_id : string} = queryValue;
      const { parent_vgphstm_uuid }: {parent_vgphstm_uuid : string} = queryValue;
      ...
      if(parent_vgphstm_uuid){ 
        query.parent_vgphstm_uuid = { [queryCondition['parent_vgphstm_uuid']]: parent_vgphstm_uuid };
      }
      if(product_code){ 
      ...
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.vgph_source_tran_main.findMany({
          select:columns,
          where: query,          
        });
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_source_tran_main');
      ...
      const totalItems = await this.prismaService.vgph_source_tran_main.count({
        where: query,
      });
...
prctm_id :true,        trs_created_date:true,
        trs_created_by:true,
        trs_modified_date:true,
        trs_modified_by:true,
        trs_next_status:true,
...
,vgphstm_id?:number) {
    try{
      ...
      if (vgphstm_id) {
        whereClause.vgphstm_id = vgphstm_id;
      }
      const res = await this.prismaService.vgph_source_tran_main.findMany({ 
      where: whereClause,
      select: {vgphstm_id:true,tenant_id:true,parent_vgphstm_uuid:true,product_code:true,direction:true,process_type:true,tran_category:true,message_code:true,channel_name:true,channel_reference:true,tran_date:true,tran_reference:true,tran_seq_no:true,value_date:true,settlement_date:true,dr_account:true,dr_bank_code_type:true,dr_bank_code:true,dr_name:true,dr_amount:true,dr_currency:true,cr_account:true,cr_bank_code_type:true,cr_bank_code:true,cr_name:true,cr_amount:true,cr_currency:true,remittance_info:true,product_basic:true,product_additional:true,charge_type:true,uuid:true,vgphsm_uuid:true,          vgphsm_id :true,    
          vgph_destination_tran_main:{
              select:{
              vgphdtm_id:true,              tenant_id:true,              product_code:true,              direction:true,              process_type:true,              tran_category:true,              vgphdm_uuid:true,              vgphstm_uuid:true            ,
          trs_created_date:true,
  ...
  async create(createvgph_source_tran_mainDto: Prisma.vgph_source_tran_mainCreateInput,token:string) {
    try{

      enum direction_vgph_source_tran_main{
        OUTBOUND="OUTBOUND",
        INBOUND="INBOUND",
      }
      enum process_type_vgph_source_tran_main{
        OP="OP",
        IR="IR",
        IP="IP",
        OR="OR",
      }
      enum tran_category_vgph_source_tran_main{
        Financial="Financial",
        Non_Financial="Non_Financial",
      }
      enum dr_bank_code_type_vgph_source_tran_main{
        IFSC="IFSC",
        BIC="BIC",
        IBAN="IBAN",
      }
      enum cr_bank_code_type_vgph_source_tran_main{
        IFSC="IFSC",
        BIC="BIC",
        IBAN="IBAN",
      }
      enum charge_type_vgph_source_tran_main{
        SHA="SHA",
        OUR="OUR",
        BEN="BEN",
      }
      const dataSchema:any =  v.object({
            ...
            parent_vgphstm_uuid :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            product_code :v.pipe(v.string(),v.maxLength(16 )) , 
            direction :v.enum(direction_vgph_source_tran_main,"Invalid direction_vgph_source_tran_main enum")  , 
            process_type :v.enum(process_type_vgph_source_tran_main,"Invalid process_type_vgph_source_tran_main enum")  , 
            tran_category :v.enum(tran_category_vgph_source_tran_main,"Invalid tran_category_vgph_source_tran_main enum")  , 
            ...
            dr_bank_code_type :  v.optional(v.enum(dr_bank_code_type_vgph_source_tran_main,"Invalid dr_bank_code_type_vgph_source_tran_main enum")), 
            ...
            cr_bank_code_type :  v.optional(v.enum(cr_bank_code_type_vgph_source_tran_main,"Invalid cr_bank_code_type_vgph_source_tran_main enum")), 
            ...
            charge_type :  v.optional(v.enum(charge_type_vgph_source_tran_main,"Invalid charge_type_vgph_source_tran_main enum")), 
            uuid :v.pipe(v.string(),v.maxLength(64 )) , 
            vgphsm_uuid :v.pipe(v.string(),v.maxLength(64 )) , 
        });
        let validate : any = v.safeParse(dataSchema,createvgph_source_tran_mainDto);
        if (!validate.success) {
   ...
   */
  async createMaster(
    createvgph_source_tran_mainDto: Prisma.vgph_source_tran_mainCreateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_tran_main creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_tran_main creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      const encryptedData = await this.encryptData(createvgph_source_tran_mainDto, 'vgph_source_tran_main', 'create');
      ...
      if(role === 'MAKER')
      {
        ...
        const result = await this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_source_tran_main',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphstm_id',
        ...
        return {
          success: true,
          message: 'vgph_source_tran_main creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphstm_id:number, updatevgph_source_tran_mainDto: Prisma.vgph_source_tran_mainUpdateInput,token:string) {   
    try{
      ...
      const dataSchema:any =  v.object({
          ...
          direction :  v.optional(v.enum(direction_vgph_source_tran_main,"Invalid direction_vgph_source_tran_main enum")), 
          process_type :  v.optional(v.enum(process_type_vgph_source_tran_main,"Invalid process_type_vgph_source_tran_main enum")), 
          tran_category :  v.optional(v.enum(tran_category_vgph_source_tran_main,"Invalid tran_category_vgph_source_tran_main enum")), 
          message_code :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
          channel_name :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
          channel_reference :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_source_tran_mainDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
...
/**
  ...
  async updateMaster(
vgphstm_id:number,
    updatevgph_source_tran_mainDto: Prisma.vgph_source_tran_mainUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphstm_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_source_tran_main update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_tran_main update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      const existingRecord = await this.prismaService.vgph_source_tran_main.findUnique({
        where: {vgphstm_id}
      });
      ...
      // Encrypt data if needed
      const encryptedData = await this.encryptData(updatevgph_source_tran_mainDto, 'vgph_source_tran_main', 'update');
      ...
      return {
        success: true,
        message: 'vgph_source_tran_main update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphstm_id:number,token : string) {
    try{
...
vgphstm_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphstm_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_source_tran_main deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_source_tran_main deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_source_tran_main deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.vgph_source_tran_main.findFirst({ 
