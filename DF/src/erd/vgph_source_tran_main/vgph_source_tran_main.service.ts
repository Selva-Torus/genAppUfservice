...
export class vgph_source_tran_mainService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      const { dr_bank_code_type }: {dr_bank_code_type : string} = queryValue;
      ...
      const { cr_bank_code_type }: {cr_bank_code_type : string} = queryValue;
      ...
      const { vgphsm_id }: {vgphsm_id : bigint} = queryValue;
      const { prctm_id }: {prctm_id : bigint} = queryValue;
      const { source_content }: {source_content : string} = queryValue;
      ...
      if(vgphsm_id){ 
        query.vgphsm_id = { [queryCondition['vgphsm_id']]: vgphsm_id };
      }
      if(prctm_id){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_source_tran_main.findMany({
          select:columns,
          where: query,          
        }));
      ...
      this.prismaService.vgph_source_tran_main.count({
        where: query,
      }));
...
,vgphstm_id?:number) {
  ...
  async create(createvgph_source_tran_mainDto: Prisma.vgph_source_tran_mainCreateInput,token:string) {
    try{
      ...
      const dataSchema:any =  v.object({
            ...
            dr_bank_code_type :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
            ...
            cr_bank_code_type :  v.optional(v.pipe(v.string(),v.maxLength(16 ))), 
      ...
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_source_tran_main.create({
          data: encryptedData,
          select:{vgphstm_id:true,parent_vgphstm_uuid:true,direction:true,process_type:true,tran_category:true,message_code:true,channel_name:true,channel_reference:true,tran_date:true,tran_reference:true,tran_seq_no:true,value_date:true,settlement_date:true,dr_account:true,dr_bank_code_type:true,dr_bank_code:true,dr_name:true,dr_amount:true,dr_currency:true,cr_account:true,cr_bank_code_type:true,cr_bank_code:true,cr_name:true,cr_amount:true,cr_currency:true,remittance_info:true,product_basic:true,product_additional:true,charge_type:true,uuid:true,vgphsm_uuid:true,vgphntm_uuid:true,vgphsm_id:true,prctm_id:true,source_content:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,trs_prev_process_code:true,trs_prev_status:true,trs_prev_process_status:true,trs_process_code:true,trs_status:true,trs_process_status:true,trs_next_process_code:true,trs_next_status:true,trs_next_process_status:true,}          
        })
      );
  ...
  async update(uuid: string, updatevgph_source_tran_mainDto: Prisma.vgph_source_tran_mainUpdateInput,token:string) {   
    try{
      ...
      this.prismaService.vgph_source_tran_main.updateMany({
      where: {uuid},
      data: encryptedData
    }));
...
uuid: string,
  ...
  async remove(vgphstm_id:number,token : string) {
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_tran_main.deleteMany({
      where: {vgphstm_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_source_tran_main');
    let decryptedRes: any = [];
...
vgphstm_id:number,
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_source_tran_main.findFirst({ 
