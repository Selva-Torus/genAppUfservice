...
@Injectable()
export class vgph_corporate_relationshipService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphcr_id }: {vgphcr_id : bigint} = queryValue;
      ...
      if(vgphcr_id){ 
        query.vgphcr_id = { [queryCondition['vgphcr_id']]: vgphcr_id };
      }
      if(parent_company){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_corporate_relationship.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_corporate_relationship');
      ...
      this.prismaService.vgph_corporate_relationship.count({
        where: query,
      }));
...
,vgphcr_id?:number) {
    try{
      ...
      if (vgphcr_id) {
        whereClause.vgphcr_id = vgphcr_id;
      }
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_corporate_relationship.findMany({ 
      where: whereClause,
  ...
  async create(createvgph_corporate_relationshipDto: Prisma.vgph_corporate_relationshipCreateInput,token:string) {
    try{
      ...
      const dataSchema:any =  v.object({
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_corporate_relationshipDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_corporate_relationshipDto, 'vgph_corporate_relationship', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_corporate_relationship.create({
          data: encryptedData,
          select:{vgphcr_id:true,parent_company:true,company:true,branch:true,parent_company_process_flag:true,company_process_flag:true,branch_process_flag:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_corporate_relationshipDto: Prisma.vgph_corporate_relationshipCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_corporate_relationship creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_corporate_relationship creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_corporate_relationship',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphcr_id',
        ...
        return {
          success: true,
          message: 'vgph_corporate_relationship creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphcr_id:number, updatevgph_corporate_relationshipDto: Prisma.vgph_corporate_relationshipUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_corporate_relationshipDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_corporate_relationshipDto,'vgph_corporate_relationship','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_corporate_relationship.updateMany({
      where: {vgphcr_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgphcr_id:number,
    updatevgph_corporate_relationshipDto: Prisma.vgph_corporate_relationshipUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphcr_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_corporate_relationship update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_corporate_relationship update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_corporate_relationship update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphcr_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_corporate_relationship.deleteMany({
      where: {vgphcr_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_corporate_relationship');
    let decryptedRes: any = [];
...
vgphcr_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphcr_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_corporate_relationship deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_corporate_relationship deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_corporate_relationship deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_corporate_relationship.findFirst({ 
