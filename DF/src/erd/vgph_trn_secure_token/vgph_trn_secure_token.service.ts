...
@Injectable()
export class vgph_trn_secure_tokenService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphtst_id }: {vgphtst_id : bigint} = queryValue;
      const { token_id }: {token_id : string} = queryValue;
      const { token_data }: {token_data : any } = queryValue;
      const { encryption_key_id }: {encryption_key_id : string} = queryValue;
      ...
      if(vgphtst_id){ 
        query.vgphtst_id = { [queryCondition['vgphtst_id']]: vgphtst_id };
      }
      if(token_id){ 
        query.token_id = { [queryCondition['token_id']]: token_id };
      }
      if(token_data){ 
        query.token_data = { [queryCondition['token_data']]: token_data };
      }
      if(encryption_key_id){ 
        query.encryption_key_id = { [queryCondition['encryption_key_id']]: encryption_key_id };
      }
      if(setup_code){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_trn_secure_token.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_trn_secure_token');
      ...
      this.prismaService.vgph_trn_secure_token.count({
        where: query,
      }));
  ...
  async create(createvgph_trn_secure_tokenDto: Prisma.vgph_trn_secure_tokenCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            vgphtst_id :v.number() , 
            token_id :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            token_data :  v.optional(v.any() ), 
            encryption_key_id :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            setup_code :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_trn_secure_tokenDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_trn_secure_tokenDto, 'vgph_trn_secure_token', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_trn_secure_token.create({
          data: encryptedData,
          select:{vgphtst_id:true,token_id:true,token_data:true,encryption_key_id:true,setup_code:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_trn_secure_tokenDto: Prisma.vgph_trn_secure_tokenCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_trn_secure_token creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_trn_secure_token creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_trn_secure_token',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphtst_id',
        ...
        return {
          success: true,
          message: 'vgph_trn_secure_token creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphtst_id:number, updatevgph_trn_secure_tokenDto: Prisma.vgph_trn_secure_tokenUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_trn_secure_tokenDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_trn_secure_tokenDto,'vgph_trn_secure_token','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_trn_secure_token.updateMany({
      where: {vgphtst_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgphtst_id:number,
    updatevgph_trn_secure_tokenDto: Prisma.vgph_trn_secure_tokenUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphtst_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_trn_secure_token update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_trn_secure_token update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_trn_secure_token update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphtst_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_trn_secure_token.deleteMany({
      where: {vgphtst_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_trn_secure_token');
    let decryptedRes: any = [];
...
vgphtst_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphtst_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_trn_secure_token deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_trn_secure_token deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_trn_secure_token deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_trn_secure_token.findFirst({ 
