...
import { itax_source_tran_docEntity } from './entity/itax_source_tran_doc.entity';
import { CustomException } from 'src/customException';
@Injectable()
export class itax_source_tran_docService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.itax_source_tran_doc.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'itax_source_tran_doc');
      ...
      this.prismaService.itax_source_tran_doc.count({
        where: query,
      }));
  ...
  async findOne(itaxstd_id:number,token : string) {
    try{
      ...
      this.prismaService.itax_source_tran_doc.findUnique({ 
      where: {itaxstd_id},
      select: {itaxstd_id:true,category:true,doc_type:true,doc_group:true,doc_name:true,url:true,doc_additional:true,doc_content_text:true,doc_content_byte:true,itaxst_id :true,        trs_created_date:true,
        trs_created_by:true,
  ...
  async findAll(token : string,trs_created_date?: Date,trs_created_by?: string,trs_modified_date?: Date,trs_modified_by?: string,trs_process_id?: string,trs_access_profile?: string,trs_org_grp_code?: string,trs_org_code?: string,trs_role_grp_code?: string,trs_role_code?: string,trs_ps_grp_code?: string,trs_ps_code?: string,trs_sub_org_grp_code?: string,trs_sub_org_code?: string,trs_locked_by?: string,trs_locked_time?: Date,trs_tenant_id?:string,trs_app_code?:string,trs_product_code?:string,trs_event_process_status?:string,trs_event_status?:string,trs_token_id?:string,trs_prev_process_code?:string,trs_prev_status?:string,trs_prev_process_status?:string,trs_process_code?:string,trs_status?:string,trs_process_status?:string,trs_next_process_code?:string,trs_next_status?:string,trs_next_process_status?:string) {
    try{
      ...
      this.prismaService.itax_source_tran_doc.findMany({ 
      where: whereClause,
      select: {itaxstd_id:true,category:true,doc_type:true,doc_group:true,doc_name:true,url:true,doc_additional:true,doc_content_text:true,doc_content_byte:true,          itaxst_id :true,    
      ...
      for (const indiviual of res) {
        ...
        const decryptedData = await this.decryptData(plain, 'itax_source_tran_doc');
        decryptedRes.push(decryptedData);
      }
      return decryptedRes;
    } catch (error) {
  ...
  async create(createitax_source_tran_docDto: Prisma.itax_source_tran_docCreateInput,token:string) {
    try{

      enum category_itax_source_tran_doc{
        front="front",
        back="back",
      }
      enum doc_type_itax_source_tran_doc{
        tiff="tiff",
        jpg="jpg",
        uv="uv",
        color="color",
      }
      enum doc_group_itax_source_tran_doc{
        cheque="cheque",
        credit_application="credit_application",
        credit_approval="credit_approval",
      }
      const dataSchema:any =  v.object({
            category :  v.optional(v.enum(category_itax_source_tran_doc,"Invalid category_itax_source_tran_doc enum")), 
            doc_type :  v.optional(v.enum(doc_type_itax_source_tran_doc,"Invalid doc_type_itax_source_tran_doc enum")), 
            doc_group :  v.optional(v.enum(doc_group_itax_source_tran_doc,"Invalid doc_group_itax_source_tran_doc enum")), 
        ...
        let validate : any = v.safeParse(dataSchema,createitax_source_tran_docDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createitax_source_tran_docDto, 'itax_source_tran_doc', 'create');
      const res = await this.prismaService.withConnection(() =>
   ...
   */
  async createMaster(
    createitax_source_tran_docDto: Prisma.itax_source_tran_docCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_source_tran_doc creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_source_tran_doc creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'itax_source_tran_doc',
        ...
        return {
          success: true,
          message: 'itax_source_tran_doc creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(itaxstd_id:number, updateitax_source_tran_docDto: Prisma.itax_source_tran_docUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updateitax_source_tran_docDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updateitax_source_tran_docDto,'itax_source_tran_doc','update');
      const res = await this.prismaService.withConnection(() =>
...
/**
  ...
  async updateMaster(
itaxstd_id:number,
    updateitax_source_tran_docDto: Prisma.itax_source_tran_docUpdateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'itax_source_tran_doc update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_source_tran_doc update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'itax_source_tran_doc update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
...
itaxstd_id:number,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'itax_source_tran_doc deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'itax_source_tran_doc deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'itax_source_tran_doc deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.itax_source_tran_doc.findFirst({ 
