...
import { itax_source_tran_docService } from './itax_source_tran_doc.service';
...
//import { CreateItaxSourceTranDocDto } from '../prisma/dto/create-itaxSourceTranDoc.dto';
//import { UpdateItaxSourceTranDocDto } from '../prisma/dto/update-itaxSourceTranDoc.dto';
import { Createitax_source_tran_docDto } from './dto/Createitax_source_tran_doc.dto';
import { Updateitax_source_tran_docDto } from './dto/Updateitax_source_tran_doc.dto';
import { plainToInstance } from 'class-transformer';
import { UfService } from 'src/Torus/v1/uf/uf.service';

 
@Controller('itax_source_tran_doc')
@ApiTags('ERD API')
export class itax_source_tran_docController {
  constructor(
    private readonly itax_source_tran_docService: itax_source_tran_docService,
    private readonly ufservice: UfService
  ) {}

  @Get("/schema")
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: itax_source_tran_docEntity })
  @ApiOperation({
    summary: 'schema validation',
    description: 'Retrive the datatype of the itax_source_tran_doc table',
  })
  async findSchema(@Headers() authHeader: string,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    return this.itax_source_tran_docService.findSchema(token);
  }

  @Get('/get')
  @ApiBearerAuth('JWT-auth')
  @ApiOkResponse({ type: itax_source_tran_docEntity, isArray: true })
  @ApiOperation({
    summary: 'Filter the records',
    description: 'Filter all the records from the itax_source_tran_doc table',
  })
  ...
  async findAllmethod(@Headers() authHeader: string,@Query() query: any,@Body() body: any,@Req() req: any) {
    ...
    return this.itax_source_tran_docService.findAllmethod(query, +limit,selectColumns,token);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read only one records from the itax_source_tran_doc table',
  })
  
  async findOne(@Headers() authHeader: string,@Param('itaxstd_id') itaxstd_id:number,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = this.itax_source_tran_docService.findOne(+itaxstd_id,token);
    return plainToInstance(itax_source_tran_docEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read all the records from the itax_source_tran_doc table',
  })
  ...
  async findAll(@Headers() authHeader: string,@Req() req: any,@Query("trs_created_date") trs_created_date?: Date,@Query("trs_created_by") trs_created_by?: string,@Query("trs_modified_date") trs_modified_date?: Date,@Query("trs_modified_by") trs_modified_by?: string,@Query("trs_process_id") trs_process_id?: string,@Query("trs_access_profile") trs_access_profile?: string,@Query("trs_org_grp_code") trs_org_grp_code?: string,@Query("trs_org_code") trs_org_code?: string,@Query("trs_role_grp_code") trs_role_grp_code?: string,@Query("trs_role_code") trs_role_code?: string,@Query("trs_ps_grp_code") trs_ps_grp_code?: string,@Query("trs_ps_code") trs_ps_code?: string,@Query("trs_sub_org_grp_code") trs_sub_org_grp_code?: string,@Query("trs_sub_org_code") trs_sub_org_code?: string,@Query("trs_locked_by") trs_locked_by?: string,@Query("trs_locked_time") trs_locked_time?: Date,@Query("trs_tenant_id") trs_tenant_id?: string,@Query("trs_app_code") trs_app_code?: string,@Query("trs_product_code") trs_product_code?: string,@Query("trs_event_process_status") trs_event_process_status?: string,@Query("trs_event_status") trs_event_status?: string,@Query("trs_prev_process_code") trs_prev_process_code?: string,@Query("trs_prev_status") trs_prev_status?: string,@Query("trs_prev_process_status") trs_prev_process_status?: string,@Query("trs_process_code") trs_process_code?: string,@Query("trs_status") trs_status?: string,@Query("trs_process_status") trs_process_status?: string,@Query("trs_next_process_code") trs_next_process_code?: string,@Query("trs_next_status") trs_next_status?: string,@Query("trs_next_process_status") trs_next_process_status?: string,@Query() query?: Record<string, any>) {
    ...
    const result = this.itax_source_tran_docService.findAll(token,trs_created_date,trs_created_by,trs_modified_date,trs_modified_by,trs_process_id,trs_access_profile,trs_org_grp_code,trs_org_code,trs_role_grp_code,trs_role_code,trs_ps_grp_code,trs_ps_code,trs_sub_org_grp_code,trs_sub_org_code,trs_locked_by,trs_locked_time,trs_tenant_id,trs_app_code,trs_product_code,trs_event_process_status,trs_event_status,trs_prev_process_code,trs_prev_status,trs_prev_process_status,trs_process_code,trs_status,trs_process_status,trs_next_process_code,trs_next_status,trs_next_process_status,);
    return plainToInstance(itax_source_tran_docEntity, result);
  } 
  ...
  @ApiBody({ type: Createitax_source_tran_docDto })
  @ApiCreatedResponse({ type: itax_source_tran_docEntity })
  @ApiOperation({
    summary: 'Create the record',
    description: 'Create the record for the itax_source_tran_doc table',
  })
  ...
  async create(
    ...
    @Body() createitax_source_tran_docDto: Prisma.itax_source_tran_docCreateInput,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.itax_source_tran_docService.createMaster(createitax_source_tran_docDto, makerInfo, token);
      return result;
    }

    const result = this.itax_source_tran_docService.create(createitax_source_tran_docDto,token);
    return plainToInstance(itax_source_tran_docEntity, result);
  }
  ...
  @ApiHeader({ name: 'xCdcaRemarks', required: false })
  @ApiHeader({ name: 'xCdcaApprovalStatus', required: false })
  @ApiBody({ type: Updateitax_source_tran_docDto })
  @ApiOkResponse({ type: itax_source_tran_docEntity })
  @ApiOperation({
    summary: 'Update the record',
    description: 'Update the record for the itax_source_tran_doc table',
  })
...
@Param('itaxstd_id') itaxstd_id:number,
    @Body() updateitax_source_tran_docDto: Prisma.itax_source_tran_docUpdateInput,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    ...
    if (mcRole && mcUsername) {
      ...
      const result = await this.itax_source_tran_docService.updateMaster(+itaxstd_id,updateitax_source_tran_docDto,makerInfo,token);
      return result;
    }

    const result = this.itax_source_tran_docService.update(+itaxstd_id,updateitax_source_tran_docDto,token);
    return plainToInstance(itax_source_tran_docEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Delete the record for the itax_source_tran_doc table',
  })
...
@Param('itaxstd_id') itaxstd_id:number,
    ...
    if (mcRole && mcUsername) {
      const makerInfo = { role: mcRole, username: mcUsername, remarks: mcRemarks,approvalStatus: mcApprovalStatus };
      const result = await this.itax_source_tran_docService.deleteMaster(+itaxstd_id,makerInfo,token);
      return result;
    }

    const result =  this.itax_source_tran_docService.remove(+itaxstd_id,token);
    return plainToInstance(itax_source_tran_docEntity, result);
  }  
  ...
  @ApiOperation({
    ...
    description: 'Read first record from the itax_source_tran_doc table',
  })
  
  async findFirst(@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = this.itax_source_tran_docService.findFirst(token);
    return plainToInstance(itax_source_tran_docEntity, result);
  }
  ...
  @ApiOperation({
    ...
    description: 'Read last record from the itax_source_tran_doc table',
  })
  
  async findLast(@Headers() authHeader: string,@Param() params: any, @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = this.itax_source_tran_docService.findLast(token);
    return plainToInstance(itax_source_tran_docEntity, result);
  }
}