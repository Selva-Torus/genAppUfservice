...
@Injectable()
export class vgph_clearing_sessionService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgph_csid }: {vgph_csid : bigint} = queryValue;
      ...
      const { processing_day }: {processing_day : Date} = queryValue;
      const { session_info }: {session_info : any } = queryValue;
      const { start_date }: {start_date :  Date} = queryValue;
      const { end_date }: {end_date :  Date} = queryValue;
      const { rule_code }: {rule_code : string} = queryValue;
      ...
      if(vgph_csid){ 
        query.vgph_csid = { [queryCondition['vgph_csid']]: vgph_csid };
      }
      if(direction){ 
      ...
      if(processing_day){ 
        query.processing_day = { [queryCondition['processing_day']]: processing_day };
      }
      if(session_info){ 
        query.session_info = { [queryCondition['session_info']]: session_info };
      }
      if(start_date){ 
      ...
      if(rule_code){ 
        query.rule_code = { [queryCondition['rule_code']]: rule_code };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_clearing_session.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_clearing_session');
      ...
      this.prismaService.vgph_clearing_session.count({
        where: query,
      }));
  ...
  async create(createvgph_clearing_sessionDto: Prisma.vgph_clearing_sessionCreateInput,token:string) {
    try{

      enum direction_vgph_clearing_session{
        OUTBOUND="OUTBOUND",
        INBOUND="INBOUND",
      }
      enum process_type_vgph_clearing_session{
        OP="OP",
        IR="IR",
        IP="IP",
        OR="OR",
      }
      enum processing_day_vgph_clearing_session{
        same_buz_day="same_buz_day",
        next_buz_day="next_buz_day",
      }
      const dataSchema:any =  v.object({
            vgph_csid :v.pipe(v.number(),v.maxValue(9999999999999999999999999999999999999999999999999999999999999999 )) , 
            direction :  v.optional(v.enum(direction_vgph_clearing_session,"Invalid direction_vgph_clearing_session enum")), 
            process_type :  v.optional(v.enum(process_type_vgph_clearing_session,"Invalid process_type_vgph_clearing_session enum")), 
            name :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            currency :  v.optional(v.pipe(v.string(),v.maxLength(3 ))), 
            processing_day :v.enum(processing_day_vgph_clearing_session,"Invalid processing_day_vgph_clearing_session enum")  , 
            session_info :  v.optional(v.any() ), 
            start_date :  v.optional((v.any())), 
            end_date :  v.optional((v.any())), 
            rule_code :  v.optional(v.string()), 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_clearing_sessionDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_clearing_sessionDto, 'vgph_clearing_session', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_clearing_session.create({
          data: encryptedData,
          select:{vgph_csid:true,direction:true,process_type:true,name:true,currency:true,processing_day:true,session_info:true,start_date:true,end_date:true,rule_code:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_clearing_sessionDto: Prisma.vgph_clearing_sessionCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_clearing_session creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_clearing_session creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_clearing_session',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgph_csid',
        ...
        return {
          success: true,
          message: 'vgph_clearing_session creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgph_csid:number, updatevgph_clearing_sessionDto: Prisma.vgph_clearing_sessionUpdateInput,token:string) {   
    try{
      ...
      const dataSchema:any =  v.object({
          ...
          name :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
          currency :  v.optional(v.pipe(v.string(),v.maxLength(3 ))), 
          processing_day :  v.optional(v.enum(processing_day_vgph_clearing_session,"Invalid processing_day_vgph_clearing_session enum")), 
          session_info :  v.optional(v.any()), 
          start_date :  v.optional((v.any())), 
          end_date :  v.optional((v.any())), 
          rule_code :  v.optional(v.string()), 
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_clearing_sessionDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_clearing_sessionDto,'vgph_clearing_session','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_clearing_session.updateMany({
      where: {vgph_csid},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgph_csid:number,
    updatevgph_clearing_sessionDto: Prisma.vgph_clearing_sessionUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgph_csid;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_clearing_session update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_clearing_session update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_clearing_session update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgph_csid:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_clearing_session.deleteMany({
      where: {vgph_csid}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_clearing_session');
    let decryptedRes: any = [];
...
vgph_csid:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgph_csid;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_clearing_session deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_clearing_session deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_clearing_session deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_clearing_session.findFirst({ 
