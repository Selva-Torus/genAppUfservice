...
@Injectable()
export class vgph_participant_branchService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphpbr_id }: {vgphpbr_id : bigint} = queryValue;
      const { country_code }: {country_code : string} = queryValue;
      const { bank_name }: {bank_name : string} = queryValue;
      const { branch_name }: {branch_name : string} = queryValue;
      ...
      if(vgphpbr_id){ 
        query.vgphpbr_id = { [queryCondition['vgphpbr_id']]: vgphpbr_id };
      }
      if(country_code){ 
      ...
      if(branch_name){ 
        query.branch_name = { [queryCondition['branch_name']]: branch_name };
      }
      if(sort_code){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_participant_branch.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_participant_branch');
      ...
      this.prismaService.vgph_participant_branch.count({
        where: query,
      }));
  ...
  async create(createvgph_participant_branchDto: Prisma.vgph_participant_branchCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            vgphpbr_id :v.pipe(v.number(),v.maxValue(9999999999999999999999999999999999999999999999999999999999999999 )) , 
            country_code :v.pipe(v.string(),v.maxLength(2 )) , 
            bank_name :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
            branch_name :  v.optional(v.pipe(v.string(),v.maxLength(128 ))), 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_participant_branchDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_participant_branchDto, 'vgph_participant_branch', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_participant_branch.create({
          data: encryptedData,
          select:{vgphpbr_id:true,country_code:true,bank_name:true,branch_name:true,sort_code:true,ifsc:true,address_1:true,address_2:true,city:true,state_code:true,zip_code:true,is_active:true,email_id:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_participant_branchDto: Prisma.vgph_participant_branchCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_participant_branch creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_participant_branch creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_participant_branch',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphpbr_id',
        ...
        return {
          success: true,
          message: 'vgph_participant_branch creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphpbr_id:number, updatevgph_participant_branchDto: Prisma.vgph_participant_branchUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_participant_branchDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_participant_branchDto,'vgph_participant_branch','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_participant_branch.updateMany({
      where: {vgphpbr_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgphpbr_id:number,
    updatevgph_participant_branchDto: Prisma.vgph_participant_branchUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphpbr_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_participant_branch update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_participant_branch update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_participant_branch update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphpbr_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_participant_branch.deleteMany({
      where: {vgphpbr_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_participant_branch');
    let decryptedRes: any = [];
...
vgphpbr_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphpbr_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_participant_branch deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_participant_branch deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_participant_branch deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_participant_branch.findFirst({ 
