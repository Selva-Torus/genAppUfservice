...
@Injectable()
export class prc_tokens_mainService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.prc_tokens_main.findMany({
          select:columns,
          where: query,          
        });
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'prc_tokens_main');
      ...
      const totalItems = await this.prismaService.prc_tokens_main.count({
        where: query,
      });
  ...
  async findOne(prctm_id:number,token : string) {
    try{
      const res = await this.prismaService.prc_tokens_main.findUnique({ 
      where: {prctm_id},
      select: {prctm_id:true,process_name:true,version:true,            vgph_source_tran_main:{
              select:{
    ...
    return  await this.decryptData(res, 'prc_tokens_main');
  } catch (error) {
...
,prctm_id?:number) {
    try{
      ...
      if (prctm_id) {
        whereClause.prctm_id = prctm_id;
      }
      const res = await this.prismaService.prc_tokens_main.findMany({ 
      where: whereClause,
      select: {prctm_id:true,process_name:true,version:true,          vgph_source_tran_main:{
              select:{
              vgphstm_id:true,              tenant_id:true,              parent_vgphstm_uuid:true,              product_code:true,              direction:true,              process_type:true,              tran_category:true,              message_code:true,              channel_name:true,              channel_reference:true,              tran_date:true,              tran_reference:true,              tran_seq_no:true,              value_date:true,              settlement_date:true,              dr_account:true,              dr_bank_code_type:true,              dr_bank_code:true,              dr_name:true,              dr_amount:true,              dr_currency:true,              cr_account:true,              cr_bank_code_type:true,              cr_bank_code:true,              cr_name:true,              cr_amount:true,              cr_currency:true,              remittance_info:true,              product_basic:true,              product_additional:true,              charge_type:true,              uuid:true,              vgphsm_uuid:true            ,
  ...
  async create(createprc_tokens_mainDto: Prisma.prc_tokens_mainCreateInput,token:string) {
    try{
      ...
      const dataSchema:any =  v.object({
        ...
        let validate : any = v.safeParse(dataSchema,createprc_tokens_mainDto);
        if (!validate.success) {
      ...
      const res = await this.prismaService.prc_tokens_main.create({ 
      data: await this.encryptData(createprc_tokens_mainDto,'prc_tokens_main','create'),
      select:{prctm_id:true,process_name:true,version:true,vgph_source_tran_main:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
   ...
   */
  async createMaster(
    createprc_tokens_mainDto: Prisma.prc_tokens_mainCreateInput,
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'prc_tokens_main creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'prc_tokens_main creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      const encryptedData = await this.encryptData(createprc_tokens_mainDto, 'prc_tokens_main', 'create');
      ...
      if(role === 'MAKER')
      {
        ...
        const result = await this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'prc_tokens_main',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'prctm_id',
        ...
        return {
          success: true,
          message: 'prc_tokens_main creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(prctm_id:number, updateprc_tokens_mainDto: Prisma.prc_tokens_mainUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updateprc_tokens_mainDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const res = await this.prismaService.prc_tokens_main.update({
      where: {prctm_id},
      data: await this.encryptData(updateprc_tokens_mainDto,'prc_tokens_main','update'),
      select: {prctm_id:true,process_name:true,version:true,vgph_source_tran_main:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return await this.decryptData(res, 'prc_tokens_main');
    } catch (error) {
...
/**
  ...
  async updateMaster(
prctm_id:number,
    updateprc_tokens_mainDto: Prisma.prc_tokens_mainUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =prctm_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'prc_tokens_main update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'prc_tokens_main update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      const existingRecord = await this.prismaService.prc_tokens_main.findUnique({
        where: {prctm_id}
      });
      ...
      // Encrypt data if needed
      const encryptedData = await this.encryptData(updateprc_tokens_mainDto, 'prc_tokens_main', 'update');
      ...
      return {
        success: true,
        message: 'prc_tokens_main update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(prctm_id:number,token : string) {
    try{
      const res = await this.prismaService.prc_tokens_main.delete({
      where: {prctm_id },
      select: {prctm_id:true,process_name:true,version:true,vgph_source_tran_main:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return res;
...
prctm_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =prctm_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'prc_tokens_main deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'prc_tokens_main deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'prc_tokens_main deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.prc_tokens_main.findFirst({ 
