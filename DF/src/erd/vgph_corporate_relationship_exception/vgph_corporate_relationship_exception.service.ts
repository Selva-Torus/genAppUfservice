...
@Injectable()
export class vgph_corporate_relationship_exceptionService {
  ...
  async findSchema (token) {
    const data = {
            ...
            company:"string",
            branch:"string",
            parent_company_process_flag:"string",
            company_process_flag:"string",
            branch_process_flag:"string",
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { vgphcre_id }: {vgphcre_id : bigint} = queryValue;
      const { processing_period }: {processing_period : string} = queryValue;
      const { parent_company }: {parent_company : string} = queryValue;
      const { company }: {company : string} = queryValue;
      const { branch }: {branch : string} = queryValue;
      const { parent_company_process_flag }: {parent_company_process_flag : string} = queryValue;
      const { company_process_flag }: {company_process_flag : string} = queryValue;
      const { branch_process_flag }: {branch_process_flag : string} = queryValue;
      const { processing_file_id }: {processing_file_id : string} = queryValue;
      ...
      if(vgphcre_id){ 
        query.vgphcre_id = { [queryCondition['vgphcre_id']]: vgphcre_id };
      }
      if(processing_period){ 
        query.processing_period = { [queryCondition['processing_period']]: processing_period };
      }
      if(parent_company){ 
        query.parent_company = { [queryCondition['parent_company']]: parent_company };
      }
      if(company){ 
        query.company = { [queryCondition['company']]: company };
      }
      if(branch){ 
        query.branch = { [queryCondition['branch']]: branch };
      }
      if(parent_company_process_flag){ 
        query.parent_company_process_flag = { [queryCondition['parent_company_process_flag']]: parent_company_process_flag };
      }
      if(company_process_flag){ 
        query.company_process_flag = { [queryCondition['company_process_flag']]: company_process_flag };
      }
      if(branch_process_flag){ 
        query.branch_process_flag = { [queryCondition['branch_process_flag']]: branch_process_flag };
      }
      if(processing_file_id){ 
        query.processing_file_id = { [queryCondition['processing_file_id']]: processing_file_id };
      }
      if(trs_created_date){ 
      ...
      if (Object.keys(query).length > 0) {
        ...
        this.prismaService.vgph_corporate_relationship_exception.findMany({
          select:columns,
          where: query,          
        }));
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'vgph_corporate_relationship_exception');
      ...
      this.prismaService.vgph_corporate_relationship_exception.count({
        where: query,
      }));
...
,vgphcre_id?:number) {
    try{
      ...
      if (vgphcre_id) {
        whereClause.vgphcre_id = vgphcre_id;
      }
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_corporate_relationship_exception.findMany({ 
      where: whereClause,
  ...
  async create(createvgph_corporate_relationship_exceptionDto: Prisma.vgph_corporate_relationship_exceptionCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            vgphcre_id :v.number() , 
            processing_period :  v.optional(v.pipe(v.string(),v.maxLength(32 ))), 
            parent_company :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            company :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            branch :  v.optional(v.pipe(v.string(),v.maxLength(64 ))), 
            parent_company_process_flag :  v.optional(v.pipe(v.string(),v.maxLength(1 ))), 
            company_process_flag :  v.optional(v.pipe(v.string(),v.maxLength(1 ))), 
            branch_process_flag :  v.optional(v.pipe(v.string(),v.maxLength(1 ))), 
            processing_file_id :  v.optional(v.pipe(v.string(),v.maxLength(256 ))), 
        ...
        let validate : any = v.safeParse(dataSchema,createvgph_corporate_relationship_exceptionDto);
        if (!validate.success) {
      ...
      const encryptedData = await this.encryptData(createvgph_corporate_relationship_exceptionDto, 'vgph_corporate_relationship_exception', 'create');
      const res = await this.prismaService.withConnection(() =>
        this.prismaService.vgph_corporate_relationship_exception.create({
          data: encryptedData,
          select:{vgphcre_id:true,processing_period:true,parent_company:true,company:true,branch:true,parent_company_process_flag:true,company_process_flag:true,branch_process_flag:true,processing_file_id:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_grp_code:true,trs_sub_org_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true,trs_token_id:true,trs_version:true,}          
        })
      );
   ...
   */
  async createMaster(
    createvgph_corporate_relationship_exceptionDto: Prisma.vgph_corporate_relationship_exceptionCreateInput,
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_corporate_relationship_exception creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_corporate_relationship_exception creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      if(role === 'MAKER')
      {
        ...
        this.prismaService.$queryRaw<any[]>`
          SELECT ct006_torus202610.request_change(
            p_table_name     := 'vgph_corporate_relationship_exception',
            p_operation_type := 'INSERT',
            p_record_id      := NULL,
            p_record_id_column := 'vgphcre_id',
        ...
        return {
          success: true,
          message: 'vgph_corporate_relationship_exception creation request submitted for approval',
          approval_id: approvalId,
          status: 'CREATED'
        };
      }
  ...
  async update(vgphcre_id:number, updatevgph_corporate_relationship_exceptionDto: Prisma.vgph_corporate_relationship_exceptionUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updatevgph_corporate_relationship_exceptionDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const encryptedData = await this.encryptData(updatevgph_corporate_relationship_exceptionDto,'vgph_corporate_relationship_exception','update');
      await this.prismaService.withConnection(() =>
      this.prismaService.vgph_corporate_relationship_exception.updateMany({
      where: {vgphcre_id},
      data: encryptedData
    }));
...
/**
  ...
  async updateMaster(
vgphcre_id:number,
    updatevgph_corporate_relationship_exceptionDto: Prisma.vgph_corporate_relationship_exceptionUpdateInput,
    userInfo: { role: string; username: string; remarks?: string,approvalStatus?:string },
    token:string
  ) {
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =vgphcre_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_corporate_relationship_exception update approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_corporate_relationship_exception update rejected',
              approvalId: approvalId,
              record_id: updateMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_corporate_relationship_exception update request submitted for approval',
        approval_id: approvalId,
        record_id: updateMaster_id,
        status: 'CREATED'
      };
  ...
  async remove(vgphcre_id:number,token : string) {
    try{
    ...
    await this.prismaService.withConnection(() =>
      this.prismaService.vgph_corporate_relationship_exception.deleteMany({
      where: {vgphcre_id}
    })); 
    // return await this.decryptData(await this.commonDecimalDatahandle(res), 'vgph_corporate_relationship_exception');
    let decryptedRes: any = [];
...
vgphcre_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =vgphcre_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'vgph_corporate_relationship_exception deletion approved and applied successfully',
            approval_id: approvalId,
          ...
          if (success) {
            return {
              success: true,
              message: 'vgph_corporate_relationship_exception deletion rejected',
              approval_id: approvalId,
              record_id: deleteMaster_id,
              status: 'REJECTED'
            };
      ...
      return {
        success: true,
        message: 'vgph_corporate_relationship_exception deletion request submitted for approval',
        approval_id: approvalId,
        record_id: deleteMaster_id,
        status: 'CREATED'
      };
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.withConnection(() =>
      this.prismaService.vgph_corporate_relationship_exception.findFirst({ 
